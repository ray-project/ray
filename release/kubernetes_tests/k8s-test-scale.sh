...
KUBERNETES_OPERATOR_TEST_NAMESPACE=scale-test KUBERNETES_OPERATOR_TEST_IMAGE="$IMAGE" python ../ray/python/ray/tests/kubernetes_e2e/test_k8s_operator_scaling.py
kubectl -n scale-test delete --all rayclusters
kubectl -n scale-test2 delete --all rayclusters
kubectl delete -f ../ray/deploy/components/operator_cluster_scoped.yaml
