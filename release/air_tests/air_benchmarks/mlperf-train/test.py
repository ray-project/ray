...
# torch_dataset = build_torch_dataset("~/data", 32, transform=get_transform(True))
# for i in range(3):
    # iterate(torch_dataset, "torch +transform")
# torch_dataset = build_torch_dataset("~/data", 32, transform=torchvision.transforms.ToTensor())
# for i in range(3):
    # iterate(torch_dataset, "torch")


# ray_dataset = ray.data.read_images("~/data").map_batches(crop_and_flip_image_batch)
# for i in range(3):
    # iterate(ray_dataset.iter_torch_batches(batch_size=32), "ray.data +transform")
# 
# ray_dataset = ray.data.read_images("~/data").map_batches(crop_and_flip_image_batch, zero_copy_batch=True)
# for i in range(3):
    # iterate(ray_dataset.iter_torch_batches(batch_size=32), "ray.data +transform +zero copy")

ray_dataset = ray.data.read_images("~/data")
for i in range(3):
    iterate(ray_dataset.iter_torch_batches(batch_size=None), "ray.data")

ray_dataset = ray.data.read_images("~/data").map_batches(lambda x: x, batch_format="pyarrow", batch_size=None)
for i in range(3):
    iterate(ray_dataset.iter_torch_batches(batch_size=None), "ray.data +dummy pyarrow transform")

ray_dataset = ray.data.read_images("~/data").map_batches(lambda x: x, batch_format="numpy", batch_size=None)
for i in range(3):
    iterate(ray_dataset.iter_torch_batches(batch_size=None), "ray.data +dummy np transform")

def load(batch):
    batch["image"] = [torchvision.io.read_image(path) for path in batch["image"]]
    return batch
        
DIR = "/home/ray/data/dog"
...
for i in range(3):
    iterate(ray_dataset.iter_torch_batches(batch_size=32), "ray.data manual load")
...
#ray_dataset = ray.data.read_images("~/data", transform=transform)
#for i in range(3):
#    iterate(ray_dataset.iter_batches(batch_size=32), "ray.data +fuse")