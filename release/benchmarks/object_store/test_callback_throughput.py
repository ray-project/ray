...
def _start_profiler():
    """Start a CPU profiler. Tries perf, then py-spy. Returns a handle dict or None."""
    import shutil
    import subprocess
    ...
    if perf_bin:
        test = subprocess.run(
            [perf_bin, "record", "-o", "/tmp/perf_test.data", "--", "true"],
            capture_output=True,
        )
        if test.returncode == 0:
            ...
            print("  perf available but cannot record (container restrictions?)")

    # Fall back to py-spy
    subprocess.run(["pip", "install", "py-spy"], capture_output=True)
    pyspy_bin = shutil.which("py-spy")
    if pyspy_bin:
        svg_path = "/tmp/artifacts/flamegraph.svg"
        proc = subprocess.Popen(
            [
                pyspy_bin,
                "record",
        ...
        time.sleep(0.5)
        return {"type": "pyspy", "proc": proc, "svg": svg_path}

    return None


def _stop_profiler(handle, num_blocks, drain_time):
    """Stop the profiler and generate the flame graph."""
    import signal
    import subprocess
    ...
    label = handle["type"]
    print(f"  [{label}] Drain time: {drain_time:.4f}s ({num_blocks} blocks)")

    if handle["type"] == "perf":
        collapse = os.path.join(handle["fg_dir"], "stackcollapse-perf.pl")
        flamegraph_pl = os.path.join(handle["fg_dir"], "flamegraph.pl")
        svg_path = "/tmp/artifacts/flamegraph.svg"
        subprocess.run(
            f"{handle['bin']} script -i {handle['data']}"
            f" | perl {collapse}"
            f" | perl {flamegraph_pl} --title 'Burst drop {num_blocks} blocks'"
            f" > {svg_path}",
            shell=True,
        )
        if os.path.isfile(svg_path) and os.path.getsize(svg_path) > 0:
            ...
            txt_path = "/tmp/artifacts/perf_stacks.txt"
            subprocess.run(
                f"{handle['bin']} script -i {handle['data']} > {txt_path}",
                shell=True,
            )
            print(f"  Flame graph failed; raw stacks at {txt_path}")

    elif handle["type"] == "pyspy":
        svg_path = handle["svg"]
        if os.path.isfile(svg_path) and os.path.getsize(svg_path) > 0:
            print(f"  Flame graph saved to {svg_path}")
        else:
            print("  py-spy flame graph generation failed")


def profile_burst_drop(num_blocks=10000, timeout_s=60):
    """Profile a 10k burst drop and generate a flame graph.

    Tries perf first (better C++ stacks), falls back to py-spy.
    """
    core_worker = ray._private.worker.global_worker.core_worker
    on_freed, fire_times, done = _make_timing_callback(num_blocks)
    refs = _produce_blocks(num_blocks)
    ...
    if profiler is None:
        print("  No profiler available, skipping CPU profile")
        return

    drop_time = time.perf_counter()
    del refs, ref
    done.wait(timeout=timeout_s)
    drain_time = time.perf_counter() - drop_time

    _stop_profiler(profiler, num_blocks, drain_time)
