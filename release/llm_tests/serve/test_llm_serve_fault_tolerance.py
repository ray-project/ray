...
def test_recovery_from_replica_failure(app) -> None:
    ...
    start_ray_serve(tensor_parallel_size=1)
    wait_for_deployment_status("llm_deployment", "HEALTHY", timeout_s=60)

    # Kill both replicas
    replica_ids = find_replica_ids("llm_deployment")
    for replica_id in replica_ids:
        print(f"Killing replica {replica_id}")
        kill_replica(replica_id)

    # wait for deployment to get unhealthy
    wait_for_deployment_status("llm_deployment", "UNHEALTHY", timeout_s=60)
