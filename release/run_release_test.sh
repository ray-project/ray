...
: "${RAY_TEST_SCRIPT:=ray_release/scripts/run_release_test.py}"
: "${RAY_TEST_REPO:=https://github.com/ray-project/ray.git}"
: "${RAY_TEST_BRANCH:=master}"
: "${RELEASE_RESULTS_DIR:=/tmp/artifacts}"

export RAY_TEST_REPO RAY_TEST_BRANCH RELEASE_RESULTS_DIR
export BUILDKITE_MAX_RETRIES=1
export BUILDKITE_RETRY_CODE=79
export BUILDKITE_TIME_LIMIT_FOR_RETRY=1800
...
if [[ -z "${NO_INSTALL}" ]]; then
  ...
  if [[ "${INSTALL_MATCHING_RAY}" == "true" ]]; then
    # Find ray-wheels parameter and install locally.
    PARSED_RAY_WHEELS=""
    LAST_ARG=""
    for arg in "$@"; do
      if [[ "${LAST_ARG}" == "--ray-wheels" ]]; then
        PARSED_RAY_WHEELS="${arg}"
        LAST_ARG=""
      else
        LAST_ARG="$arg"
      fi
...
if [[ -z "${NO_CLONE}" ]]; then
  ...
  if [[
    ...
    "${RAY_TEST_BRANCH-}" == "master" &&
    -n "${RAY_COMMIT_OF_WHEEL-}" &&
    "${HEAD_COMMIT}" != "${RAY_COMMIT_OF_WHEEL}"
  ]]; then
    echo "The checked out test code doesn't match with the installed wheel."
    echo "This is likely due to a racing condition when a PR is landed between"
    echo "a wheel is installed and test code is checked out."
    echo
