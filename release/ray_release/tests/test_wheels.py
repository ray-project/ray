...
from ray_release.wheels import (get_ray_version, DEFAULT_REPO,
                                get_ray_wheels_url, find_ray_wheels_url,
...
class WheelsFinderTest(unittest.TestCase):
    ...
    def testGetRayVersion(self):
        init_file = os.path.join(
            os.path.dirname(__file__), "..", "..", "..", "python", "ray",
            "__init__.py")
        ...
        with patch("urllib.request.urlopen",
                   lambda _: []), self.assertRaises(RayWheelsNotFoundError):
    ...
    def testGetRayWheelsURL(self):
        ...
        url = get_ray_wheels_url(
            ...
            ray_version="2.0.0.dev0")
        self.assertEqual(
            url, "https://s3-us-west-2.amazonaws.com/ray-wheels/"
    ...
    def testFindRayWheelsBuildkite(self):
        ...
        self.assertEqual(url, get_ray_wheels_url(repo, branch, commit,
                                                 version))
    ...
    def _testFindRayWheelsCheckout(self, repo: str, branch: str, commit: str,
                                   version: str, search_str: str):
        with patch("ray_release.wheels.get_latest_commits",
                   lambda *a, **kw: [commit]), patch(
                       "ray_release.wheels.url_exists",
                       lambda *a, **kw: False), self.assertRaises(
        ...
        with patch("ray_release.wheels.get_latest_commits",
                   lambda *a, **kw: [commit]), patch(
                       "ray_release.wheels.url_exists", lambda *a, **kw: True):
            # Succeeds
            url = find_ray_wheels_url(search_str)

            self.assertEqual(url,
                             get_ray_wheels_url(repo, branch, commit, version))
    ...
    def testFindAndWaitWheels(self):
        ...
        with freeze_time(auto_tick_seconds=10):
            with patch(
                    "ray_release.wheels.url_exists",
                    TrueAfter(400)), self.assertRaises(RayWheelsTimeoutError):
                find_and_wait_for_ray_wheels_url(commit, timeout=300.)

        with freeze_time(auto_tick_seconds=10):
            with patch("ray_release.wheels.url_exists", TrueAfter(200)):
                url = find_and_wait_for_ray_wheels_url(commit, timeout=300.)
