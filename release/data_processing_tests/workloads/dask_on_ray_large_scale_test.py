...
NUM_CHANNELS = 3
NUM_MINS_TO_PROCESS_PER_DATA_PRODUCER = 43800  # Number of minutes we process per data producer.
...
class ProcessingChainRoutines:
    ...
    Small scale:
        ...
        Since NUM_MINS_TO_PROCESS_PER_DATA_PRODUCER is 20 and NUM_MINS_PER_INPUT_FILE is 10, this means that we process
          20 / 10 = 2 input files. Since NUM_MINS_PER_OUTPUT_FILE is 5, we'd produce 20 / 5 = 4 output files.
    """
    @staticmethod
    def load_input_file() -> np.ndarray:
        ...
        """
        audio_tensor = np.ones(SAMPLING_RATE_PER_SECOND * NUM_CHANNELS * NUM_MINS_PER_INPUT_FILE * 60, dtype=np.float32)
        audio_tensor = np.reshape(audio_tensor, (NUM_CHANNELS, -1))  # reshape
        return audio_tensor

    @staticmethod
    def load_xarray(num_minutes_to_load_for_data_producer: int) -> xarray.Dataset:
        """
        Load an Xarray representing `num_minutes` amount of audio data.
        Each Xarray's underlying data variable is a dask.array that's lazily constructed.
        Therefore, computing the Xarray will trigger Dask computations.
        """
        dask_array_lists = list()
        num_files = int(num_minutes_to_load_for_data_producer / NUM_MINS_PER_INPUT_FILE)

        # Load num_files number of files lazily
        for i in range(0, num_files):
            delayed_obj = dask.delayed(ProcessingChainRoutines.load_input_file)()
            dask_arr = dask.array.from_delayed(
                delayed_obj, shape=(NUM_CHANNELS, SAMPLING_RATE_PER_SECOND * NUM_MINS_PER_INPUT_FILE * 60), dtype=np.float32
            )
        ...
        # Rechunk the array
        # Chunk size in the time dimension should be a power of two for the later FFT operations.
        final_dask_array = dask.array.rechunk(final_dask_array, chunks=(NUM_CHANNELS, 2 << 23))
    ...
    @staticmethod
    def infer_chunk_shape_after_fft_transform(n_fft: int, hop_length: int, time_chunk_sizes: List) -> tuple:
        """
        Infer the chunk shapes after applying FFT transformation.
        Infer is necessary for lazy transformations in Dask when transformations do not preserve chunk shape.
        """
    ...
    def fft_xarray(xr_input: xarray.Dataset, n_fft: int, hop_length: int):
        ...
        """
        # Infer the output chunk shape since FFT does not preserve input chunk shape.
        output_chunk_shape = ProcessingChainRoutines.infer_chunk_shape_after_fft_transform(
            n_fft=n_fft, hop_length=hop_length, time_chunk_sizes=xr_input.chunks["time"]
        )
        ...
        return xarray.Dataset(
            ...
            coords={
                "fft_freq": ("fft_freq", np.arange(transformed_audio.shape[1])),
...
def main():
    data_producer_ids = ["dpid_{}".format(i) for i in range(NUM_DATA_PRODUCERS)]
    n_fft = 4096
    hop_length = 2048
    file_save_dirpath = "." # change to save files to a different place
    BATCH_SIZE = 2000  # Adjust this to change how many _output_ files are saved at a time.

    ray.init(address="auto")
    logging.info("Ray available resources = {}".format(ray.available_resources()))
    ...
    for data_producer_id in data_producer_ids:
        ...
        xr1 = ProcessingChainRoutines.load_xarray(
            num_minutes_to_load_for_data_producer=NUM_MINS_TO_PROCESS_PER_DATA_PRODUCER)

        # 2. Apply FFT
        xr2 = ProcessingChainRoutines.fft_xarray(xr_input=xr1, n_fft=n_fft, hop_length=hop_length)

        num_output_files = int(NUM_MINS_TO_PROCESS_PER_DATA_PRODUCER / NUM_MINS_PER_OUTPUT_FILE)
        start_time = 0

        # 3. Produce indices for
        for step in range(num_output_files):
            segment_start = start_time + (NUM_MINS_PER_OUTPUT_FILE * step)  # in minutes
            segment_start_index = int(60 * NUM_MINS_PER_OUTPUT_FILE * step * SAMPLING_RATE_PER_SECOND / hop_length)
            segment_end = segment_start + NUM_MINS_PER_OUTPUT_FILE
            segment_len_sec = (segment_end - segment_start) * 60
            segment_end_index = int(
                segment_start_index + segment_len_sec * SAMPLING_RATE_PER_SECOND / hop_length
            )
            xr_segment = copy.deepcopy(xr2.isel(time=slice(segment_start_index, segment_end_index)))
