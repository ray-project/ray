...
def torch_setup(rank, world_size, num_workers_per_node, address, port):
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = ",".join(
        [str(i) for i in range(num_workers_per_node)])
...
def torch_run_single_node(train_func, world_size, num_workers_per_node,
    ...
    print("Total time for vanilla torch.distributed: ",
...
def torch_run_distributed(num_nodes, num_workers_per_node):
    ...
    per_node_bundle = {
        "CPU": num_workers_per_node,
        "GPU": num_workers_per_node
    }
    ...
    ray.get([
        task.remote(
            ...
            num_workers_per_node=num_workers_per_node,
            address=address,
            port=port) for task in remote_tasks
    ])
    total_time = time.time() - current_time
    return total_time
...
def multi_node_benchmark():
    ...
    torch_time = torch_run_distributed(
        num_nodes=NUM_NODES, num_workers_per_node=NUM_GPUS_PER_NODE)

    # Time using Ray Train.
    ray_time = ray_train_run(
        train_func=train_func, world_size=NUM_NODES * NUM_GPUS_PER_NODE)
