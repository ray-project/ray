...
The gpu-cu130 BYOD image (+ its python_depset) provides torch/ray. The DeepSpeed LLM adapter also needs
...
we don't keep that variant — `ray_torch_distributed` already isolates the Train control
