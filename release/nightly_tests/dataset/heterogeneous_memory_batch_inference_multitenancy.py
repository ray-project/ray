...
DATA_TASK_NAME_PREFIXES = ("MapBatches(", "Read", "Write", "map", "reduce")
...
def verify_placement() -> dict:
    ...
    node_subcluster = {n.node_id: (n.labels or {}).get("ray-subcluster") for n in nodes}
    ...
    for t in list_tasks(detail=True, limit=LIST_TASKS_LIMIT):
        ...
        if node_sc in SUBCLUSTERS:
            tasks_on_labeled += 1
            want = _selector_for(t).get("ray-subcluster")
            if want != node_sc:
