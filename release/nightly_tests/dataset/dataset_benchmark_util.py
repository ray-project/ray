...
# source: https://github.com/formigone/tf-imagenet/blob/master/LOC_synset_mapping.txt
IMAGENET_WNID_TO_LABEL = {
...
SORTED_WNIDS = sorted(list(IMAGENET_WNID_TO_LABEL.keys()))
IMAGENET_WNID_TO_ID = {
    wnid: SORTED_WNIDS.index(wnid) for wnid in SORTED_WNIDS
}
...
def get_prop_raw_image_paths(num_workers, target_worker_gb):
    ...
    sorted_class_ids = sorted(list(IMAGENET_WNID_TO_LABEL.keys()))
    file_paths = [
        f"{img_s3_root}/{class_id}/"
        for class_id in sorted_class_ids[:TARGET_NUM_DIRS]
    ]
    return file_paths
