...
Next, from $SCRIPT_DIR:
  ...
  # Point the box at the bucket's region first (cross-region S3 skews the timing):
  aws s3api get-bucket-location --bucket arrowrs-bench-21f6c795   # null == us-east-1
  export AWS_DEFAULT_REGION=<that region>

  python gen_s3_fixtures.py --bucket s3://arrowrs-bench-21f6c795
  python run_matrix.py --wide-path     s3://arrowrs-bench-21f6c795/wide_schema/primitives \\
                       --imagenet-path s3://arrowrs-bench-21f6c795/imagenet/parquet
EOF
