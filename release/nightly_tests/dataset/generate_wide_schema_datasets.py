...
def create_nested_struct_data(num_rows: int, num_columns: int) -> list:
    ...
    for col_idx in range(num_columns):
        ...
        struct = {"x": base_x, "y": f"string_{base_str}", "z": base_z}
...
def calculate_rows_for_target_size(
    data_type: str, num_columns: int, target_gb: float
) -> int:
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate wide schema datasets for benchmarking"
    )
    parser.add_argument(
        "--target-size-gb",
        type=float,
        default=5.0,
        help="Target size in GB per dataset",
    )
    parser.add_argument(
        "--num-columns", type=int, default=5000, help="Number of columns per dataset"
    )
    parser.add_argument(
        "--chunk-size", type=int, default=10000, help="Chunk size for writing"
    )
    ...
    for data_type, generator_func in generators.items():
        ...
        target_rows = calculate_rows_for_target_size(
            data_type, args.num_columns, args.target_size_gb
        )
        ...
        max_rows_per_file = max(
            1000, target_rows // 20
        )  # Use //20 to get ~20 files, minimum 1000 rows

        print(
            f"Writing to {output_path} with max_rows_per_file={max_rows_per_file:,}..."
        )
        ...
        print(
            f"   Estimated {estimated_files} parquet files (min_rows_per_file={max_rows_per_file:,})"
        )
