...
and compression variance. Reads are unchanged (--use-version v1/v2).

    python parquet_dataset_gen.py --mode mixed \\
        --file-size 256MiB --num-files 40 \\
        --output ./out/mixed --use-version v2
"""
import argparse
import io
import os
...
def _read_back(
    ...
    # Route through the requested DataSource. ``v1`` is the legacy DataSource V1;
    # ``v2`` is DataSource V2 (row-group-aware Parquet chunking + file-affinity
    # partitioning). DataContext is a slot-less dataclass, so a plain ``setattr``
    # on an unknown name would silently create a dead attribute on builds that
    # lack the field -- only set flags the build actually exposes, and warn
    # loudly when a flag a version needs turned ON can't be honored.
    version_flags = {
        "v1": {"use_datasource_v2": False},
        "v2": {"use_datasource_v2": True},
    }
...
def generate_mixed(
    ...
    from one another AND row groups vary within a file -- stressing the V2
...
def _apply_read_knobs(args) -> None:
    ...
    """
    knobs = {
        "partitioner_max_bucket_size_bytes": args.max_bucket_size,
        "parquet_reader_target_batch_size_bytes": args.batch_target_bytes,
        "parquet_in_memory_var_width_factor": args.var_width_factor,
...
def main():
    ...
    p.add_argument(
        ...
        default="v2",
        choices=["v1", "v2"],
        help="Datasource: v1=DataSource V1; v2=DataSource V2 (row-group-aware "
        "chunking + file-affinity partitioning).",
    )
    p.add_argument(
        "--max-bucket-size",
        type=parse_size,
        default=None,
        help="Override DataContext.partitioner_max_bucket_size_bytes "
        "(max in-memory bytes per read partition, e.g. 2GiB). Bundles more "
        "row groups per read task. Default: build default (1 GiB).",
    )
    p.add_argument(
        "--batch-target-bytes",
        type=parse_size,
        ...
        "(per-decode-batch target, e.g. 8MiB). Default: build default (128 MiB).",
    )
    p.add_argument(
        "--var-width-factor",
        type=float,
        default=None,
        help="Override DataContext.parquet_in_memory_var_width_factor: the "
        "expansion factor applied to a variable-width / nested column's footer "
        "uncompressed bytes in the partition in-memory size estimate. "
        "Default: build default (2.0).",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        "Set 1 to read one scan at a time per task. Default: build default (2).",
    )
    p.add_argument(
        "--batch-readahead",
        type=int,
        ...
        "peak). Lower it (e.g. 1-2) to shrink per-task memory. "
        "Default: build default (4).",
    )
    args = p.parse_args()
