...
# @ray.remote
def get_urls(bucket, path, max_files=10 * 1000):
...
image_ds = ray.experimental.data.from_items(s3_paths, parallelism=parallelism).map(read_file)

processed = image_ds.map(preprocess)
inferred = processed.map(infer, num_gpus=1)


print(next(inferred.iter_rows()))
