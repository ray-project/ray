...
def test_placement_group_perf(num_pgs, num_bundles, num_pending_pgs):
    ...
    def placement_group_create(num_pgs):
        pgs = [
            ray.util.placement_group(bundles=[
                {"custom": 0.001} for _ in range(num_bundles)
    ...
    print(
        "P50 scheduling latency ms: "
        f"{latencies[int(len(latencies) * 0.5)]}")
    print(
        "P95 scheduling latency ms: "
    ...
    return {
        ...
        "p50_scheduling_latency_ms": latencies[
            int(len(latencies) * 0.5)],
        "p50_e2e_pg_creation_latency_ms": e2e_latencies[
            int(len(e2e_latencies) * 0.5)]
    }
...
def run_full_benchmark(num_pending_pgs):
    ...
    for num_pgs in num_pgs_test:
        results.append(
            test_placement_group_perf(
                num_pgs, num_bundles, num_pending_pgs))
...
if __name__ == "__main__":
    ...
    if args.num_pgs != -1 and args.num_bundles != -1:
        test_placement_group_perf(
            args.num_pgs, args.num_bundles, args.num_pending_pgs)
    else:
