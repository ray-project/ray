# Claude Remaining Plan After GCS-6 Round 5
...
- Rust still resolves success callbacks before the async actor-table persistence completes
- C++ resolves those callbacks in the `ActorTable().Put()` completion path, after persistence and publication

## Non-Negotiable Rule

Do not call `GCS-6` fixed unless a successful actor-creation callback implies that the corresponding actor-table state has already been persisted.

## Required Work Order

### 1. Re-derive the exact C++ callback ordering contract
...
Add targeted tests for callback ordering:
...
- `test_creation_success_callbacks_observe_post_persist_state`
...
1. move callback resolution for `on_actor_creation_success()` into the async post-persist path
2. preserve the intended ordering:
   - persist
   - publish
   - resolve callbacks
...
- success callbacks do not fire before persistence completes
- success callbacks correspond to already-persisted actor state
