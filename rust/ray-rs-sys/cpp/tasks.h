...
std::vector<std::unique_ptr<::ray::TaskArg>> TransformArgs(
  ...
  for (auto& arg : args) {
    ...
    if (arg.is_value()) {
      ...
      // std::string s(reinterpret_cast<const char *>(p), 30);

      auto id = ObjectID::FromBinary(*arg.object_ref());
...
std::unique_ptr<ObjectID> Submit(const std::string& name, const rust::Vec<RustTaskArg>& args) {
  auto &core_worker = CoreWorkerProcess::GetCoreWorker();
  TaskOptions options{};
  std::vector<rpc::ObjectReference> return_refs;
  BundleID bundle_id = std::make_pair(PlacementGroupID::Nil(), -1);
  auto function_descriptor = FunctionDescriptorBuilder::BuildCpp(name);
