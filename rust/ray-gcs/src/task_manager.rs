...
pub struct GcsTaskManager {
    ...
    task_events: DashMap<(TaskID, i32), ray_proto::ray::rpc::TaskEvents>,
    /// Job index: job_id → set of (task_id, attempt).
    job_index: DashMap<JobID, Vec<(TaskID, i32)>>,
    /// Max events to store.
    max_events: usize,
...
impl GcsTaskManager {
    ...
    fn add_task_event(&self, event: ray_proto::ray::rpc::TaskEvents) {
        ...
        // Use DashMap entry API for atomic insert-or-merge
        match self.task_events.entry(key) {
            dashmap::mapref::entry::Entry::Vacant(e) => {
                e.insert(event);
                self.num_events.fetch_add(1, Ordering::Relaxed);
                // Update job index
                self.job_index.entry(job_id).or_default().push(key);
            }
            dashmap::mapref::entry::Entry::Occupied(mut e) => {
                // Merge with existing event — keep the latest state update
                if event.state_updates.is_some() {
                    e.get_mut().state_updates = event.state_updates;
                }
            }
        }
    }
    ...
    pub fn handle_get_task_events(
        ...
        let collected: Vec<ray_proto::ray::rpc::TaskEvents> = if let Some(job_id) = job_id {
            if let Some(keys) = self.job_index.get(job_id) {
                keys.iter()
                    .filter_map(|k| self.task_events.get(k).map(|v| v.clone()))
                    .collect()
            } else {
                Vec::new()
            }
            ...
            task_ids
                ...
                .flat_map(|tid| {
                    self.task_events
                        .iter()
                        .filter(|entry| entry.key().0 == *tid)
                        .map(|entry| entry.value().clone())
                        .collect::<Vec<_>>()
                })
                .collect()
        } else {
            self.task_events.iter().map(|e| e.value().clone()).collect()
        };
    ...
    pub fn on_job_finished(&self, job_id: &JobID) {
        if let Some(keys) = self.job_index.get(job_id) {
            for key in keys.iter() {
                if let Some(mut _event) = self.task_events.get_mut(key) {
