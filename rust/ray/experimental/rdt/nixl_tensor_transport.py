"""NIXL tensor transport for the Rust Ray backend.

Adapted from python/ray/experimental/gpu_object_manager/nixl_tensor_transport.py
for standalone use without C++ backend dependencies.
"""
...
# Hardcoded instead of importing from ray._private.ray_constants
NIXL_REMOTE_AGENT_CACHE_MAXSIZE = 1000

# Module-level variable set by each GPU worker process
...
class NixlTensorTransport(TensorTransportManager):
    def __init__(self):
        ...
        # LRU cache of remote agent names.
    ...
    def get_communicator_metadata(
        self, src_actor, dst_actor, backend=None,
    ...
    def recv_multiple_tensors(
        ...
        target_tensors: Optional[List["torch.Tensor"]] = None,
    ) -> List["torch.Tensor"]:
        from ray.experimental.rdt.util import create_empty_tensors_from_metadata

        if target_tensors is not None:
            # Validate shapes match
            for i, (t, (shape, dtype)) in enumerate(
                zip(target_tensors, tensor_transport_metadata.tensor_meta)
            ):
                if tuple(t.shape) != tuple(shape):
                    raise ValueError(
                        f"Shape of tensor_buffer at index {i} does not match "
                        f"expected shape: {tuple(t.shape)} vs {tuple(shape)}"
                    )
    ...
    def send_multiple_tensors(
        self, tensors, tensor_transport_metadata, communicator_metadata,
    ):
    ...
    def register_nixl_memory(self, tensor: "torch.Tensor"):
        """Register a tensor's memory with NIXL and bump the metadata count.

        This persists the registration even after the associated ObjectRef
        goes out of scope.
        """
        with self._cache_lock:
    ...
    def _add_tensor_descs(self, tensors: List["torch.Tensor"]):
        """Register full underlying pytorch storage objects with NIXL.

        Deduplicates by storage data pointer and increments reference count.
        """
        for tensor in tensors:
            key = tensor.untyped_storage().data_ptr()
            if key in self._tensor_desc_cache:
