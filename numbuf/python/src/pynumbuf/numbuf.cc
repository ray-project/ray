...
static PyObject* read_from_buffer(PyObject* self, PyObject* args) {
  ...
  ARROW_CHECK_OK(read_batch(schema_buffer, header_end_offset, reinterpret_cast<uint8_t*>(metadata_buffer->buf), metadata_buffer->len, batch));
...
static PyObject* store_list(PyObject* self, PyObject* args) {
  ...
  plasma_connection *conn;
  PyObject *value;
  if (!PyArg_ParseTuple(args, "O&O&O", PyStringToUniqueID, &obj_id, PyObjectToPlasmaConnection, &conn, &value)) {
    return NULL;
  }
  if (!PyList_Check(value)) {
    return NULL;
