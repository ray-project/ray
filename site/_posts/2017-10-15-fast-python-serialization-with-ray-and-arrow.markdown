...
- Arrow objects can naturally be stored in shared memory and read by multiple
processes (requirements 1 and 3).
...
Arrow encodes the schema as a flatbuffers object. **Using only the schema, we
can compute the offsets of each value in the data blob without scanning through
the data blob.** This means that we can avoid reading large arrays and other
