...
if [[ "$PYTHON" == "2.7" ]] && [[ "$platform" == "linux" ]]; then
  sudo apt-get update
  sudo apt-get install -y cmake pkg-config build-essential autoconf curl libtool python-dev python-numpy python-pip unzip socat
  # Install miniconda.
  ...
  sudo apt-get update
  sudo apt-get install -y cmake pkg-config python-dev python-numpy build-essential autoconf curl libtool unzip socat
  # Install miniconda.
  ...
  brew install cmake pkg-config automake autoconf libtool openssl bison socat > /dev/null
  # Install miniconda.
