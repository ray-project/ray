...
def doctest(files, gpu = False, name="doctest", deps=[], srcs=[], data=[], args=[], size="small", tags=[], **kwargs):
    ...
    for file in files:
        native.py_test(
            ...
            args = [
                ...
                "-v"
            ] + args + ["$(location :%s)" % file for file in [file]],
            data = ["//bazel:conftest.py"] + [file] + data,
            python_version = "PY3",
            srcs_version = "PY3",
