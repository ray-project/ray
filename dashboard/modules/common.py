...
def submission_client_cluster_info(
    address: str,
...
def parse_cluster_info(
    ...
    if module_string in {"http", "https", "ray"}:
        return submission_client_cluster_info(
            inner_address,
    ...
    # Try to dynamically import the function to get cluster info.
        ...
        assert "submission_client_cluster_info" in dir(module), (
            f"Module: {module_string} does "
            "not have `submission_client_cluster_info`."
        )

        return module.submission_client_cluster_info(
            inner_address,
            create_cluster_if_needed=create_cluster_if_needed,
            cookies=cookies,
            metadata=metadata,
