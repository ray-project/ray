...
class CpuProfilingManager:
    ...
    async def _check_sudo_required(self, password: str):
        ...
        if sudo_p.returncode != 0:
            ...
            raise EnvironmentError(
                "Running CPU profiler require a sudo access. "
                "Please provide the password or use "
                f"passwordless sudo. {stdout}\n{stderr}"
            )
        return sudo
    ...
    async def cpu_profile(
        self, pid: int, format="flamegraph", duration: float = 5, password: str = None
    ):
