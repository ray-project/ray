...
METRICS_GAUGES = {
    ...
    "node_mem_used": Gauge(
        "node_mem_used", "Memory usage on a ray node", "bytes", ["ip", "ClusterId"]
    ),
    ...
    "node_mem_total": Gauge(
        "node_mem_total", "Total memory on a ray node", "bytes", ["ip", "ClusterId"]
    ),
    ...
    "node_disk_io_read": Gauge(
        "node_disk_io_read", "Total read from disk", "bytes", ["ip", "ClusterId"]
    ),
    "node_disk_io_write": Gauge(
        "node_disk_io_write", "Total written to disk", "bytes", ["ip", "ClusterId"]
    ),
    "node_disk_io_read_count": Gauge(
        "node_disk_io_read_count", "Total read ops from disk", "io", ["ip", "ClusterId"]
    ),
    "node_disk_io_write_count": Gauge(
        "node_disk_io_write_count", "Total write ops to disk", "io", ["ip", "ClusterId"]
    ),
    "node_disk_io_read_speed": Gauge(
        "node_disk_io_read_speed", "Disk read speed", "bytes/sec", ["ip", "ClusterId"]
    ),
    "node_disk_io_write_speed": Gauge(
        "node_disk_io_write_speed", "Disk write speed", "bytes/sec", ["ip", "ClusterId"]
    ),
    "node_disk_read_iops": Gauge(
        "node_disk_read_iops", "Disk read iops", "iops", ["ip", "ClusterId"]
    ),
    "node_disk_write_iops": Gauge(
        "node_disk_write_iops", "Disk write iops", "iops", ["ip", "ClusterId"]
    ),
    ...
    "node_network_sent": Gauge(
        "node_network_sent", "Total network sent", "bytes", ["ip", "ClusterId"]
    ),
    "node_network_received": Gauge(
        "node_network_received", "Total network received", "bytes", ["ip", "ClusterId"]
    ),
...
class ReporterAgent(
    ...
    def __init__(self, dashboard_agent):
        ...
        self._cluster_id = dashboard_agent.cluster_id
            ...
            try:
                ...
                if not self._metrics_collection_disabled:
                    ...
                    self._metrics_agent.record_and_export(
                        records_reported, global_tags={"ClusterId": self._cluster_id}
                    )
