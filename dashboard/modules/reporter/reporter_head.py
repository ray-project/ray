...
class ReportHead(dashboard_utils.DashboardHeadModule):
    ...
    async def run_task_memory_profile(self, req) -> aiohttp.web.Response:
        ...
        duration = req.query.get("duration", None)
        ...
        logger.info(
            "Sending memory profiling request to {}:{} for {} with native={}".format(
                ip, pid, task_id, native
            )
        )

        reply = await reporter_stub.RunMemoryProfile(
            reporter_pb2.MemoryProfilingRequest(
    ...
    @routes.get("/worker/get_memory_profile")
    async def get_memory_profile(self, req) -> aiohttp.web.Response:
        if "ip" in req.query:
            reporter_stub = self._stubs[req.query["ip"]]
        else:
            reporter_stub = list(self._stubs.values())[0]
