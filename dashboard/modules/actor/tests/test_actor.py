...
def actor_killed_loop(worker_pid: str, timeout_secs=3) -> bool:
    dead = False
    for _ in range(timeout_secs):
        time.sleep(1)
        if actor_killed(worker_pid):
            dead = True
            break
    return dead

def kill_actor_using_dashboard_gcs(webui_url: str, actor_id: str, force_kill=False):
    resp = requests.get(
        webui_url + "/logical/kill_actor_gcs",
        params={
            "actor_id": actor_id,
...
def test_kill_actor_gcs(ray_start_with_dashboard):
    ...
    # Kill the actor
    kill_actor_using_dashboard_gcs(webui_url, actor_id, force_kill=False)
    assert actor_killed_loop(worker_pid)
    ...
    resp_json = kill_actor_using_dashboard_gcs(webui_url, actor_id, force_kill=False)
    print(resp_json)
    assert not actor_killed_loop(worker_pid, timeout=1)

    # Force kill the actor
    kill_actor_using_dashboard_gcs(webui_url, actor_id, force_kill=True)
    assert actor_killed_loop(worker_pid)
