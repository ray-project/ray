...
DATA_GRAFANA_PANELS = [
    ...
    Panel(
        ...
        description="Size in bytes of output blocks that are already taken by downstream operators.",
        unit="bytes",
        targets=[
            Target(
    ...
    Panel(
        ...
        targets=[
            Target(
                expr="sum(ray_data_num_outputs_of_finished_tasks{{{global_filters}}}) by (dataset, operator)",
                legend="Blocks Taken: {{dataset}}, {{operator}}",
            )
        ],
        fill=0,
    ...
    Panel(
        ...
        description="Size in bytes of generated output blocks that are from finished tasks.",
        unit="bytes",
        targets=[
            Target(
                expr="sum(ray_data_bytes_outputs_of_finished_tasks{{{global_filters}}}) by (dataset, operator)",
                legend="Bytes Taken: {{dataset}}, {{operator}}",
            )
        ],
        fill=0,
