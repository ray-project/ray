...
class MetricsHead(dashboard_utils.DashboardHeadModule):
    ...
    def _create_prometheus_query_for_progress(
        ...
        filter_for_terminal_states = ['State=~"FINISHED|FAILED"', f'SessionName="{self._session_name}"'] + filters
        filter_for_non_terminal_states = ['State!~"FINISHED|FAILED"', f'SessionName="{self._session_name}"'] + filters
