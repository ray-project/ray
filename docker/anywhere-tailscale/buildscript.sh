...
if [ -n "$cuda_version" ] && ! [ $cuda_version = gpu]; then
  ...
  sudo docker build --shm-size=$shm_memory --cache-from=index.docker.io/rayproject/ray-ml:py38-gpu ~/build -t jcoffi/cluster-anywhere:gpu -t jcoffi/cluster:gpu --build-arg IMAGETYPE=gpu
else
  sudo docker build --shm-size=$shm_memory --cache-from=index.docker.io/rayproject/ray:py38-cpu ~/build -t jcoffi/cluster-anywhere:cpu -t jcoffi/cluster-anywhere:latest -t jcoffi/cluster:cpu-latest --build-arg IMAGETYPE=gpu 
fi 