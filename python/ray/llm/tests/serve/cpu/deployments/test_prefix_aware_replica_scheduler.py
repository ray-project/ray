...
from test_pow_2_replica_scheduler import FakeRunningReplica # Reuse the FakeRunningReplica from the Pow2 test

from ray.serve._private.replica_scheduler.prefix_aware_scheduler import PrefixAwareReplicaScheduler
from ray.llm._internal.serve.replica_scheduler.prefix_aware.prefix_tree import PrefixTreeActor
from ray.serve._private.common import DeploymentHandleSource, DeploymentID, ReplicaID, RequestMetadata
...
class TestPow2FallbackBehavior:
    ...
    async def test_fallback_when_no_prompt(self, prefix_scheduler):
        ...
        tenant_to_char_count = ray.get(prefix_scheduler._tree_actor.getattr.remote("tenant_to_char_count"))
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("prefix_scheduler", [{"imbalanced_threshold": 2}], indirect=True)
    async def test_fallback_when_imbalanced(self, prefix_scheduler):
        ...
        ray.get(prefix_scheduler._tree_actor.insert.remote("hello world", r2.replica_id.to_full_id_str(), time.time()))
        ...
        matched_text, matched_tenants = ray.get(prefix_scheduler._tree_actor.prefix_match.remote("hello world"))
...
class TestPrefixAwareLogic:
    ...
    async def test_high_match_rate_selects_matching_replica(self, prefix_scheduler):
        ...
        prefix_scheduler.update_replicas([r1, r2])
        ray.get(prefix_scheduler._tree_actor.insert.remote("Hello", r2.replica_id.to_full_id_str(), time.time()))
        # Verify prefix match and smallest tenants
        matched_text, matched_tenants = ray.get(prefix_scheduler._tree_actor.prefix_match.remote("Hello world"))
        assert matched_text == "Hello"
        assert matched_tenants == [r2.replica_id.to_full_id_str()]
        
        tenant_counts = ray.get(prefix_scheduler._tree_actor.getattr.remote("tenant_to_char_count"))
        ...
        for _ in range(10):
            ...
            assert chosen == r2
        chat_req = fake_pending_request(messages=[{"content": "Hello"}, {"content": " world"}])
        for _ in range(10):
    ...
    async def test_low_match_rate_uses_smallest_tree(self, prefix_scheduler):
        ...
        ray.get(prefix_scheduler._tree_actor.insert.remote("hi", r1.replica_id.to_full_id_str(), time.time()))
        ray.get(prefix_scheduler._tree_actor.insert.remote("longtext", r2.replica_id.to_full_id_str(), time.time()))
...
class TestEvictionBehavior:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("prefix_scheduler", [{
        "do_eviction": True,
    ...
    async def test_eviction_threshold_behavior(self, prefix_scheduler):
        ...
        ray.get(prefix_scheduler._tree_actor.insert.remote("verylongtext", r1.replica_id.to_full_id_str(), time.time()))
        ray.get(prefix_scheduler._tree_actor.insert.remote("anotherlongtext", r1.replica_id.to_full_id_str(), time.time()))
