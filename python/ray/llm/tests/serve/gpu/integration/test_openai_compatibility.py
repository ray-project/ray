...
class TestOpenAICompatibility:
    ...
    def test_responses_without_store(self, testing_model):  # noqa: F811
        """The SDK omits unset params, so this is what a first call looks like.

        ``store`` defaults to true in the OpenAI schema, so an omitted ``store``
        must still be served rather than rejected as an opt-in to server state.
        """
        client, model = testing_model
        response = client.responses.create(
            model=model,
    ...
    def test_responses_stream(self, testing_model):  # noqa: F811
        ...
        assert events
        assert any(getattr(e, "type", "") == "response.completed" for e in events)

    @pytest.mark.skipif(
        os.environ.get("RAY_SERVE_LLM_ENABLE_DIRECT_STREAMING") == "1",
        reason="The stateless guard lives in the ingress, which direct "
        "streaming bypasses in favor of the engine's own ASGI app.",
    )
    def test_responses_store_unsupported(self, testing_model):  # noqa: F811
        client, model = testing_model
    ...
    def test_responses_previous_response_id_unsupported(
        self, testing_model
    ):  # noqa: F811
        """A stored response is not visible to the replica serving the follow-up."""
        client, model = testing_model
        with pytest.raises(openai.APIStatusError) as exc_info:
            client.responses.create(
                model=model,
