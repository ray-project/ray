...
def _make_fatal_error(name: str = "EngineDeadError") -> Exception:
    cls = type(name, (Exception,), {})
    return cls("engine is dead")
...
class TestIsFatalEngineError:
    ...
    def test_bare_engine_dead_error(self):
        assert server_utils._is_fatal_engine_error(_make_fatal_error("EngineDeadError"))

    def test_ray_wrapped_engine_dead_error(self):
        assert server_utils._is_fatal_engine_error(
            _make_fatal_error("RayTaskError(EngineDeadError)")
        )
...
class TestFatalEngineErrorLogHandler:
    ...
    def test_first_fatal_logs_full_traceback(self, handler):
        exc = _make_fatal_error("EngineDeadError")
...
class TestFatalEngineErrorRateLimiting:
    ...
    def test_first_fatal_sets_state(self, handler):
        with _patched_logger_and_time():
            handler.log(
                _make_fatal_error("EngineDeadError"),
                request_id="r1",
                status_code=500,
            )
