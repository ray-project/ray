...
def _ray_scheduling_strategy_fn(
    num_devices_per_instance: int, accelerator_type: str, accelerator_name: str = "GPU"
):
    """
    Create a Ray scheduling strategy for vLLM engine.

    Args:
        num_devices_per_instance: The number of GPUs per instance.
    ...
    def _get_bundle() -> Dict[str, float]:
        bundle: Dict[str, float] = {accelerator_name: 1, "CPU": 1}
        if accelerator_type:
            bundle[f"accelerator_type:{accelerator_type}"] = 0.001
        return bundle

    pg = ray.util.placement_group(
        [_get_bundle()] * num_devices_per_instance,
...
class vLLMEngineStage(StatefulStage):
    ...
    def post_init(cls, values):
        ...
        Returns:
            The updated values.
        """
        accelerator_name = values.get("accelerator_name", "GPU")
        ...
        # Setup num_devices required per vLLM engine.
        ...
        if executor_backend == "ray" and num_devices > 1:
        ...
        if accelerator_name.upper() == "GPU":
            map_batches_kwargs["num_gpus"] = num_devices
        else:
            ray_remote_args["resources"] = {accelerator_name: num_devices}
