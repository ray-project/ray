...
from ray.llm._internal.serve.deployments.llm.vllm import kv_transfer_backends
...
class NixlConnectorBackend(BaseConnectorBackend):
    ...
    def _set_side_channel_port(self):
        ...
        if not vllm_envs.is_set("VLLM_NIXL_SIDE_CHANNEL_PORT"):
            base_port: int = int(self.llm_config.experimental_configs.get(
                "NIXL_SIDE_CHANNEL_PORT_BASE",
                vllm_utils.get_open_port()
            ))
