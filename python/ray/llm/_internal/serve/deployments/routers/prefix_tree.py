...
import json
from typing import Optional, List, Tuple, Dict, Any, TypeVar, Union, cast
...
class Node:
    ...
    def __init__(self, text: str = "", parent: Optional["Node"] = None):
        ...
        self.tenant_last_access_time: Dict[str, int] = {}  # Maps tenant -> timestamp in ms (int)
...
class PrefixTree:
    ...
    def __init__(self) -> None:
        self.root: Node = Node()
        self.tenant_char_count: Dict[str, int] = defaultdict(int)  # Maps tenant -> character count
        ...
        with self.lock:
            ...
            while i < len(text):
                ...
                if first_char not in curr_node.children:
                    ...
                    shared_count: int = self.shared_prefix_count(matched_node.text, curr_text)
    ...
    def prefix_match(self, text: str, available_tenants: Optional[List[str]] = None) -> Tuple[str, Optional[List[str]]]:
        """
        ...
        with self.lock:
            ...
            while i < text_len:
                ...
                if first_char in curr_node.children:
                    ...
                    if available_tenants:
                        if not any(tenant in matched_node.tenant_last_access_time for tenant in available_tenants):
                            break
            ...
            if available_tenants:
                matching_tenants = [tenant for tenant in available_tenants if tenant in curr_node.tenant_last_access_time]
                if matching_tenants:
