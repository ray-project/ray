...
def get_mnist_dataset(train: bool = True) -> ray.data.Dataset:
    ...
    """
    mnist_dataset_factory = lambda: torchvision.datasets.MNIST("./data",
                                                               download=True,
                                                               train=train)
    mnist_dataset = ray.data.read_datasource(SimpleTorchDatasource(),
                                             dataset_factory=mnist_dataset_factory)

    # TODO: provide a better way to apply torchvision transforms.
    # Do some initial preprocessing on the PIL Image directly, and then convert to numpy.
    if train:
        mnist_dataset = mnist_dataset.map_batches(lambda batch: [(RandomCrop(28,
                                                                           padding=4)(
            image), label) for (image, label) in batch])
    mnist_dataset = mnist_dataset.map_batches(lambda batch: [(np.asarray(image), label) for (image,
                                                                        label) in batch])
...
class PermutedMNISTStream:
    ...
    def __init__(self, n_tasks: int = 3):
        ...
        self.permutations = [np.random.permutation(28*28) for _ in range(self.n_tasks)]

    def random_permute_dataset(self, dataset: ray.data.Dataset, permutation: np.ndarray):
        ...
        class PixelsPermutation(object):
            def __call__(self, batch):
                permuted_batch = [
                    (image.reshape(-1)[permutation].reshape(28, 28), label) for
                    (image, label) in batch]
    ...
    def generate_train_stream(self) -> Iterator[ray.data.Dataset]:
        for permutation in self.permutations:
            ...
            permuted_mnist_dataset = self.random_permute_dataset(mnist_dataset, permutation)
...
def train_loop_per_worker(config: dict):
    ...
    dataset_shard = train.get_dataset_shard("train").to_torch(label_column="label",
                                                              batch_size=batch_size,
                                                              unsqueeze_feature_tensors=False, unsqueeze_label_tensor=False)
...
def preprocess_images(df: pd.DataFrame) -> pd.DataFrame:
    ...
    torchvision_transforms = transforms.Compose([ToTensor(), transforms.Normalize((0.1307,), (0.3081,))])
...
def predict(checkpoint: Checkpoint, test_dataset_stream: Iterator[ray.data.Dataset]):
    ...
    acc_results = []
    batch_predictor = BatchPredictor.from_checkpoint(checkpoint=checkpoint,
                                                     predictor_cls=TorchPredictor,
                                                     model=SimpleMLP(num_classes=10))

    for test_dataset in test_dataset_stream:
        model_output = batch_predictor.predict(data=test_dataset,
        ...
        prediction_results = model_output.map_batches(convert_logits_to_classes,
                                                      batch_format="pandas")
        ...
        def calculate_prediction_scores(df):
            ...
            return df
        correct_dataset = zipped_dataset.map_batches(calculate_prediction_scores,
                                                     batch_format="pandas")
...
if __name__ == "__main__":
    ...
    predict(Checkpoint.from_directory("./checkpoint"),
            dataset_stream.generate_test_stream())
