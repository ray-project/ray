...
def test_deprecation_warning_warn():
    with patch('ray._common.deprecation.logger.warning') as mock_warning:
        ...
        assert "DeprecationWarning: `old_feature` has been deprecated. Use `new_feature` instead." in args[0]
...
def test_deprecated_decorator_function():
    with patch('ray._common.deprecation.logger.warning') as mock_warning, \
         patch('ray._common.deprecation.log_once') as mock_log_once:
