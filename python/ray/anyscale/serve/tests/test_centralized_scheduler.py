...
def test_state():
    ...
    t1 = 1001
    state.assign_request("req_1", "m1", "r1", t1)
    ...
    t2 = 1002
    state.assign_request("req_2", "m1", "r1", t2)
    ...
    state.assign_request("req_3", "m2", "r2", t3)
    assert state.find_evictable_replica("m3", 1, 10, t3) == ("r3", None)
    state.assign_request("req_4", "m3", "r3", t3 + 5)
    ...
    state.assign_request("req_5", "m4", "r2", t3 + 11, model_to_evict="m2")
...
def test_sync_scheduler_enforces_limits():
    ...
    assert not sched.try_schedule("req1", "m1")
    sched.update_running_replicas(["r1"])
    assert sched.try_schedule("req1", "m1") == "r1"
    assert sched.try_schedule("req2", "m1") == "r1"
    assert not sched.try_schedule("req3", "m1")
    assert not sched.try_schedule("req3", "m2")
    sched.notify_completed("req1")
    assert sched.try_schedule("req3", "m1") == "r1"
    sched.update_running_replicas(["r1", "r2"])
    assert not sched.try_schedule("req4", "m1")
    assert not sched.try_schedule("req4", "m1")
    assert sched.try_schedule("req5", "m2") == "r2"
    assert sched.try_schedule("req6", "m2") == "r2"
    assert not sched.try_schedule("req7", "m2")
    sched.notify_completed("req5")
    assert sched.try_schedule("req7", "m2") == "r2"
...
def test_sync_scheduler_eviction_policy():
    ...
    assert sched.try_schedule("req2", "m1") == "r1"
    assert not sched.try_schedule("req3", "m1")
    assert not sched.try_schedule("req4", "m2")

    now += 15

    assert not sched.try_schedule("req5", "m1")  # Cannot evict self
    assert sched.try_schedule("req6", "m2") == "r1"  # But can evict others.
