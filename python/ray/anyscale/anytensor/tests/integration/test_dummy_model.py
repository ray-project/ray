...
def _load_and_assert_equal(
    ...
    *,
    ...
    if populate_existing_state_dict:
        ...
        state_dict = client.load_state_dict(url, device=device)
...
def test_basic_load(
    ...
    client = AnytensorClient(local_cache_dir=local_cache_dir)
    (
        url,
        source_file_path,
        file_name,
    ...
    with pytest.raises(Exception):  # noqa
        if populate_existing_state_dict:
            client.populate_state_dict(
                url, state_dict=_gen_matching_empty_state_dict(source_state_dict)
            )
        else:
            client.load_state_dict(url)
