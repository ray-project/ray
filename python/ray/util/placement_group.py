...
def placement_group(bundles: List[Dict[str, float]],
    ...
    Args:
        ...
        lifetime(str): Either `None`, which defaults to the placement group will
            fate share with its creator and will be deleted once its creator is 
            dead, or "detached", which means the placement group will live as a 
            global object independent of the creator.
    ...
    if lifetime is None:
        ...
        raise ValueError(
            "placement group lifetime must be either `None` or 'detached'")
