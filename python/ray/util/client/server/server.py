    ...
    def ray_connect_handler(job_config: JobConfig = None, **ray_init_kwargs):
        if redis_address:
            if redis_password:
                ...
                ray.init(
                    address=redis_address, job_config=job_config, **ray_init_kwargs
                )
        else:
...
def try_create_redis_client(
    redis_address: Optional[str], redis_password: Optional[str]
) -> Optional[Any]:
    """
...
def main():
    ...
    ray_connect_handler = create_ray_handler(args.redis_address, args.redis_password)
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                logger.error(
                    f"[{args.mode}] Failed to put health check "
