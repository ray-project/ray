...
def _load_val_sol(val_sol_path):
    assert val_sol_path is not None, \
        "Must supply the path of LOC_val_solution.csv"
    solutions = {}
    classes = set()
    ...
    class_to_idx = \
        {classes_list[i]: i for i in range(len(classes_list))}
    return solutions, classes_list, class_to_idx


class ImageNetDataset(LocalDataset):
    """An example implementation of a dataset using imagenet (which can be
    downloaded here
    ...
    def __init__(self,
        ...
        super(ImageNetDataset, self).__init__(
            loc, max_paths=max_paths, transform=transform)
        self.split = split
        if self.split == "validate":
            self._id_label_map, self.classes, self.class_to_idx = \
                _load_val_sol(val_sol_path)
        else:
