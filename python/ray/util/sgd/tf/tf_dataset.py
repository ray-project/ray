...
class TFMLDataset:
    ...
    def __init__(
        ...
        feature_types: Optional[List[tf.DType]],
        label_column: Any,
    ...
    def _check_and_convert(self):
        ...
        if self._feature_types:
            ...
            for i in range(len(self._feature_types)):
                assert all(
                    isinstance(dtype, tf.DType) for dtype in self._feature_types
    ...
    def get_shard(
        ...
        ds = tf.data.Dataset.from_generator(
            make_generator, output_types=output_types, output_shapes=output_shapes
        )
        return ds
