...
void scatter_wrapper(const std::shared_ptr<gloo::Context> &context,
  ...
  case glooDataType_t::glooInt8:
    scatter<int8_t>(context, sendbuf, recvbuf, size, root);
    break;
  case glooDataType_t::glooUint8:
    scatter<uint8_t>(context, sendbuf, recvbuf, size, root);
    break;
  case glooDataType_t::glooInt32:
    scatter<int32_t>(context, sendbuf, recvbuf, size, root);
    break;
  case glooDataType_t::glooUint32:
    scatter<uint32_t>(context, sendbuf, recvbuf, size, root);
    break;
  case glooDataType_t::glooInt64:
    scatter<int64_t>(context, sendbuf, recvbuf, size, root);
    break;
  case glooDataType_t::glooUint64:
    scatter<uint64_t>(context, sendbuf, recvbuf, size, root);
    break;
  case glooDataType_t::glooFloat16:
    scatter<gloo::float16>(context, sendbuf, recvbuf, size, root);
    break;
  case glooDataType_t::glooFloat32:
    scatter<float_t>(context, sendbuf, recvbuf, size, root);
    break;
  case glooDataType_t::glooFloat64:
    scatter<double_t>(context, sendbuf, recvbuf, size, root);
    break;
  default:
