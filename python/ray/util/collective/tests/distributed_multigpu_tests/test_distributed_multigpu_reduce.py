...
def test_reduce_different_name(ray_start_distributed_multigpu_2_nodes_4_gpus,
    ...
    results = ray.get(
        [a.do_reduce_multigpu.remote(group_name, dst_rank=dst_rank, dst_gpu_index=dst_gpu_index)
         for a in actors])
...
@pytest.mark.parametrize("dst_rank", [0, 1])
@pytest.mark.parametrize("dst_gpu_index", [0, 1])
def test_reduce_different_array_size(ray_start_distributed_multigpu_2_nodes_4_gpus,
                                     array_size, dst_rank, dst_gpu_index):
    world_size = 2
    num_gpu_per_worker = 2
    actual_world_size = world_size * num_gpu_per_worker
    ...
    results = ray.get([a.do_reduce_multigpu.remote(dst_rank=dst_rank, dst_gpu_index=dst_gpu_index)
                       for a in actors])
    for i in range(world_size):
        for j in range(num_gpu_per_worker):
            if i == dst_rank and j == dst_gpu_index:
...
def test_reduce_torch_cupy(ray_start_distributed_multigpu_2_nodes_4_gpus,
    ...
    ray.get(actors[0].set_buffer.remote([10], value0=2, value1=3))
    ray.get(actors[1].set_buffer.remote([10], value0=4, value1=5,
                                        tensor_type0='torch', tensor_type1="torch"))
...
def test_reduce_invalid_rank(ray_start_distributed_multigpu_2_nodes_4_gpus, dst_rank, dst_gpu_index):
    ...
    with pytest.raises(ValueError):
        _ = ray.get([a.do_reduce_multigpu.remote(dst_rank=dst_rank, dst_gpu_index=dst_gpu_index)
                     for a in actors])
