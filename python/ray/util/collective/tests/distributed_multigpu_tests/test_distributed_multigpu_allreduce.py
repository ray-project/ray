...
def test_allreduce_multiacc_different_name(
    ray_start_distributed_multiacc_2_nodes_4_accs, group_name
):
    world_size = 2
    ...
    results = ray.get([a.do_allreduce_multiacc.remote(group_name) for a in actors])
...
def test_allreduce_multiacc_different_array_size(
    ray_start_distributed_multiacc_2_nodes_4_accs, array_size
):
    world_size = 2
    num_acc_per_worker = 2
    actual_world_size = world_size * num_acc_per_worker
    ...
    results = ray.get([a.do_allreduce_multiacc.remote() for a in actors])
    assert (
...
def test_allreduce_multiacc_destroy(
    ray_start_distributed_multiacc_2_nodes_4_accs, backend="nccl", group_name="default"
):
    world_size = 2
    num_acc_per_worker = 2
    actual_world_size = world_size * num_acc_per_worker
...
def test_allreduce_multiacc_multiple_group(
    ray_start_distributed_multiacc_2_nodes_4_accs, backend="nccl", num_groups=5
):
    world_size = 2
    num_acc_per_worker = 2
    actual_world_size = world_size * num_acc_per_worker
...
def test_allreduce_multiacc_different_op(ray_start_distributed_multiacc_2_nodes_4_accs):
    world_size = 2
    actors, _ = create_collective_multiacc_workers(world_size)
    # check product
    ...
    results = ray.get(
        [a.do_allreduce_multiacc.remote(op=ReduceOp.PRODUCT) for a in actors]
    )
    ...
    ray.get(actors[0].set_buffer.remote([10], value0=2, value1=3))
    ray.get(actors[1].set_buffer.remote([10], value0=4, value1=5))
    results = ray.get([a.do_allreduce_multiacc.remote(op=ReduceOp.MIN) for a in actors])
    ...
    ray.get(actors[0].set_buffer.remote([10], value0=2, value1=3))
    ray.get(actors[1].set_buffer.remote([10], value0=4, value1=5))
    results = ray.get([a.do_allreduce_multiacc.remote(op=ReduceOp.MAX) for a in actors])
...
def test_allreduce_multiacc_different_dtype(
    ray_start_distributed_multiacc_2_nodes_4_accs, dtype
):
    world_size = 2
    num_acc_per_worker = 2
    actual_world_size = world_size * num_acc_per_worker
...
def test_allreduce_torch_cupy(ray_start_distributed_multiacc_2_nodes_4_accs):
    # import torch
    world_size = 2
    actual_world_size = 4
    actors, _ = create_collective_multiacc_workers(world_size)
