...
def test_allreduce_multigpu_different_array_size(
    ...
    ray.get([
        a.set_buffer.remote(array_size)
        for a in actors
    ])
...
def test_allreduce_multigpu_destroy(ray_start_distributed_multigpu_2_nodes_4_gpus,
                                    backend="nccl",
                                    group_name="default"):
    world_size = 2
    num_gpu_per_worker = 2
...
def test_allreduce_multigpu_multiple_group(ray_start_distributed_multigpu_2_nodes_4_gpus,
                                           backend="nccl",
                                           num_groups=5):
    world_size = 2
    num_gpu_per_worker = 2
...
def test_allreduce_multigpu_different_dtype(ray_start_distributed_multigpu_2_nodes_4_gpus,
                                            dtype):
    world_size = 2
    num_gpu_per_worker = 2
    actual_world_size = world_size * num_gpu_per_worker
