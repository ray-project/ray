...
def test_broadcast_different_name(ray_start_distributed_multigpu_2_nodes_4_gpus,
                                  group_name, src_rank, src_gpu_index):
    world_size = 2
    ...
    results = ray.get([
        a.do_broadcast_multigpu.remote(group_name=group_name,
                                       src_rank=src_rank,
...
@pytest.mark.parametrize("src_rank", [0, 1])
@pytest.mark.parametrize("src_gpu_index", [0, 1])
def test_broadcast_different_array_size(ray_start_distributed_multigpu_2_nodes_4_gpus,
                                        array_size, src_rank, src_gpu_index):
    world_size = 2
    num_gpu_per_worker = 2
...
def test_broadcast_torch_cupy(ray_start_distributed_multigpu_2_nodes_4_gpus, src_rank, src_gpu_index):
    ...
    results = ray.get(
        [a.do_broadcast_multigpu.remote(src_rank=src_rank, src_gpu_index=src_gpu_index)
         for a in actors])
    for i in range(world_size):
        for j in range(num_gpu_per_worker):
            val = (src_rank + 1) * 2 + src_gpu_index
...
def test_broadcast_invalid_rank(ray_start_distributed_multigpu_2_nodes_4_gpus,
    ...
    with pytest.raises(ValueError):
        _ = ray.get([a.do_broadcast_multigpu.remote(
