...
def test_broadcast_different_name(
    ray_start_distributed_multiacc_2_nodes_4_accs, group_name, src_rank, src_acc_index
):
    world_size = 2
    ...
    results = ray.get(
        [
            a.do_broadcast_multiacc.remote(
                group_name=group_name, src_rank=src_rank, src_acc_index=src_acc_index
            )
            for a in actors
        ]
    )
    for i in range(world_size):
        for j in range(num_acc_per_worker):
            val = (src_rank + 1) * 2 + src_acc_index
...
def test_broadcast_different_array_size(
    ray_start_distributed_multiacc_2_nodes_4_accs, array_size, src_rank, src_acc_index
):
    world_size = 2
    num_acc_per_worker = 2
    ...
    results = ray.get(
        [
            a.do_broadcast_multiacc.remote(
                src_rank=src_rank, src_acc_index=src_acc_index
            )
            for a in actors
        ]
    )
...
def test_broadcast_torch_cupy(
    ray_start_distributed_multiacc_2_nodes_4_accs, src_rank, src_acc_index
):
    import torch
...
@pytest.mark.parametrize("src_acc_index", [2, 3])
