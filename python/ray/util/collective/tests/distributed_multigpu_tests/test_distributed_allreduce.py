...
def test_allreduce_multigpu_different_name(ray_start_distributed_2_nodes_4_gpus, group_name):
    world_size = 2
...
def test_allreduce_multigpu_different_array_size(
    ...
    ray.get([
        a.set_buffer_by_size.remote(array_size)
        for a in actors
    ])
...
def test_allreduce_multigpu_destroy(ray_start_distributed_2_nodes_4_gpus,
                                    backend="nccl",
                                    group_name="default"):
    world_size = 2
    num_gpu_per_worker = 2
    ...
    results = ray.get([a.do_allreduce_multigpu.remote() for a in actors])
    assert (results[0] == cp.ones(
        (10, ), dtype=cp.float32) * actual_world_size * actual_world_size).all()
    assert (results[1] == cp.ones(
        (10, ), dtype=cp.float32) * actual_world_size * actual_world_size).all()


def test_allreduce_multigpu_multiple_group(ray_start_distributed_2_nodes_4_gpus,
                                           backend="nccl",
                                           num_groups=5):
    world_size = 2
    num_gpu_per_worker = 2
...
def test_allreduce_multigpu_different_op(ray_start_distributed_2_nodes_4_gpus):
    world_size = 2
    actors, _ = create_collective_multigpu_workers(world_size)
    # check product
    ray.get(actors[0].set_buffer_by_size.remote([10], 2, 3))
    ray.get(actors[1].set_buffer_by_size.remote([10], 4, 5))
    results = ray.get(
...
def test_allreduce_multigpu_different_dtype(ray_start_distributed_2_nodes_4_gpus,
                                            dtype):
    world_size = 2
    num_gpu_per_worker = 2
    actual_world_size = world_size * num_gpu_per_worker
    actors, _ = create_collective_multigpu_workers(world_size)
    ray.wait([a.set_buffer_by_size.remote([10], dtype=dtype) for a in actors])
    results = ray.get([a.do_allreduce_multigpu.remote() for a in actors])
...
def test_allreduce_torch_cupy(ray_start_distributed_2_nodes_4_gpus):
    ...
    actual_world_size = 4
    actors, _ = create_collective_multigpu_workers(world_size)
    ray.wait(a.set_buffer_by_size())

    ray.wait([actors[1].set_buffer_by_size.remote(torch.ones(10, ).cuda())])
    results = ray.get([a.do_allreduce_multigpu.remote() for a in actors])
