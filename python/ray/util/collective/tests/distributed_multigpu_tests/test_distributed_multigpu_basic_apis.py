...
def test_init_two_actors(ray_start_distributed_multiacc_2_nodes_4_accs, group_name):
    world_size = 2
    actors, results = create_collective_multiacc_workers(world_size, group_name)
    for i in range(world_size):
        assert results[i]


def test_report_num_accs(ray_start_distributed_multiacc_2_nodes_4_accs):
    world_size = 2
    actors, results = create_collective_multiacc_workers(world_size)
    num_accs = ray.get([actor.report_num_accs.remote() for actor in actors])
    assert num_accs == [2, 2]


def test_get_rank(ray_start_distributed_multiacc_2_nodes_4_accs):
    world_size = 2
...
def test_is_group_initialized(ray_start_distributed_multiacc_2_nodes_4_accs):
    world_size = 2
    actors, _ = create_collective_multiacc_workers(world_size)
    # check group is_init
...
def test_destroy_group(ray_start_distributed_multiacc_2_nodes_4_accs):
    world_size = 2
    actors, _ = create_collective_multiacc_workers(world_size)
