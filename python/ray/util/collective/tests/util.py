...
from ray.util.collective.collective_group.nccl_util import get_num_accs
...
class MultiACCWorker:
    ...
    def do_allreduce_multiacc(self, group_name="default", op=ReduceOp.SUM):
        col.allreduce_multiacc([self.buffer0, self.buffer1], group_name, op)
        cp.cuda.Device(0).synchronize()
        cp.cuda.Device(1).synchronize()
        return self.buffer0

    def do_reduce_multiacc(
        self, group_name="default", dst_rank=0, dst_acc_index=0, op=ReduceOp.SUM
    ):
        col.reduce_multiacc(
            [self.buffer0, self.buffer1], dst_rank, dst_acc_index, group_name, op
        )
        cp.cuda.Device(0).synchronize()
        cp.cuda.Device(1).synchronize()
        return self.buffer0, self.buffer1

    def do_broadcast_multiacc(self, group_name="default", src_rank=0, src_acc_index=0):
        col.broadcast_multiacc(
            [self.buffer0, self.buffer1], src_rank, src_acc_index, group_name
        )
        return self.buffer0, self.buffer1

    def do_allgather_multiacc(self, group_name="default"):
        col.allgather_multiacc(
    ...
    def do_reducescatter_multiacc(self, group_name="default", op=ReduceOp.SUM):
        col.reducescatter_multiacc(
            [self.buffer0, self.buffer1],
            [self.list_buffer0, self.list_buffer1],
            group_name,
            op,
    ...
    def do_send_multiacc(
        self, group_name="default", dst_rank=0, dst_acc_index=0, src_acc_index=0
    ):
        if src_acc_index == 0:
            col.send_multiacc(self.buffer0, dst_rank, dst_acc_index, group_name)
            cp.cuda.Device(0).synchronize()
            return self.buffer0
        elif src_acc_index == 1:
            col.send_multiacc(self.buffer1, dst_rank, dst_acc_index, group_name)
            cp.cuda.Device(1).synchronize()
            return self.buffer1
        else:
            raise RuntimeError()

    def do_recv_multiacc(
        self, group_name="default", src_rank=0, src_acc_index=0, dst_acc_index=0
    ):
        if dst_acc_index == 0:
            col.recv_multiacc(self.buffer0, src_rank, src_acc_index, group_name)
            cp.cuda.Device(0).synchronize()
            return self.buffer0
        elif dst_acc_index == 1:
            col.recv_multiacc(self.buffer1, src_rank, src_acc_index, group_name)
            cp.cuda.Device(1).synchronize()
            return self.buffer1
        else:
            raise RuntimeError()
    ...
    def report_num_accs(self):
        n_accs = get_num_accs()
        return n_accs


def create_collective_multiacc_workers(
    num_workers=2, group_name="default", backend="nccl"
):
    actors = [None] * num_workers
    for i in range(num_workers):
        actor = MultiACCWorker.remote()
...
def init_tensors_for_gather_scatter_multiacc(
