...
def test_init_two_actors(ray_start_single_node_2_npus, group_name, backend):
    world_size = 2
...
@pytest.mark.parametrize("backend", [Backend.HCCL])
def test_init_multiple_groups(ray_start_single_node_2_npus, backend):
    world_size = 2
    num_groups = 10
    ...
    for i in range(num_groups):
        ...
        init_results = ray.get(
            [
                actor.init_group.remote(world_size, k, backend=backend, group_name=group_name)
...
@pytest.mark.parametrize("backend", [Backend.HCCL])
def test_get_rank(ray_start_single_node_2_npus, backend):
    world_size = 2
    ...
    ray.get(
        [
            actor.init_group.remote(
                world_size, world_size - 1 - i, backend=backend, group_name=new_group_name
            )
...
@pytest.mark.parametrize("backend", [Backend.HCCL])
def test_get_collective_group_size(ray_start_single_node_2_npus, backend):
    world_size = 2
    actors, _ = create_collective_workers(world_size, backend=backend)
...
@pytest.mark.parametrize("backend", [Backend.HCCL])
def test_is_group_initialized(ray_start_single_node_2_npus, backend):
    world_size = 2
    actors, _ = create_collective_workers(world_size, backend=backend)
    # check group is_init
...
@pytest.mark.parametrize("backend", [Backend.HCCL])
def test_destroy_group(ray_start_single_node_2_npus, backend):
    world_size = 2
    actors, _ = create_collective_workers(world_size, backend=backend)
    ...
    init_results = ray.get(
        [actor.init_group.remote(world_size, i, backend=backend) for i, actor in enumerate(actors)]
    )
    for i in range(world_size):
