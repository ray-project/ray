...
def test_reduce_different_name(
    ray_start_single_node_2_npus, group_name, array_size, dst_rank, backend
):
    world_size = 2
    actors, _ = create_collective_workers(num_workers=world_size, group_name=group_name, backend=backend)
    ray.wait(
        [
            a.set_buffer.remote(torch.ones(array_size, dtype=torch.float32).npu() * (i + 1))
    ...
    for i in range(world_size):
        assert (
            results[i] == torch.ones(array_size, dtype=torch.float32).npu() * (src_rank + 1)
        ).all()


@pytest.mark.parametrize("backend", [Backend.HCCL])
@pytest.mark.parametrize("dst_rank", [0, 1])
def test_sendrecv_torch_npu(ray_start_single_node_2_npus, dst_rank, backend):
    world_size = 2
    ...
    results = ray.get(refs)
    if dst_rank == 0:
        assert (results[0] == torch.ones((10,)).npu() * 2).all()
        assert (results[1] == torch.ones((10,)).npu() * 2).all()
    else:
        assert (results[0] == torch.ones((10,)).npu()).all()
        assert (results[1] == torch.ones((10,)).npu()).all()


@pytest.mark.parametrize("backend", [Backend.HCCL])
def test_sendrecv_invalid_rank(ray_start_single_node_2_npus, backend, dst_rank=3):
    world_size = 2
    actors, _ = create_collective_workers(world_size, backend=backend)
