...
def test_allreduce_different_name(
    ray_start_distributed_2_nodes_4_accs, group_name, world_size
):
...
def test_allreduce_different_array_size(
    ray_start_distributed_2_nodes_4_accs, array_size
):
    world_size = 4
...
def test_allreduce_destroy(
    ray_start_distributed_2_nodes_4_accs, backend="nccl", group_name="default"
):
    world_size = 4
    actors, _ = create_collective_workers(world_size)
...
def test_allreduce_multiple_group(
    ray_start_distributed_2_nodes_4_accs, backend="nccl", num_groups=5
):
    world_size = 4
    actors, _ = create_collective_workers(world_size)
...
def test_allreduce_different_op(ray_start_distributed_2_nodes_4_accs):
    world_size = 4
    actors, _ = create_collective_workers(world_size)
...
def test_allreduce_different_dtype(ray_start_distributed_2_nodes_4_accs, dtype):
    world_size = 4
    actors, _ = create_collective_workers(world_size)
...
def test_allreduce_torch_cupy(ray_start_distributed_2_nodes_4_accs):
    # import torch
    world_size = 4
    actors, _ = create_collective_workers(world_size)
    ray.wait(
