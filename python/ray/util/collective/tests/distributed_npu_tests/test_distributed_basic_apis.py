...
import ray
from ray.util.collective.tests.npu_util import Worker, create_collective_workers
...
def test_init_two_actors(
    ray_start_distributed_multinpu_2_nodes_4_npus, world_size, group_name, backend
):
...
@pytest.mark.parametrize("world_size", [2, 3, 4])
def test_init_multiple_groups(ray_start_distributed_multinpu_2_nodes_4_npus, world_size):
    num_groups = 1
...
@pytest.mark.parametrize("world_size", [2, 3, 4])
def test_get_rank(
    ray_start_distributed_multinpu_2_nodes_4_npus, world_size, backend
):
...
@pytest.mark.parametrize("backend", [Backend.HCCL])
@pytest.mark.parametrize("world_size", [2, 3, 4])
def test_get_collective_group_size(
    ray_start_distributed_multinpu_2_nodes_4_npus, world_size, backend
):
    actors, _ = create_collective_workers(world_size, backend=backend)
