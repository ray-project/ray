...
import pytest
import torch
import torch_npu  # replace cupy usage
import numpy as np
...
@pytest.mark.parametrize("tensor_backend", ["torch"])  # remove cupy option
...
def test_allgather_different_dtype(ray_start_distributed_2_nodes_4_npus, dtype, backend):
    world_size = 4
    actors, _ = create_collective_workers(world_size, backend=backend)
    ...
    for i in range(world_size):
        for j in range(world_size):
            assert (
                results[i][j]
                == torch.ones(10, dtype=dtype).npu() * (j + 1)
            ).all()


@pytest.mark.parametrize("backend", [Backend.HCCL])
@pytest.mark.parametrize("length", [0, 1, 3, 4, 7, 8])
def test_unmatched_tensor_list_length(
    ray_start_distributed_2_nodes_4_npus, length, backend
):
    world_size = 4
    actors, _ = create_collective_workers(world_size, backend=backend)
...
def test_unmatched_tensor_shape(
    ray_start_distributed_2_nodes_4_npus, shape, backend
):
    world_size = 4
    actors, _ = create_collective_workers(world_size, backend=backend)
