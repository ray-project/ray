...
@pytest.mark.parametrize("dst_rank", [0, 1])
def test_reduce_different_array_size(ray_start_single_node_2_gpus,
                                     array_size,
                                     dst_rank):
    world_size = 2
...
def test_reduce_multiple_group(ray_start_single_node_2_gpus, dst_rank, num_groups=5):
    ...
    for i in range(num_groups):
        ...
        results = ray.get([a.do_reduce.remote(dst_rank=dst_rank,
                                              group_name=group_name) for a in actors])
