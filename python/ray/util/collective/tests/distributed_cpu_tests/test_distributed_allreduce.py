...
def test_allreduce_different_op(ray_start_distributed_2_nodes,
    ...
    assert (results[0] == np.ones((10, ), dtype=np.float32) * 120).all()
    assert (results[1] == np.ones((10, ), dtype=np.float32) * 120).all()
    ...
    assert (results[0] == np.ones((10, ), dtype=np.float32) * 5).all()
    assert (results[1] == np.ones((10, ), dtype=np.float32) * 5).all()
