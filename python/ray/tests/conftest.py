...
def _restore_token_auth_env(_token_auth_env_baseline):
    ...
    A new local cluster started by ``ray.init`` enables token auth by default and
    sets ``RAY_AUTH_MODE=token`` in ``os.environ``, which persists in the driver
    process. Left in place, it makes later tests spawn auth-enabled clusters they
    don't expect (e.g. dashboard HTTP calls without a token get 401). Only the
    env vars are restored here; the token file and the C++ loader cache are left
    untouched so a still-running module- or session-scoped cluster keeps working.
    """
    yield
    ...
    # Also drop a token generated by a default-on ray.init() so the next test's
    # `ray start --head` doesn't auto-enable auth from it. Only when no cluster
    # is live: a module/session-scoped cluster keeps Ray initialized across its
    # tests and its processes still need the token file, so leave it in place.
