...
def test_try_create_redis_client(redis_password):
    ...
    with patch("ray._private.services", create_redis_client=create_mock), patch(
        "ray._private.services.find_redis_address",
        side_effect=[["address0", "address1"], [], ["address0"]],
    ):
        # Two redis addresses found
        assert client_server.try_create_redis_client(None, redis_password) is None
        create_mock.assert_not_called()
        # No redis addresses found
        assert client_server.try_create_redis_client(None, redis_password) is None
        create_mock.assert_not_called()
        # Exactly one redis address found
        assert client_server.try_create_redis_client(None, redis_password) == "address0"
