...
def _setup_client_and_exporter(controller):
    exporter = Exporter(MOCK_DASHBOARD_ID,
                        MOCK_DASHBOARD_ADDRESS,
                        controller)
    client = MetricsExportClient(MOCK_DASHBOARD_ADDRESS,
        controller, MOCK_DASHBOARD_ID, exporter)
...
def test_authentication(auth_request, mock_controller):
    auth_request.return_value = AuthResponse(
        dashboard_url=MOCK_DASHBOARD_ADDRESS,
...
@patch("ray.dashboard.dashboard.DashboardController")
@patch("ray.dashboard.metrics_exporter.api.authentication_request")
def test_start_exporting_metrics_without_authentication(auth_request, mock_controller, start):
    """
...
@patch("ray.dashboard.dashboard.DashboardController")
@patch("ray.dashboard.metrics_exporter.api.authentication_request")
def test_start_exporting_metrics_with_authentication(auth_request, mock_controller, start):
    """
...
def test_start_exporting_metrics_succeed(auth_request, mock_controller, start):
    ...
    assert result == True
    assert error == None
    assert client.is_exporting_started == True
    start.call_count == 1
