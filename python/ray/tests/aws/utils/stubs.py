...
def create_tag_specifications(resource_types, kvs):
    tag_specs = []
    for r in resource_types:
        tag_specs.append({
            ...
            "Tags": [{
                "Key": k,
                "Value": v
            } for kv in kvs for k, v in kv.items()]
        })
    return tag_specs


def describe_launch_template_versions_echo(ec2_client_stub,
                                           has_iam=True,
                                           has_key_name=True,
                                           has_tags=True,
                                           has_image=True,
    ...
    ec2_client_stub.add_response(
        "describe_launch_template_versions", service_response=response)
