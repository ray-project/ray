...
from ray.tests.aws.utils.constants import AUX_SUBNET, DEFAULT_SUBNET, \
    ...
    DEFAULT_SG_WITH_RULES_AUX_SUBNET, DEFAULT_SG_WITH_RULES, AUX_SG, DEFAULT_SG_WITH_RULES_AND_NAME
...
def test_create_sg_with_custom_inbound_rules_and_name(iam_client_stub, ec2_client_stub):
    ...
    stubs.create_sg_echo(ec2_client_stub, DEFAULT_SG_WITH_RULES_AND_NAME)
    # expect new head security group details to be retrieved after creation
    stubs.describe_sgs_on_vpc(
        ec2_client_stub,
        [DEFAULT_SUBNET["VpcId"]],
        [DEFAULT_SG_WITH_RULES_AND_NAME],
    )
    ...
    assert config["provider"]["security_group"]["GroupName"] == DEFAULT_SG_WITH_RULES_AND_NAME["GroupName"]
    assert config["provider"]["security_group"]["IpPermissions"] == DEFAULT_SG_WITH_RULES_AND_NAME["IpPermissions"]
