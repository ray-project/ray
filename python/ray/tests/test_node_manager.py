...
ray.get(f.options(num_cpus=99999999).remote())
  """
    ...
    def get_infeasible_queued():
        ...
        lmr = get_load_metrics_report(webui_url=ctx.address_info["webui_url"])
        print(lmr)

        return infeasible_queued

    def check_backlog(expect_infeasible) -> bool:
        infeasible_queued = get_infeasible_queued()
    ...
    wait_for_condition(
        check_backlog, timeout=10, retry_interval_ms=1000, expect_infeasible=True
    )

    os.kill(proc.pid, 9)

    wait_for_condition(
        check_backlog, timeout=10, retry_interval_ms=1000, expect_infeasible=False
    )
...
def test_kill_driver_clears_nested_infeasible(call_ray_start):
    driver = """
import ray
