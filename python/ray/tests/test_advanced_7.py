...
def test_wait_makes_object_local(ray_start_cluster_enabled):
    ...
    ray.get(x_id)
    assert ray._internal.worker.global_worker.core_worker.object_exists(x_id)
...
def test_future_resolution_skip_plasma(ray_start_cluster_enabled):
    ...
    def g(x):
        ...
        # in Plasma.
        assert ray._internal.worker.global_worker.core_worker.object_exists(
...
def test_task_output_inline_bytes_limit(ray_start_cluster_enabled):
    ...
    def sum():
        ...
        for i, ref in enumerate(numbers):
            result += ray.get(ref)
            inlined = ray._internal.worker.global_worker.core_worker.object_exists(
