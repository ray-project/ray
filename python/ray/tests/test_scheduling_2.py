...
def test_load_balancing_under_constrained_memory(
    ...
    @ray.remote
    def f(i, x):
        print(i, ray._internal.worker.global_worker.node.unique_id)
        time.sleep(0.1)
...
def test_default_scheduling_strategy(ray_start_cluster, connect_to_client):
    ...
    with connect_to_client_or_not(connect_to_client):
        ...
        def get_node_id_1():
            return ray._internal.worker.global_worker.current_node_id
        ...
        class Actor1:
            def get_node_ids(self):
                return [
                    ray._internal.worker.global_worker.current_node_id,
