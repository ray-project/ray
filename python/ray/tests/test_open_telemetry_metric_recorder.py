...
def test_record_histogram_aggregated_batch(
    ...
    """
    mock_meter = MagicMock()
    # Use NoOpHistogram which passes isinstance check, then wrap with MagicMock for call tracking
    ...
    #   2 observations in bucket 0 (midpoint 0.5)
    #   3 observations in bucket 1 (midpoint 5.5)
    #   0 observations in bucket 2 (midpoint 55.0)
    #   1 observation in bucket 3 (Inf bucket, midpoint 200.0)
    recorder.record_histogram_aggregated_batch(
        name="test_histogram",
        data_points=[
