...
class TestAbfssStorageAccountExtraction:
    ...
    def test_valid_abfss_urls(self):
        ...
        test_cases = [
            ...
            (
                "abfss://data@mystorageaccount.dfs.core.windows.net/path/file.zip",
                "mystorageaccount",
            ),
            (
                "abfss://logs@company123.dfs.core.windows.net/2024/data.zip",
                "company123",
            ),
            (
                "abfss://backup@test-storage.dfs.core.windows.net/backup.zip",
                "test-storage",
            ),
            (
                "abfss://ml-data@azurestorage001.dfs.core.windows.net/models/model.zip",
                "azurestorage001",
            ),
        ]
        ...
        for uri, expected_account in test_cases:
            ...
            ), f"URI should have correct hostname: {uri}"

            # Test the extraction logic
            extracted_account = parsed.hostname.split(".")[0]
            assert (
                extracted_account == expected_account
            ), f"Expected {expected_account}, got {extracted_account} for URI: {uri}"
    ...
    def test_abfss_url_parsing_edge_cases(self):
        ...
        for uri in test_cases:
            ...
            assert parsed.hostname.endswith(
                ".dfs.core.windows.net"
            ), f"Should have correct hostname: {uri}"
            ...
            assert (
                "." not in account_name
            ), f"Account name should not contain dots: {uri}"
    ...
    def test_abfss_handler_with_url_extraction(self):
            ...
            def _handle_abfss_protocol(cls):
                ...
                def open_file(uri, mode, *, transport_params=None):
                    ...
                    mock_file.read.return_value = (
                        f"Data from {azure_storage_account_name}".encode()
                    )
        ...
        for uri in test_uris:
            ...
            assert (
                content == f"Data from {expected_account}".encode()
            ), f"Should extract correct account name for {uri}"
    ...
    def test_abfss_url_format_validation(self):
        ...
        for uri in valid_formats:
            ...
            assert parsed.hostname.endswith(
                ".dfs.core.windows.net"
            ), f"Should end with .dfs.core.windows.net: {uri}"
            ...
            assert (
                len(netloc_parts) == 2
            ), f"Should have container@account format: {uri}"
