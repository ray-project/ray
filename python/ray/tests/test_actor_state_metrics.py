...
def test_get_all_actors_info(shutdown_only):
    ...
    actors_info = ray._private.state.actors()
    assert len(actors_info) == 2

    job_id_hex = ray.get_runtime_context().get_job_id()
    job_id = ray.JobID(hex_to_binary(job_id_hex))
    actors_info = ray._private.state.actors(job_id=job_id)
    assert len(actors_info) == 2
    actors_info = ray._private.state.actors(job_id=ray.JobID.from_int(100))
    assert len(actors_info) == 0

    # To filter actors by state
    actor_3 = Actor.remote()
    wait_for_condition(
        lambda: len(ray._private.state.actors(actor_state_name="PENDING_CREATION")) == 1
    )
    assert (
        actor_3._actor_id.hex()
        in ray._private.state.actors(actor_state_name="PENDING_CREATION").keys()
    )

    with pytest.raises(ValueError, match="not a valid actor state name"):
        actors_info = ray._private.state.actors(actor_state_name="UNKONWN_STATE")
