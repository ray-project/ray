...
def test_ray_processes_can_match_agents_on_windows():
    """Verify that RAY_PROCESSES contains entries that can match the dashboard
    agent and runtime_env agent on Windows, where setproctitle does not change
    the process cmdline visible to psutil.
    ...
    assert win32_ray_processes[-1][0] == "gcs_server"

    # On Windows, setproctitle only creates a named event object — it does
    ...
    for agent_name, cmdline in simulated_agent_cmdlines.items():
        cmdline_str = subprocess.list2cmdline(cmdline)
        matched = any(
            keyword in ("python.exe" if filter_by_cmd else cmdline_str)
            for keyword, filter_by_cmd in win32_ray_processes
        )
        assert matched, (
            f"No RAY_PROCESSES entry matches the {agent_name} cmdline on "
            f"Windows: {cmdline_str}"
        )
