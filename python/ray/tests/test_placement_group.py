...
def test_placement_group_pack(ray_start_cluster_enabled, connect_to_client):
...
@pytest.mark.parametrize("connect_to_client", [False, True])
def test_placement_group_strict_pack(ray_start_cluster_enabled, connect_to_client):
    @ray.remote(num_cpus=2)
    class Actor(object):
        def __init__(self):
            self.n = 0
...
@pytest.mark.parametrize("connect_to_client", [False, True])
def test_placement_group_spread(ray_start_cluster_enabled, connect_to_client):
    @ray.remote
    class Actor(object):
        def __init__(self):
            self.n = 0
