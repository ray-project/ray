...
def test_memory_pressure_kill_worker(shutdown_only):
    ...
    ray.init(
        ...
        _system_config={
            ...
            "node_high_memory_monitor_min_interval_ms": node_high_memory_monitor_min_interval_ms,
        },
    )
    ...
    ray.get(leaker.allocate.remote(bytes_to_alloc, node_high_memory_monitor_min_interval_ms * 3))
...
def test_memory_pressure_kill_newest_worker(shutdown_only):
    ...
    with pytest.raises(ray.exceptions.RayActorError) as _:
        ray.get(leaker2.allocate.remote(bytes_to_alloc, node_high_memory_monitor_min_interval_ms * 3))
...
def test_memory_pressure_above_single_actor(shutdown_only):
    ...
    node_high_memory_monitor_min_interval_ms = 200
    ...
    num_tasks = 5
    result_refs = []
    result_refs.append(no_retry.remote(bytes_used_per_task, 5, node_high_memory_monitor_min_interval_ms / 1000))
    result_refs.extend([inf_retry.remote(bytes_used_per_task, 5, node_high_memory_monitor_min_interval_ms / 1000) for _ in range(num_tasks-1)])
