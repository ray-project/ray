...
def test_fetch_gpu_object_to_driver(ray_start_regular):
    ...
    result = ray.get(ref, _get_tensor_options=GetTensorOptions(use_object_store=True))
...
def test_dynamic_tensor_transport_via_options(
    ...
    if enable_tensor_transport:
        ...
        tensor = ray.get(
            ref, _get_tensor_options=GetTensorOptions(use_object_store=True)
        )
        result = ray.get(
            receiver.double.remote(ref),
            _get_tensor_options=GetTensorOptions(use_object_store=True),
        )
        assert result == pytest.approx(tensor * 2)
    else:
...
def test_send_back_and_dst_warning(ray_start_regular):
    ...
    with pytest.warns(UserWarning, match=warning_message):
        ...
        ray.get([t1, t2], _get_tensor_options=GetTensorOptions(use_object_store=True))
    ...
    ray.get(t3, _get_tensor_options=GetTensorOptions(use_object_store=True))
