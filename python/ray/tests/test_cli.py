...
def _gcs_addresses():
    """Force-refresh and return the set of GCS addresses on the host."""
...
def _gcs_addresses_contains(addresses, target) -> bool:
...
def test_ray_stop_address_targets_specific_multi_node_cluster(
    ...
    wait_for_condition(
        lambda: _gcs_addresses_contains(_gcs_addresses(), gcs_a)
        and _gcs_addresses_contains(_gcs_addresses(), gcs_b),
        timeout=60,
    )
    ...
    try:
        ...
        wait_for_condition(
            lambda: not _gcs_addresses_contains(_gcs_addresses(), gcs_a), timeout=60
        )
        _fail_if_false(
            _gcs_addresses_contains(_gcs_addresses(), gcs_b),
            stop_result.output,
        ...
        wait_for_condition(
            lambda: not _gcs_addresses_contains(_gcs_addresses(), gcs_b), timeout=60
        )
...
def test_ray_stop_address_targets_specific_multi_node_cluster_dynamic_port(
    configure_lang, monkeypatch, tmp_path, cleanup_ray
):
    """Same isolation contract as the fixed-port variant but exercises GCS
    dynamic port binding (--port=0). This covers Strategy 5 of
    _extract_gcs_address_from_cmdline, which reads the actual bound port from
    <session_dir>/ports/<node_id>.<port_name> via _read_persisted_gcs_server_port.

    Because --port=0 means the GCS address is unknown until the head node has
    actually bound, each cluster is spawned in two phases: head first, then
    capture the new entry from find_gcs_addresses() as that cluster's GCS
    address, then start the worker against it.
    """
    monkeypatch.setenv("RAY_USAGE_STATS_CONFIG_PATH", str(tmp_path / "config.json"))
    runner = CliRunner(env={"RAY_USAGE_STATS_PROMPT_ENABLED": "0"})
    ...
    # Phase 1: spawn cluster A head, capture its dynamically bound GCS address.
    before_a = _gcs_addresses()
    _die_on_error(runner.invoke(scripts.start, cluster_a_head))
    wait_for_condition(
        lambda: len(_gcs_addresses() - before_a) == 1,
        timeout=60,
    )
    gcs_a = next(iter(_gcs_addresses() - before_a))
    ...
    _die_on_error(runner.invoke(scripts.start, cluster_a_worker))

    # Phase 2: spawn cluster B head, capture its dynamically bound GCS address.
    before_b = _gcs_addresses()
    _die_on_error(runner.invoke(scripts.start, cluster_b_head))
    wait_for_condition(
        lambda: len(_gcs_addresses() - before_b) == 1,
        timeout=60,
    )
    gcs_b = next(iter(_gcs_addresses() - before_b))
    ...
    _die_on_error(runner.invoke(scripts.start, cluster_b_worker))
