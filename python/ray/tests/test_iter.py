...
def test_from_items(ray_start_regular_shared):
    ...
    assert repr(it) == "ParIterator[from_items[int, 4, shards=2]]"
...
def test_gather_sync(ray_start_regular_shared):
    it = from_range(4)
    it = it.gather_sync()
    assert (repr(it) == "LocalIterator[ParIterator[from_range[4, shards=2]]"
            ".gather_sync()]")
...
def test_batch_across_shards(ray_start_regular_shared):
    ...
    assert (repr(it) == "LocalIterator[ParIterator[from_iterators[shards=2]]"
            ".batch_across_shards()]")
...
def test_union(ray_start_regular_shared):
    ...
    it = it1.union(it2)
    assert (repr(it) == "ParIterator[ParallelUnion[ParIterator["
            "from_items[str, 3, shards=1]], ParIterator["
...
def test_union_local_async(ray_start_regular_shared):
    ...
    assert (repr(it) == "LocalIterator[LocalUnion[LocalIterator["
            "ParIterator[from_iterators[shards=1].for_each()]"
            ".gather_async()], LocalIterator[ParIterator["
...
def test_serialization(ray_start_regular_shared):
    ...
    assert (repr(it) == "LocalIterator[ParIterator["
