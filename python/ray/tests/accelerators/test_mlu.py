...
@patch("glob.glob")
def test_autodetect_num_mlus(mock_glob):
    with patch.dict(sys.modules):
        sys.modules["torch_mlu"] = None
        mock_glob.return_value = [f"/dev/cambricon_dev{i}" for i in range(64)]
        assert Accelerator.get_current_node_num_accelerators() == 64


@patch("glob.glob")
def test_autodetect_num_mlus_without_devices(mock_glob):
    with patch.dict(sys.modules):
        sys.modules["torch_mlu"] = None
...
def test_cambricon_mlu_accelerator_manager_api():
...
def test_visible_cambricon_mlu_type(monkeypatch, shutdown_only):
    with patch.object(
...
def test_visible_cambricon_mlu_ids(monkeypatch, shutdown_only):
    with patch.dict(sys.modules):
        sys.modules["torch_mlu"] = __import__("mock_mlu")
...
def test_torch_mlu_api_function(shutdown_only):
    with patch.dict(sys.modules):
        sys.modules["torch_mlu"] = __import__("mock_mlu")
