...
def test_tpu_pod_resources():
    ...
    env_vars = {
        ray_constants.RESOURCES_ENVIRONMENT_VARIABLE: {
            "TPU": 4,
            f"tpu-{_MOCK_ACCELERATOR_TYPE}": 1,
        },
    }
    def test_command_run(self, environment_variables, **kwargs):
        ...
        assert _MOCK_TPU_NAME in resources
        if self._worker_id == 0:
            assert f"tpu-{_MOCK_ACCELERATOR_TYPE}" in resources
        else:
            assert f"tpu-{_MOCK_ACCELERATOR_TYPE}" not in resources
