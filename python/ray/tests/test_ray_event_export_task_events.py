...
def get_job_ids_and_driver_script_task_ids_from_events(events: json) -> tuple[str, str]:
...
def check_task_event_base_fields(event: json):
...
class TestNormalTaskEvents:
    ...
    def test_normal_task_succeed(
        self, ray_start_cluster_head_with_env_vars, httpserver
    ):
        script = """
import ray
ray.init()
...
ray.get(normal_task.remote())
    """
        ...
        def validate_events(events):
            ...
            for event in events:
                if event["eventType"] == "TASK_DEFINITION_EVENT":
                    check_task_event_base_fields(event)
                    ...
                    if event["taskDefinitionEvent"]["taskType"] == "DRIVER_TASK":
                        ...
                        assert event["taskDefinitionEvent"]["taskName"] == "normal_task"
                        assert event["taskDefinitionEvent"]["requiredResources"] == {
                            "CPU": 1.0
                        }
                        assert (
...
try:
    ...
    pass
            ...
            for event in events:
                if event["eventType"] == "TASK_DEFINITION_EVENT":
                    ...
                    if event["taskDefinitionEvent"]["taskType"] == "DRIVER_TASK":
                        ...
                        assert event["taskDefinitionEvent"]["taskName"] == "node-killed"
                        assert event["taskDefinitionEvent"]["requiredResources"] == {
                            "CPU": 2.0
                        }
                        assert (
        ...
        get_and_validate_events(httpserver, validate_task_killed)
            ...
            for event in events:
                if event["eventType"] == "TASK_DEFINITION_EVENT":
                    ...
                    if event["taskDefinitionEvent"]["taskType"] == "DRIVER_TASK":
                        ...
                        actor_creation_task_id = event["taskDefinitionEvent"]["taskId"]
                        ...
                        assert (
                            event["taskDefinitionEvent"]["taskName"] == "Actor.__init__"
                        )
                        assert event["taskDefinitionEvent"]["requiredResources"] == {
                            "CPU": 1.0
                        }
                    ...
                    assert event["actorTaskDefinitionEvent"]["requiredResources"] == {}
                    assert (
