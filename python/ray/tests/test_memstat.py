...
def test_task_status(ray_start_regular):
    ...
    wait_for_condition(lambda: count(filtered_summary(), RUNNING) == 2)
    ...
    wait_for_condition(lambda: count(filtered_summary(), RUNNING) == 4)
    ...
    wait_for_condition(lambda: count(filtered_summary(), WAITING_FOR_DEPENDENCIES) == 0)
    wait_for_condition(lambda: count(filtered_summary(), RUNNING) == 0)
    wait_for_condition(lambda: count(filtered_summary(), SCHEDULED) == 1)
