...
def test_get_instances_packing_heuristic():
    assert get_instances_for(TYPES_A, {}, 9999, [{
        "GPU": 8
    }]) == [("p2.8xlarge", 1)]
    assert get_instances_for(TYPES_A, {}, 9999, [{
        "GPU": 1
    ...
    assert get_instances_for(TYPES_A, {}, 9999, [{
        "CPU": 16
    }] * 5) == [("m4.16xlarge", 1), ("m4.4xlarge", 1)]
    assert get_instances_for(TYPES_A, {}, 9999, [{
        "CPU": 8
    }] * 10) == [("m4.16xlarge", 1), ("m4.4xlarge", 1)]
    assert get_instances_for(TYPES_A, {}, 9999, [{
        "CPU": 1
    }] * 100) == [("m4.16xlarge", 1), ("m4.4xlarge", 2), ("m4.large", 2)]
    assert get_instances_for(TYPES_A, {}, 9999, [{
        "GPU": 1
    }] + ([{
        "CPU": 1
    }] * 64)) == [("m4.16xlarge", 1), ("p2.xlarge", 1)]
    assert get_instances_for(TYPES_A, {}, 9999, ([{
        "GPU": 1
    }] * 8) + ([{
        "CPU": 1
    }] * 64)) == [("m4.16xlarge", 1), ("p2.8xlarge", 1)]
