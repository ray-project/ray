...
class RayOnSparkACCClusterTestBase(RayOnSparkCPUClusterTestBase, ABC):
    num_total_accs = None
    num_accs_per_spark_task = None

    def test_acc_allocation(self):
        for num_worker_nodes, num_cpus_worker_node, num_accs_worker_node in [
            (
                self.max_spark_tasks // 2,
                self.num_cpus_per_spark_task,
                self.num_accs_per_spark_task,
            ),
            (
            ...
            (
                ...
                self.num_accs_per_spark_task * 2,
            ),
            (
                self.max_spark_tasks // 2,
                self.num_cpus_per_spark_task,
            ...
            with _setup_ray_cluster(
                ...
                num_ray_task_slots = self.max_spark_tasks // (
                    num_accs_worker_node // self.num_accs_per_spark_task
                )
                (
                    mem_worker_node,
                ...
                for worker_res in worker_res_list:
                    ...
                    assert worker_res["ACC"] == num_accs_worker_node
                ...
                @ray.remote(
                    num_cpus=num_cpus_worker_node, num_accs=num_accs_worker_node
                )
                def f(_):
                    ...
                    return [
                        int(acc_id)
                        for acc_id in os.environ["CUDA_VISIBLE_DEVICES"].split(",")
                    ]

                futures = [f.remote(i) for i in range(num_worker_nodes)]
                results = ray.get(futures)
                merged_results = functools.reduce(lambda x, y: x + y, results)
                # Test all ray tasks are assigned with different ACCs.
                assert sorted(merged_results) == list(
                    range(num_accs_worker_node * num_worker_nodes)
                )

    def test_acc_autoscaling(self):
        for num_worker_nodes, num_cpus_worker_node, num_accs_worker_node in [
            (
                self.max_spark_tasks,
                self.num_cpus_per_spark_task,
            ...
            with _setup_ray_cluster(
                ...
                assert len(worker_res_list) == num_worker_nodes and all(
                    ...
                    and worker_res_list[i]["ACC"] == num_accs_worker_node
...
class TestBasicSparkACCCluster(RayOnSparkACCClusterTestBase):
    @classmethod
    def setup_class(cls):
        cls.num_total_cpus = 2
        cls.num_total_accs = 2
        cls.num_cpus_per_spark_task = 1
        cls.num_accs_per_spark_task = 1
        cls.max_spark_tasks = 2
        acc_discovery_script_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "discover_2_acc.sh"
        )
        os.environ["SPARK_WORKER_CORES"] = "4"
        cls.spark = (
            SparkSession.builder.master("local-cluster[1, 2, 1024]")
            .config("spark.task.cpus", "1")
            .config("spark.task.resource.acc.amount", "1")
            .config("spark.executor.cores", "2")
            .config("spark.worker.resource.acc.amount", "2")
            .config("spark.executor.resource.acc.amount", "2")
            ...
            .config("spark.executorEnv.RAY_ON_SPARK_WORKER_ACC_NUM", "2")
            .getOrCreate()
        )
