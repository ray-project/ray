...
MY_PLUGIN_CLASS_PATH = "ray.tests.test_runtime_env.PluginToTestURI"
MY_PLUGIN_NAME = "PluginToTestURI"


class PluginToTestURI(RuntimeEnvPlugin):

    name = MY_PLUGIN_NAME

    PLUGIN_WORKING_DIR = "PluginToTestURI"

    def get_uris(self, runtime_env):
        dirname = runtime_env[MY_PLUGIN_NAME]["DIRNAME"]
        return [os.path.join(dirname, self.PLUGIN_WORKING_DIR)]

    async def create(self, uri, runtime_env, context, logger) -> float:
        dirname = runtime_env[MY_PLUGIN_NAME]["DIRNAME"]
        function_name = runtime_env[MY_PLUGIN_NAME]["function_name"]
        assert uri == os.path.join(dirname, self.PLUGIN_WORKING_DIR)
        assert not os.path.exists(uri)
        logger.info(f"in create: {function_name}")
        # touch the file
        with open(uri, "w") as f:
            f.write("test")
        # Make sure the second runtime env will try to create current
        # URI by PluginToTestURI.
...
def test_run_same_uri_in_diff_runtime_env(
    set_runtime_env_retry_times, set_runtime_env_plugins, ray_start_regular
):
    """This case is to test that the same plugin in different runtime_env will access
    the same directory when processing the same URI, resulting in conflicts. For example
    `PipPlugin`, will create virtualenv in a directory generated by URI, if two
    different runtime_env have the same pip_uri, there will raise an exception.
    """
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        f1 = f.options(
            ...
            runtime_env={
                MY_PLUGIN_NAME: {
                    "DIRNAME": tmpdir,
                    # just make the two normal tasks have
                    # different runtime_env
                    "function_name": "f1",
                }
            },
        ).remote()
        ...
        assert ray.get(f2) and ray.get(f1)
