...
@pytest.mark.parametrize("_setup_cluster_for_test", ["recommended"], indirect=True)
def test_recommended_cardinality(_setup_cluster_for_test):
    """
    Test that the ray_tasks and ray_actors metric are reported without the WorkerId tag
    """
    TEST_TIMEOUT_S = 30
...
@pytest.mark.skipif(prometheus_client is None, reason="Prometheus not installed")
@pytest.mark.parametrize("_setup_cluster_for_test", ["legacy"], indirect=True)
def test_legacy_cardinality(_setup_cluster_for_test):
    """
    Test that the ray_tasks and ray_actors metric are reported without the WorkerId tag
    """
    TEST_TIMEOUT_S = 30
