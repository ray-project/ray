...
def test_get_task_function(shutdown_only):
    ...
    expected_task_function = __name__ + ".get_task_function_for_normal_task"
    task_function = ray.get(get_task_function_for_normal_task.remote())
    assert (
        task_function == expected_task_function
    ), f"Check normal task function failed. expected={expected_task_function}, \
        actual={task_function}"
    ...
    expected_task_function = __name__ + ".Actor.get_task_function_for_actor_task"
    actor = Actor.remote()
    task_function = ray.get(actor.get_task_function_for_actor_task.remote())
    assert (
        task_function == expected_task_function
    ), f"Check actor task function failed. expected={expected_task_function}, \
        actual={task_function}"
    ...
    expected_task_function = (
        __name__ + ".ThreadedActor.get_task_function_for_threaded_actor_task"
    )
    threaded_actor = ThreadedActor.options(max_concurrency=2).remote()
    ...
    expected_task_function = (
        __name__
        + ".test_get_task_function.<locals>.AsyncActor.\
            get_task_function_for_async_actor_task"
    )
    async_actor = AsyncActor.remote()
