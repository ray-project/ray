...
def test_error_isolation(call_ray_start):
    ...
    assert len(ray.errors(all_jobs=False)) == 0
    ...
    while len(ray.errors(all_jobs=False)) != 1:
        time.sleep(0.1)
        print("Waiting for error to appear.")

    # Make sure we got the error.
    assert len(ray.errors(all_jobs=False)) == 1
    assert error_string1 in ray.errors(all_jobs=False)[0]["message"]
...
while len(ray.errors(all_jobs=False)) != 1:
    print(len(ray.errors(all_jobs=False)))
    time.sleep(0.1)
assert len(ray.errors(all_jobs=False)) == 1

assert "{}" in ray.errors(all_jobs=False)[0]["message"]
