...
def test_remove_placement_group(ray_start_cluster_enabled, connect_to_client):
...
def test_remove_placement_group_worker_startup_slowly(ray_start_cluster_enabled):
    cluster = ray_start_cluster_enabled
    cluster.add_node(num_cpus=4)
    ray.init(address=cluster.address)
...
@pytest.mark.parametrize("connect_to_client", [False, True])
def test_remove_pending_placement_group(ray_start_cluster_enabled, connect_to_client):
    cluster = ray_start_cluster_enabled
    cluster.add_node(num_cpus=4)
    ray.init(address=cluster.address)
...
@pytest.mark.parametrize("connect_to_client", [False, True])
def test_placement_group_table(ray_start_cluster_enabled, connect_to_client):
    @ray.remote(num_cpus=2)
    class Actor(object):
        def __init__(self):
            self.n = 0
...
def test_placement_group_stats(ray_start_cluster_enabled):
    cluster = ray_start_cluster_enabled
    num_nodes = 1
    for _ in range(num_nodes):
...
@pytest.mark.parametrize("connect_to_client", [False, True])
def test_cuda_visible_devices(ray_start_cluster_enabled, connect_to_client):
