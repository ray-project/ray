...
def test_ray_task_cancel_and_retry_race_condition(ray_start_cluster):
    ...
    def create_big_object() -> np.ndarray:
        return np.zeros(1024 * 1000)

    @ray.remote(num_cpus=2)
    def get_big_object(object_refs: List[ObjectRef[np.ndarray]]) -> np.ndarray:
    ...
    big_object_ref = create_big_object.remote()
    ray.wait([big_object_ref], fetch_local=False)
    ray.cancel(big_object_ref)
    ...
    with pytest.raises(TaskCancelledError):
        ray.get(get_big_object.remote([big_object_ref]))
