...
def test_task_cancel_and_failure_race_condition(ray_start_cluster):
    ...
    cluster.add_node(
        num_cpus=0,
        resources={"head": 1},
        # This delays allows the following method sequences to happen, which then
        # triggers a logical race condition this test is trying to test.
        # NormalTaskSubmitter::PushNormalTask-callback -->
        # NormalTaskSubmitter::CancelTask -->
        # NormalTaskSubmitter::HandleGetTaskFailureCause
        _system_config={
            "testing_asio_delay_us": (
                "NodeManagerService.grpc_client.GetTaskFailureCause=1000000:1000000"
            ),
        },
    )
    ray.init(address=cluster.address)
    # Add one worker node.
    worker_node = cluster.add_node(num_cpus=2, resources={"worker": 1})
    ...
    def node_preemption():
        time.sleep(0.5)
        cluster.remove_node(worker_node)  # Trigger NormalTaskSubmitter::PushNormalTask-callback
        ray.cancel(task_to_cancel_ref)  # Trigger NormalTaskSubmitter::CancelTask

    preemption_thread = threading.Thread(target=node_preemption)
    preemption_thread.start()
    # Sleep for 2 second to ensure NodeManagerService.grpc_client.GetTaskFailureCause
