...
def run_basic_workload():
                 append_path=ray.ray_constants.DEFAULT_OBJECT_PREFIX):	    """Run the workload that requires spilling."""
    # append_path is used because the file based spilling will append	    arr = np.random.rand(5 * 1024 * 1024)  # 40 MB
    # new directory path.	    refs = []
    num_files = 0	    refs.append([ray.put(arr) for _ in range(2)])
    temp_folder = temp_folder / append_path	    ray.get(ray.put(arr))
