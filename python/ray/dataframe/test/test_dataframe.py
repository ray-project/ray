...
from ray.dataframe.utils import (to_pandas, from_pandas)
...
def test_fillna_sanity(num_partitions=2):
    ...
    ray_df = from_pandas(df,
                         num_partitions).fillna(value={'Date': df['Date2']})
    assert ray_df_equals_pandas(ray_df, result)
...
def test_max(ray_df, pandas_df):
    ...
    assert (ray_series_equals_pandas(
        ray_df.max(axis=1), pandas_df.max(axis=1)))
...
def test_min(ray_df, pandas_df):
    ...
    assert (ray_series_equals_pandas(
        ray_df.min(axis=1), pandas_df.min(axis=1)))
