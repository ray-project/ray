...
def concat(
    ...
    try:
        type_check = next(
            ...
            not isinstance(obj,
                           (pandas.Series, pandas.DataFrame, DataFrame))
        )
    ...
    if all_series:
        return pandas.concat(
            objs, axis, join, join_axes, ignore_index, keys, levels, names,
    ...
    if join not in ['inner', 'outer']:
        raise ValueError(
            "Only can inner (intersect) or outer (union) join the"
            " other axis"
        )
    ...
    objs = [
        DataFrame(obj) if isinstance(obj,
                                     (pandas.DataFrame, pandas.Series)) else obj
        for obj in objs
    ]
