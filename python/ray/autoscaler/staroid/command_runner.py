...
class StaroidCommandRunner(KubernetesCommandRunner):
    def __init__(self, log_prefix, namespace, node_id, auth_config,
                process_runner, kube_api_server = None):

        super(StaroidCommandRunner, self).__init__(log_prefix, namespace, node_id, auth_config,
            process_runner)

        if kube_api_server != None:
    ...
    def _rewrite_target_home_dir(self, target):
        # Staroid forces containers to run non-root permission.
        # Ray docker image does not have a support for non-root user at the moment.
    ...
    def run_cp_up(self, source, target):
        target = self._rewrite_target_home_dir(target)
        super().run_cp_up(source, target, options)
    ...
    def run_cp_down(self, source, target):
        target = self._rewrite_target_home_dir(target)
        super().run_cp_down(source, target, options)
