...
# Not doing it as of now since there will be name conflicts with the existing
# functions in utils.py , i.e. ResourceRequestByCount
...
def resource_requests_by_count(
    ...
    for serialized_request, count in resource_requests_by_count.items():
        ...
        results.append(ResourceRequestByCount(request=request, count=count))
...
class ScheduleContext:
    ...
    def from_schedule_request(
        ...
        for node in req.pending_instances:
            node_config = req.schedule_config.node_type_configs[node.ray_node_type_name]
            assert node_config is not None, f"Node config for {node} not found."
            nodes.append(
...
class SimpleResourceScheduler(IResourceScheduler):
    ...
    def schedule_resource_bundles(
        ...
        for node_type, count in ctx.get_cluster_shape().items():
            if count > cur_shape.get(node_type, 0):
                to_launch[node_type] = count - cur_shape.get(node_type, 0)
