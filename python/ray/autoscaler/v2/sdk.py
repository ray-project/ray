...
def request_cluster_resources(to_request: List[dict], timeout : int = 10):
    ...
    Args:
        to_request (List[dict]): A list of resource bundles to request the cluster to have.
            Each bundle is a dict of resource name to resource quantity, e.g:
            [{"CPU": 1}, {"GPU": 1}].
        timeout (int): Timeout in seconds for the request to be timeout

    """

    # NOTE: We could also use a GCS python client. However, current GCS rpc client expects
    # GcsStatus as part of the reply, which is a protocol internal to Ray. So we use the 
    # rpc stub directly to avoid that dependency.
    stub = _autoscaler_state_service_stub()
    min_bundles = [autoscaler_pb2.ResourceRequest(resources_bundle=bundle) for bundle in to_request]
