...
def test_ippr_apply_provider_suggestions():
    scheduler = ResourceDemandScheduler(event_logger)
    ...
    ippr_status = IPPRStatus(
        ...
        current_cpu=1,
        current_memory=1 * 1024 * 1024 * 1024,
        desired_cpu=4,  # original desired higher than suggestion
        desired_memory=8 * 1024 * 1024 * 1024,
        suggested_max_cpu=2.0,
        suggested_max_memory=2 * 1024 * 1024 * 1024,
    )

    request = sched_request(
        node_type_configs=node_type_configs,
        resource_requests=[ResourceRequestUtil.make({"CPU": 3})],
        instances=[instance],
        ippr_specs=ippr_specs,
        ippr_statuses={"pod-1": ippr_status},
    )
    ...
    reply = scheduler.schedule(request)
    # Should produce an IPPR action applying provider suggestions (not max caps)
    assert reply.to_ippr[0].cloud_instance_id == "pod-1"
    assert reply.to_ippr[0].desired_cpu == 2.0
    assert reply.to_ippr[0].desired_memory == 2 * 1024 * 1024 * 1024
    # Should launch a new node for the 3-CPU request
    to_launch, _ = _launch_and_terminate(reply)
    assert to_launch == {"type_1": 1}
    assert len(reply.to_ippr) == 1


def test_ippr_revert_error_and_timeout_produce_actions():
    scheduler = ResourceDemandScheduler(event_logger)
    ...
    # Node A: provider suggested limits
    ...
    # Node B: last resize errored -> revert to current
    ...
    # Node C: last resize timed out -> revert to current
    instance_c = make_autoscaler_instance(
        ray_node=NodeState(
            ray_node_type_name="type_1",
            available_resources={"CPU": 1},
            total_resources={"CPU": 1},
    ...
    ippr_specs = IPPRSpecs(
        groups={
            "type_1": IPPRGroupSpec(
                ...
                min_memory=1 * 1024 * 1024 * 1024,
                max_memory=8 * 1024 * 1024 * 1024,
                resize_timeout=1,  # small timeout to trigger timeout case
            )
        }
    )
    ...
    status_a = IPPRStatus(
        ...
        desired_cpu=1,
        desired_memory=1 * 1024 * 1024 * 1024,
        suggested_max_cpu=3.0,
        suggested_max_memory=3 * 1024 * 1024 * 1024,
    )
    status_b = IPPRStatus(
        cloud_instance_id="pod-b",
        spec=ippr_specs.groups["type_1"],
    ...
    status_c = IPPRStatus(
        ...
        desired_cpu=2,
        desired_memory=2 * 1024 * 1024 * 1024,
        resized_at=int(time.time()) - 10,  # already timed out
        resized_status="inprogress",
    )
    ...
    reply = scheduler.schedule(request)
    # Should produce three IPPR actions: suggestion (A), revert error (B), revert timeout (C)
    to_launch, _ = _launch_and_terminate(reply)
    assert to_launch == {"type_1": 1}
    assert len(reply.to_ippr) == 3
    actions = {a.cloud_instance_id: a for a in reply.to_ippr}

    # Suggestion applied as-is, not overridden to max caps
    assert actions["pod-a"].desired_cpu == 3.0
    assert actions["pod-a"].desired_memory == 3 * 1024 * 1024 * 1024

    # Error revert to current
    assert actions["pod-b"].desired_cpu == 1
    assert actions["pod-b"].desired_memory == 1 * 1024 * 1024 * 1024

    # Timeout revert to current
    assert actions["pod-c"].desired_cpu == 1
    assert actions["pod-c"].desired_memory == 1 * 1024 * 1024 * 1024
