...
def test_autoscaler_instance():
    ...
    assert not i.is_valid()[0], "Empty instance should be invalid"

    i = AutoscalerInstance(im_instance=Instance(status=Instance.QUEUED))
    assert i.is_valid()[0], "Instance with only im_instance should be valid"

    i = AutoscalerInstance(
        ray_node=NodeState(status=NodeStatus.RUNNING, instance_id="i-123"),
        cloud_instance_id="i-123",
    )
    assert i.is_valid()[0], i.is_valid()[1]

    i = AutoscalerInstance(
        ray_node=NodeState(status=NodeStatus.RUNNING),
    )
    assert not i.is_valid()[0], "Missing cloud node id."
    ...
    assert not i.is_valid()[0], "cloud instance id should match"

    i = AutoscalerInstance(
        im_instance=Instance(status=Instance.QUEUED, cloud_instance_id="i-123"),
        cloud_instance_id="i-123",
    )
    assert not i.is_valid()[0], "cloud instance id is not possible with queued state"

    i = AutoscalerInstance(
        im_instance=Instance(status=Instance.ALLOCATED, cloud_instance_id="i-123"),
    )
    assert not i.is_valid()[0], "cloud instance id should also be set"
