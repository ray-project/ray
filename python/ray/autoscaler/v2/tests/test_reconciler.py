...
class MockScheduler(IResourceScheduler):
    def __init__(
        self, to_launch=None, to_terminate=None, cluster_idle_termination_expired=False
    ):
        ...
        self.cluster_idle_termination_expired = cluster_idle_termination_expired

    def schedule(self, req):
        return SchedulingReply(
            to_launch=self.to_launch,
            to_terminate=self.to_terminate,
            cluster_idle_termination_expired=self.cluster_idle_termination_expired,
        )
...
class TestClusterIdleDriverMask:
    ...
    def test_no_op_when_predicate_not_fired(self):
        reply = SchedulingReply(cluster_idle_termination_expired=False)
    ...
    def test_preserved_when_no_active_drivers(self):
        reply = SchedulingReply(cluster_idle_termination_expired=True)
        Reconciler._apply_cluster_idle_driver_mask(
...
class TestClusterIdleDispatch:
    ...
    def test_annotation_dispatched_when_predicate_fires(self, setup):
        from ray.autoscaler.v2.instance_manager.cloud_providers.kuberay.cloud_provider import (  # noqa: E501
        ...
        gcs_client = MagicMock()
        gcs_client.get_all_job_info.return_value = {}  # no alive drivers

        Reconciler.reconcile(
            instance_manager,
            scheduler=MockScheduler(cluster_idle_termination_expired=True),
    ...
    def test_annotation_not_dispatched_when_predicate_silent(self, setup):
        instance_manager, instance_storage, _, cloud_resource_monitor = setup
        provider, mock_client = self._setup_kuberay_provider()

        Reconciler.reconcile(
            instance_manager,
            scheduler=MockScheduler(cluster_idle_termination_expired=False),
            cloud_provider=provider,
            cloud_resource_monitor=cloud_resource_monitor,
            ray_cluster_resource_state=ClusterResourceState(),
            non_terminated_cloud_instances={},
    ...
    def test_annotation_not_dispatched_when_layer_3_masks(self, setup):
        instance_manager, instance_storage, _, cloud_resource_monitor = setup
        provider, mock_client = self._setup_kuberay_provider()

        # GCS reports a live driver → mask must downgrade predicate to False.
        class _Job:
            def __init__(self, dead):
                self.is_dead = dead
