...
class AutoscalerMonitor:
    ...
    def __init__(
        ...
        if log_dir:
            try:
                self.ray_event_logger = get_event_logger(
                    RayEvent.SourceType.AUTOSCALER, log_dir
                )
            except Exception:
                self.ray_event_logger = None
        else:
            self.ray_event_logger = None
        ...
        logger.info("AutoscalerMonitor: Started")
        ...
        while True:
            try:
                ...
                if self.retry_on_failure:
                    logger.exception("AutoscalerMonitor: Execution exception. Trying again...")
                else:
                    raise
    ...
    def destroy_autoscaler_workers(self):
        ...
        if self.autoscaling_config is None:
            ...
            logger.error("AutoscalerMonitor: Cleanup failed due to lack of autoscaler config.")
            return

        logger.info("AutoscalerMonitor: Exception caught. Taking down workers...")
        clean = False
        while not clean:
            try:
                teardown_cluster(
                ...
                clean = True
                logger.info("AutoscalerMonitor: Workers taken down.")
            except Exception:
                logger.error("AutoscalerMonitor: Cleanup exception. Trying again...")
                time.sleep(2)
    ...
    def _signal_handler(self, sig, frame):
        try:
            ...
            logger.exception("AutoscalerMonitor: Failure in signal handler.")
        sys.exit(sig + 128)
