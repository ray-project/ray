...
class Monitor:
    ...
    def emit_metrics(self, load_metrics_summary, autoscaler_summary, node_types):
        ...
        for resource_name in ["CPU", "GPU", "TPU", "HPU"]:
    ...
    def update_event_summary(self):
        ...
        To avoid log spam, only cluster size changes (CPU, GPU, TPU or HPU count change)
        ...
        if not self.readonly_config and avail_resources != self.last_avail_resources:
            self.event_summarizer.add(
                "Resized to {}.",  # e.g., Resized to 100 CPUs, 4 GPUs, 4 TPUs, 4 HPUs.
