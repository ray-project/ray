...
class AzureNodeProvider(NodeProvider):
    ...
    def _create_node(self, node_config, tags, count):
        ...
        if requested_zones and available_zones == []:
            raise ValueError(
        ...
        # Create nodes with zone distribution
        for i in range(count):
            ...
            template_params[
                "vmCount"
            ] = 1  # Create one VM at a time for zone distribution
            ...
            if zones_explicitly_disabled:
                ...
                logger.info(
                    f"Creating VM {vm_name} with zones explicitly disabled (no availability zone)"
                )
            elif available_zones:
                # Round-robin distribution across available zones
                selected_zone = available_zones[i % len(available_zones)]
                template_params["zones"] = [selected_zone]
                logger.info(
                    f"Creating VM {vm_name} in availability zone {selected_zone}"
                )
            else:
                # For non-zonal deployments (no zones available), use empty array
                template_params["zones"] = []
                logger.info(f"Creating VM {vm_name} without specific availability zone")
