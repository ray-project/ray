...
def _configure_iam_role(config):
    ...
    if profile is None:
        log.info("Creating new instance profile {}".format(
    ...
    if not profile.roles:
        ...
        if role is None:
            log.info("Creating new role {}".format(DEFAULT_RAY_IAM_ROLE))
    ...
    log.info("Role not specified for head node, using {}".format(
        profile.arn))
...
def _configure_key_pair(config):
    ...
    log.info("KeyName not specified for nodes, using {}".format(key_name))
...
def _configure_security_group(config):
    ...
    if security_group is None:
        log.info("Creating new security group {}".format(group_name))
