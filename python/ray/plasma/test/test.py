...
from ray.plasma.utils import (
    random_object_id, create_object_with_id, create_object
)
...
class TestPlasmaManager(unittest.TestCase):
    ...
    def test_fetch_multiple(self):
        for _ in range(20):
            ...
            # updated.
            while (
                (not self.client2.contains(object_id1))
        ...
        for i in range(10):
            if i % 2 == 0:
                create_object_with_id(
                    self.client1, object_ids_perm[i], 2000, 2000
                )
            else:
                create_object_with_id(
                    self.client2, object_ids_perm[i], 2000, 2000
                )
    ...
    def test_transfer(self):
        ...
        for _ in range(100):
            ...
            for i in range(num_attempts):
                ...
                buff = self.client2.get_buffers([object_id1], timeout_ms=100)[0]
                if buff is not None:
                    break
            ...
            for i in range(num_attempts):
                ...
                buff = self.client1.get_buffers([object_id2], timeout_ms=100)[0]
                if buff is not None:
                    break
            self.assertNotEqual(buff, None)
            del buff
...
class TestPlasmaManagerRecovery(unittest.TestCase):
    def setUp(self):
        ...
        self.store_name, self.p2 = start_plasma_store(use_valgrind=USE_VALGRIND)
        # Start a Redis server.
