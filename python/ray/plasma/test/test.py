...
class TestPlasmaManager(unittest.TestCase):
  ...
  def setUp(self):
    ...
    redis_address1 = services.address("127.0.0.1", services.start_redis()[0])
    redis_address2 = services.address("127.0.0.1", services.start_redis()[0])
    # Start two PlasmaManagers.
    manager_name1, self.p4, self.port1 = plasma.start_plasma_manager(
        store_name1, redis_address1, "[" + redis_address1 + "," + redis_address2 + "]", use_valgrind=USE_VALGRIND)
    manager_name2, self.p5, self.port2 = plasma.start_plasma_manager(
        store_name2, redis_address1, "[" + redis_address1 + "," + redis_address2 + "]", use_valgrind=USE_VALGRIND)
