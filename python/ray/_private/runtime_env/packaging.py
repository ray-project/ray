...
def _dir_travel(
    ...
    def _apply_includes_and_handle(
        paths: List[Path], includes: List[Callable], handler: Callable, root: Path
    ):
        """
        ...
        for path in paths:
            ...
            if includes is not None and len(includes) != 0:
                ...
                include = True  # If no includes are specified, assume it's included
        ...
        for path in sorted(processed_paths, key=lambda p: len(p.parts)):
            try:
                # Pass the root directory as the hash value to the handler
                handler(path, root)
