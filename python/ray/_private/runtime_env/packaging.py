...
def _dir_travel(
    ...
    Note:
    The priority of excludes is higher than that of includes. Therefore, when both are assigned values simultaneously, 
    the system will first check excludes, and then filter the remaining files using includes.
    If the values of includes and excludes are the same, includes will not take effect.              
    ...
    def _apply_includes_and_handle(
        paths: List[Path], includes: Optional[List[Callable]], handler: Callable, root: Path
    ):
        """
