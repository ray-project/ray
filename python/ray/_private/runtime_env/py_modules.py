...
def upload_ray_libraries_if_needed(
    ...
    if isinstance(libraries, (str, ModuleType)):
        ...
        raise TypeError(
            "ray_libraries must be a str, module, or list of strs " "and modules."
        )
    ...
    for library in libraries:
        if isinstance(library, str):
            ...
            if len(library.__path__) > 1:
                raise ValueError(
                    "ray_libraries only supports modules whose "
                    "__path__ has length 1."
                )
            [library_path] = library.__path__
        else:
            raise TypeError(
                "ray_libraries must be a list of file paths or "
        ...
        if not library_path.startswith("/"):
            ...
            raise ValueError(
                "ray_libraries entries must be a sub-directory "
...
class RayLibrariesManager:
    def __init__(self, resources_dir: str):
        self._resources_dir = os.path.join(resources_dir, "ray_libraries_modules_files")
        try_to_create_directory(self._resources_dir)
        assert _internal_kv_initialized()
    ...
    async def create(
        ...
        def _create():
            ...
            [ray_path] = ray.__path__
            for relative_path, _uri in runtime_env["ray_libraries"].items():
                target_path = os.path.join(ray_path, relative_path)
                logger.info(f"Unpacking URI {_uri} to {target_path}.")
                library_dir = download_and_unpack_package(
                    _uri, self._resources_dir, logger=logger
                )
