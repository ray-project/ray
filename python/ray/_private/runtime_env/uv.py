...
# Matches environment variable placeholders such as
# ${RAY_RUNTIME_ENV_CREATE_WORKING_DIR} or $VAR, mirroring the pattern the pip
# plugin uses to detect them.
_ENV_VAR_PATTERN = re.compile(r"\$\{([^}]+)\}|\$([A-Za-z_][A-Za-z0-9_]*)")


def _expand_env_vars(value: str, env: Dict[str, str]) -> str:
    """Expand `${VAR}`/`$VAR` in `value` using `env`, leaving unknown names as-is.

    `os.path.expandvars` would read the live `os.environ` instead. That is not safe
    here: `RAY_RUNTIME_ENV_CREATE_WORKING_DIR` is set process-wide by
    `WorkingDirPlugin.with_working_dir_env` for the duration of one setup, so a
    concurrent setup can change it across the `await`s in this coroutine, and a
    user's `env_vars` overrides only exist in the per-setup environment anyway.
    """
...
class UvProcessor:
    ...
    async def _install_uv_packages(
        ...
        virtualenv_path = virtualenv_utils.get_virtualenv_path(path)
        python = virtualenv_utils.get_virtualenv_python(path)
        # Expand ${RAY_RUNTIME_ENV_CREATE_WORKING_DIR} (and other env vars) so uv
        # packages can reference files inside the unpacked working_dir, matching
        # pip/conda behavior. See #59343. Expand against `pip_env`, the environment
        # this install actually runs with, not the live process environment.
        expanded_packages = [_expand_env_vars(pkg, pip_env) for pkg in uv_packages]
        ...
        if uv_opt_list:
            # Expand env vars in install options (e.g. --requirements=${...}).
            uv_install_cmd += [_expand_env_vars(opt, pip_env) for opt in uv_opt_list]
