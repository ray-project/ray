...
class AsyncGeneratorActor:
    ...
    async def small_value_batch(self, n):
        yield await asyncio.wait([small_value.remote() for _ in range(n)])
...
class GeneratorClient:
    ...
    def small_value_batch(self, n):
        ...
        for s in self.servers:
            results.extend(
                [
                    s.small_value.options(num_returns="streaming").remote()
                    for _ in range(n)
                ]
            )
        refs = []
    ...
    def small_value_batch_arg(self, n):
        ...
        for s in self.servers:
            results.extend(
                [
                    s.small_value_arg.options(num_returns="streaming").remote(x)
                    for _ in range(n)
                ]
            )
        refs = []
...
def main(results=None):
    ...
    """
    a = AsyncGeneratorActor.remote()

    def actor_sync_generator():
        ray.get(list(a.small_value.options(num_returns="streaming").remote()))

    results += timeit("1:1 async-actor-generator calls sync", actor_sync_generator)

    a = AsyncGeneratorActor.options().remote()

    def async_actor_generator():
        gens = [
            a.small_value.options(num_returns="streaming").remote() for _ in range(1000)
        ]
        refs = []
        for gen in gens:
            refs.extend(list(gen))
    ...
    def async_actor_generator():
        gens = [
            a.small_value_with_arg.options(num_returns="streaming").remote(i)
            for i in range(1000)
        ]
        refs = []
        for gen in gens:
    ...
    n = 5000
    n_cpu = multiprocessing.cpu_count() // 2
    actors = [AsyncGeneratorActor.remote() for _ in range(n_cpu)]
    ...
    m = 4
    n_cpu = multiprocessing.cpu_count() // 2
    a = [AsyncGeneratorActor.remote() for _ in range(n_cpu)]

    @ray.remote
    def async_actor_work_generator(actors):
        gens = [
            actors[i % n_cpu].small_value.options(num_returns="streaming").remote()
            for i in range(n)
        ]
        refs = []
        for gen in gens:
    ...
    def async_actor_multi_generator():
        ray.get([async_actor_work_generator.remote(a) for _ in range(m)])
