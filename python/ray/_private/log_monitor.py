        ...
        for file_info in self.open_file_infos:
            ...
            for _ in range(max_num_lines_to_read):
                try:
                    ...
                    elif "Windows fatal exception: access violation" in next_line:
                        # We are suppressing the 'Windows fatal exception: access violation'
                        # message on Windows here. As far as we know it is harmless, but
                        # frequently popping up if Python functions are run inside the core
                        # worker C extension. See the investigation in
                        # github.com/ray-project/ray/issues/18944
                        # Also skip the following line, which is an empty line.
