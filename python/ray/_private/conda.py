...
# TODO(architkulkarni): remove mlflow dependency
from mlflow.exceptions import ExecutionException
from mlflow.utils import process

"""Utilities for conda.  Adapted from https://github.com/mlflow/mlflow."""

# TODO(architkulkarni): remove MLflow-specific artifacts
...
def get_conda_activate_commands(conda_env_name):
    ...
    if os.name != "nt" and ("CONDA_EXE" in os.environ or RAY_CONDA_HOME in os.environ):
        ...
        # activate_conda_env += ["conda activate {} 1>&2".format(conda_env_name)] TODO (architkulkarni) reinstate
        activate_conda_env += ["conda activate {}".format(conda_env_name)]
...
def get_conda_bin_executable(executable_name):
    ...
    ``RAY_CONDA_HOME`` environment variable. If
...
def get_or_create_conda_env(conda_env_path):
    ...
    :param conda_env_path: Path to a conda environment yaml file.
    """
    conda_path = get_conda_bin_executable("conda")
    try:
        ...
        raise ExecutionException(
            ...
            "to the path of the Conda executable".format(conda_path, RAY_CONDA_HOME)
        )
