...
def _get_addrinfo_from_sock_kind(address, kind, family=0):
    ...
    kind is socket kind such as socket.SOCK_STREAM or socket.SOCK_DGRAM
    family is INET address family.  0 is both IPv4 and IPv6.  socket.AF_INET is IPv4.  socket.AF_INET6 is IPv6

    Returns a List of tuples with (family, ip) as items.

    Example return value for _get_addrinfo_from_sock_kind("localhost", socket.SOCK_STREAM):

        [(socket.AF_INET6, "::1"), (socket.AF_INET, "127.0.0.1")]
    """
    return [(x[0], x[-1][0]) for x in socket.getaddrinfo(host, None, family) if x[1] == kind]
...
def _get_addrinfo_from_sock_kind_ipv4_fallback_ipv6(address, kind):
    ...
    if len(inet_addresses) > 0:
        ...
        return _get_addrinfo_from_sock_kind(host, kind)
...
def _get_sock_stream_from_host(address):
    """Returns a socket.socket which can either be IPv4 or IPv6 with socket.SOCK_STREAM.
...
def _get_sock_dgram_from_host(address):
    """Returns a socket.socket which can either be IPv4 or IPv6 with socket.SOCK_DGRAM.
...
def _get_private_ip_addresses():
    """Connects to Google DNS over IPv4, IPv6, or both to discover private IP addresses."""
    private_ip_addresses = []
    for public_host in ["2001:4860:4860::8888", "8.8.8.8"]:
        try:
            s = _get_socket_dualstack_fallback_single_stack_laddr(socket.SOCK_DGRAM)
            if not socket.has_dualstack_ipv6():
                # be aware we're single stack ip networking
                # we should skip addresses that are not compatible with our supported stack
                if ":" in public_host and not s.family == socket.AF_INET6:
                    continue
...
def _get_socket_dualstack_fallback_single_stack_laddr(kind=socket.SOCK_STREAM):
    """Similar to socket.socket() except that if networking is dualstack it will listen on all address family interfaces.

    If single-stack, then the loopback interface is queried for which stack is allowed (IPv4 or IPv6)"""
    if socket.has_dualstack_ipv6():
        ...
        # fall back to supporting the host single network stack (IPv4 or IPv6 only)
...
def _parse_ip_port(address: str) -> list[str]:
    ...
    # if no colon is in the address then assume a List with the original value is desired.
    #
    ...
    if not pattern.match(address):
        raise ValueError(f"Malformed address (expected address to include IP or DNS host and port e.g. ip:port): {address}")
