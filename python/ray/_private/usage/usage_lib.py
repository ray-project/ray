...
def generate_report_data(cluster_metadata: dict) -> dict:
    ...
    Returns:
        UsageStats in a dictionary type.
    """
    data = UsageStats(
...
class UsageReportClient:
    ...
    def _write_usage_data(self, data: UsageStats, dir_path: str) -> None:
    ...
    def _report_usage_data(self, url: str, data: UsageStats) -> None:
    ...
    async def write_usage_data_async(self, data: UsageStats, dir_path: str) -> None:
        loop = asyncio.get_running_loop()
        with ThreadPoolExecutor(max_workers=1) as executor:
            await loop.run_in_executor(executor, self._write_usage_data, data, dir_path)

    async def report_usage_data_async(self, url: str, data: UsageStats) -> None:
        loop = asyncio.get_running_loop()
        with ThreadPoolExecutor(max_workers=1) as executor:
