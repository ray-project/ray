...
ONEAPI_DEVICE_TYPE = "acc"


class IntelACCAcceleratorManager(AcceleratorManager):
    """Intel ACC accelerators."""
    ...
    def get_current_process_visible_accelerator_ids() -> Optional[List[str]]:
        oneapi_visible_devices = os.environ.get(
            IntelACCAcceleratorManager.get_visible_accelerator_ids_env_var(), None
        )
    ...
    def get_current_node_accelerator_type() -> Optional[str]:
        """Get the name of first Intel ACC. (supposed only one ACC type on a node)
        Example:
            name: 'Intel(R) Data Center ACC Max 1550'
            return name: 'Intel-ACC-Max-1550'
        Returns:
            A string representing the name of Intel ACC type.
        """
        try:
            import dpctl
        except ImportError:
        ...
        try:
            ...
            accelerator_type = "Intel-ACC-" + "-".join(dev.name.split(" ")[-2:])
        except Exception:
            accelerator_type = None
    ...
    def set_current_process_visible_accelerator_ids(
        ...
        prefix = ONEAPI_DEVICE_BACKEND_TYPE + ":"
        os.environ[
            IntelACCAcceleratorManager.get_visible_accelerator_ids_env_var()
