...
def _get_larger_3d_topologies(
    ...
    topologies = set()
    for x in range(DEFAULT_TPU_NUM_CHIPS_PER_HOST, max_x, DEFAULT_TPU_NUM_CHIPS_PER_HOST):
        for y in range(DEFAULT_TPU_NUM_CHIPS_PER_HOST, max_y, DEFAULT_TPU_NUM_CHIPS_PER_HOST):
            for z in range(DEFAULT_TPU_NUM_CHIPS_PER_HOST, max_z, DEFAULT_TPU_NUM_CHIPS_PER_HOST):
...
VALID_TPU_TOPOLOGY = {
    ...
    "v4" : {"2x2x1", "2x2x2", "2x2x4", "2x4x4"}.add(_get_larger_3d_topologies(12, 12, 16)),
    "v5p" : {"2x2x1", "2x2x2", "2x2x4", "2x4x4",}.add(_get_larger_3d_topologies(16, 16, 24)),
...
def get_chips_per_host(
    topology: str,
    accelerator_verison: str
) -> int:
    ...
    if total_chips <= 8 and accelerator_verison.strip().lower() in SINGLE_HOST_8_CHIPS_TPU_TYPES:
        return total_chips
...
class TPUAcceleratorManager(AcceleratorManager):
    ...
    @staticmethod
    def is_valid_tpu_accelerator_topology(tpu_accelerator_version: str, tpu_topology: str) -> bool:
        ...
        if tpu_version_formatted.lower() not in VALID_TPU_TOPOLOGY or tpu_topology.strip().lower() not in VALID_TPU_TOPOLOGY[tpu_version_formatted]:
            return False
        return True
