...
class MBLTAcceleratorManager(AcceleratorManager):
    ...
    def get_current_node_num_accelerators() -> int:
        try:
            ...
            max_probe = max(_hint_num_from_os() or 0, 256)
            count = 0
            for dev_no in range(max_probe):
                try:
                    ...
                    logger.debug("MBLT probe error at dev %d: %s", dev_no, e)
                    break
            return count
        except ImportError:
            logger.debug(
                "qbruntime not installed on this node; skipping MBLT detection"
            )
            return 0
        except Exception as e:
            logger.debug("Failed to detect MBLT devices: %s", e)
            return 0
    ...
    def get_current_node_accelerator_type() -> Optional[str]:
        ...
        try:
            ...
            logger.debug(
                "qbruntime not installed on this node; skipping MBLT type detection"
            )
        except Exception as e:
            logger.warning("Failed to detect MBLT type via qbruntime: %s", e)
