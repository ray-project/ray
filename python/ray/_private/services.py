...
class NodeMetadataManager:
    def __init__(self, session_dir_path: str):
        ...
        self.file_path = os.path.join(session_dir_path, "node_metadata.json")
        with FileLock(self.file_path + ".lock"):
            if not os.path.exists(self.file_path):
                with open(self.file_path, "w") as f:
                    json.dump({}, f)

    def get_or_update_cached_port(self, unique_node_id: str, port_name: str, port: int):
        id_to_node_metadata: Dict[str, NodeMetadata] = {}
        with FileLock(self.file_path + ".lock"):
            with open(self.file_path, "r") as f:
                id_to_node_metadata.update(json.load(f))

            node_metadata = id_to_node_metadata.get(unique_node_id, None)
            if node_metadata:
                node_metadata = NodeMetadata(**node_metadata)
                ports = node_metadata.ports
    ...
    def get_or_update_node_ip_address(self, unique_id: str, ip_address: str):



def get_or_write_node_metadata(session_dir_path: str):
    file_path = os.path.join(session_dir_path, "node_metadata.json")
