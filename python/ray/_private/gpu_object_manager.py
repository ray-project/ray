...
class GPUObjectRefCountUpdater:
    def __init__(self):
        # A dictionary that maps from an ObjectRef to an object ID. The ObjectRef is the
        # ObjectRef of the `__ray_send__` task which sends the GPU object to the destination actor
        # out of band. The object ID is the ID of the object generated by actor tasks annotated with
        # `@ray.method(tensor_transport=...)`.
        self.obj_ref_to_obj_id: Dict[ObjectRef, str] = {}
        # A flag to indicate if the thread should shutdown.
    ...
    def add_obj_ref(self, obj_ref: ObjectRef, obj_id: str):
        ...
        with self._condition:
            if not self._shutdown:
                self.obj_ref_to_obj_id[obj_ref] = obj_id
    ...
    def _run(self):
        while not self._shutdown:
            with self._condition:
                while not self.obj_ref_to_obj_id and not self._shutdown:
                    self._condition.wait()

                if self._shutdown:
                    break

                obj_refs_to_wait = list(self.obj_ref_to_obj_id.keys())

            if obj_refs_to_wait:
                ready_refs, _ = ray.wait(
                    obj_refs_to_wait, num_returns=len(obj_refs_to_wait), timeout=1
                )
                completed_obj_refs = []
                with self._condition:
                    for obj_ref in ready_refs:
                        if obj_ref in self.obj_ref_to_obj_id:
                            obj_id = self.obj_ref_to_obj_id.pop(obj_ref)
                            completed_obj_refs.append(obj_id)

                if completed_obj_refs:
                    worker = ray._private.worker.global_worker
                    worker.core_worker.decrement_gpu_object_ref_count(
                        completed_obj_refs
                    )
...
class GPUObjectManager:
    def __init__(self):
        ...
        # A thread that updates the GPU object ref count.
        self.ref_count_updater = GPUObjectRefCountUpdater()

    def shutdown(self):
        """Shutdown the GPU object manager and its background threads."""
        self.ref_count_updater.shutdown()
    ...
    def _send_gpu_object(
        ...
        util = _get_or_import_util()
        ref = src_actor.__ray_call__.remote(
            util.__ray_send__, communicator_name, obj_id, dst_rank
        )
        self.ref_count_updater.add_obj_ref(ref, obj_id)
