...
class Router:
    ...
    async def __init__(self, instance_name=None):
        ...
        # the central actor. We use a "pull-based" approach instead of pushing
        # them from the central so that the router can transparently recover
        # from failure.
        serve.init(name=instance_name)
        central_actor = serve.api._get_central_actor()

        traffic_policies = ray.get(central_actor.get_traffic_policies.remote())
        ...
        backend_configs = ray.get(central_actor.get_backend_configs.remote())
