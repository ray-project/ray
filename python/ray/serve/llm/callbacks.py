...
class CallbackBase(_CallbackBase):
    ...
    - ``on_before_node_init``: Before default model download on the head
      node. Use to download from custom storage or skip default init via
      ``self.ctx.run_init_node = False``.
    - ``on_after_node_init``: After default model download on the head
      node. Use to register reasoning parsers, tokenizers, or plugins.
    - ``on_before_download_model_files_distributed``: On each worker node
      before it downloads model files. Use to download auxiliary files.
            ...
            class ReasoningParserCallback(CallbackBase):
                async def on_after_node_init(self) -> None:
                    ...
                    ReasoningParserManager.import_reasoning_parser(
                        self.llm_config.engine_kwargs["reasoning_parser_plugin"]
                    )

            llm_config = LLMConfig(
                model_loading_config=dict(
                    model_id="nemotron-nano",
                    model_source="nvidia/Nemotron-3-Nano",
                ),
                accelerator_type="A10G",
                engine_kwargs=dict(
                    reasoning_parser_plugin="my_custom_parser",
                ),
