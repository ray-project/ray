...
class BackendState:
    ...
    def _checkpoint(self) -> None:
        self._kv_store.put(
            ...
            pickle.dumps(
                (self._replicas, self._backend_metadata, self._target_replicas,
                self._goals, list(self.pending_goals.keys()))
            )
        )
    ...
    def get_replica_handles(
        ...
        return             {backend_tag : dict(backend_replica._replica_tag : backend_replica.get_handle() for handle in state_to_replica_dict[ReplicaState.RUNNING])
    ...
    def _notify_replica_handles_changed(self) -> None:
        self._long_poll_host.notify_changed(
            ...
            {backend_tag : list(replica_dict.values()) for backend_tag, replica_dict in 
            self.get_replica_handles()})
    ...
    def _completed_goals(self) -> List[GoalId]:
        ...
        for backend_tag in all_tags:
            desired_info = self._backend_metadata.get(backend_tag)
