...
# Used for testing purposes only. If this is set, the central actor will crash
# after writing each checkpoint with the specified probability.
_CRASH_AFTER_CHECKPOINT_PROBABILITY = 0.0
CHECKPOINT_KEY = "serve-central-checkpoint"

# Feature flag for central actor resource checking. If true, central actor will
...
class ServeCentral:
    ...
    The central actor implements fault tolerance by persisting its state in
    ...
    All other actors started by the central actor are named, detached actors
    so they will not fate share with the central actor if it crashes.

    The following guarantees are provided for state-changing calls to the
    central actor:
        - If the call succeeds, the change was made and will be reflected in
          the system even if the central actor or other actors die unexpectedly.
