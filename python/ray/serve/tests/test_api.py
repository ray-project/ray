...
def test_updating_config(serve_instance):
    ...
    old_replica_tag_list = ray.get(
        central_actor._list_replicas.remote("bsimple:v1"))
    ...
    for worker_dict in ray.get(
            central_actor.get_all_worker_handles.remote()).values():
...
def test_create_infeasible_error(serve_instance):
    ...
    replicas = ray.get(serve.api.central_actor._list_replicas.remote("f1"))
...
def test_shutdown(serve_instance):
    ...
    def check_dead():
        for actor_name in [
                constants.SERVE_CENTRAL_NAME, constants.SERVE_PROXY_NAME,
