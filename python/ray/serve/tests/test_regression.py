    ...
    def whitelist(phase, info):
        ...
        for item in gc.garbage[:]:
            if getattr(type(item), "__name__", None) == "_Metadatum":
                ...
                print("whitelisted", item)
                gc.garbage.remove(item)
            elif getattr(type(item), "__name__", None) == "FreezableDefaultDict":
                print("whitelisted", item)
                gc.garbage.remove(item)
            elif getattr(type(item), "__name__", None) == "cell":
                print("whitelisted", item)
                gc.garbage.remove(item)
            elif (
                getattr(type(item), "__name__", None) == "function"
                and hasattr(item, "__name__")
                and ("freeze" in item.__name__ or "<lambda>" in item.__name__)
            ):
                print("whitelisted", item)
                gc.garbage.remove(item)
            elif isinstance(item, dict) and "_frozen" in item:
                print("whitelisted", item)
                gc.garbage.remove(item)
            elif getattr(type(item), "__name__", None) == "_SelectorSocketTransport":
                print("whitelisted", item)
                gc.garbage.remove(item)
                ...
                print("whitelisted", item)
                gc.garbage.remove(item)
            elif getattr(type(item), "__name__", None) == "TransportSocket":
                print("whitelisted", item)
                gc.garbage.remove(item)
            elif hasattr(item, "__name__") and "_read_ready" in str(item.__name__):
                print("whitelisted", item)
                gc.garbage.remove(item)
            elif getattr(type(item), "__name__", None) == "socket":
                print("whitelisted", item)
                gc.garbage.remove(item)
