...
def test_shutdown(mock_deployment_state_manager):
    """
    Test that shutdown waits for all deployments to be deleted and the
    deployments are force-killed without a grace period.
    """
    (deployment_state_manager, timer,
     goal_manager) = mock_deployment_state_manager

    tag = "test"

    b_info_1, b_version_1 = deployment_info()
    create_goal, updating = deployment_state_manager.deploy_deployment(
        tag, b_info_1)
...
def test_resume_deployment_state_from_replica_tags(
    ...
    # Step 3: Create new deployment_state by resuming from passed in replica
    # tags
