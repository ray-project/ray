...
class TestMultiplexBatchingEnd2End:
    ...
    async def test_model_caching_and_lru_eviction(self, start_serve_with_context):
        ...
        assert (
            stats["active_models"] == 3
        ), "Expected 3 active models, got {stats['active_models']}"
        ...
        assert (
            cache_size <= 3
        ), f"Should have at most 3 models in cache, got {cache_size}"
        ...
        assert (
            final_cache_size <= 3
        ), f"Should maintain max 3 models, got {final_cache_size}"
    ...
    async def test_batching_efficiency_metrics(self, start_serve_with_context):
        ...
        assert all(
            "batch_batched_model" in result for result in results
        ), f"Expected batch results, got: {results[:3]}"
        ...
        assert (
            model_no_batch.batch_predict_count == 0
        ), "Batch predict should not be called"
    ...
    async def test_concurrent_model_access_patterns(self, start_serve_with_context):
        ...
        assert (
            stats["active_models"] == 3
        ), f"Should have 3 models loaded, got {stats['active_models']}"
...
class TestMultiplexBatchingIntegration:
    ...
    async def test_multiple_models_with_batching(self, start_serve_with_context):
        ...
        print(f"Stats: {stats}")
        print(
            f"Model 1 predictions: {TrackableModel._instances['model_1'].predict_count}"
        )
        print(
            f"Model 2 predictions: {TrackableModel._instances['model_2'].predict_count}"
        )
        print(
            f"Model 3 predictions: {TrackableModel._instances['model_3'].predict_count}"
        )
