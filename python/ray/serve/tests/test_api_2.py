...
def test_get_application_urls(serve_instance):
    ...
    assert get_application_urls(RequestProtocol.HTTP, use_localhost=False) == [f"http://{node_ip}:8000"]
    assert get_application_urls(RequestProtocol.GRPC, use_localhost=False) == [f"{node_ip}:9000"]
...
def test_get_application_urls_with_app_name(serve_instance):
    ...
    controller_details = ray.get(serve_instance._controller.get_actor_details.remote())
    node_ip = controller_details.node_ip
    assert get_application_urls("HTTP", app_name="app1", use_localhost=False) == [f"http://{node_ip}:8000"]
    assert get_application_urls("gRPC", app_name="app1", use_localhost=False) == [f"{node_ip}:9000"]
