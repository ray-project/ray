...
def test_http_user_bring_own_driver_route_prefix(serve_instance):
    ...
    assert exposed_deployment.route_prefix == "/hello"
    for deployment in deployments[:-1]:
        assert deployment.route_prefix is None


def test_http_no_non_exposed_deployment_rout_prefix(serve_instance):
    with InputNode() as dag_input:
    ...
    with pytest.raises(
        ValueError, match="Route prefix is only configurable on the exposed deployment"
    ):
        _ = pipeline_build(serve_dag)


def test_http_we_provide_default_route_prefix_cls(serve_instance):
    """Ensure the default exposed deployment route is '/' instead of driver
    class name
    """
    with InputNode() as dag_input:
        m1 = Model.bind(1)
    ...
    deployments = pipeline_build(serve_dag)
    exposed_deployment = get_and_validate_exposed_deployment(deployments)
    assert exposed_deployment.route_prefix == "/"
    for deployment in deployments[:-1]:
        assert deployment.route_prefix is None
...
def test_http_non_default_route_prefix_on_root(serve_instance):
    with InputNode() as dag_input:
        m1 = Model.bind(1)
        m2 = Model.bind(2)
        m1_output = m1.forward.bind(dag_input[0])
    ...
    with pytest.raises(
        ValueError,
        match="Route prefix is only configurable on the exposed deployment",
    ):
        _ = get_and_validate_exposed_deployment(deployments)
