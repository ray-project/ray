...
def generate_mock_backend_info(num_replicas: Optional[int]=None) -> BackendInfo:
    backend_info = BackendInfo(worker_class=lambda x: x,backend_config=BackendConfig(),replica_config=ReplicaConfig(lambda x: x))
    if num_replicas:
...
def mock_backend_state_inputs() -> Tuple[BackendState, Mock, Mock]:
    with patch("ray.serve.kv_store.RayInternalKVStore") as mock_kv_store, patch("ray.serve.long_poll.LongPollHost") as mock_long_poll:
        mock_kv_store.get = Mock(return_value=None)
        backend_state = BackendState("name", True,mock_kv_store, mock_long_poll)
