...
from ray.serve._private.common import ApplicationStatus, ReplicaState, DeploymentID
...
def test_recover_start_from_replica_actor_names(serve_instance):
    ...
    deployment_id = DeploymentID("app", "recover_start_from_replica_actor_names")
    assert len(deployment_dict[deployment_id]) == 2

    replica_version_hash = None
    for replica in deployment_dict[deployment_id]:
...
def test_replica_deletion_after_controller_recover(serve_instance):
    ...
    def check_replica(replica_state=None):
        try:
            replicas = ray.get(
                controller._dump_replica_states_for_testing.remote("V1", "app")
            )
...
def test_recover_deleting_application(serve_instance):
    ...
    def application_deleting():
        ...
        assert serve_instance.get_serve_status().app_status.status == "DELETING"
