...
def test_recover_deleting_application(serve_instance):
    ...
    print("Started task to delete application `app`")

    def application_deleting():
        check = True
        # Confirm application is in deleting state
        check |= serve_instance.get_serve_status("app").app_status.status == "DELETING"

        # Confirm deployment is in updating state
        status = serve_instance.get_all_deployment_statuses()[0]
        check |= status.name == "app_A" and status.status == "UPDATING"

        # # Confirm replica is in stopping state
        replicas = ray.get(
    ...
    print("Confirmed that application is stuck on deleting.")
