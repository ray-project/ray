...
class MockClassificationModel:
    ...
    async def predict(self, text: str) -> Dict[str, float]:
        ...
        return {f"class_{i}": probs[i] for i in range(self.num_classes)}
    ...
    async def batch_predict(self, texts: List[str]) -> List[Dict[str, float]]:
        ...
        for text in texts:
            ...
            hash_val = int(
                hashlib.md5(f"{text}_{self.model_id}".encode()).hexdigest(), 16
            )
...
def embedding_model_loader():
    ...
    async def loader(model_id: str) -> MockEmbeddingModel:
        if model_id not in models:
            ...
            dims = {"mini": 384, "base": 768, "large": 1024}
...
class MultiModelTestScenario:
    ...
    async def execute_scenario(self, wrapper):
        ...
        for model_id, task in all_tasks:
            result = await task
            results.append(
                {"model_id": model_id, "result": result, "timestamp": time.time()}
            )
    ...
    def analyze_results(self, results: List[Dict], total_time: float):
        ...
        analysis = {
            ...
            "requests_per_model": {
                model_id: len(model_results[model_id]) for model_id in model_results
            },
...
TEST_SCENARIOS = {
    "embedding_workload": MultiModelTestScenario(
        ...
        request_patterns={
            ...
            "large": [
                "This is a longer text that requires more sophisticated "
...
def test_fixtures_return_correct_types(
    embedding_model_loader,
    classification_model_loader,
