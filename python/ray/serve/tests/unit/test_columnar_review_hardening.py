...
def test_missing_numpy_warns_once_only_when_it_would_have_helped(monkeypatch):
    """Without numpy the columnar path silently disappears, so a deployment wide enough
    to have benefited gets one warning -- and a narrow one, which would have stayed on
    the object path regardless, stays quiet."""
    monkeypatch.setattr(codec, "RAY_SERVE_COLUMNAR_METRICS_MIN_REPLICAS", 8)
    monkeypatch.setattr(codec, "np", None)
    warn = mock.Mock()
    monkeypatch.setattr(codec.logger, "warning", warn)

    monkeypatch.setattr(codec, "_WARNED_NUMPY_MISSING", False)
    assert codec.should_encode_columnar(_wide_report(4)) is False
    assert warn.call_count == 0  # below the gate: columnar would not have been used
