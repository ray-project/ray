...
class TestAutoscaling:
    ...
    def test_scale_log_reuses_decision_aggregate(self, mock_deployment_state_manager):
        """The scale up/down log reads the decision's stashed aggregate, not a second
        aggregation. Drive a real upscale: the decision stashes the total, and
        autoscale() logs it without calling get_total_num_requests_for_deployment again.
        """
        ...
        # 2 ongoing requests per replica -> total 6; target_ongoing 1 -> upscale 3->6.
        ...
        if RAY_SERVE_COLLECT_AUTOSCALING_METRICS_ON_HANDLE:
            asm.record_request_metrics_for_handle(
                HandleMetricReport(
                    ...
                    aggregated_metrics={
                        RUNNING_REQUESTS_KEY: {
                            r._actor.replica_id.to_full_id_str(): 2 for r in replicas
                        }
                    },
                    metrics={
                        RUNNING_REQUESTS_KEY: {
                            r._actor.replica_id.to_full_id_str(): [
        ...
        # Decision phase stashes the aggregate.
        app_name = TEST_DEPLOYMENT_ID.app_name
        curr_target = dsm.get_deployment_details(TEST_DEPLOYMENT_ID).target_num_replicas
        decisions = asm.get_decision_num_replicas(
            app_name, {TEST_DEPLOYMENT_ID: curr_target}
        )
        decision = decisions[TEST_DEPLOYMENT_ID]
        stashed = asm.get_last_decision_total_num_requests_for_deployment(
            TEST_DEPLOYMENT_ID
        )
        assert decision > curr_target  # a real upscale, so the log fires
        assert stashed == 6.0  # 2 requests x 3 replicas

        # Log phase must read the stash, not re-aggregate.
        with patch.object(
            asm, "get_total_num_requests_for_deployment"
        ) as agg_spy, patch(
            "ray.serve._private.deployment_state.logger"
        ) as mock_logger:
            assert dsm.autoscale(TEST_DEPLOYMENT_ID, decision) is True
        agg_spy.assert_not_called()
        logged = " ".join(
            str(c.args[0]) for c in mock_logger.info.call_args_list if c.args
        )
        assert f"Current ongoing requests: {stashed:.2f}" in logged
