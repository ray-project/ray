...
class TestStopReplicasOnDrainingNodes:
    ...
    def test_migration_replacement_excludes_draining_node(
        self, mock_deployment_state_manager
    ):
        """Replacements must stay off the node their replicas are leaving."""
        ...
        # Same order as an update: mark the migration, then scale up.
        ds.migrate_replicas_on_draining_nodes({node_2: 60 * 1000})
        upscale, _ = ds.scale_deployment_replicas()

        assert len(upscale) == 1
        assert upscale[0].excluded_node_ids == {node_2}

    def test_only_migration_replacements_are_excluded(
        self, mock_deployment_state_manager
    ):
        """An upscale alongside a migration only restricts the replacement."""
        ...
        # One replica migrates off node_2 while the deployment upscales to 4.
        b_info_2, _ = deployment_info(num_replicas=4, version="1")
        assert dsm.deploy(TEST_DEPLOYMENT_ID, b_info_2)
        ds.migrate_replicas_on_draining_nodes({node_2: 60 * 1000})
        upscale, _ = ds.scale_deployment_replicas()

        assert len(upscale) == 3
        excluded = [r.excluded_node_ids for r in upscale]
        assert excluded.count({node_2}) == 1
        assert excluded.count(None) == 2

    def test_plain_upscale_excludes_no_nodes(self, mock_deployment_state_manager):
        """Replicas added without a migration in flight can go anywhere."""
        ...
        b_info_1, v1 = deployment_info(num_replicas=1, version="1")
        assert dsm.deploy(TEST_DEPLOYMENT_ID, b_info_1)
        ds = dsm._get_deployment_state_for_testing(TEST_DEPLOYMENT_ID)

        dsm.update()
        replica = ds._replicas.get()[0]
        replica._actor.set_node_id(node_1)
        replica._actor.set_ready()
        dsm.update()
        check_counts(ds, total=1, by_state=[(ReplicaState.RUNNING, 1, v1)])

        b_info_2, _ = deployment_info(num_replicas=2, version="1")
        assert dsm.deploy(TEST_DEPLOYMENT_ID, b_info_2)
        upscale, _ = ds.scale_deployment_replicas()

        assert len(upscale) == 1
        assert upscale[0].excluded_node_ids is None
