...
from ray.serve._private.utils import run_coroutine_threadsafe
...
async def test_run_coroutine_threadsafe_with_basic_coroutine(separate_loop):
    ...
    future = run_coroutine_threadsafe(sample_coro(), separate_loop)
...
async def test_run_coroutine_threadsafe_with_future(separate_loop):
    ...
    future = run_coroutine_threadsafe(async_future, separate_loop)
    result = future.result(timeout=1)
    assert result == "ok2"
...
async def test_run_coroutine_threadsafe_with_future_from_other_loop(separate_loop):
    ...
    with pytest.raises(AssertionError):
        run_coroutine_threadsafe(future, separate_loop)
