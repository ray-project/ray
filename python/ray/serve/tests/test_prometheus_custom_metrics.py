...
class TestPrometheusCustomMetrics:
    ...
    def test_prometheus_custom_metrics_integration(self, serve_instance):
        ...
        basic_metrics = ["ray_node_cpu_utilization", "ray_node_mem_used"]
        metrics = fetch_prometheus_metrics(["localhost:9999"])
        node_metrics = {k: v for k, v in metrics.items() if "ray_node_" in k}
        # serve_metrics = {k: v for k, v in metrics.items() if "ray_serve_" in k}

        for metric in basic_metrics:
            assert (
                metric in node_metrics
            ), f"Basic metric {metric} not found in Prometheus"
        ...
        metrics_c = get_autoscaling_metrics_from_controller(client, dep_c_id)
    ...
    def test_prometheus_custom_metrics_with_custom_queries(self, serve_instance):
        ...
        assert check_autoscaling_metrics_include_prometheus(
            client, dep_id, ["custom_cpu_metric"]
    ...
    def test_prometheus_custom_metrics_error_handling(self, serve_instance):
        ...
        # Wait for metrics to be collected (should handle errors gracefully)
        time.sleep(2)

        # The deployment should still be healthy even with invalid prometheus queries
        wait_for_condition(
            check_deployment_status,
            name="InvalidQueryDeployment",
            expected_status="HEALTHY",
