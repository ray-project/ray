...
def test_multiplexed_replica_info(serve_instance):
    ...
    def check_replica_information(
        ...
        if RAY_SERVE_ENABLE_NEW_ROUTING:
            for replica in handle._get_or_create_router()._replica_scheduler.curr_replicas.values():
                if (
            ...
            for replica in handle._get_or_create_router()._replica_scheduler.in_flight_queries.keys():
...
def check_model_id_in_replicas(handle: RayServeHandle, model_id: str) -> bool:
    if RAY_SERVE_ENABLE_NEW_ROUTING:
        ...
        return model_id in handle._get_or_create_router()._replica_scheduler.multiplexed_replicas_table
