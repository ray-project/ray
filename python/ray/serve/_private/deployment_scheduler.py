...
class DeploymentScheduler:
    ...
    def on_deployment_created(
        ...
        assert id not in self._pending_replicas
        assert id not in self._launching_replicas
        assert id not in self._recovering_replicas
        assert id not in self._running_replicas
        self._deployments[id] = scheduling_policy

    def on_deployment_deleted(self, id: DeploymentID) -> None:
        """Called whenever a deployment is deleted."""
        assert not self._pending_replicas[id]
        self._pending_replicas.pop(id, None)

        assert not self._launching_replicas[id]
        self._launching_replicas.pop(id, None)

        assert not self._recovering_replicas[id]
        self._recovering_replicas.pop(id, None)

        assert not self._running_replicas[id]
        self._running_replicas.pop(id, None)

        del self._deployments[id]
    ...
    def _schedule_driver_deployment(self, deployment_id: str) -> None:
        if isinstance(deployment_id, str):
            import traceback
    ...
    def _get_replicas_to_stop(
        self, deployment_id: str, max_num_to_stop: int
    ) -> Set[str]:
        if isinstance(deployment_id, str):
            import traceback
