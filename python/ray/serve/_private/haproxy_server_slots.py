"""Runtime-managed HAProxy backend membership.

With dynamic servers enabled, each backend renders a fixed number of
placeholder servers (`server-template`) instead of one `server` line per
replica, so replica membership is not part of haproxy.cfg. Replicas are
assigned to those slots through the Runtime API (admin socket). A membership
change therefore never reloads HAProxy, which means there is no soft-stopping
worker holding frontend keep-alive connections (and a frozen backend view)
until hard-stop-after.
...
formatting; HAProxyApi owns the socket I/O.
"""
