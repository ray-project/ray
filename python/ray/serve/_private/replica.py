...
class ReplicaBase(ABC):
    ...
    async def initialize(self, deployment_config: DeploymentConfig):
        ...
        try:
            ...
            async with self._user_callable_initialized_lock:
                ...
                if not self._user_callable_initialized:
                    ...
                    if self._user_callable_asgi_app:
                        ...
                        logger.critical(f"[SEIJI] Set docs_path: {self._docs_path}")
                    await self._on_initialized()
                    logger.critical("[SEIJI] Called _on_initialized()")
                    self._user_callable_initialized = True
                    logger.critical("[SEIJI] Marked user callable as initialized")
                else:
                    logger.critical(
                        "[SEIJI] User callable already initialized, skipping"
                    )
                ...
                if deployment_config:
                    ...
                    logger.critical("[SEIJI] Set threadpool limit")
                    await asyncio.wrap_future(
                    ...
                    logger.critical("[SEIJI] No deployment config provided")
            ...
            logger.critical("[SEIJI] Starting health check")
            await self.check_health()
            logger.critical(
                "[SEIJI] Health check passed, replica initialization complete"
            )
        except Exception:
            logger.error(
...
class ReplicaActor:
    ...
    async def __init__(
        ...
        logger.critical("[SEIJI] calling create_replica_impl")
    ...
    async def initialize_and_get_metadata(
        ...
        logger.critical(f"[SEIJI] initialize_and_get_metadata: {deployment_config}")
