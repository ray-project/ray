...
class PortAllocator:
    ...
    def update_port_if_missing(self, replica_id: str, port: Optional[int]):
        ...
        assert port is not None, (
            f"Port is None for {self._protocol} protocol on replica {replica_id} "
        ...
        logger.info(
            f"Recovered {self._protocol} port {port} for replica {replica_id} "
            f"on node {self._node_id}"
        )
        return port
    ...
    def allocate(self, replica_id: str) -> int:
        ...
        while self._available_ports:
            ...
            if port not in self._blocked_ports:
                self._allocated_ports[replica_id] = port
                logger.info(
                    f"Allocated {self._protocol} port {port} to replica {replica_id} "
                    f"on node {self._node_id}"
                )
                return port
    ...
    def release(self, replica_id: str, port: int, block_port: bool = False):
        ...
        Args:
            ...
            block_port: Whether to block the port from being allocated again. Use this
                in situations where the port is being released due some other process
                is using it.
        """
        if replica_id not in self._allocated_ports:
            raise ValueError(
                f"{self._protocol} port not allocated for replica {replica_id} "
                f"on node {self._node_id}"
            )
        expected_port = self._allocated_ports[replica_id]
        assert expected_port == port, (
            f"{self._protocol} port mismatch for replica {replica_id} on node "
            f"{self._node_id}: expected {expected_port}, got {port}"
        )

        heapq.heappush(self._available_ports, port)
        del self._allocated_ports[replica_id]
        logger.info(
            f"Released {self._protocol} port {port} for replica {replica_id} "
            f"on node {self._node_id}"
        )
    ...
    def prune(self, active_replica_ids: Set[str]):
        for replica_id in list(self._allocated_ports.keys()):
            if replica_id not in active_replica_ids:
                ...
                logger.info(
                    f"Cleaning up {self._protocol} port {port} for stale replica "
...
class NodePortManager:
    ...
    and is only used in direct ingress mode, where each Serve replica is exposed
    individually via a Kubernetes or GCP or AWS Ingress.

    The primary goal of this class is to assign ports in a consistent and efficient
    manner, minimizing EndpointSlice fragmentation in Kubernetes. It uses a min-heap
    strategy to allocate ports incrementally, ensuring that all nodes tend to reuse
    the same port numbers.

    Background:
    Kubernetes groups endpoints into EndpointSlices based on the set of ports exposed
    by each Pod. If Pods expose different port combinations (e.g., due to random port
    assignment), Kubernetes generates separate EndpointSlices per unique port list.
    This leads to unnecessary fragmentation and increased resource consumption.
    ...
    Although Kubernetes does not allow users to explicitly configure the ports included
    in EndpointSlices, maintaining a uniform port layout across nodes is still
    beneficial.

    Port lifecycle:
    - Replicas are expected to release their ports when stopped
    - If a replica crashes without releasing its port, the controller loop will detect
      and reclaim leaked ports during reconciliation
    ...
    def get_node_manager(cls, node_id: str) -> "NodePortManager":
        # this doesn't need to be behind a lock because it will already be
