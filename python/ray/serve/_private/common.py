...
class RunningReplicaInfo:
    ...
    # Maps multiplex dimension name -> list of IDs cached on the replica.
    # e.g. {"model": ["lora_v1", "lora_v2"], "session": ["user_1"]}
    multiplex_dim_to_ids: Dict[str, List[str]] = field(default_factory=dict)
        ...
        hash_val = hash(
            " ".join(
                [
                    ...
                    str(sorted(self.multiplex_dim_to_ids.items())),
...
class RequestRoutingInfo:
    ...
    multiplex ids, and routing stats.
    """

    replica_id: ReplicaID
    # Maps multiplex dimension name -> list of IDs cached on the replica.
    # e.g. {"model": ["lora_v1", "lora_v2"], "session": ["user_1"]}
    multiplex_dim_to_ids: Optional[Dict[str, List[str]]] = None
...
def parse_multiplex_ids_from_headers(
    headers: Iterable[Tuple[Union[str, bytes], Union[str, bytes]]],
) -> Dict[str, str]:
    """Extract a canonical multiplex_ids dict from HTTP/WS/gRPC headers.

    Recognizes two incoming header forms, case-insensitive and with `-`/`_`
    interchangeable (some fronting proxies mangle one for the other). The hyphen
    form is commonly produced by proxies like nginx and AWS API Gateway.

    1. ``serve_multiplexed_{dimension}_id`` (e.g. ``serve_multiplexed_model_id``,
       ``serve_multiplexed_session_id``).
    2. ``x-session-id`` — conventional session header, maps to the "session"
       dimension when no explicit ``serve_multiplexed_session_id`` is present.

    Args:
        headers: iterable of ``(key, value)`` pairs. Both may be ``bytes``
            (ASGI scope["headers"] / proxy request headers) or ``str``
            (gRPC ``invocation_metadata()``); the helper normalizes both.

    Returns:
        A dict mapping multiplex dimension name (e.g. ``"model"``,
        ``"session"``) to the ID value parsed from the corresponding header.
    """
    multiplex_ids: Dict[str, str] = {}
    for key, value in headers:
        ...
        if normalized_key.startswith(
            SERVE_MULTIPLEX_HEADER_PREFIX
        ) and normalized_key.endswith(SERVE_MULTIPLEX_HEADER_SUFFIX):
            dimension = normalized_key[
                len(SERVE_MULTIPLEX_HEADER_PREFIX) : -len(SERVE_MULTIPLEX_HEADER_SUFFIX)
            ]
            if dimension:
                multiplex_ids[dimension] = value
        elif normalized_key == SERVE_SESSION_ID_HEADER:
            multiplex_ids.setdefault("session", value)
    return multiplex_ids
...
class RequestMetadata:
    ...
    # e.g. {"model": "lora_v2", "session": "user_123"}
    ...
    def multiplexed_model_id(self) -> str:
        ...
        `multiplex_ids`). Preserved as a property because several internal
        readers still access it as a scalar; there is no public guarantee."""
