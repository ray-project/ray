...
class DeploymentHandle(_DeploymentHandleBase[T]):
    ...
    def get_request_router(
        ...
        Returns the underlying RequestRouter instance if the handle has been
        initialized, or None otherwise. This can be used to make local routing
        decisions (e.g., via choose_replicas()) without sending a request
        through the full DeploymentHandle transport path.
        """
        if self._router is None:
            return None
