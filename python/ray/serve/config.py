...
class BackendConfig:
    def __init__(self, *args, **kwargs):
        raise DeprecationWarning(
            "BackendConfig is deprecated, please use DeployementConfig")
...
class DeploymentConfig(BaseModel):
    ...
    Args:
        ...
        experimental_graceful_shutdown_wait_loop_s (Optional[float]): Duration
            that deployment workers will wait until there is no more work to be
...
class ReplicaConfig:
    def __init__(self, deployment_def, *init_args, ray_actor_options=None):
        ...
        if isinstance(deployment_def, str):
            ...
            if len(init_args) != 0:
                raise ValueError(
                    "init_args not supported for function deployment.")
        elif inspect.isclass(deployment_def):
            self.func_or_class_name = deployment_def.__name__
        else:
            raise TypeError(
                "Deployment must be an import path, function or class, "
                "it is {}.".format(type(deployment_def)))
