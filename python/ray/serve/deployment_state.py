    ...
    def check_ready(
        ...
        if len(ready) == 0:
            ...
            try:
                ...
                self._health_check_period_s = (
                    deployment_config.health_check_period_s)
                self._health_check_timeout_s = (
                    deployment_config.health_check_timeout_s)
            except Exception:
                logger.exception(
    ...
    def check_health(self) -> bool:
        ...
        if len(ready) == 1:
            ...
            try:
                ...
                randomized_period = (
                    self._health_check_period_s * random.uniform(0.9, 1.1))
                if time_since_last > randomized_period:
                    self._last_health_check_time = time.time()
                    self._health_check_ref = self._actor_handle.check_health.remote(  # noqa: E501
                    )
            except RayError as e:
                logger.info(f"Health check for replica {self._replica_tag} "
                            f"failed: {e}")
                healthy = False
        elif time.time(
        ) - self._last_health_check_time > self._health_check_timeout_s:
            logger.info(
