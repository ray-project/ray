...
class RayServeDAGHandle:
    ...
    async def remote(self, *args, _cache_refs: bool = False, **kwargs) -> ray.ObjectRef:
        ...
        if FLAG_SERVE_DEPLOYMENT_HANDLE_IS_SYNC:
            return self.dag_node.execute(*args, _cache_refs=_cache_refs, **kwargs)
        else:
            return await self.dag_node.execute(*args, _cache_refs=_cache_refs, **kwargs)
