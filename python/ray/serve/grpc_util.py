...
class gRPCDIReceiveProxy(AsyncIterator[T]):
    ...
    (a separate thread by default). Awaiting the server-loop iterator directly from
    the user-code loop raises "got Future attached to a different loop".
    ...
    (mirroring `ASGIDIReceiveProxy`). User code iterates this proxy on the user-code
    loop, only ever touching the queue. Decoupling the read side from the consume
    side lets the server read ahead while user code processes the previous message,
    which is what makes it efficient for high-throughput streaming.
    """
    ...
    def _put(self, item: Any):
        # Always invoked on the user-code loop (via call_soon_threadsafe), so the
        # queue is created and operated on a single loop.
    ...
    async def _fetch_until_done(self):
        ...
        A terminal exception is forwarded to the consumer; a sentinel is always
        enqueued at the end so the consumer terminates.
        """
        try:
            ...
            self._user_event_loop.call_soon_threadsafe(self._put, e)
        finally:
            self._user_event_loop.call_soon_threadsafe(self._put, _STREAM_END)

    def __aiter__(self) -> "gRPCDIReceiveProxy":
        return self
