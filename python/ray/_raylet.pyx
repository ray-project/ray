...
cdef class CoreWorker:
    ...
    def put_serialized_object(self, serialized_object,
        ...
        put_threshold = RayConfig.instance().max_direct_call_object_size()
        put_small_object_in_memory_store = RayConfig.instance().put_small_object_in_memory_store()
        ...
        if not object_already_exists:
            if total_bytes > 0:
                (<SerializedObject>serialized_object).write_to(
                    Buffer.make(data))
            if self.is_local_mode or (put_small_object_in_memory_store and <int64_t>total_bytes < put_threshold):
