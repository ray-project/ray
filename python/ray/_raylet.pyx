...
from ray.includes.stream_redirection cimport (
    ...
    RedirectStdout,
    RedirectStderr,
    RedirectStdoutAndGetFd,
    RedirectStderrAndGetFd,
)
...
cdef class StreamRedirector:
    ...
    @staticmethod
    def redirect_stdout_and_get_fd(const c_string &file_path, uint64_t rotation_max_size, uint64_t rotation_max_file_count, c_bool tee_to_stdout, c_bool tee_to_stderr):
        cdef CStreamRedirectionOptions opt = CStreamRedirectionOptions()
        opt.file_path = file_path
        opt.rotation_max_size = rotation_max_size
        opt.rotation_max_file_count = rotation_max_file_count
        opt.tee_to_stdout = tee_to_stdout
        opt.tee_to_stderr = tee_to_stderr
        return RedirectStdoutAndGetFd(opt)

    @staticmethod
    def redirect_stderr_and_get_fd(const c_string &file_path, uint64_t rotation_max_size, uint64_t rotation_max_file_count, c_bool tee_to_stdout, c_bool tee_to_stderr):
        cdef CStreamRedirectionOptions opt = CStreamRedirectionOptions()
        opt.file_path = file_path
        opt.rotation_max_size = rotation_max_size
        opt.rotation_max_file_count = rotation_max_file_count
        opt.tee_to_stdout = tee_to_stdout
        opt.tee_to_stderr = tee_to_stderr
        return RedirectStderrAndGetFd(opt)
