...
cdef execute_streaming_generator(
    ...
    if is_async:
        worker.core_worker.run_async_func_or_coro_in_event_loop(
        ...
        execute_streaming_generator_sync(
            generator,
            generator_id,
            return_obj,
            task_type,
...
async def execute_streaming_generator_async(
    ...
    while True:
        ...
        done = execute_streaming_generator_unit(
            output,
            generator_index,
            True, # is_async
            worker,
...
def execute_streaming_generator_sync(
        generator,
        const CObjectID &generator_id,
        c_pair[CObjectID, shared_ptr[CRayObject]] &return_obj,
        CTaskType task_type,
...
cdef execute_streaming_generator_unit(
        output,
        generator_index,
        is_async,
        worker,
    ...
    if isinstance(output, AsyncioActorExit):
        # Make the task handle this exception.
        raise
    elif isinstance(output, StopAsyncIteration):
        return True
    elif isinstance(output, StopIteration):
        return True
