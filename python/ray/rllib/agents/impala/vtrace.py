...
def from_logits(behaviour_policy, target_policy, actions,
    ...
    Args:
      ...
      target_policy: A float32 tensor of shape [T, B, NUM_ACTIONS] with
        un-normalized log-probabilities parametrizing the softmax target
        policy.
      ...
      clip_rho_threshold: A scalar float32 tensor with the clipping threshold
        for importance weights (rho) when calculating the baseline targets (vs)
...
def _from_importance_weights(rhos, discounts, rewards, values, bootstrap_value,
    ...
    Args:
      rhos: A float32 tensor of shape [T, B, NUM_ACTIONS] representing the
        ...
        i.e. target_policy(a) / behaviour_policy(a).
