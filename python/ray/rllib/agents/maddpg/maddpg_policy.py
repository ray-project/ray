...
class MADDPGPostprocessing(object):
    ...
    def postprocess_trajectory(self,
                               ...
                               episode=None):
        # FIXME: Get done from info is required since agentwise done is not supported now.
...
class MADDPGTFPolicy(MADDPGPostprocessing, TFPolicy):
    def __init__(self, obs_space, act_space, config):
        ...
        self.config = config = dict(ray.rllib.agents.maddpg.DEFAULT_CONFIG, **config)
        ...
        obs_space_n = [_make_continuous_space(space) for space in config["obs_space_dict"].values()]
        act_space_n = [_make_continuous_space(space) for space in config["act_space_dict"].values()]

        # _____ Placeholders
        # Placeholders for policy evaluation and updates
        def _make_ph_n(space_n, name=""):
            return [tf.placeholder(tf.float32, shape=(None,) + space.shape, name=name + "_%d" % i)
        ...
        critic, _, critic_model_n, critic_vars = \
            self._build_critic_network(obs_ph_n, act_ph_n, obs_space_n, act_space_n,
                                       hiddens=config["critic_hiddens"],
                                       activation=getattr(tf.nn, config["critic_hidden_activation"]),
                                       scope="critic")

        # Build critic network for t + 1.
        target_critic, _, _, target_critic_vars = \
            self._build_critic_network(new_obs_ph_n, new_act_ph_n, obs_space_n, act_space_n,
                                       hiddens=config["critic_hiddens"],
                                       activation=getattr(tf.nn, config["critic_hidden_activation"]),
                                       scope="target_critic")
        ...
        act_sampler, actor_feature, actor_model, actor_vars = \
            self._build_actor_network(obs_ph_n[agent_id], obs_space_n[agent_id], act_space_n[agent_id],
        ...
        self.target_act_sampler, _, _, target_actor_vars = \
            self._build_actor_network(self.new_obs_ph, obs_space_n[agent_id], act_space_n[agent_id],
                                      hiddens=config["actor_hiddens"],
                                      activation=getattr(tf.nn, config["actor_hidden_activation"]),
                                      scope="target_actor")
        ...
        critic, _, _, _ = \
            self._build_critic_network(obs_ph_n, act_n, obs_space_n, act_space_n,
                                       hiddens=config["critic_hiddens"],
                                       activation=getattr(tf.nn, config["critic_hidden_activation"]),
                                       scope="critic")
        ...
        def _make_set_weight_op(variables):
            ...
            for v in variables.values():
                vs += v
            phs = [tf.placeholder(tf.float32, shape=v.get_shape(), name=v.name.split(":")[0] + "_ph") for v in vs]
    ...
    def _build_critic_network(self, obs_n, act_n, obs_space_n, act_space_n,
                              hiddens, activation=None, scope=None):
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE) as scope:
            if self.config["use_state_preprocessor"]:
                model_n = [
                    ...
                    for obs, obs_space, act_space in zip(obs_n, obs_space_n, act_space_n)
                ]
    ...
    def _build_actor_network(self, obs, obs_space, act_space,
                             hiddens, activation=None, scope=None):
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE) as scope:
            if self.config["use_state_preprocessor"]:
