...
def _build_action_network(q_values, observations, num_actions, stochastic, eps):
...
class ModelAndLoss(object):
    ...
    def __init__(
        self, registry, num_actions, config, obs_t, act_t, rew_t, obs_tp1,
...
class DQNGraph(object):
    def __init__(self, registry, env, config, logdir):
        ...
        def build_loss(
            ...
            return ModelAndLoss(
                registry, num_actions, config, obs_t, act_t, rew_t, obs_tp1,
                done_mask, importance_weights
            )
    ...
    def compute_apply(
        ...
        td_err, _ = sess.run([self.td_error, self.train_expr],
                             feed_dict={
                                 self.obs_t: obs_t,
                                 self.act_t: act_t,
                                 self.rew_t: rew_t,
