...
class PPOEvaluator(PolicyEvaluator):
    ...
    def __init__(self, registry, env_creator, config, logdir, is_remote):
        ...
        if is_remote:
            ...
            self.batch_size = int(config["sgd_batchsize"] / len(devices)
                                  ) * len(devices)
        ...
        def build_loss(obs, vtargets, advs, acts, plog, pvf_preds):
            return ProximalPolicyLoss(
                self.env.observation_space, self.env.action_space, obs,
                vtargets, advs, acts, plog, pvf_preds, self.logit_dim,
    ...
    def load_data(self, trajectories, full_trace):
        ...
        return self.par_opt.load_data(
            self.sess, [
                trajectories["obs"], trajectories["value_targets"] if use_gae
                else dummy, trajectories["advantages"], trajectories["actions"],
                trajectories["logprobs"], trajectories["vf_preds"]
                if use_gae else dummy
            ],
