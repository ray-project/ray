...
class SharedModel(TFPolicy):
    ...
    def __init__(self, registry, ob_space, ac_space, config, **kwargs):
        super(SharedModel, self).__init__(registry, ob_space, ac_space, config,
                                          **kwargs)
    ...
    def compute_gradients(self, samples):
        ...
        self.local_steps += 1
        if self.summarize:
            grad, summ = self.sess.run(
                [self.grads, self.summary_op], feed_dict=feed_dict)
            info['summary'] = summ
        else:
    ...
    def compute(self, ob, *args):
        action, vf = self.sess.run([self.sample, self.vf], {self.x: [ob]})
