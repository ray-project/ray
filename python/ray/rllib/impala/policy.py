...
class Policy(object):
    ...
    def setup_loss(self, action_space):
        ...
        log_diff = log_prob - self.actor_logprobs
        ...
        is_weights_ruo = tf.exp(log_ruo_trunc)
        is_weights_c = self.config["lambda"] * tf.exp(log_c_trunc)
        delta_v = self.rwd[:-1] + (self.config["gamma"] * self.vf)[1:] - self.vf[:-1]
