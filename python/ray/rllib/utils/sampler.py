...
class mSyncSampler(object):
    """This class interacts with the environment and tells it what to do for multiagent environments.
    ...
    def __init__(self, env, policy, obs_filter,
        ...
        self.policy = policy
        self._obs_filter = obs_filter
        self.rollout_provider = _m_env_runner(
            self.env, self.policy, self.num_local_steps, self.horizon,
            self._obs_filter)
        self.metrics_queue = queue.Queue()
    ...
    def get_obs_filter(self, flush=False):
        ...
        for i in range(len(self._obs_filter)):
            snapshot.append(self._obs_filter[i].copy())
            if flush and hasattr(self._obs_filter[i], "clear_buffer"):
                self._obs_filter[i].clear_buffer()
        return snapshot
...
def _m_env_runner(env, policy, num_local_steps, horizon, obs_filter):
    """This implements the logic of the thread runner.
    ...
    for i in range(n_agents):
        last_observation.append(obs_filter[i](reset_state[i]))
    horizon = horizon if horizon else env.spec.tags.get(
        "wrapper_config.TimeLimit.max_episode_steps")
    assert horizon > 0
    if hasattr(policy, "get_initial_features"):
