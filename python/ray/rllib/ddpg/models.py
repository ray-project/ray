...
def _build_action_network(
    ...
    return tf.cond(
        stochastic, lambda: stochastic_actions, lambda: deterministic_actions
    )
...
class ModelAndLoss(object):
    ...
    def __init__(
        self, registry, dim_actions, low_action, high_action, config, obs_t,
        act_t, rew_t, obs_tp1, done_mask, importance_weights
    ):
        ...
        with tf.variable_scope("target_p_func") as scope:
            self.p_tp1 = _build_p_network(
                registry, obs_tp1, dim_actions, config
            )
        ...
        with tf.variable_scope("a_func", reuse=True):
            ...
            output_actions = _build_action_network(
                self.p_t, low_action, high_action, deterministic_flag, zero_eps,
        ...
        with tf.variable_scope("q_func", reuse=True):
            self.q_tp0 = _build_q_network(
                registry, obs_t, output_actions, config
            )

        # target q network evalution
        with tf.variable_scope("target_q_func") as scope:
            self.q_tp1 = _build_q_network(
                registry, obs_tp1, output_actions_estimated, config
            )
...
class DDPGGraph(object):
    def __init__(self, registry, env, config, logdir):
        ...
        with tf.variable_scope(a_scope_name, reuse=True):
            ...
            self.reset_noise_op = tf.assign(
                exploration_sample, dim_actions * [.0]
            )
        ...
        def build_loss(
            ...
            return ModelAndLoss(
                registry, dim_actions, low_action, high_action, config, obs_t,
                act_t, rew_t, obs_tp1, done_mask, importance_weights
            )
        ...
        if config["l2_reg"] is not None:
            ...
            for var in q_func_vars:
                if "bias" not in var.name:
                    weighted_error += config["l2_reg"
                                             ] * 0.5 * tf.nn.l2_loss(var)
    ...
    def compute_gradients(
        ...
        td_err, grads = sess.run([self.td_error, self.grads],
                                 feed_dict={
                                     self.obs_t: obs_t,
                                     self.act_t: act_t,
                                     self.rew_t: rew_t,
    ...
    def apply_gradients(self, sess, grads):
        ...
        sess.run([self.critic_train_expr, self.actor_train_expr],
                 feed_dict=feed_dict)
