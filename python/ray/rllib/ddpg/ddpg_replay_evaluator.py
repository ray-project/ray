...
class DDPGReplayEvaluator(DDPGEvaluator):
    """Wraps DDPGEvaluators to provide replay buffer functionality.
    ...
    def __init__(self, registry, env_creator, config, logdir):
        DDPGEvaluator.__init__(self, registry, env_creator, config, logdir)

        # Create extra workers if needed
        if self.config["num_workers"] > 1:
            remote_cls = ray.remote(num_cpus=1)(DDPGEvaluator)
            self.workers = [
    ...
    def sample(self, no_replay=False):
        ...
        if self.workers:
            ...
            samples = [DDPGEvaluator.sample(self)]
    ...
    def compute_critic_gradients(self, samples):
        td_errors, grad = self.ddpg_graph.compute_critic_gradients(
    ...
    def stats(self):
        if self.workers:
            ...
            return DDPGEvaluator.stats(self)

    def save(self):
        return [
            DDPGEvaluator.save(self),
            ray.get([w.save.remote() for w in self.workers]),
            self.beta_schedule,
            self.replay_buffer]

    def restore(self, data):
        DDPGEvaluator.restore(self, data[0])
