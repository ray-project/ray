...
class EnvContext(dict):
    ...
    Attributes:
        ...
        env_index (int): When there are multiple envs per worker, this uniquely
            identifies the env index within the worker.
    """

    def __init__(self, env_config, worker_index, env_index=0):
        dict.__init__(self, env_config)
        self.worker_index = worker_index
        self.env_index = env_index

    def with_env_index(self, env_index):
        return EnvContext(
            self, worker_index=self.worker_index, env_index=env_index)
