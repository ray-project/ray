...
@dataclass
class XGBoostConfigImpl(BackendConfig):
...
class _XGBoostRabitBackend(Backend):
    ...
    def on_training_start(
        self, worker_group: WorkerGroup, backend_config: XGBoostConfigImpl
    ):
        assert backend_config.xgboost_communicator == "rabit"
        self._setup_xgboost_distributed_backend(worker_group)

    def on_shutdown(self, worker_group: WorkerGroup, backend_config: XGBoostConfigImpl):
        timeout = 5
