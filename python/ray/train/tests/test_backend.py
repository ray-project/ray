...
def gen_execute_special(special_f):
    def execute_async_special(self, name: str, f):
        """Runs f on worker 0, special_f on other workers."""
        futures = [
            self.workers[0].actor._RayTrainWorker__execute.options(name=name).remote(f)
        ]
        for worker in self.workers[1:]:
            futures.append(
                worker.actor._RayTrainWorker__execute.options(name=name).remote(
                    special_f
                )
            )
        return futures
