...
def prepare_dataset():
    ...
    train_dataset_raw: ray.data.Dataset = ray.data.read_datasource(SimpleTorchDatasource(), dataset_factory=train_dataset_factory)
    test_dataset_raw: ray.data.Dataset = ray.data.read_datasource(SimpleTorchDatasource(), dataset_factory=test_dataset_factory)
...
def test_callback():
    ...
    trainer_init_config['callbacks'] = [EarlyStopper("Accuracy", "my_evaluator", min_delta=0.9, patience=0)]
...
def test_resumed_training():
    ...
    result2 = trainer.fit()
    assert result.metrics_dataframe['iterations_since_restore'].max() == result2.metrics_dataframe['iterations_since_restore'].max()

    # Train for 1 more epoch, now loading using `load_path` instead of resume from checkpoint
    trainer_init_config["load_path"] = str(result.checkpoint.to_dict()["last_checkpoint"][0])
    trainer = MosaicTrainer(
        trainer_init_per_worker=trainer_init_per_worker,
        datasets={"train": train_dataset, "evaluation": test_dataset},
        trainer_init_config=trainer_init_config,
