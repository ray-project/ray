...
class Checkpoint:
    ...
    def __init__(
        ...
        # and the path. This ensures that each checkpoint has a distinct UUID,
        # preventing conflicts with other checkpoints.
        # The UUID is used to create a temporary directory name on the local filesystem,
        # which will be used for downloading checkpoint data.
        # This ensures that if multiple processes download data from the same location,
        # only one process performs the actual download while the others wait
        # (using a FileLock). This prevents duplicated download efforts and data.
        self._uuid = uuid.uuid5(
            _CHECKPOINT_UUID_NAMESPACE, self.filesystem.type_name + self.path
        )
