...
def _resolve_storage_type(
    storage_path_type: str, tmp_path: Path, mock_s3_bucket_uri: str
) -> Tuple[str, Optional[pyarrow.fs.FileSystem]]:
    storage_path, storage_filesystem = None, None

    if storage_path_type == "cloud":
...
def test_trainer(
    ...
    with _resolve_storage_type(storage_path_type, tmp_path, mock_s3_bucket_uri) as (
        storage_path,
        storage_filesystem,
    ):
