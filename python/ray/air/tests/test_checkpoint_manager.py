...
def test_unlimited_persistent_checkpoints():
    ...
    for i in range(10):
        cpm.register_checkpoint(_TrackedCheckpoint({"data": i}))

    assert len(cpm._top_checkpoints) == 10
...
def test_limited_persistent_checkpoints():
    ...
    assert len(cpm._top_checkpoints) == 2
...
def test_no_persistent_checkpoints():
    ...
    assert len(cpm._top_checkpoints) == 0
...
def test_keep_best_checkpoints():
    ...
    assert [cp.tracked_checkpoint.metrics["metric"] for cp in cpm._top_checkpoints] == [
        1,
        0,
    ]
