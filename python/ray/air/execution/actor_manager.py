...
def _resolve_many_futures(
    ...
    try:
        ...
        # Resolve every future to detect which actor failed
        # Todo: continue
        return [ActorFailed]
...
class ActorManager:
    ...
    def __init__(self, resource_manager: ResourceManager):
        ...
        self._actors_to_remove: Dict[ray.actor.ActorHandle, bool] = {}
    ...
    def remove_actor(
        ...
        If ``resolve_futures=True``, we will cache the actor removal and only
        remove it once all its futures are resolved.

        If ``kill=True``, this will use ``ray.kill()`` to forcefully terminate the
        actor. Otherwise, it will schedule actor deconstruction after all running
        ...
        Args:
            ...
            kill: If set, will forcefully terminate the actor.
        """
        if resolve_futures and (
            ...
            self._actors_to_remove[actor] = kill
            return
        ...
        self._next_events.append(
            ActorStopped(actor_id=actor._actor_id.hex(), actor_info=info)
        )
    ...
    def _start_new_actors(self):
        ...
        for actor_request in self._actor_requests:
            if self._resource_manager.has_resources_ready(actor_request.resources):
                ...
                [annotated_actor_cls] = allocated_resources.annotate_remote_objects(
                    [remote_actor_cls]
                )
                actor = annotated_actor_cls.remote(**actor_request.kwargs)
                actor_info = ActorInfo(
                    actor_request=actor_request, allocated_resource=allocated_resources
                )
