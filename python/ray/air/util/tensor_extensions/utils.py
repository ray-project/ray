...
def _should_convert_to_tensor(
    ...
    if res:
        print(f">>> [DBG] _should_convert_to_tensor(column_values): {column_values=}, {type(column_values[0])=}, {_is_ndarray_tensor(column_values)=}, {_is_ndarray_like_not_pyarrow_array_or_pd_series(column_values)=}, {_is_ndarray_like_not_pyarrow_array_or_pd_series(column_values[0])=} {res=}")
...
def _is_ndarray_like_not_pyarrow_array_or_pd_series(column_values):
    return (
        ...
        not _is_arrow_array(column_values)
