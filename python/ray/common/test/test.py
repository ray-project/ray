...
class TestObjectID(unittest.TestCase):
    ...
    def test_equality_comparisons(self):
        x1 = local_scheduler.ObjectID(ray._RAY_ID_SIZE * b"a")
        x2 = local_scheduler.ObjectID(ray._RAY_ID_SIZE * b"a")
        y1 = local_scheduler.ObjectID(ray._RAY_ID_SIZE * b"b")
        y2 = local_scheduler.ObjectID(ray._RAY_ID_SIZE * b"b")
        self.assertEqual(x1, x2)
        self.assertEqual(y1, y2)
        self.assertNotEqual(x1, y1)

        random_strings = [
            np.random.bytes(ray._RAY_ID_SIZE) for _ in range(256)
        ]
        object_ids1 = [
