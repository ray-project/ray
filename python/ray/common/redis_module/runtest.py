...
class TestGlobalStateStore(unittest.TestCase):
    ...
    def testInvalidObjectTableAdd(self):
        ...
        with self.assertRaises(redis.ResponseError):
            self.redis.execute_command(
                "RAY.OBJECT_TABLE_ADD", "object_id2", "one", "hash2",
                "manager_id1"
            )
        with self.assertRaises(redis.ResponseError):
            self.redis.execute_command(
                "RAY.OBJECT_TABLE_ADD", "object_id2", 1, "hash2", "manager_id1",
                "extra argument"
            )
        ...
        self.redis.execute_command(
            "RAY.OBJECT_TABLE_ADD", "object_id1", 1, "hash1", "manager_id1"
        )
        response = self.redis.execute_command(
            "RAY.OBJECT_TABLE_LOOKUP", "object_id1"
        )
        self.assertEqual(set(response), {b"manager_id1"})
        with self.assertRaises(redis.ResponseError):
            self.redis.execute_command(
                "RAY.OBJECT_TABLE_ADD", "object_id1", 1, "hash2", "manager_id2"
            )
        ...
        self.redis.execute_command(
            "RAY.OBJECT_TABLE_ADD", "object_id1", 1, "hash2", "manager_id1"
        )
        self.redis.execute_command(
            "RAY.OBJECT_TABLE_ADD", "object_id1", 1, "hash2", "manager_id1"
        )
        ...
        self.redis.execute_command(
            "RAY.OBJECT_TABLE_ADD", "object_id1", 2, "hash2", "manager_id2"
        )
        response = self.redis.execute_command(
            "RAY.OBJECT_TABLE_LOOKUP", "object_id1"
        )
    ...
    def testObjectTableAddAndLookup(self):
        ...
        self.redis.execute_command(
            "RAY.OBJECT_TABLE_ADD", "object_id1", 1, "hash1", "manager_id2"
        )
        response = self.redis.execute_command(
            "RAY.OBJECT_TABLE_LOOKUP", "object_id1"
        )
        ...
        self.redis.execute_command(
            "RAY.OBJECT_TABLE_ADD", "\x00object_id3", 1, "hash1", "manager_id1"
        )
        self.redis.execute_command(
            "RAY.OBJECT_TABLE_ADD", "\x00object_id4", 1, "hash2", "manager_id1"
        )
        # Check that NULL characters in the hash are handled properly.
        self.redis.execute_command(
            "RAY.OBJECT_TABLE_ADD", "object_id3", 1, "\x00hash1", "manager_id1"
        )
        with self.assertRaises(redis.ResponseError):
            self.redis.execute_command(
                "RAY.OBJECT_TABLE_ADD", "object_id3", 1, "\x00hash2",
                "manager_id1"
            )
    ...
    def testObjectTableAddAndRemove(self):
        ...
        with self.assertRaises(redis.ResponseError):
            self.redis.execute_command(
                "RAY.OBJECT_TABLE_REMOVE", "object_id1", "manager_id1"
            )
        # Try calling RAY.OBJECT_TABLE_LOOKUP with an object ID that has not
        # been added yet.
        response = self.redis.execute_command(
        ...
        # same set.
        self.redis.execute_command(
            "RAY.OBJECT_TABLE_REMOVE", "object_id1", "manager_id3"
        )
        response = self.redis.execute_command(
            "RAY.OBJECT_TABLE_LOOKUP", "object_id1"
        )
        ...
        self.redis.execute_command(
            "RAY.OBJECT_TABLE_REMOVE", "object_id1", "manager_id2"
        )
        response = self.redis.execute_command(
            "RAY.OBJECT_TABLE_LOOKUP", "object_id1"
        )
    ...
    def testObjectTableSubscribeToNotifications(self):
        ...
        # notifications.
        def check_object_notification(
            notification_message, object_id, object_size, manager_ids
        ):
            notification_object = (
                SubscribeToNotificationsReply.
                GetRootAsSubscribeToNotificationsReply(notification_message, 0)
            )
            self.assertEqual(notification_object.ObjectId(), object_id)
            self.assertEqual(notification_object.ObjectSize(), object_size)
            self.assertEqual(
                notification_object.ManagerIdsLength(), len(manager_ids)
            )
        ...
        self.redis.execute_command(
            "RAY.OBJECT_TABLE_ADD", "object_id1", data_size, "hash1",
            "manager_id2"
        )
        ...
        self.redis.execute_command(
            "RAY.OBJECT_TABLE_REQUEST_NOTIFICATIONS", "manager_id1",
            "object_id1"
        )
        ...
        self.redis.execute_command(
            "RAY.OBJECT_TABLE_ADD", "object_id3", data_size, "hash1",
            "manager_id1"
        )
        self.redis.execute_command(
            "RAY.OBJECT_TABLE_ADD", "object_id3", data_size, "hash1",
        ...
        self.redis.execute_command(
            "RAY.OBJECT_TABLE_ADD", "object_id2", data_size, "hash1",
            "manager_id3"
        )
        # Verify that the notification is correct.
        check_object_notification(
    ...
    def testResultTableAddAndLookup(self):
        ...
        # added.
        response = self.redis.execute_command(
            "RAY.RESULT_TABLE_LOOKUP", "object_id1"
        )
        ...
        task_id = b"task_id1"
        self.redis.execute_command(
            "RAY.RESULT_TABLE_ADD", "object_id1", task_id, 0
        )
        response = self.redis.execute_command(
            "RAY.RESULT_TABLE_LOOKUP", "object_id1"
        )
        ...
        task_id = b"task_id2"
        self.redis.execute_command(
            "RAY.RESULT_TABLE_ADD", "object_id2", task_id, 1
        )
        response = self.redis.execute_command(
            "RAY.RESULT_TABLE_LOOKUP", "object_id2"
        )
    ...
    def testInvalidTaskTableAdd(self):
        ...
        with self.assertRaises(redis.ResponseError):
            self.redis.execute_command(
                "RAY.TASK_TABLE_ADD", "task_id", 3, "node_id"
            )
        with self.assertRaises(redis.ResponseError):
            # Non-integer scheduling states should not be added.
            self.redis.execute_command(
                "RAY.TASK_TABLE_ADD", "task_id", "invalid_state", "node_id",
                "task_spec"
            )
        with self.assertRaises(redis.ResponseError):
            # Should not be able to update a non-existent task.
            self.redis.execute_command(
                "RAY.TASK_TABLE_UPDATE", "task_id", 10, "node_id", b""
            )
    ...
    def testTaskTableAddAndLookup(self):
        ...
        response = self.redis.execute_command(
            "RAY.TASK_TABLE_ADD", "task_id", *task_args
        )
        ...
        self.redis.execute_command(
            "RAY.TASK_TABLE_UPDATE", "task_id", *task_args[:4]
        )
        response = self.redis.execute_command("RAY.TASK_TABLE_GET", "task_id")
        check_task_reply(response, task_args)
        ...
        response = self.redis.execute_command(
            "RAY.TASK_TABLE_TEST_AND_UPDATE", "task_id", *task_args[:3]
        )
        check_task_reply(response, task_args[1:], updated=True)
        # Check that the task entry is still the same.
        get_response = self.redis.execute_command(
            "RAY.TASK_TABLE_GET", "task_id"
        )
        ...
        response = self.redis.execute_command(
            "RAY.TASK_TABLE_TEST_AND_UPDATE", "task_id", *new_task_args[:3]
        )
        check_task_reply(response, task_args[1:], updated=False)
        # Check that the update did not happen.
        get_response2 = self.redis.execute_command(
            "RAY.TASK_TABLE_GET", "task_id"
        )
    ...
    def check_task_subscription(self, p, scheduling_state, local_scheduler_id):
        ...
        self.assertEqual(
            notification_object.ExecutionDependencies(), task_args[3]
        )
    ...
    def testTaskTableSubscribe(self):
        ...
        p.psubscribe(
            "{prefix}{local_scheduler_id}:*".format(
