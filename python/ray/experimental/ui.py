...
class UI(object):
    ...
    # recursions.
    def __init__(self):
        self.total_time_value = "% total time"
        self.total_tasks_value = "% total tasks"
    ...
    def get_sliders(self, update):
        ...
        breakdown_opt = widgets.Dropdown(
            options=[self.total_time_value, self.total_tasks_value],
            value=self.total_tasks_value,
        ...
        # currently executing
        out_recursion = self._EventRecursionContextManager()
        ...
        def update_wrapper(event):
            ...
            with out_recursion:
                ...
                if num_tasks is not 0:
                    ...
                    if event == INIT_EVENT:
                        if breakdown_opt.value == self.total_tasks_value:
                            num_tasks_box.value = -min(10000, num_tasks)
                            range_slider.value = (int(100 -
                             (100. * -num_tasks_box.value) / num_tasks),
                             100)
                        else:
                            low, high = map(lambda x: x / 100.,
                                            range_slider.value)
                    ...
                    if breakdown_opt.value == self.total_time_value:
                    ...
                    # run.)
                    elif breakdown_opt.value == self.total_tasks_value:
    ...
    def task_timeline(self):
        ...
        start_box, end_box, range_slider, time_opt = self.get_sliders(False)
        ...
        def handle_submit(sender):
            ...
            if time_opt.value == self.total_time_value:
                tasks = ray.global_state.task_profiles(
                    start=smallest + diff * low,
                    end=smallest + diff * high)
            elif time_opt.value == self.total_tasks_value:
                if range_slider.value[0] == 0:
                    tasks = ray.global_state.task_profiles(
                        num_tasks=int(num_tasks * high),
                        fwd=True)
    ...
    def task_completion_time_distribution(self):
        ...
        self.get_sliders(task_completion_time_update)
    ...
    def cpu_usage(self):
        ...
        def plot_utilization():
            ...
            def update_plot(abs_earliest, abs_latest, abs_num_tasks, tasks):
                num_buckets = 100
                left, right, top = self.compute_utilizations(abs_earliest,
                                                        abs_latest,
                                                        abs_num_tasks,
                                                        tasks,
                                                        num_buckets)
    ...
    def cluster_usage(self):
        ...
        def heat_map_update(abs_earliest, abs_latest, abs_num_tasks, tasks):
            ...
            for node_ip, task_dict in node_to_tasks.items():
                left, right, top = self.compute_utilizations(earliest,
                                                             latest,
                                                             abs_num_tasks,
                                                             task_dict,
                                                             100,
        ...
        self.get_sliders(heat_map_update)
