...
class RaySDKWorker(object):
    ...
    def run(self):
        ...
        actor = BeamWorker.options(
            override_environment_variables=env_dict).remote()
        try:
    ...
    def process_bundle(
        ...
        def execute(part_map_input_timers):
            ...
            return bundle_manager.process_bundle(
                part_map, expected_outputs, input_timers,
                expected_output_timers, dry_run)
        ...
        with thread_pool_executor.shared_unbounded_instance() as executor:
            for result, split_result in executor.map(
                    execute, zip(part_inputs, timer_inputs)):
                #print("===== result below =====:")
                #print(result)
                #print("===== result above =====:")
