...
class BaseImageManager(ABC):
    ...
    def create_oci_spec(
        ...
        Args:
            ...
            resolv_conf_source: Optional file to bind-mount read-only at
                /etc/resolv.conf (the host's file, or one generated by
                prepare_oci_bundle from the dns setting).
    ...
    def prepare_oci_bundle(
        ...
        Args:
            ...
            network: Sandbox network mode. With "public", a generated
                resolv.conf (from ``dns``, default public resolvers) is
                mounted; with "host", the host's own resolv.conf is mounted
                unless ``dns`` overrides it.
...
class ImageManager(BaseImageManager):
    ...
    def create_oci_spec(
        ...
        if network in ("host", "public"):
            ...
            # runsc was launched in, which is what host-side networking
            # ("host" and "public") means.
        ...
        # Host-side networking shares the host's network stack but not its
        # resolver configuration, and most images ship an empty
        # /etc/resolv.conf because container engines inject one at run time.
        # The caller (prepare_oci_bundle) decides which file to inject: the
        # host's own for network="host", or a generated one for "public"/dns.
    ...
    def prepare_oci_bundle(
        ...
        if network in ("public", "host"):
            ...
            if nameservers is not None:
                # A synthetic, portable resolv.conf: nothing about the host's
                # resolver configuration (search domains, internal VIPs,
                # ndots) is inherited.
