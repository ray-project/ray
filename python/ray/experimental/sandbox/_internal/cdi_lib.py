...
calls to `ray.experimental.sandbox._internal.cdi.get_spec`, passing it
a resource type (e.g. "GPU"). This resource type then gets resolved by
Ray to the right accelerator manager (e.g. `NvidiaGPUAcceleratorManager`),
whose `generate_cdi_spec` gets passed in as the callback this module
needs. Once a `CDISpec` is in hand, `select_devices` looks up specific
devices by id, and `apply_edits` merges their `containerEdits` into an
OCI runtime spec in place.

Vendor-agnostic on purpose, even though `ray.experimental.sandbox` is
its only caller today: nothing here references any specific
accelerator vendor, so a second caller wouldn't need to fork it.
"""
...
class CDISpec:
    ...
    def generate(
        ...
        `ray.experimental.sandbox._internal.cdi.get_spec`) needs,
        ...
        Args:
            ...
            generate_fn: Returns the parsed CDI spec, or None if this kind
                doesn't support generation. Vendor-specific (e.g. shells
                out to nvidia-ctk). Any exception it raises propagates
                unchanged.

        Returns:
            A `CDISpec`, or None if `generate_fn` returned None.
        """
        spec = generate_fn()
