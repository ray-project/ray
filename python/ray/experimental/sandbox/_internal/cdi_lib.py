...
`ray.experimental.sandbox._internal.cdi.get_spec`. Always generating
avoids silently picking up an incompatible spec.

Lives under `ray.experimental.sandbox._internal`, not `ray/_private/`:
`cdi.py` depends on this module and itself must resolve accelerator
managers via `ray._private.accelerators`, but this module has no need
to reach into `ray._private` at all, so tying it to that location
would be an unwarranted coupling.
...
`ray.experimental.sandbox._internal.cdi` builds NVIDIA GPU support
(and, mechanically, any future vendor's) on top of this by resolving
`kind` from
...
class CDISpec:
    ...
    def generate(
        ...
        spec" function (e.g. `ray.experimental.sandbox._internal.cdi.get_spec`) needs,
        ...
        `ray.experimental.sandbox._internal.cdi`'s module docstring for why that's fine.
