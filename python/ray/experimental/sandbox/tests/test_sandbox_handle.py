...
def test_sandbox_runtime_interface(tmp_path):
    ...
    mock_backend.create_sandbox.return_value = "sb-rt-1"
    mock_backend.exec_command.return_value = ExecResult(
        exit_code=0, stdout="Hello world\n", stderr="", duration_seconds=0.1
    )
    mock_backend.read_file.return_value = b"runtime read content"

    rt = SandboxRuntime(backend=mock_backend)
    config = SandboxConfig()
    instance_id = rt.create(config)
    assert instance_id == "sb-rt-1"
    ...
    mock_backend.write_file.assert_called_with(
        "sb-rt-1", "/workspace/rt_up.txt", b"rt upload bytes"
    )

    # Test download_file
    local_down = tmp_path / "rt_down.txt"
    rt.download_file(instance_id, "/workspace/rt_up.txt", str(local_down))
    assert local_down.read_bytes() == b"runtime read content"

    rt.delete(instance_id)
    mock_backend.delete_sandbox.assert_called_with("sb-rt-1")
...
def test_sandbox_actor_wrapper():
    ...
    rt = GVisorSandboxRuntime(runsc_path_override="/bin/sh")
