...
def test_gpu_ids_select_overlayfs_rootfs():
    """A sandbox with GPUs runs on an overlayfs rootfs, however it's created,
    and one without on the EROFS image."""
    with patch("ray.get_gpu_ids", return_value=["0"]):
        assert SandboxConfig(image="python:3.10-slim")._rootfs_type == "erofs"
        config = SandboxConfig(image="python:3.10-slim", gpu_ids=["0"])
    assert config._rootfs_type == "overlayfs"
