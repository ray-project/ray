...
# Docker; pass ``capabilities: []`` to keep Ray Sandbox's minimal set.
...
class CreateSandboxRequest(BaseModel):
    ...
    capabilities: Optional[List[str]] = Field(
        default=None,
        description=(
            "Linux capabilities granted to the sandbox. null applies the "
            "server default (Docker's 14-capability set); [] keeps Ray "
            "Sandbox's minimal set."
        ),
    )
...
class StartExecRequest(BaseModel):
    ...
    A string command runs through the image's ``/bin/sh -c``; a list is
    executed argv-style without a shell.
    """
