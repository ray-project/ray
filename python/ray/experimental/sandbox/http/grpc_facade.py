...
async def _bounded(
    ...
    for calls that must return promptly). Unreachable actors map to
...
class RaySandboxRouterServicer(TaskCommandRouterBase):
    ...
    async def TaskExecStart(self, stream: Any) -> None:
        ...
        if request.exec_id in state.execs:
            # A retry of a start whose reply was lost: _bounded can answer
            # UNAVAILABLE while the actor call is still queued, and the SDK
            # retries that status. Starting the command again would orphan
            # the first job, so the retry acknowledges the existing one.
            logger.debug(
                "exec %s on %s already started", request.exec_id, request.task_id
            )
