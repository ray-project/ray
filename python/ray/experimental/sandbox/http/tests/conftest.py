...
class FakeSandboxRuntime:
    ...
    def __init__(self) -> None:
        ...
        # Same for create, holding the sandbox in the "starting" state.
        self.create_gate: Optional[threading.Event] = None
    ...
    def create(self, image: str, **kwargs: Any) -> str:
        ...
        if self.create_gate is not None:
            if not self.create_gate.wait(timeout=30):
                raise RuntimeError("create gate never released")
    ...
    def delete(self, instance_id: str) -> None:
        if self.delete_gate is not None:
            if not self.delete_gate.wait(timeout=30):
                raise RuntimeError("delete gate never released")
...
class FakeResolver:
    ...
    def create(
        ...
        self.create_options.append(
            {"name": name, "get_if_exists": get_if_exists, **actor_options}
        )
        if name in self.handles:
            # get_if_exists semantics: racing creates converge on one host;
            # without it Ray refuses a taken name.
