...
from ray.experimental.sandbox.exceptions import SandboxExecError
from ray.experimental.sandbox.http.app import RayActorHandleResolver, create_app
...
def test_create_omitted_ttl_defaults_to_server_cap(
    fake_resolver: FakeResolver,
) -> None:
    """Omitting ttl_seconds and sending null agree: both take the server cap,
    so a cap below one hour never rejects an ordinary create."""
    settings = SandboxAPISettings(max_ttl_seconds=600)
    with _client(fake_resolver, settings) as client:
        response = client.post(f"{BASE}/sandboxes", json={"image": "x"})
        assert response.status_code == 202, response.text
        assert response.json()["ttl_seconds"] == 600
...
def test_client_token_makes_create_idempotent(
    ...
    with _client(fake_resolver) as client:
        ...
        assert fake_resolver.create_options[0]["get_if_exists"] is True


def test_random_ids_are_long_and_skip_get_if_exists(
    fake_resolver: FakeResolver,
) -> None:
    """A fresh id has 88 random bits, so its create needs no get-or-create
    (which would attach a colliding client to someone else's sandbox)."""
    with _client(fake_resolver) as client:
        info = _create_sandbox(client)
    assert re.fullmatch(r"sb-[0-9a-f]{22}", info["sandbox_id"])
    assert fake_resolver.create_options[0]["get_if_exists"] is False
...
def test_delete_on_scheduling_actor_reports_terminating_without_killing(
    fake_resolver: FakeResolver,
) -> None:
    """An actor that does not answer within the grace is either unscheduled or
    busy with a slow runsc delete; killing it could orphan a running
    container, so the API reports "terminating" and leaves the queued
    terminate() to finish (the host exits on its own afterwards)."""
    client = _client(fake_resolver, _fast_settings())
    fake_resolver.handles["sb-stalled0001"] = _StalledHandle()
    ...
    assert response.status_code == 200, response.text
    assert response.json() == {
        "sandbox_id": "sb-stalled0001",
        "status": "terminating",
    }
    assert fake_resolver.killed == []
...
def test_list_reports_unavailable_sandboxes_as_pending(
    fake_resolver: FakeResolver,
) -> None:
    """One restarting actor must not turn the whole listing into a 503."""
    client = _client(fake_resolver, _fast_settings())
    healthy = _create_sandbox(client)
    fake_resolver.handles["sb-unavail0001"] = _UnavailableHandle()
    ...
    assert response.status_code == 200, response.text
    statuses = {s["sandbox_id"]: s["status"] for s in response.json()["sandboxes"]}
    assert statuses["sb-unavail0001"] == "pending"
    assert healthy["sandbox_id"] in statuses
...
def test_dead_actor_maps_to_404(fake_resolver: FakeResolver) -> None:
    ...
    assert response.json()["error"]["code"] == "sandbox_not_found"
    assert fake_resolver.forgotten == ["sb-deadone0001"]


class _BrokenHandle:
    """Mimics a handle whose calls fail with an unexpected error."""
...
def test_list_skips_sandboxes_that_fail_unexpectedly(
    fake_resolver: FakeResolver,
) -> None:
    """One broken sandbox must not fail the whole listing."""
    client = _client(fake_resolver, _fast_settings())
    healthy = _create_sandbox(client)
    fake_resolver.handles["sb-broken00001"] = _BrokenHandle()

    response = client.get(f"{BASE}/sandboxes")

    assert response.status_code == 200, response.text
    ids = [s["sandbox_id"] for s in response.json()["sandboxes"]]
    assert ids == [healthy["sandbox_id"]]


def test_file_write_failure_maps_to_409(fake_resolver: FakeResolver) -> None:
    runtime = FakeSandboxRuntime()
    runtime.write_error = SandboxExecError("sh: can't create /etc/x: Read-only")
    fake_resolver.next_runtime = runtime
    with _client(fake_resolver) as client:
        sandbox_id = _create_sandbox(client)["sandbox_id"]
        _wait_running(client, sandbox_id)
        response = client.put(
            f"{BASE}/sandboxes/{sandbox_id}/files",
            params={"path": "/etc/x"},
...
def test_resolver_reuses_one_actor_class_and_caches_fresh_handles(
    ...
    resolver = RayActorHandleResolver(SandboxAPISettings())

    fresh = resolver.create("sb-fresh", {}, {}, get_if_exists=False)
    resolver.create("sb-token", {}, {}, get_if_exists=True)

    # One ActorClass (one class export) for every create.
    assert len(exported) == 1
    # A fresh random name is served from the cache ...
    assert resolver.get("sb-fresh") is fresh
    # ... a deterministic one always goes to ray.get_actor, which follows
    # deaths and re-creations under the same name.
    assert resolver.get("sb-token") == "handle-sb-token"
    resolver.forget("sb-fresh")
    assert resolver.get("sb-fresh") == "handle-sb-fresh"
    assert lookups == ["sb-token", "sb-fresh"]


class _LosesFirstBoot(FakeResolver):
    """The first create's boot() never reaches the actor, as when its replica
    dies right after registering the actor."""
...
def test_client_token_retry_boots_a_stuck_sandbox() -> None:
    resolver = _LosesFirstBoot()
    with _client(resolver) as client:
        body = {"image": "x", "client_token": "trial-lost-boot"}
        sandbox_id = client.post(f"{BASE}/sandboxes", json=body).json()["sandbox_id"]
        assert resolver.runtimes == []  # never booted
        retry = client.post(f"{BASE}/sandboxes", json=body)
        assert retry.status_code == 200
        info = _wait_running(client, sandbox_id)
    assert info["status"] == "running"


def test_package_and_resolver_import_without_fastapi() -> None:
    """Only the REST app needs the Serve extra; importing the package or the
    resolver must not import FastAPI."""
    import subprocess

    code = (
        "import sys; import ray.experimental.sandbox.http; "
        "import ray.experimental.sandbox.http.resolver; "
