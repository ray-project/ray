...
# ----------------------------------------------------------------------
# Terminate racing the boot
# ----------------------------------------------------------------------


async def _await_status(host: SandboxHost, status: str) -> None:
    for _ in range(500):
        if (await host.describe())["status"] == status:
            return
        await asyncio.sleep(0.01)
    raise AssertionError(f"host never reached {status}")


def test_terminate_during_start_deletes_the_container() -> None:
    """A terminate() while runtime.create runs must not orphan the container:
    it waits for the create to settle and deletes the new instance."""
    runtime = FakeSandboxRuntime()
    runtime.create_gate = threading.Event()
    host = _make_host(runtime)

    async def scenario():
        boot = asyncio.create_task(host.boot())
        await _await_status(host, "starting")
        terminate = asyncio.create_task(host.terminate())
        await asyncio.sleep(0.1)
        waited = not terminate.done()
        runtime.create_gate.set()
        result = await asyncio.wait_for(terminate, 5)
        await boot
        return waited, result, await host.describe()

    waited, result, info = asyncio.run(scenario())
    assert waited, "terminate() returned before the starting container existed"
    assert result == {"ok": True}
    assert info["status"] == "terminated"
    assert runtime.deleted == [runtime.instance_id]


def test_terminate_during_pull_skips_the_create() -> None:
    """No container exists during the pull, so terminate() returns at once
    and the boot creates nothing once the pull finishes."""
    runtime = FakeSandboxRuntime()
    runtime.pull_gate = threading.Event()
    host = _make_host(runtime)
    ...
    async def scenario():
        boot = asyncio.create_task(host.boot())
        await _await_status(host, "pulling")
        result = await asyncio.wait_for(host.terminate(), 5)
        runtime.pull_gate.set()
        await boot
        return result, await host.describe()

    result, info = asyncio.run(scenario())
    assert result == {"ok": True}
    assert info["status"] == "terminated"
...
def test_ttl_during_start_deletes_the_container() -> None:
    runtime = FakeSandboxRuntime()
    runtime.create_gate = threading.Event()
    host = _make_host(runtime, spec_overrides={"ttl_seconds": 0.05})
    ...
    async def scenario():
        boot = asyncio.create_task(host.boot())
        await _await_status(host, "starting")
        await asyncio.sleep(0.2)  # the TTL fired and is waiting on the create
        deleted_early = list(runtime.deleted)
        runtime.create_gate.set()
        await boot
        await _await_status(host, "terminated")
        return deleted_early

    deleted_early = asyncio.run(scenario())
    assert deleted_early == []
    assert runtime.deleted == [runtime.instance_id]


def test_boot_after_terminate_starts_nothing() -> None:
    runtime = FakeSandboxRuntime()
    host = _make_host(runtime)
...
def test_write_failure_is_reported_not_raised() -> None:
    ...
    async def scenario():
        await host.boot()
        return await host.write_file("/etc/x", b"hi")
...
def test_work_is_refused_while_terminating() -> None:
    ...
    async def scenario():
        await host.boot()
        terminate = asyncio.create_task(host.terminate())
        await asyncio.sleep(0.1)  # terminate() is blocked in the delete
        results = [
