...
class SandboxHost:
    ...
    async def boot(self) -> None:
        ...
        try:
            ...
            self._instance_id = await asyncio.to_thread(
                ...
                # Never bind a host scratch dir over the workdir: the image's
                # own WORKDIR content must stay visible (readonly=False covers
                # writability via the per-sandbox overlay).
                mount_workdir=False,
    ...
    async def start_exec(
        ...
        task = asyncio.create_task(
            self._run_exec(job, command, cwd, env, timeout_seconds)
        )
