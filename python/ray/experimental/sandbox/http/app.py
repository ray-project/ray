...
from ray.experimental.sandbox.http.resolver import (
    ...
    _is_unschedulable,
    _new_random_sandbox_id,
    _sandbox_name_for_token,
)
    ...
    *,
    ...
    async def _bounded_call(
        ...
        try:
            ...
            if exc.code == "sandbox_not_found":
                # The actor is gone; a cached handle must not outlive it.
                resolver.forget(sandbox_id)
            raise

    # Resolver calls can wait on a GCS round trip (an actor create, a cache
    # miss), so they run in worker threads and never stall the event loop
    # that serves every other request.
    async def _get(sandbox_id: str) -> Optional[Any]:
        return await asyncio.to_thread(resolver.get, sandbox_id)

    async def _lookup(sandbox_id: str) -> Any:
        handle = await _get(sandbox_id)
        if handle is None:
            raise _sandbox_not_found(sandbox_id)
        return handle

    async def _kill(sandbox_id: str, handle: Any) -> None:
        resolver.forget(sandbox_id)
        await asyncio.to_thread(resolver.kill, handle)

    def _pending_info(sandbox_id: str) -> Dict[str, Any]:
        # The actor owns the spec, so report a shape-complete pending info
        # with the fields only it knows left empty.
        return {
    ...
    async def _describe_or_pending(
        ...
        try:
            ...
            return _pending_info(sandbox_id)

    async def _listing_entry(sandbox_id: str, handle: Any) -> Dict[str, Any]:
        """Describe one sandbox for the listing.

        A transiently unreachable actor is a fact about one sandbox, not the
        listing: it shows as pending rather than failing the whole request
        (a single GET on it still reports 503).
        """
        try:
            return await _describe_or_pending(sandbox_id, handle)
        except _ApiError as exc:
    ...
    async def create_sandbox(request: CreateSandboxRequest) -> JSONResponse:
        ...
        if request.client_token is not None:
            ...
            existing = await _get(sandbox_id)
            if existing is not None:
                try:
                    ...
                    await _kill(sandbox_id, existing)
                else:
                    # boot() is idempotent. Sending it again covers a first
                    # create whose boot call never reached the actor (its
                    # replica died), which would leave the sandbox pending
                    # with no TTL to reclaim it.
                    existing.boot.remote()
                    return JSONResponse(status_code=200, content=info)
        else:
            sandbox_id = _new_random_sandbox_id()
        ...
        handle = await asyncio.to_thread(
            ...
            get_if_exists=request.client_token is not None,
        )
    ...
    async def list_sandboxes(
        ...
        names = await asyncio.to_thread(resolver.list_names)
        handles = await asyncio.gather(*(_get(name) for name in names))
        named = list(zip(names, handles))
        # Concurrently: sequential describes would make the listing scale
        # linearly with the number of sandboxes.
        listed = [(name, handle) for name, handle in named if handle is not None]
        listed_names = [name for name, _ in listed]
        results = await asyncio.gather(
            *(_listing_entry(name, handle) for name, handle in listed),
            return_exceptions=True,
        )
        sandboxes: List[Dict[str, Any]] = []
        for name, info in zip(listed_names, results):
            if isinstance(info, _ApiError) and info.code == "sandbox_not_found":
                continue
            if isinstance(info, BaseException):
                # One broken sandbox is a fact about that sandbox, not the
                # listing (a GET on it still reports the error).
                logger.warning("Listing: failed to describe sandbox %s: %r", name, info)
                continue
    ...
    async def get_sandbox(
        ...
        handle = await _lookup(sandbox_id)
    ...
    async def delete_sandbox(sandbox_id: str) -> Dict[str, str]:
        ...
        if handle is not None:
            try:
                ...
                # Still being scheduled, or mid-way through a slow runsc
                # delete: the two look alike from here. Its queued terminate()
                # finishes the job and the host exits on its own afterwards;
                # killing it now could orphan a running container.
                return {"sandbox_id": sandbox_id, "status": "terminating"}
            except _ApiError:
                # Dead, unreachable, or unschedulable actors are killed below.
                pass
            await _kill(sandbox_id, handle)
    ...
    async def put_file(
        ...
        if result.get("error_code") in ("conflict", "write_failed"):
            raise _ApiError(409, result["error_code"], result["message"])
