...
def receive(sources, timeout=10**12):
    """Get all signals from each source in sources that have been not received yet.

    A source can be either (1) an object id returned by the task (we want to receive
    signals from), or (2) an actor handle.

    For each source S, this function returns all signals associated to S since
    the last receive() or forget() were invoked on S. If this is the first call on
    S, this function returns all past signals generated by S so far.

    Args:
        sources: list of sources from which caller waits for signals. A source is
            either an object id identifying the task returning the object,
            or an actor handle.
        timeout: Time this function waits to get a signal from a source in sources.
            If none, return when timeout expires. Measured in seconds.
...
def forget(sources):
    ...
    The index of the next expected signal from S is set to the index of the last
    signal that S sent plus 1. This means that the next receive() on S
