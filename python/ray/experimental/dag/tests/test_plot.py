...
def test_serve_pipeline_single_func_deployment_dag_plot():
    ...
    graph = ray.experimental.dag.vis_utils.dag_to_dot(serve_dag)
    to_string = graph.to_string()
    assert "input_node -> input_attributed_node" in to_string
    assert "input_node -> input_attributed_node_1" in to_string
    assert "input_attributed_node -> combine" in to_string
    assert "input_attributed_node_1 -> combine" in to_string
...
def test_serve_pipeline_func_class_with_class_method_plot():
    ...
    assert "input_node -> input_attributed_node" in to_string
    assert "input_node -> input_attributed_node_1" in to_string
    assert "input_node -> input_attributed_node_2" in to_string
    assert "Model -> forward" in to_string
    assert "input_attributed_node -> forward" in to_string
    assert "Model_1 -> forward_1" in to_string
    assert "input_attributed_node_1 -> forward_1" in to_string
    assert "input_attributed_node_2 -> combine" in to_string
