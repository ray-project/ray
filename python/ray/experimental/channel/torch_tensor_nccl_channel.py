...
from ray.experimental.channel.communicator_info_holder import CommunicatorInfoHolder
...
def _do_init_communicator(
    ...
    if custom_communicator is not None:
        ...
        ctx.communicators[group_id] = AcceleratorRuntime.get().get_communicator(
            world_size,
            comm_id,
            rank,
            actor_handles,
            AcceleratorRuntime.get().current_stream(),
            use_communication_streams,
        )
...
def _do_check_has_communicator(self) -> bool:
    ...
    return AcceleratorRuntime.get().has_communicator()


def _do_get_unique_communication_id(self) -> bool:
    from ray.experimental.channel.utils import AcceleratorRuntime

    return AcceleratorRuntime.get().get_unique_id()
