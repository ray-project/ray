...
def _do_init_communicator(
    ...
    if not custom_communicator:
        assert (
            _do_get_accelerator_name(self) != "cpu"
...
def _do_get_accelerator_name(self) -> str:
    return AcceleratorContext.get().get_module_name()
...
def _init_communicator(
    ...
    accelerator_names = ray.get(
        [actor.__ray_call__.remote(_do_get_accelerator_name) for actor in actors]
    )
    for accelerator_name, actor in zip(accelerator_names, actors):
        if accelerator_name != accelerator_names[0] and not is_cpu_communicator:
            raise ValueError(
                f"Inconsistent accelerator usage detected: Actor {actor} "
                "is using accelerator '{accelerator_name}', "
                "but other actors in the same group are using '{accelerator_names[0]}'. "
                "Ensure all actors in the group use the same accelerator "
                "when returning tensors with type hint "
                "TorchTensor(transport='nccl') or TorchTensor(transport=nccl_group_handle)."
            )
