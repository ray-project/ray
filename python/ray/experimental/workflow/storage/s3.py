...
class S3StorageImpl(Storage):
    ...
    async def load_step_input_metadata(self, workflow_id: str,
                                       step_id: StepID) -> Awaitable[Dict[str, Any]]:
        path = self._get_s3_path(workflow_id, STEPS_DIR, step_id, STEP_INPUTS_METADATA)
    ...
    async def load_step_output_metadata(self, workflow_id: str,
        ...
        path = self._get_s3_path(workflow_id, STEPS_DIR, step_id, STEP_OUTPUTS_METADATA)
        data = await self._get_object(path, True)
        return data
    ...
    async def load_step_func_body(self, workflow_id: str,
        ...
        path = self._get_s3_path(workflow_id, STEPS_DIR, step_id, STEP_FUNC_BODY)
        data = await self._get_object(path)
        return data
    ...
    async def get_step_status(self, workflow_id: str,
        ...
        async with self._client() as s3:
            response = await s3.list_objects_v2(
                Bucket=self._bucket,
                Prefix=path)
            keys = set({item["Key"].split("/")[-1] for item in response["Contents"]})
            print(keys)
            return StepStatus(
    ...
    async def _put_object(self, path: str, data: Any, is_json: bool=False) -> Awaitable[None]:
        with tempfile.SpooledTemporaryFile(mode="w+b", max_size=MAX_RECEIVED_DATA_MEMORY_SIZE) as tmp_file:
            if is_json:
    ...
    async def _get_object(self, path: str, is_json: bool=False) -> Awaitable[Any]:
        with tempfile.SpooledTemporaryFile(mode='w+b', max_size=MAX_RECEIVED_DATA_MEMORY_SIZE) as tmp_file:
            async with self._client() as s3:
    ...
    def _get_s3_path(self, *args) -> str:
        return self._s3_path + '/' + '/'.join(args)
