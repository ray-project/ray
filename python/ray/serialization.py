...
class SerializationContextMap(object):
    ...
    def __getitem__(self, driver_id):
        """Get the SerializationContext of the driver
        that this worker is processing.
    ...
    def register_custom_serializer(self,
        ...
        This method runs the register_class function defined below
        on every worker,
        ...
        Args:
            cls (type):
                The class that ray should use this custom serializer for.
            ...
            serializer: The custom serializer to use.
                This should be provided if and only if
                use_pickle and use_dict are False.
            deserializer: The custom deserializer to use.
                This should be provided
                if and only if use_pickle and use_dict are False.
            local: True if the serializers should only be
                registered on the current worker. This should usually be False.
        ...
        Raises:
            Exception: An exception is raised if pickle=False and
                the class cannot be efficiently serialized by Ray.
        ...
        if use_dict:
            # Raise an exception if cls cannot be serialized efficiently
            # by Ray.
            check_serializable(cls)

        if class_id is None:
            if not local:
                # In this case, the class ID will be used to deduplicate
                # the class across workers.
                # Note that cloudpickle unfortunately does not
                # produce deterministic strings,
                # so these IDs could be different
                ...
                try:
                    ...
                    # each worker.
                    # However, determinism is not guaranteed, and the
        ...
        def register_class_for_serialization(worker_info):
            ...
            # subsequent calls to register_custom_serializer that were
            # made by the system.
