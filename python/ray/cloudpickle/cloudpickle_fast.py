...
class CloudPickler(Pickler):
    ...
    try:
        from pydantic.fields import ModelField
        _dispatch_table[ModelField] = _pydantic_model_field_reduce
    except ImportError:
        pass

    try:
        from starlette.datastructures import State
        _dispatch_table[State] = _starlette_state_reduce
    except ImportError:
        pass
    ...
    if pickle.HIGHEST_PROTOCOL >= 5:
        ...
        # Note that when using protocol 5, `CloudPickler.dispatch` is not an
        ...
        # CloudPickler used `CloudPickler.dispatch` as a class-level attribute
        # storing all reducers implemented by cloudpickle, but the attribute
        # name was not a great choice given the meaning of
        # `CloudPickler.dispatch` when `CloudPickler` extends the pure-python
        #  pickler.
