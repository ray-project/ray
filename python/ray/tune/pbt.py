...
class PopulationBasedTraining(FIFOScheduler):
    ...
    def __init__(
            ...
            grace_period=10.0, perturbation_interval=6.0, hyperparameter_mutations=None):
    ...
    def _exploit(self, hyperparameter_mutations, best_trial, trial, trial_runner, time):
        ...
        trial.experiment_tag = trial.experiment_tag + "-mutated@" + str(time)
        trial.start()
    ...
    def _truncation_ready(self, result, trial, time):
        #function checks if appropriate time has passed and trial is in the bottom 20% of all trials, and if so, is ready
            ...
            if time_since_last >= self._perturbation_interval:
                ...
                for i in range(0, max_index):
                    if trial == sorted_result_keys[i]:
                        ...
                        return True
                print("{0} is not in the bottomn 20 percent of {1}, truncation is not ready".format(trial, [x.experiment_tag for x in sorted_result_keys]))
            else:
    ...
    def debug_string(self):
        ...
        for trial in self._completed_trials:
            ...
            if getattr(last_result, self._time_attr) < min_time or min_time == 0:
                ...
                best_trial = trial
        if best_trial != None:
            return ("The Best Trial is currently {0} finishing in {1} iterations, with the hyperparameters of {2}".format(best_trial, min_time, best_trial.config))
        else:
