...
class DistributeResources:
    ...
    def _get_upper_limits(self) -> Tuple[int, int]:
        ...
        if self.min_cpu == 0:
            ...
            upper_cpu_limit = math.ceil(
                self.total_available_cpus / num_running_trials)
            ...
            if self.add_bundles:
                upper_cpu_limit = math.ceil(
                    upper_cpu_limit /
                    self.min_cpu_bundle) * self.min_cpu_bundle
        ...
        if self.min_gpu == 0:
            ...
            upper_gpu_limit = math.ceil(
                self.total_available_gpus / num_running_trials)
            # Ensure we don't go below per-bundle minimum
            if self.add_bundles:
                upper_gpu_limit = math.ceil(
                    upper_gpu_limit /
                    self.min_gpu_bundle) * self.min_gpu_bundle
    ...
    def _modify_lower_limits_with_increase_by_times(self) -> None:
        if self.increase_by:
            ...
            if required_cpus and required_gpus:
                multiplier = min(self.free_cpus // required_cpus,
    ...
    def __call__(self, trial_runner: "trial_runner.TrialRunner", trial: Trial,
        ...
        # Get the number of CPUs and GPUs avaialble in total (not just free)
        self.total_available_cpus, self.total_available_gpus = self._get_total_available_resources(
        )
        ...
        self.trial_used_cpus, self.trial_used_gpus = self._get_used_cpus_and_gpus(
            trial)

        # Check how many CPUs and GPUs are currently being used by live trials
        used_cpus_and_gpus = [
            self._get_used_cpus_and_gpus(t)
        ...
        new_cpu = min(self.upper_cpu_limit,
                      max(self.trial_used_cpus + self.free_cpus, self.min_cpu))
        new_gpu = min(self.upper_gpu_limit,
        ...
        if self.add_bundles:
            if self.min_cpu_bundle and self.min_gpu_bundle:
                multiplier = min(new_cpu // self.min_cpu_bundle,
                                 new_gpu // self.min_gpu_bundle)
            elif self.min_gpu_bundle:
                multiplier = new_gpu // self.min_gpu_bundle
            else:
                multiplier = new_cpu // self.min_cpu_bundle
            new_bundles = [{
                "CPU": self.min_cpu_bundle,
                "GPU": self.min_gpu_bundle
            }] * int(multiplier)
    ...
    def __init__(
        ...
        if mode not in ("min", "max"):
            raise ValueError("The mode parameter can only be"
                             " either min or max.")
        super().__init__(add_bundles, increase_by, increase_by_times,
                         reserve_resources)
        self.metric = metric
        self.mode = mode
    ...
    def _get_upper_limits(self) -> Tuple[int, int]:
        ...
        if self.increase_by and self.increase_by_times > 0:
            ...
            def is_trial_below_limit(trial: Trial):
                ...
                ret = (resources.get("CPU", -np.inf) < upper_cpu_limit
                       and resources.get("GPU", -np.inf) < upper_gpu_limit)
                return ret

            best_trial = next((t for t in sorted_trials
                               if is_trial_below_limit(t)), sorted_trials[0])
        else:
...
def evenly_distribute_cpus_gpus(trial_runner: "trial_runner.TrialRunner",
    ...
    return DistributeResources(add_bundles=False)(trial_runner, trial, result,
                                                  scheduler)
...
def evenly_distribute_cpus_gpus_distributed(
    ...
    return DistributeResources(add_bundles=True)(trial_runner, trial, result,
                                                 scheduler)
