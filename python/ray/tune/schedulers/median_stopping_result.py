...
class MedianStoppingResult(FIFOScheduler):
    ...
    def fetch_lowest_common_interval(self):
        _trials = self.fetch_active_trials()
    ...
    def on_trial_result(self, trial_runner, trial, result):
        ...
        lc_interval, _active_trials = self.fetch_lowest_common_interval()
        ...
        if  (lc_interval*self._eval_interval > self._grace_period) and \
            ...
            _means, _best_results = self._get_trial_metrics(_active_trials, lc_interval * self._eval_interval)
            _median = np.median(means)

            if self._mode == "max":
                _trials_to_stop = np.array(_trials)[best_result < _median for best_result in _best_results]
            else:
                _trials_to_stop = np.array(_trials)[best_result > _median for best_result in _best_results]

            if trial in _trials_to_stop:
                self._stopped_trials.add(trial)
                _return = TrialScheduler.STOP

            for _trial in _trials_to_stop if _trial != trial:
                # if self._verbose:
                #     logger.info("MedianStoppingResult: "
                #                 "early {} {} best res={} vs median res={} at t={}".format(
                #                     "stopping" if self._hard_stop else "pausing",
        ...
        if _waiting and _return==TrialScheduler.CONTINUE:
