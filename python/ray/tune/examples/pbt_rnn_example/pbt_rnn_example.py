...
#
# We will be building and training a basic RNN to classify words.
...
#
# To represent a single letter,
# we use a "one-hot vector" of size `<1 x n_letters>`.
# A one-hot vector is filled with 0s except for
# a 1 at index of the current letter,
# e.g. `"b" = <0 1 0 0 0 ...>`.
#
# To make a word we join a bunch of those into
# a 2D matrix `<line_length x 1 x n_letters>`.
#
...
# This means you can implement a RNN in a very "pure" way,
...
def random_training_pair():
    ...
    category_tensor = Variable(
        torch.LongTensor([all_categories.index(category)]))
...
class PBTExperiment(Trainable):
    ...
    def _train(self):
        for i in range(100):
            training_pair = random_training_pair()
            category_tensor = training_pair[2]
            line_tensor = training_pair[3]
            output, loss = train(self.model, category_tensor, line_tensor,
                                 self.optimizer, i)
        ...
        self.epoch += 1
        return {
            "mean_accuracy": self.accuracy / (100 * self.epoch),
...
analysis = run(
    ...
    **{
        ...
        "config": {
            "lr": tune.sample_from(
                lambda spec: 0.0001 * random.randint(1, 50)),
