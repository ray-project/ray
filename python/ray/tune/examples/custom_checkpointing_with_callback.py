...
class SmartCheckpointCallback(Callback):
    ...
    def on_trial_result(self, iteration, trials, trial, result, **info):
        ...
        if self.checkpoint_on_improvement:
            if current_loss < self.best_loss_per_trial[trial_id] * 0.9:  # 10% improvement
                should_checkpoint = True
        ...
        if self.checkpoint_on_instability and current_step > 5:
            ...
            if len(recent_losses) == 5:
                variance = sum((x - sum(recent_losses)/5)**2 for x in recent_losses) / 5
        ...
        if should_checkpoint:
            print(f"Callback requesting checkpoint for trial {trial_id} at step {current_step}: {reason}")
...
if __name__ == "__main__":
    ...
    checkpoint_callback = SmartCheckpointCallback(
        ...
        checkpoint_on_instability=True
    )
