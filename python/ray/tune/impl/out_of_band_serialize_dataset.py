...
def _deserialize_and_fully_execute_if_needed(serialized_ds: bytes):
    ds = ray.data.Datastream.deserialize_lineage(serialized_ds)
    return ds


def _reduce(ds: ray.data.Datastream):
...
def out_of_band_serialize_dataset():
    ...
    try:
        context._register_cloudpickle_reducer(ray.data.Datastream, _reduce)
        yield
    finally:
        context._unregister_cloudpickle_reducer(ray.data.Datastream)
