...
class BeliefModel:
  ...
  def predict_xiid(self, xx, tt, yy, Budgets, oldhyp, sampler, debug=False):
    ...
    if x_:
      ...
      logdist = lambda hyp: self.ft_log_likelihood(x_, t_, y_,
                       ...
                       t_kernel_params=dict(alpha=hyp[0], beta=hyp[1], scale=hyp[2], log_noise=hyp[3],
                                            tscale=self.tscale))
    ...
    self.set_kernel_params(x_kernel_params=dict(scale=newhyp[4]),
                           t_kernel_params=dict(alpha=newhyp[0], beta=newhyp[1], scale=newhyp[2], log_noise=newhyp[3],
                                                tscale=self.tscale))
  ...
  def predict_asymptote(self, xx, tt, yy, maxL, oldhyp, sampler, debug=False):
    ...
    if x_:
      ...
      logdist = lambda hyp: self.ft_log_likelihood(x_, t_, y_,
                                                   x_kernel_params=dict(scale=hyp[4]),
                                                   t_kernel_params=dict(alpha=hyp[0], beta=hyp[1], scale=hyp[2],
                                                                        log_noise=hyp[3],
                                                                        tscale=self.tscale))
...
def slice_sample_bounded_max(logdist, oldhyp, sampler, step_out=True, debug=False):
  ...
  for ii in range(N + burn):
    ...
    if debug is True:
      print \
        ('oldhyp/bound = %s' % " ".join(
          ["%.2f/(%.2f, %.2f)" % (oldhyp[s], bounds[s][0], bounds[s][1]) for s in range(len(oldhyp))]))
...
if __name__ == "__main__":
  ...
  x_scale = 10
  x_ell = [0.000001, 1, 0.1, 0.5, 100]

  bounds = [[0.5, 5],
            [0.1, 5],
            [80, 120],
            [np.log(0.0001), np.log(1.0)],
            [1, 30],
            [0.000001, 0.1],
            [0.01, 3],
            [0.01, 3],
  ...
  hyp0 = [alpha, beta, scale, log_noise, x_scale] #, x_ell
  # hyp1 = [alpha, beta, scale, log_noise, x_scale] + x_ell
  matpath = 'data/resnet_sept.mat'
  # x, y, t, lens = process_cifar(matpath)
  ...
  #     yobs[i] = []
  #   else:
  #     yobs[i] = list(y[i][0:times[i][-1]+1])
  # 
  # FreezeThaw = BeliefModel(x_kernel_params=dict(scale=x_scale), 
  #              t_kernel_params=dict(alpha=alpha, beta=beta, scale=scale, log_noise=log_noise))
  # # xx, tt, yy, Budgets, oldhyp, sampler, 
  # sampler = dict(bounds = bounds)
  # t_star, mu, var, _ = FreezeThaw.predict_xiid(xn, times, yobs, B, hyp0, sampler, debug=True)
  # Tpred, ymu, yv = FreezeThaw.convergence(t_star, mu, var)