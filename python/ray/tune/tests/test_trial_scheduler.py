...
class PopulationBasedTestingSuite(unittest.TestCase):
    ...
    def testLogConfig(self):
        ...
        for log_file in log_files:
            self.assertTrue(
                os.path.exists(
                    os.path.join(self.storage.experiment_local_staging_path, log_file)
                )
            )
            raw_policy = open(
                os.path.join(self.storage.experiment_local_staging_path, log_file), "r"
            ).readlines()
    ...
    def testReplay(self):
        ...
        for i, trial in enumerate(trials):
            ...
            replay = PopulationBasedTrainingReplay(
                os.path.join(
                    self.storage.experiment_local_staging_path,
