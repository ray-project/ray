"""Serialize Iceberg task write results for checkpoint recovery."""
...
_FORMAT_VERSION = 1
_FORMAT_VERSION_KEY = b"ray.data.iceberg-checkpoint.format-version"
_PAYLOAD_SCHEMA = pa.schema(
    ...
    metadata={_FORMAT_VERSION_KEY: str(_FORMAT_VERSION).encode()},
)
_DATA_FILE_FIELDS = tuple(
    field.name
    for field in _PAYLOAD_SCHEMA
    if field.name not in ("record_type", "payload")
)
_REQUIRED_DATA_FILE_FIELDS = (
    "content",
    "file_path",
    "file_format",
    "partition",
...
def _deserialize_arrow_table(data: bytes) -> pa.Table:
    try:
        ...
        raise ValueError("Invalid Arrow IPC checkpoint payload") from exc
    return table
...
def serialize_write_result(write_result: IcebergWriteResult) -> bytes:
    """Serialize the Iceberg task output needed for APPEND recovery."""
...
def _row_to_data_file(row: Dict[str, Any]) -> "DataFile":
    ...
    if any(row[field] is None for field in _REQUIRED_DATA_FILE_FIELDS):
        raise ValueError("Invalid data file in Iceberg checkpoint payload")
    if row["payload"] is not None:
        raise ValueError("Invalid data file payload in Iceberg checkpoint")
    ...
    try:
        ...
        raise ValueError("Invalid data file in Iceberg checkpoint payload") from exc
    return data_file
...
def deserialize_write_result(data: bytes) -> IcebergWriteResult:
    ...
    table = _deserialize_arrow_table(data)
    if not table.schema.equals(_PAYLOAD_SCHEMA, check_metadata=True):
        raise ValueError("Unsupported Iceberg checkpoint Arrow schema or version")
    ...
    for row in table.to_pylist():
        ...
        elif row["record_type"] == "schema":
            if row["payload"] is None or any(
                row[field] is not None for field in _DATA_FILE_FIELDS
            ):
                raise ValueError("Invalid schema in Iceberg checkpoint payload")
