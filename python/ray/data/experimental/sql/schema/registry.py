...
from ray.data.experimental.sql.utils import setup_logger, validate_table_name
...
class DatasetRegistry:
    ...
    def register(self, name: str, dataset: Dataset) -> None:
        ...
        # Log successful registration (without expensive count() operation)
        self._logger.debug(f"Registered dataset '{name}' successfully")
    ...
    def get_table_info(self, name: str) -> Dict[str, Any]:
        ...
        # Return table summary without expensive count() operation
        # Note: row_count omitted for performance (count() materializes dataset)
        return {
            "name": name,
