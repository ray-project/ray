...
from ray.data.experimental.sql.base import DataHandler, ExecutionEngine, ExecutionResult
...
class NativeOrderHandler(DataHandler):
    ...
    def apply(self, dataset: Dataset, plan: ExecutionPlan) -> Dataset:
        ...
        if len(plan.ordering.columns) == 1:
            return dataset.sort(
                plan.ordering.columns[0], descending=plan.ordering.descending[0]
            )
        else:
            return dataset.sort(
                plan.ordering.columns, descending=plan.ordering.descending
            )
