...
class AggregateFn:
    ...
    Args:
        init (Callable[[Any], Any]):
            Function that creates an initial aggregator for each group. Receives a key
            (the group key) and returns the initial accumulator state (commonly 0, 
            an empty list, or an empty dictionary).
        accumulate_block (Optional[Callable[[Any, Block], Any]]):
            Function that processes an entire block of rows at once. It receives the
            ...
            is provided.
        accumulate_row (Optional[Callable[[Any, dict], Any]]):
            Function that processes an individual row. It receives the current
            accumulator and a row, then returns an updated accumulator. Cannot be
            used if `accumulate_block` is provided.
        merge (Callable[[Any, Any], Any]):
            Function that merges two accumulators generated by different workers
            into one accumulator.
        finalize (Callable[[Any], Any]):
            Function that finishes the aggregation by transforming the final
            ...
            as-is.
        name (str):
            An optional display name for the aggregator. Useful for debugging.
