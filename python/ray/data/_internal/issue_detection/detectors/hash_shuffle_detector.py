...
class HashShuffleAggregatorIssueDetector(IssueDetector):
    ...
    def detect(self) -> List[Issue]:
        ...
        for op in self._executor._topology.keys():
            ...
            aggregator_info = pool.get_aggregator_health_info()
    ...
    def _should_emit_warning(self, op_id: str, current_time: float, info: dict) -> bool:
        """Check if we should emit a warning for this operator."""
    ...
    def _format_health_warning(self, info: dict) -> str:
        ...
        return (
            ...
            f"(available CPUs: {available_cpus}, required CPUs: {info['required_cpus']}). "
            f"Cluster only has {available_memory / GiB:.2f} GiB available memory, "
