...
# Sorted reduces (map_groups / repartition(sort=True)) hold the decoded
# shards and sort_by's take copy simultaneously (~2.2x of input at peak),
# and reduce phases carry node-level memory the scheduler cannot see
# (local object store pages, raylet, spill workers). Requesting 3x packs
# fewer reducers per node, keeping nodes under the OOM kill threshold.
_SORT_SHUFFLE_PEAK_MEMORY_MULTIPLIER = 3
...
def _plan_hash_shuffle_repartition_v2(
    ...
    peak_memory_multiplier = (
        _SORT_SHUFFLE_PEAK_MEMORY_MULTIPLIER
        if logical_op.sort
