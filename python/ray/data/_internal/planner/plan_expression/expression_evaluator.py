...
def eval_projection(projection_exprs: List[Expr], block: Block) -> Block:
    ...
    if isinstance(projection_exprs[0], StarExpr):
        ...
        # renaming.
        #
        # Note: ``Project.__post_init__`` eagerly expands ``StarExpr`` to
        # explicit ``col()`` refs whenever the input schema is known, so
        # this runtime branch is hit only on the UDF-fallback path
        # (``Project`` on top of an opaque-schema input).
