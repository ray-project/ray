...
class ParquetFileReader(FileReader, SupportsMetadata):
    ...
    - **Footer-only metadata** (:class:`SupportsMetadata`): row counts read
      straight from the Parquet footer, so ``Dataset.count()`` can answer
      without reading data columns (see ``PushdownCountFiles``).
