...
class ParquetFileChunker(FileChunker):
    ...
    def generate_chunk_metadatas(
        ...
        def _emit(start: int, end: int, on_disk_size: int):
            return (
                create_chunk_metadata(
        ...
        # atomic read unit). ``RowGroupMetaData`` exposes only the *uncompressed*
        ...
        for rg_idx in range(num_row_groups):
            ...
            if running_size > 0 and running_size + rg_size > self._target_chunk_size:
                yield _emit(start, rg_idx, running_size)
                start = rg_idx
                running_size = 0
            running_size += rg_size

        # Flush the final bundle.
        yield _emit(start, num_row_groups, running_size)
