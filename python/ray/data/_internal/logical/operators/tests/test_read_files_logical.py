...
# ----------------------------------------------------------------------
# v2c — projection-aware sizing: ``ProjectionPushdown`` records the pushed
# column set on the upstream ``ListFiles`` so partition sizing (which runs in
# ``ListFiles``) can size by only the projected columns.
# ----------------------------------------------------------------------


def _optimize_projection(read_op: ReadFiles, project_exprs) -> "object":
    """Wrap ``read_op`` in a ``Project`` and run ``ProjectionPushdown``."""
    from ray.data._internal.logical.interfaces import LogicalPlan
    from ray.data._internal.logical.operators.map_operator import Project
    from ray.data._internal.logical.rules import ProjectionPushdown
    from ray.data.context import DataContext

    project = Project(exprs=project_exprs, input_dependencies=[read_op])
    plan = LogicalPlan(project, DataContext.get_current())
    return ProjectionPushdown().apply(plan).dag


def test_projection_pushdown_records_columns_on_list_files(tmp_path):
    op = _mk_read_files(tmp_path)  # columns: a, b
    assert op.input_dependencies[0].projected_columns is None  # default unset

    # ``select_columns(["a"])`` is a pure prune -> the ``Project`` is
    # subsumed and the optimized top op is the pruned ``ReadFiles``.
    top = _optimize_projection(op, [col("a")])
    assert isinstance(top, ReadFiles)
    list_files = top.input_dependencies[0]
    assert isinstance(list_files, ListFiles)
    assert list_files.projected_columns == ["a"]
    # The read itself was pruned to the same column.
    assert top.scanner.columns == ("a",)


def test_projection_pushdown_records_sorted_multi_column(tmp_path):
    op = _mk_read_files(tmp_path)
    top = _optimize_projection(op, [col("b"), col("a")])
    list_files = top.input_dependencies[0]
    # Recorded sorted for a stable, comparable value.
    assert list_files.projected_columns == ["a", "b"]


def test_projection_pushdown_is_idempotent(tmp_path):
    op = _mk_read_files(tmp_path)
    top1 = _optimize_projection(op, [col("a")])

    # Re-running the rule on the already-optimized plan must not change the
    # recorded columns (and must reach a fixed point, not loop).
    from ray.data._internal.logical.interfaces import LogicalPlan
    from ray.data._internal.logical.rules import ProjectionPushdown
    from ray.data.context import DataContext

    plan2 = LogicalPlan(top1, DataContext.get_current())
    top2 = ProjectionPushdown().apply(plan2).dag
    assert top2.input_dependencies[0].projected_columns == ["a"]


def test_no_projection_leaves_list_files_unset(tmp_path):
    # Without a projecting ``Project`` above it, the ``ListFiles`` keeps its
    # default ``None`` -> sizing covers the full row group.
    op = _mk_read_files(tmp_path)
    assert op.input_dependencies[0].projected_columns is None
