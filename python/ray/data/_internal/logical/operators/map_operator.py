...
class MapBatches(LogicalOperator):
    ...
    def get_physical_dag(self) -> PhysicalOperator:
        """Get the corresponding DAG of physical operators for map_batches."""
        assert len(self.input_dependencies) == 1, self.input_dependencies

        # Get physical operators for input dependencies first.
        input_physical_dag = self.input_dependencies[0].get_physical_dag()

        compute = get_compute(self._compute)
        block_fn = self._block_fn

        if isinstance(self._fn, CallableClass):
            ...
            fn_constructor_args = self._fn_constructor_args or ()
            fn_constructor_kwargs = self._fn_constructor_kwargs or {}
            fn_ = self._fn
        ...
        return MapOperator(
            ...
            min_rows_per_bundle=self._target_block_size,
