...
class KinesisDatasource(UnboundDatasource):
    ...
    def __init__(
        ...
        if enhanced_fan_out and not consumer_name:
            raise ValueError("consumer_name is required when enhanced_fan_out is enabled")
    ...
    def _get_read_tasks_for_partition(
        ...
        session_kwargs = {
            'region_name': self.kinesis_config.get('region_name') or 
                          self.kinesis_config.get('aws_region')
        }
        
        if 'aws_access_key_id' in self.kinesis_config:
            session_kwargs['aws_access_key_id'] = self.kinesis_config['aws_access_key_id']
        if 'aws_secret_access_key' in self.kinesis_config:
            session_kwargs['aws_secret_access_key'] = self.kinesis_config['aws_secret_access_key']
        if 'aws_session_token' in self.kinesis_config:
            session_kwargs['aws_session_token'] = self.kinesis_config['aws_session_token']
        ...
        for shard_id_val in shard_ids:
            ...
            def create_kinesis_read_fn(
                ...
                def kinesis_read_fn() -> Iterator[pa.Table]:
                    ...
                    session_kwargs = {
                        'region_name': kinesis_config.get('region_name') or 
                                      kinesis_config.get('aws_region')
                    }
                    
                    # Add explicit credentials if provided
                    # Otherwise boto3 will use the default credential chain (IAM role, env vars, etc.)
                    if 'aws_access_key_id' in kinesis_config:
                        session_kwargs['aws_access_key_id'] = kinesis_config['aws_access_key_id']
                    if 'aws_secret_access_key' in kinesis_config:
                        session_kwargs['aws_secret_access_key'] = kinesis_config['aws_secret_access_key']
                    if 'aws_session_token' in kinesis_config:
                        session_kwargs['aws_session_token'] = kinesis_config['aws_session_token']
                    ...
                    try:
                        ...
                        if enhanced_fan_out and consumer_name:
                            ...
                            try:
                                ...
                                consumer_response = kinesis_client.describe_stream_consumer(
                                ...
                                consumer_response = kinesis_client.register_stream_consumer(
                                    StreamARN=stream_arn,
                                    ConsumerName=consumer_name
                                )
                            ...
                            if start_sequence not in ['LATEST', 'TRIM_HORIZON', 'AT_TIMESTAMP']:
                            ...
                            for event in event_stream:
                                ...
                                if 'SubscribeToShardEvent' in event:
                                    ...
                                    for record in shard_event['Records']:
                                        ...
                                        records.append({
                                            "sequence_number": record['SequenceNumber'],  # Unique per shard
                                            "partition_key": record['PartitionKey'],  # Used for shard assignment
                            ...
                            if start_sequence and start_sequence not in ['LATEST', 'TRIM_HORIZON', 'AT_TIMESTAMP']:
                                iterator_kwargs['ShardIteratorType'] = 'AT_SEQUENCE_NUMBER'
                                iterator_kwargs['StartingSequenceNumber'] = start_sequence
                            
                            response = kinesis_client.get_shard_iterator(**iterator_kwargs)
                            shard_iterator = response['ShardIterator']
                            
                            # Read records from shard with proper pagination
                            while shard_iterator and records_read < max_records_per_task:
                                ...
                                response = kinesis_client.get_records(
                                    ...
                                    Limit=min(
                                        ...
                                        kinesis_config.get('max_records_per_request', 10000)
                                    )
                                )
                        ...
                        logger.error(f"Error reading from Kinesis shard {shard_id_val}: {e}")
