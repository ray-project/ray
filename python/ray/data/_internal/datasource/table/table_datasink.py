...
class TableDatasink(
    ...
    def write(
        ...
        try:
            ...
            # the framework can hand it to ``adapter.on_failure``. Include any
            # in-flight paths the adapter's writer started but hadn't yet
            # returned as completed actions (e.g. a Parquet file registered
            # right before ``pq.write_table`` raised mid-write) — these never
            # appear in ``written_paths`` and would otherwise be orphaned.
            orphan_paths = list(written_paths)
            try:
                orphan_paths += list(self._adapter.get_written_paths() or [])
            except Exception:  # noqa: BLE001 - cleanup best-effort; never mask e
                pass
    ...
    def on_write_complete(
        ...
        # complete), so ``on_write_failed`` must clean them up. This complements
        # the worker-side tracking, which only covers tasks that themselves
        # failed — not a driver-side commit failure after all workers succeeded.
    ...
    def _aggregate_and_commit(
        self, returns: List[TableWriteTaskResult[FileAction]]
    ) -> None:
        """Aggregate worker results, reconcile schema, and run the commit.

        Separated from ``on_write_complete`` so the latter can wrap the whole
        thing and forward orphaned data files to cleanup on any failure.
        """
        ...
        if self._mode in (
            ...
            self._adapter.commit_overwrite(all_actions, unified_schema, predicate)
        elif self._mode == SaveMode.UPSERT:
            # _validate_mode_against_adapter guarantees SupportsUpserts.
            upsert_adapter = cast(SupportsUpserts, self._adapter)
            predicate = upsert_adapter.build_upsert_predicate(
                upsert_keys, self._join_cols
            )
            upsert_adapter.commit_upsert(all_actions, unified_schema, predicate)
        else:
...
def _concat_tables(tables: List[pa.Table]) -> Optional[pa.Table]:
    ...
    # ``promote_types=True`` so per-task upsert-key tables with promotable type
    # diffs (e.g. int32/int64) concat successfully — matching the permissive
    # promotion the framework already applies to worker data schemas.
    return concat(tables, promote_types=True)
