"""Table-specific enums layered on top of the generic SaveMode.

`SaveMode` is the generic, public-facing enum from ``ray.data._internal.savemode``.
The table layer re-exports it for convenience and adds ``UpsertSemantics``,
which a ``TableAdapter`` uses to declare whether it supports copy-on-write or
...
class UpsertSemantics(str, Enum):
    """How a table adapter implements UPSERT.
