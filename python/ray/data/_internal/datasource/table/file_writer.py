...
matters because not every table uses Parquet (Hudi's MOR row-groups, custom
columnar stores, etc.).
...
import pyarrow.fs as pa_fs  # noqa: E402
...
# Sentinel used to normalise NaN inside partition keys. Python's ``float('nan')
# != float('nan')`` makes raw NaN values unusable as dict keys (every NaN is a
# distinct key), which both fragments per-partition buffers across calls and
# corrupts grouping within a single table. ``_PartitionKey`` maps every NaN to
# this one sentinel for hashing/equality while preserving the original values
# (real NaN / None) for directory naming.
_NAN_SENTINEL = object()


class _PartitionKey:
    """Hashable partition key with NaN-safe equality.

    Two keys are equal iff their values match element-wise, with NaN treated
    as equal to NaN and None equal to None. ``values`` retains the original
    Python values (real ``float('nan')`` / ``None``) so the adapter-supplied
    ``path_builder`` still receives them verbatim for Hive-style directory
    encoding.
    """

    __slots__ = ("values", "_norm")

    def __init__(self, values: Tuple):
        self.values = values
        self._norm = tuple(
            _NAN_SENTINEL if (isinstance(v, float) and v != v) else v for v in values
        )
    ...
    def __eq__(self, other: object) -> bool:
        return isinstance(other, _PartitionKey) and self._norm == other._norm

    def __repr__(self) -> str:
        return f"_PartitionKey({self.values!r})"
...
class ParquetFileWriter(DataFileWriter[FileAction]):
    ...
    so it can be reused by any table adapter whose file format is Parquet.
    ...
    def __init__(
        ...
        *,
        ...
        self._buffers: DefaultDict["_PartitionKey", List[pa.Table]] = defaultdict(list)
        self._buffer_bytes: DefaultDict["_PartitionKey", int] = defaultdict(int)
        self._file_seq = 0
    ...
    def add_table(self, table: pa.Table) -> List[FileAction]:
        ...
        if self._partition_cols:
            ...
            parts = {_PartitionKey(()): table}

        actions: List[FileAction] = []
        for partition_key, partition_table in parts.items():
            self._buffers[partition_key].append(partition_table)
            self._buffer_bytes[partition_key] += getattr(partition_table, "nbytes", 0)
            if self._buffer_bytes[partition_key] >= self._target_file_size_bytes:
                actions.extend(self._flush_partition(partition_key))
        return actions
    ...
    def _write_table_immediate(
        ...
        a = self._write_partition(table, _PartitionKey(()), block_idx)
        return [a] if a is not None else []

    def _flush_partition(self, partition_key: "_PartitionKey") -> List[FileAction]:
        tables = self._buffers.get(partition_key)
        if not tables:
            return []
        ...
        # behave the same as unbuffered ones.
        merged = pa.concat_tables(tables, promote_options="permissive")
        self._buffers[partition_key].clear()
        self._buffer_bytes[partition_key] = 0
        self._file_seq += 1
        action = self._write_partition(merged, partition_key, self._file_seq)
        return [action] if action is not None else []

    def _partition_table(
        self, table: pa.Table, cols: List[str]
    ) -> Dict["_PartitionKey", pa.Table]:
        """Partition a table by ``cols`` via a single sort + boundary walk.

        Sorts the table once by the partition columns (O(N log N), vectorised)
        and then makes one O(N) pass over the sorted key columns, slicing out a
        contiguous run per distinct key. This replaces the previous
        O(N * K) per-value filtering and the struct ``dictionary_encode``
        fallback.

        Keys are :class:`_PartitionKey` so NaN groups correctly (``NaN`` is
        treated as equal to ``NaN``) both within this table and across buffered
        ``add_table`` calls. ``key.values`` preserves the original Python
        values (real ``NaN`` / ``None``) for the adapter's ``path_builder``.
        """
        n = len(table)
        if n == 0:
            return {}

        # Sort once by all partition columns (nulls last is fine; grouping only
        # needs equal keys to be contiguous).
        sort_idx = pc.sort_indices(table, sort_keys=[(c, "ascending") for c in cols])
        sorted_table = table.take(sort_idx)

        # Materialise just the key columns as Python lists for the boundary
        # walk. Slicing the sorted table is zero-copy.
        key_columns = [sorted_table.column(c).to_pylist() for c in cols]

        out: Dict[_PartitionKey, pa.Table] = {}
        run_start = 0
        prev_key: Optional[_PartitionKey] = None
        for i in range(n):
            cur_values = tuple(key_columns[j][i] for j in range(len(cols)))
            cur_key = _PartitionKey(cur_values)
            if prev_key is None:
                prev_key = cur_key
            elif cur_key != prev_key:
                out[prev_key] = sorted_table.slice(run_start, i - run_start)
                if len(out) > _MAX_PARTITIONS:
                    raise ValueError(
                        f"Too many partition values ({len(out)}+). "
                        f"Max: {_MAX_PARTITIONS}"
                    )
                run_start = i
                prev_key = cur_key
        # Close the final run. ``n >= 1`` here (the ``n == 0`` case returned
        # above), so the loop set ``prev_key`` on its first iteration.
        assert prev_key is not None
        out[prev_key] = sorted_table.slice(run_start, n - run_start)
        if len(out) > _MAX_PARTITIONS:
            raise ValueError(
                f"Too many partition values ({len(out)}). Max: {_MAX_PARTITIONS}"
            )
        return out

    def _write_partition(
        self, table: pa.Table, partition_key: "_PartitionKey", block_idx: int
    ) -> Optional[FileAction]:
        if len(table) == 0:
            return None
        ...
        # ``key.values`` carries the original Python values (real NaN / None)
        # so the adapter's path_builder can apply its own encoding.
        partition_path, partition_dict = self._path_builder(
            self._partition_cols, partition_key.values
        )
    ...
    def _write_parquet(self, table: pa.Table, rel_path: str) -> int:
        ...
        if parent:
            try:
                ...
                # Non-fatal: many filesystems (e.g. object stores) create
                # parents implicitly on write, and recursive=True tolerates
                # an existing dir. Log at debug so genuine FS errors
                # (permissions, etc.) are still discoverable; the subsequent
                # write will surface a hard failure if the path is unusable.
                logger.debug(
                    "create_dir(%s) failed; relying on the write to validate "
                    "path accessibility.",
                    parent,
                    exc_info=True,
                )
        ...
        def _do_write() -> None:
            ...
            # ``FileInfo.size`` is only valid for an existing regular file; on a
            # NotFound/Directory result PyArrow raises ``ValueError``, which
            # would mask the real write failure. Check the type first.
            if info.type != pa_fs.FileType.File:
                raise RuntimeError(
                    f"Written path is missing or not a regular file: {rel_path}"
                )
            if info.size == 0:
                try:
