...
class UnboundedQueueStreamingDataOperator(PhysicalOperator):
    ...
    def _add_read_tasks(self) -> None:
        """Add new read tasks based on the current trigger and parallelism."""
        try:
            # Get read tasks from the datasource
            read_tasks = self.datasource.get_read_tasks(parallelism=self.parallelism)

            for task in read_tasks:
                # Validate ray_remote_args for security
