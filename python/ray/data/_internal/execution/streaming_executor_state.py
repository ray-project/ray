...
def select_operator_to_run(
    ...
    for op, state in topology.items():
        if resource_manager.op_resource_allocator_enabled():
            ...
            limit_reason = (
                f"(in reserved mode new tasks: "
                f"{''.join(can_submit_with_reason['usage'])})"
            ...
            limit_reason = (
                f"(not enough resources, exec allowed:"
                f" {''.join(allowed_execution['usage'])})"
                if not under_resource_limits
                else ""
            )
...
def _execution_allowed_with_reason(
    ...
    global_ok_sans_memory, global_sans_memory = new_usage.satisfies_limit_with_reason(
    ...
    downstream_memory_ok, downstream_not_satisfied_memory = ExecutionResources(
    ...
    return {
        "allowed_exec": False,
        "usage": f"G({global_sans_memory}/D{downstream_not_satisfied_memory}",
    }
