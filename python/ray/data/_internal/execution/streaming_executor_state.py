...
class OpState:
    ...
    def total_input_enqueued(self) -> int:
        ...
        """
        return self._pending_dispatch_input_count() + self.op.internal_queue_size()

    def _pending_dispatch_input_count(self) -> int:
        """Return the number of enqueued bundles across all input queues that
        are *pending* dispatching to the operator"""
    ...
    def summary_str(self, resource_manager: ResourceManager) -> str:
        ...
        # Queued blocks
        desc += f"; Queued blocks: {self.total_input_enqueued()}"
    ...
    *,
    ...
    for op, state in topology.items():
        ...
        if (
            ...
            and state._pending_dispatch_input_count() > 0
        ):
