...
class DataOpTask(OpTask):
    ...
    def __init__(
        ...
        Args:
            ...
            operator_name: The name of the operator that created this task. Used in
                warning messages to identify which pipeline stage is experiencing issues.
        """
    ...
    def on_data_ready(self, max_bytes_to_read: Optional[int]) -> int:
        ...
        while max_bytes_to_read is None or bytes_read < max_bytes_to_read:
            ...
            try:
                ...
                op_label = self._operator_name or self.__class__.__name__
                logger.warning(
                    f"Timed out ({METADATA_GET_TIMEOUT_S}s) waiting for block "
                    f"metadata from operator '{op_label}' "
                    f"(ref={self._pending_meta_ref.hex()}). "
                    f"Possible causes: slow task, worker crashed, or cluster "
                    f"overloaded. Will retry in next iteration."
                )
                break
