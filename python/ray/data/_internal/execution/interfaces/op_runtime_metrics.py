...
class OpRuntimeMetrics(metaclass=OpRuntimesMetricsMeta):
    ...
    num_prefetched_blocks: int = metric_field(
        default=0,
    ...
    block_completion_time: list[int] = metric_field(
        default_factory=list,
    ...
    block_size_rows: list[int] = metric_field(
        default_factory=list,
    ...
    def as_dict(
        ...
        with self._histogram_thread_lock:
            for metric in self.get_metrics():
                ...
                if metric.metrics_type == MetricsType.Histogram:
                    value = value.copy()
