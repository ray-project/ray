...
class ResourceManager:
    ...
    def __init__(
        ...
        # Cached set of completed operators whose usage must be excluded from
        # the resource reservation (see `_get_completed_ops_usage`). `None`
        # means "recompute on next access"; invalidated by `update_usages`.
        self._completed_ops: Optional[List["PhysicalOperator"]] = None
    ...
    def update_usages(self):
        ...
        # The set of completed operators can only change at this reconciliation
        # boundary (after `process_completed_tasks` / between scheduling-loop
        # steps), never during the inner dispatch loop. Invalidate the cached
        # set here; `_get_completed_ops_usage` recomputes it lazily and the
        # incremental `update_usages_for_ops` path reuses it.
        self._completed_ops = None
    ...
    def _get_completed_ops_usage(self) -> ExecutionResources:
        ...
        # The *set* of completed operators only changes when an operator
        # finishes execution, which happens at the `update_usages`
        # reconciliation boundary, never during the inner dispatch loop. Cache
        # the set (which requires a full topology walk) and recompute it only
        # when `update_usages` invalidates it. The per-op usages can still
        # change between dispatches (e.g. a completed op's output draining), so
        # the sum is recomputed from the current usages on every call.
        if self._completed_ops is None:
            self._completed_ops = self._compute_completed_ops()
    ...
    def _compute_completed_ops(self) -> List[PhysicalOperator]:
        """Collect the operators whose usage must be excluded from reservation:
        completed operators and their downstream ineligible operators."""
        last_completed_ops = []
        ops_to_exclude = []
        ...
        return list(set(ops_to_exclude))
...
class ReservationOpResourceAllocator(OpResourceAllocator):
    ...
    def __init__(self, resource_manager: ResourceManager, reservation_ratio: float):
        ...
        # Inputs the cached reservation (above dicts + `_total_shared`) was last
        # computed for. The reservation depends only on `limits` and the set of
        # eligible operators (each op's `min_max_resource_requirements()` is
    ...
    def _update_reservation(self, limits: ExecutionResources):
        ...
        # The reservation is a pure function of `limits` and the eligible-op
        # set; reuse the cached one when both are unchanged. This skips the
        # per-op reservation loop on every dispatch in the scheduling loop's
        # inner dispatch loop, where only operators' *usage* (not the
        # reservation) changes.
        if (
            self._cached_reservation_limits is not None
            and limits == self._cached_reservation_limits
            and eligible_ops == self._cached_reservation_eligible_ops
        ):
            return

        # Record what we're recomputing the reservation for, so subsequent
        # calls with the same inputs hit the cache above.
        self._cached_reservation_limits = limits
        self._cached_reservation_eligible_ops = eligible_ops
