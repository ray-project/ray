...
class ResourceManager:
    ...
    def update_usages_for_ops(self, ops: Iterable["PhysicalOperator"]):
        ...
        includes `_external_consumer_bytes` (iterator/streaming_split prefetch),
        which the consumer thread can update at any time via
        `set_external_consumer_bytes` and which no other op depends on, so it
        would otherwise stay stale until the next full `update_usages`.
        """
    ...
    def _get_completed_ops_usage(self) -> ExecutionResources:
        ...
        # The set of completed operators is recomputed on every call rather
        # than cached: an operator can finish *during* the inner dispatch loop
        # (e.g. `LimitOperator.mark_execution_finished` in `_add_input_inner`),
        # so a set cached at the `update_usages` boundary would go stale within
        # the same scheduling step.
        completed_ops_usage = ExecutionResources.zero()
        for op in self._compute_completed_ops():
