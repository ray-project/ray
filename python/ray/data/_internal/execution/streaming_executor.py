...
class StreamingExecutor(Executor, threading.Thread):
    ...
    def __init__(
        ...
        # `MIN_WAIT_TIMEOUT_S` and above by `WAIT_FOR_TASK_COMPLETION_TIMEOUT_S`. Starts
        # at the max so initial idle polls are cheap.
        self._adaptive_wait_timeout: float = WAIT_FOR_TASK_COMPLETION_TIMEOUT_S
    ...
    def _adapt_wait_timeout(current: float, num_ready: int) -> float:
        ...
        ``[MIN_WAIT_TIMEOUT_S, WAIT_FOR_TASK_COMPLETION_TIMEOUT_S]``.
        """
        if num_ready > 0:
            return max(MIN_WAIT_TIMEOUT_S, current * 0.5)
        return min(WAIT_FOR_TASK_COMPLETION_TIMEOUT_S, current * 2.0)
    ...
    def _scheduling_loop_step(self, topology: Topology) -> bool:
        ...
        # WAIT_FOR_TASK_COMPLETION_TIMEOUT_S]; the upper bound matches the previous
