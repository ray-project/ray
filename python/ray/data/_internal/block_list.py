...
class BlockList:
    ...
    def __init__(
        ...
        # Whether the block list can be consumed (i.e. can be deleted after read).
    ...
    def copy(self) -> "BlockList":
        ...
        return BlockList(self._blocks, self._metadata, self._consumable)
    ...
    def split(self, split_size: int) -> List["BlockList"]:
        ...
        for b, m in zip(blocks, meta):
            output.append(BlockList(b.tolist(), m.tolist(), self._consumable))
        return output
    ...
    def split_by_bytes(self, bytes_per_split: int) -> List["BlockList"]:
        ...
        for b, m in zip(self._blocks, self._metadata):
            ...
            if cur_blocks and cur_size + size > bytes_per_split:
                output.append(BlockList(cur_blocks, cur_meta, self._consumable))
                cur_blocks = []
                cur_meta = []
                cur_size = 0
            cur_blocks.append(b)
    ...
    def randomize_block_order(self, seed: Optional[int] = None) -> "BlockList":
        ...
        return BlockList(blocks, metadata, self._consumable)
