...
This is a HUGE performance win! Examples:
...
class SmartCacheUpdater:
        ...
        **transform_params,
        ...
        if preservation_rules.can_compute_columns:
            if operation_name == "add_column":
                ...
                # with_column is similar to add_column
                self._update_add_column(
                    source_key_prefix, target_key_prefix, transform_params
                )
    ...
    def _preserve_cache_entries(
        ...
        Example:
            ...
            → Copies "count_count_abc123_" to "count_count_def456_"
            → Copies "schema_count_abc123_" to "schema_count_def456_"
        """
        for op in operations:
            # Build source and target cache keys
            source_key = f"{op}_{source_prefix}_"

            # Check if source has this cached value
