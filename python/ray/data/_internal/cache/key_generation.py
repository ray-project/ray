...
def make_cache_key(logical_plan: LogicalPlan, operation_name: str, **params) -> str:
    ...
    The cache key is generated by hashing the operation name, logical plan
    DAG string representation, and serialized parameters using SHA-256.

    Args:
        logical_plan: The logical plan to generate key from.
        operation_name: Name of the operation (e.g., "count", "schema").
        **params: Additional parameters to include in the key.

    Returns:
        Cache key string in format "{operation_name}_{hash}".

    Raises:
        No exceptions raised; falls back to error key on failure.
    """
    try:
        ...
        try:
            fallback_input = (
                f"{operation_name}|{logical_plan.dag.dag_str}|{str(params)}"
            )
...
def _serialize_params(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Converts complex types (lists, dicts, objects) to string representations
    for consistent hashing. Large collections are truncated to avoid
    excessive memory usage in cache keys.

    Args:
        params: Dictionary of parameters to serialize.

    Returns:
        Dictionary with serialized parameter values suitable for hashing.
    """
