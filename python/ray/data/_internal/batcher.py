...
def _blocks_to_batches(
    ...
    of the appropriate batch size.

    Args:
        block_iter: An iterator over blocks.
        batcher: The batcher instance to use for batching.
        drop_last: Whether to drop the last batch if it's incomplete.
        batch_timer: An optional Timer object to time batch creation.
    Returns:
        An iterator over blocks of the given size that are potentially shuffled.
    """

    def get_iter_next_batch_s_timer():
        return batch_timer.timer() if batch_timer else nullcontext()
