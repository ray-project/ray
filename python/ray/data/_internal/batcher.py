...
@DeveloperAPI
class BlockIteratorInterface(ABC):
    """An iterator-based interface for iterating over blocks and yielding batches

    Note:
        Existing batcher-based implementation (e.g., one has has_batch() and next_batch()
        methods) should use BatcherBasedBlockIterator to implement this interface.
...
class BatcherBasedBlockIterator(BlockIteratorInterface):
    """A wrapper around a batcher implementation that implements the BlockIteratorInterface.

    This class encapsulates the batcher implementation and provides an iterator-based interface for
    iterating over blocks and yielding batches.

    Args:
        batcher: A batcher implementation instance (subclass of BatcherInterface).
    ...
    def iter_leftovers(self) -> Iterator[Block]:
        ...
        Yields:
            Any remaining batches in the batcher.
        """
...
def create_default_block_iterator(
    *,
    batch_size: Optional[int],
    ...
    stats: Optional[DatasetStats] = None,
    drop_last: bool = False,
    batcher_factory_fn: Optional[Callable[..., BatcherInterface]] = None,
    **kwargs,
) -> BatcherBasedBlockIterator:
    """Create a default block iterator that handles the batching of blocks.
    ...
    Args:
        ...
        stats: Optional stats object for timing batch operations.
        drop_last: Whether to drop the last incomplete batch.
        batcher_factory_fn: a factory function that creates a BatcherInterface
        **kwargs: kwargs to pass to the batcher's constructor or factory function

    Returns:
        A BatcherBasedBlockIterator.
    """

    if batcher_factory_fn is not None:
        # Use a factory function to create a batcher implementation.
        batcher = batcher_factory_fn(
    ...
    elif shuffle_buffer_min_size is not None:
        # Use a shuffling batcher implementation.
        ...
        # Use a default batcher implementation.
        batcher = Batcher(batch_size=batch_size, ensure_copy=ensure_copy)

    return BatcherBasedBlockIterator(batcher, stats, drop_last)
