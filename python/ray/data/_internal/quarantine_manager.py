...
class QuarantineManager:
    ...
    def add_quarantine_block(
        ...
        if schema_key in self._schemas:
            ...
            if current_schema and not existing_schema.equals(current_schema):
                ...
                logger.info(f"Schema evolved for quarantine path {path} (check: {check_id})")
        else:
    ...
    def write_all_quarantine_data(self):
        ...
        for (path, format, check_id), entries in grouped_entries.items():
            try:
                ...
                logger.info(f"Successfully wrote {len(entries)} quarantine entries to {actual_path} (check: {check_id})")
            except Exception as e:
    ...
    def _write_quarantine_group(self, entries: List[Dict[str, Any]], path: str, format: str, check_id: str = "default"):
        ...
        for entry in entries:
            if entry["type"] == "block":
                ...
                if isinstance(block, pa.Table):
                    ...
                    normalized_block = self._normalize_block_schema(block, unified_schema)
                    combined_blocks.append(normalized_block)
                else:
                    # Convert other block types to Arrow
                    accessor = BlockAccessor.for_block(block)
                    arrow_block = accessor.to_arrow()
                    normalized_block = self._normalize_block_schema(arrow_block, unified_schema)
                    combined_blocks.append(normalized_block)
            elif entry["type"] == "rows":
                # Convert rows to Arrow table
                try:
                    table = pa.Table.from_pylist(entry["data"])
                    normalized_block = self._normalize_block_schema(table, unified_schema)
                    combined_blocks.append(normalized_block)
                except Exception as e:
        ...
        self._write_quarantine_with_partitioning(combined_table, path, format, check_id, entries)
    ...
    def _should_partition_quarantine(self, table: pa.Table, entries: List[Dict[str, Any]]) -> bool:
    ...
    def _write_partitioned_quarantine(self, table: pa.Table, path: str, format: str, check_id: str):
        ...
        try:
            ...
            logger.info(f"Partitioned quarantine data written successfully: {write_results}")
    ...
    def _write_single_quarantine_file(self, table: pa.Table, path: str, format: str, check_id: str):
    ...
    def _normalize_block_schema(self, block: pa.Table, target_schema: pa.Schema) -> pa.Table:
    ...
    def _infer_schema_from_rows(self, rows: List[Dict[str, Any]]) -> Optional[pa.Schema]:
    ...
    def _apply_gdpr_compliance(self, data: Union[pa.Table, List[Dict[str, Any]]]) -> Union[pa.Table, List[Dict[str, Any]]]:
        ...
        if isinstance(data, pa.Table):
            ...
            for field_name in self._gdpr_fields:
                if field_name in data.column_names:
                    ...
                    redacted_values = pa.array([f"<REDACTED_{field_name.upper()}>" for _ in range(len(data))])
