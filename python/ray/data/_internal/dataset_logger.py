...
class DatastreamLogger:
    """Logger for Ray Datastreams which writes logs to a separate log file
    at `DatastreamLogger.DEFAULT_DATASET_LOG_PATH`. Can optionally turn off
    logging to stdout to reduce clutter (but always logs to the aformentioned
    Datastreams-specific log file).
    ...
    def __init__(self, log_name: str):
        """Initialize DatastreamLogger for a given `log_name`.
    ...
    def _initialize_logger(self) -> logging.Logger:
        ...
        # Datastream log file. If this is not the case (e.g. when used in Ray
        # Client), then we skip initializing the FileHandler.
        global_node = ray._private.worker._global_node
        if global_node is not None:
            # Add a FileHandler to write to the specific Ray Datastreams log file
            # at `DatastreamLogger.DEFAULT_DATASET_LOG_PATH`, using the standard
            # default logger format used by the root logger
            session_dir = global_node.get_session_dir_path()
            datastreams_log_path = os.path.join(
                session_dir,
                DatastreamLogger.DEFAULT_DATASET_LOG_PATH,
            )
            file_log_formatter = logging.Formatter(fmt=LOGGER_FORMAT)
            file_log_handler = logging.FileHandler(datastreams_log_path)
    ...
    def get_logger(self, log_to_stdout: bool = True) -> logging.Logger:
        ...
        `log_to_stdout = False`, we do not want the `DatastreamLogger` to
        propagate up to the base Logger which writes to stdout.

        This is a workaround needed due to the DatastreamLogger wrapper object
