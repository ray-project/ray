    ...
    * joining via :py:meth:`.join()`
    ...
    def random_shuffle(
        ...
        **ray_remote_args,
        ...
        seed_config = RandomSeedConfig._normalize_seed_argument(
            seed=seed,
            default_reseed_after_execution=False,
        )
    ...
    def random_sample(
        ...
        def random_sample(batch: DataBatch, seed_config: RandomSeedConfig):
            ctx = TaskContext.get_current()

            seed_tuple = seed_config._get_seed_tuple(data_context=self.context)

            if "rng" in ctx.kwargs:
                rng = ctx.kwargs["rng"]
            elif seed_tuple is None:
                rng = np.random.default_rng()
                ctx.kwargs["rng"] = rng
            else:
                rng = np.random.default_rng((ctx.task_idx, *seed_tuple))
                ctx.kwargs["rng"] = rng
