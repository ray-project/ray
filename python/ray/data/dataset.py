    ...
    def map_batches(
        ...
        **ray_remote_args,
        ...
        Args:
            ...
            memory: The heap memory in bytes to reserve for each parallel map worker.
            concurrency: This argument is deprecated. Use ``compute`` argument.
            preserves_row_count: Set to True only if the UDF always emits the same number of records it receives (no drops or duplicates). When set to True, the logical optimizer, in the presence of a limit(limit=k), will only scan k rows prior to executing the UDF, thereby saving on compute resources.
            ray_remote_args_fn: A function that returns a dictionary of remote args
                passed to each map worker. The purpose of this argument is to generate
                dynamic arguments for each actor/task, and will be called each time prior
                to initializing the worker. Args returned from this dict will always
        ...
        return self._map_batches_without_batch_size_validation(
            ...
            memory=memory,
            concurrency=concurrency,
            preserves_row_count=preserves_row_count,
            ray_remote_args_fn=ray_remote_args_fn,
            **ray_remote_args,
        )
