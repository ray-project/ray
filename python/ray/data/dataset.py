...
class Dataset(Generic[T]):
    ...
    def to_jax(
        ...
        *,
        ...
        label_column_dtype: Optional["jax.dtype"] = None,
        feature_column_dtypes: Optional[
            Union["jax.dtype", List["jax.dtype"], Dict[str, "jax.dtype"]]
        ] = None,
        batch_size: int = 1,
        prefetch_blocks: int = 0,
        drop_last: bool = False,
        ...
        It is recommended to use the returned ``IterableDataset`` directly
        instead of passing it into a jax ``DataLoader``.
        ...
        * If ``feature_columns`` is a ``Dict[str, List[str]]``, the features
        ...
        Note that you probably want to call ``.split()`` on this dataset if
        there are to be multiple jax workers consuming the data.
...
Dict[str, List[str]]]): The names of the columns
                ...
                If None, then use all columns except the label column as
                the features.
            label_column_dtype (Optional[jax.dtype]): The jax dtype to
                use for the label column. If None, then automatically infer
                the dtype.
            feature_column_dtypes (Union[None, jax.dtype, List[jax.dtype],\
 Dict[str, jax.dtype]]): The dtypes to use for the feature
                tensors. This should match the format of ``feature_columns``,
                or be a single dtype, in which case it will be applied to
                all tensors. If None, then automatically infer the dtype.
        ...
        if feature_column_dtypes and not isinstance(feature_column_dtypes, jax.dtype):
            if isinstance(feature_columns, dict):
                if not isinstance(feature_column_dtypes, dict):
                    raise TypeError(
                        "If `feature_columns` is a dict, "
                        "`feature_column_dtypes` must be None, `jax.dtype`,"
                        f" or dict, got {type(feature_column_dtypes)}."
                    )
                if set(feature_columns) != set(feature_column_dtypes):
                    raise ValueError(
        ...
        def make_generator():
            for batch in self.iter_batches(
                ...
                if label_column:
                    label_tensor = convert_pandas_to_jax_tensor(
                        batch,
                        [label_column],
                        label_column_dtype,
                        unsqueeze=unsqueeze_label_tensor,
                ...
                if isinstance(feature_columns, dict):
                    features_tensor = {
                        key: convert_pandas_to_jax_tensor(
                            batch,
                            feature_columns[key],
                            feature_column_dtypes[key]
                            if isinstance(feature_column_dtypes, dict)
                    ...
                    features_tensor = convert_pandas_to_jax_tensor(
                        batch,
                        columns=feature_columns,
                        column_dtypes=feature_column_dtypes,
                        unsqueeze=unsqueeze_feature_tensors,
        ...
        return JaxIterableDataset(make_generator)
