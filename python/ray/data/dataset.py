    ...
    def map_batches(
        ...
        **ray_remote_args,
        ...
        Args:
            ...
            memory: The heap memory in bytes to reserve for each parallel map worker.
            concurrency: This argument is deprecated. Use ``compute`` argument.
            udf_modifying_row_count: Set to ``False`` only if the UDF always emits the
                same number of records it receives (no drops or duplicates). When set to
                ``False``, the logical optimizer, in the presence of a limit (limit=k),
                will only scan k rows prior to executing the UDF, thereby saving on
                compute resources.
            ray_remote_args_fn: A function that returns a dictionary of remote args
                passed to each map worker. The purpose of this argument is to generate
                dynamic arguments for each actor/task, and will be called each time prior
