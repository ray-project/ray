...
class LazyBlockList(BlockList):
    ...
    def __init__(self,
        ...
        self._calls = calls
        self._num_partitions = len(self._calls)
        self._metadata = metadata
        if blocks:
            self._partitions = blocks
        else:
            self._partitions = [None] * len(calls)
            # Immediately compute the first block at least.
            if calls:
                self._partitions[0] = calls[0]()
        assert len(calls) == len(metadata), (calls, metadata)
        assert len(calls) == len(self._partitions), (calls, self._partitions)
    ...
    def _get_or_compute(self, i: int) -> ObjectRef[BlockPartition]:
        ...
        if not self._partitions[i]:
            # Exponentially increase the number of blocks computed per batch.
            for j in range(max(i + 1, i * 2)):
                if j >= len(self._partitions):
                    break
                if not self._partitions[j]:
                    self._partitions[j] = self._calls[j]()
