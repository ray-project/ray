...
def get_expected_mask_indices(
    num_blocks: int, num_rows_per_block: int, fraction: float, seed: int
) -> np.ndarray:
    """A helper function to verify random_sample with seed. This assumes
    there is only one batch in each block.

    Returns:
        An array of indices of the sampled rows
    """
    # Seed sequence for each block: [batch_idx, task_idx, seed]
    seeds = [[0, i, seed] for i in range(num_blocks)]

    # Generate random numbers for each block
    res = np.concatenate(
        [np.random.default_rng(s).random(num_rows_per_block) for s in seeds]
    )

    # Sample the random numbers and get the indices
    expected = np.where(res < fraction)[0]
...
def test_random_sample_fixed_seed_0001(ray_start_regular_shared):
    ...
    # Expected:
    expected = get_expected_mask_indices(
        num_blocks=10, num_rows_per_block=10, fraction=0.1, seed=1234
    )
...
@pytest.mark.parametrize("num_blocks, num_rows_per_block", [(None, 1000), (10, 100)])
...
def test_random_sample_fixed_seed_0002(
    ...
    if dtype == "pyarrow" and num_blocks is not None:
        pytest.skip("ray.data.from_arrow doesn't support multiple blocks yet")

    if num_blocks is None:
        num_blocks = 1

    total_num_rows = num_blocks * num_rows_per_block

    def generate_data(n):
        return {"item": list(range(n))}

    data = generate_data(total_num_rows)

    if dtype == "numpy":
        ds = ray.data.from_items(data["item"], override_num_blocks=num_blocks)
    elif dtype == "pandas":
        ds = ray.data.from_pandas(pd.DataFrame(data), override_num_blocks=num_blocks)
    elif dtype == "pyarrow":
        ds = ray.data.from_arrow(pa.Table.from_pydict(data))
    else:
