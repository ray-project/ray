...
@pytest.mark.parametrize(
    ...
    [
        (dtype, [95, 0, 95, 40, 7])
        for dtype in (np.int8, np.uint16, np.int64, np.float16, np.float32, np.float64)
    ]
    + [(np.float32, indices) for indices in ([], [0], list(range(96)))],
)
...
def test_variable_take_slices_empty_chunks_zero_rows_and_lifetime(take_calls):
    ...
    def prepare():
        ...
        sources = [chunk for chunk in column.chunks if len(chunk)]
        for source, prepared in zip(sources, plan.chunks):
            data, shape = source.storage.field("data"), source.storage.field("shape")
    ...
    # Source views survive releasing Arrow wrappers when prepare returns.
    plan, expected = prepare()
    gc.collect()
    ...
    if failure == "list_null":
        ...
        if failure == "wrong_rank":
            ...
            shapes[0][0] = {
                ...
                "wrong_shape": 7,
                # Multiplication by the second dimension (4) would wrap to
                # the real row length if validated with int64 arithmetic.
                "overflowing_shape": 2**62 + 4096,
                "dimension_null": None,
            }[failure]
...
def test_variable_take_null_indices_use_standard_path(take_calls):
    # Index-type normalization is covered by the shared take_table tests.
    indices = pa.array([0, None], type=pa.int64())
    column = _column()
...
def test_hash_partition_preserves_variable_tensors(take_calls):
    column = _column()
    table = pa.table(
        {
