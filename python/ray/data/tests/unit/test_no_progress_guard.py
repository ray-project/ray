...
def _make_guard(ops, timeout_s=100.0, max_stall_interval_s=float("inf")):
    ...
    # The first check only starts the clock, mirroring the first scheduling
    # step. Prime it here so each test's advances are the ones that count.
    guard.check(topology, consumer_idling=True)
    return guard, clock, topology


def test_raises_once_timeout_elapses_without_progress():
    op = _FakeOperator("MapBatches(embed)")
    guard, clock, topology = _make_guard([op], timeout_s=100.0)
...
def test_progress_by_any_operator_counts():
    ...
    guard, clock, topology = _make_guard([stalled, moving], timeout_s=100.0)
...
def test_busy_consumer_never_times_out():
    ...
    for _ in range(100):
        ...
        guard.check(topology, consumer_idling=False)
...
def test_blocking_step_is_not_a_stall():
    ...
    """
    op = _FakeOperator("Sort")
    guard, clock, topology = _make_guard(
        [op], timeout_s=100.0, max_stall_interval_s=1.0
    )
...
def test_stall_accumulates_across_steps():
    op = _FakeOperator("MapBatches(embed)")
    guard, clock, topology = _make_guard([op], timeout_s=1.0, max_stall_interval_s=0.25)
...
def test_failed_tasks_are_not_progress():
    """A job stuck retrying the same failure isn't making progress.

    `num_tasks_finished` counts failures too, so counting it alone would let a
    retry loop hold the stall clock at zero forever.
    """
    op = _FakeOperator("MapBatches(embed)")
    guard, clock, topology = _make_guard([op], timeout_s=100.0)
...
def test_disabled(timeout_s):
    op = _FakeOperator("MapBatches(embed)")
    guard, clock, topology = _make_guard([op], timeout_s=timeout_s)
...
def test_error_message_names_stalled_operators():
    ...
    guard, clock, topology = _make_guard([stalled, completed], timeout_s=1800.0)
