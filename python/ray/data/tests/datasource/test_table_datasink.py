...
class FakeAdapter(TableAdapter[_FakeFileAction, str]):
    ...
    def build_overwrite_predicate(self, overwrite_filter):
        self._record("build_overwrite_predicate", overwrite_filter=overwrite_filter)
...
def test_error_ignore_modes_route_to_commit_append(mode):
    ...
    commit = next((c for c in adapter.calls if c.method == "commit_append"), None)
...
def test_path_for_action_is_abstract_no_silent_default():
    """``path_for_action`` is abstract — there is no silent base default, so an
    adapter can't accidentally opt out of dedup/cleanup. Each adapter names its
    own field; ``FakeAdapter`` reads ``action.path``."""
    assert getattr(TableAdapter.path_for_action, "__isabstractmethod__", False)
    adapter = FakeAdapter()
...
def test_path_for_action_override_drives_duplicate_detection():
    ...
    assert (
        TableAdapter.path_for_action(adapter, _IcebergLikeFile(file_path="x.parquet"))
        is None
    )
    ...
    r2 = TableWriteTaskResult(file_actions=[dup], emitted_schemas=r1.emitted_schemas)
    with pytest.raises(ValueError, match="Duplicate file paths"):
        sink.on_write_complete(_wrap_result([r1, r2]))
...
def test_attach_written_paths_swallows_attributeerror():
    """Recording orphan paths on an exception whose attribute set fails (e.g.
    a ``__slots__`` / C-extension error with no ``__dict__``) must not raise
    and mask the original error. Regression for PR #64300 (gemini, high)."""
    from ray.data._internal.datasource.table.table_datasink import (
        _attach_written_paths,
    )

    class _NoAttr(Exception):
        # Simulate a slotted exception: setting our marker attribute fails.
        def __setattr__(self, name, value):
            if name == "_table_written_paths":
                raise AttributeError(name)
            super().__setattr__(name, value)

    # Must not raise even though the attribute can't be set.
    _attach_written_paths(_NoAttr("boom"), ["a.parquet"])

    # A normal exception still records (and accumulates) the paths.
    e = RuntimeError("x")
    _attach_written_paths(e, ["b.parquet"])
    _attach_written_paths(e, ["c.parquet"])
    assert e._table_written_paths == ["b.parquet", "c.parquet"]


def test_commit_failure_forwards_worker_paths_to_on_failure():
    """When the driver-side commit fails after workers succeeded, the files
    those workers wrote must be handed to ``on_failure`` for cleanup — not
    silently orphaned. Regression for PR #64300 (cursor, high)."""
    adapter = FakeAdapter(fail_at="commit_append")
    sink = TableDatasink(adapter, SaveMode.APPEND)
    sink.on_write_start()

    r = _drive_write(sink, [_table(2)])
    assert r.written_paths, "worker should have recorded its written file path"

    with pytest.raises(RuntimeError, match="commit_append") as ei:
        sink.on_write_complete(_wrap_result([r]))

    # The framework attaches the worker-written paths to the commit exception;
    # on_write_failed forwards them to the adapter.
    sink.on_write_failed(ei.value)
    on_failure_calls = [c for c in adapter.calls if c.method == "on_failure"]
    assert len(on_failure_calls) == 1
    assert set(on_failure_calls[0].kwargs["written_paths"]) == set(r.written_paths)
