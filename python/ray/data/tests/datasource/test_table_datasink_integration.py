...
class ToyParquetAdapter(TableAdapter[_ToyFile, str]):
    ...
    def path_for_action(self, action: "_ToyFile") -> str:
        return action.path
    ...
    def get_written_paths(self) -> List[str]:
        # Expose the writer's started-paths set so the framework can recover
        # files begun before a mid-write failure (in-flight orphans).
        return list(self._task_written)
...
def test_partition_nan_rows_grouped_together_single_column(tmp_path):
    ...
    nan = float("nan")
    table = pa.table({"k": [1.0, nan, 2.0, nan, nan], "v": ["a", "b", "c", "d", "e"]})
    r = _drive_one_task(sink, [table])
    sink.on_write_complete(_wrap_write_result([r]))
...
def test_partition_nan_buffer_coalesces_across_calls(tmp_path):
    ...
    batches = [pa.table({"k": [nan], "v": [f"r{i}"]}) for i in range(3)]
    r = _drive_one_task(sink, batches)
    sink.on_write_complete(_wrap_write_result([r]))
...
# ----------------------------------------------------------------------
# Write-failure robustness — regressions for PR #64300 bot review.
# ----------------------------------------------------------------------


def test_inflight_path_recovered_on_midwrite_failure(tmp_path, monkeypatch):
    """A file registered just before ``pq.write_table`` raises mid-write never
    appears in the returned file actions, but must still reach cleanup via
    ``get_written_paths``. Regression for PR #64300 (cursor, medium)."""
    adapter = ToyParquetAdapter(str(tmp_path))
    sink = TableDatasink(adapter, SaveMode.APPEND)
    sink.on_write_start()
    ...
    with pytest.raises(RuntimeError, match="disk full mid-write") as ei:
        _drive_one_task(sink, [pa.table({"id": [1], "v": ["x"]})])

    # The writer registered the path before the failing write; the framework
    # pulled it from get_written_paths() and attached it for cleanup.
    inflight = getattr(ei.value, "_table_written_paths", [])
    assert inflight and all(p.endswith(".parquet") for p in inflight)


def test_missing_written_file_raises_runtimeerror_not_valueerror(tmp_path, monkeypatch):
    """If the file isn't actually written, ``get_file_info`` returns NotFound;
    reading ``FileInfo.size`` on it would raise ValueError and mask the real
    failure. We must raise a clear RuntimeError instead. Regression for
    PR #64300 (gemini, medium)."""
    adapter = ToyParquetAdapter(str(tmp_path))
    sink = TableDatasink(adapter, SaveMode.APPEND)
    sink.on_write_start()

    # Write nothing -> the file never appears on disk.
    monkeypatch.setattr(pq, "write_table", lambda *a, **k: None)
