...
@pytest.mark.parametrize("batch_format", ["pandas", "pyarrow"])
def test_unique_with_nulls_ignored(ray_start_regular_shared, batch_format):
    ds = ray.data.from_items([3, 2, 3, 1, 2, 3, None])
    assert set(ds.unique("item", ignore_nulls=True)) == {1, 2, 3}
    assert len(ds.unique("item", ignore_nulls=True)) == 3
    ...
    assert set(ds.unique("a", ignore_nulls=True)) == {1}
    assert len(ds.unique("a", ignore_nulls=True)) == 1
    assert set(ds.unique("b", ignore_nulls=True)) == {1, 2, 3, 4}
    assert len(ds.unique("b", ignore_nulls=True)) == 4
    ...
    ds2 = ray.data.from_pandas(df)
    ds2 = ds2.map_batches(lambda x: x, batch_format=batch_format)
    assert set(ds2.unique("col1", ignore_nulls=True)) == {1, 2, 3}
    assert len(ds2.unique("col1", ignore_nulls=True)) == 3
    assert set(ds2.unique("col2", ignore_nulls=True)) == {2, 3}
    assert len(ds2.unique("col2", ignore_nulls=True)) == 2
    assert set(ds2.unique("col3", ignore_nulls=True)) == {1, 2}
    assert len(ds2.unique("col3", ignore_nulls=True)) == 2
    ...
    ds3 = ray.data.from_pandas(df)
    ds3 = ds3.map_batches(lambda x: x, batch_format=batch_format)
    assert set(ds3.unique("col1", ignore_nulls=True)) == {1, 2, 3}
    assert len(ds3.unique("col1", ignore_nulls=True)) == 3
    assert set(ds3.unique("col2", ignore_nulls=True)) == {2, 3}
    assert len(ds3.unique("col2", ignore_nulls=True)) == 2
    assert set(ds3.unique("col3", ignore_nulls=True)) == {"1.0", "2.0"}
    assert len(ds3.unique("col3", ignore_nulls=True)) == 2
