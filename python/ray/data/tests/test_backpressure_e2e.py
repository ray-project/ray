...
def _build_dataset(
    ...
    ctx.execution_options.resource_limits = ctx.execution_options.resource_limits.copy(object_store_memory=obj_store_limit)
...
def test_no_deadlock_with_preserve_order(
    ...
    data_context.execution_options.resource_limits = data_context.execution_options.resource_limits.copy(object_store_memory=5 * block_size)
...
def test_input_backpressure_e2e(restore_data_context, shutdown_only):  # noqa: F811
    ...
    ctx = ray.data.DataContext.get_current()
    ctx.execution_options.resource_limits = ctx.execution_options.resource_limits.copy(object_store_memory=10e6)
