...
def test_resource_utils(ray_start_10_cpus_shared):
    ...
    r3 = ExecutionResources(acc=1)
    r4 = ExecutionResources(cpu=1, acc=1, object_store_memory=100 * 1024 * 1024)
    r5 = ExecutionResources(cpu=1, acc=1, object_store_memory=1024 * 1024 * 1024)
    ...
    assert r2.add(r3) == ExecutionResources(cpu=1, acc=1)
    assert r4.add(r4) == ExecutionResources(
        cpu=2, acc=2, object_store_memory=200 * 1024 * 1024
    )
...
def test_resource_canonicalization(ray_start_10_cpus_shared):
    ...
    assert op.incremental_resource_usage() == ExecutionResources(cpu=1, acc=0)
    ...
    op = MapOperator.create(
        ...
        name="TestMapper",
        compute_strategy=TaskPoolStrategy(),
        ray_remote_args={"num_accs": 2},
    )
    assert op.base_resource_usage() == ExecutionResources()
    assert op.incremental_resource_usage() == ExecutionResources(cpu=0, acc=2)
    assert op._ray_remote_args == {"num_accs": 2}
    ...
    with pytest.raises(ValueError):
        MapOperator.create(
            ...
            name="TestMapper",
            compute_strategy=TaskPoolStrategy(),
            ray_remote_args={"num_accs": 2, "num_cpus": 1},
        )
...
def test_scheduling_strategy_overrides(ray_start_10_cpus_shared, restore_data_context):
    ...
    op = MapOperator.create(
        ...
        name="TestMapper",
        compute_strategy=TaskPoolStrategy(),
        ray_remote_args={"num_accs": 2, "scheduling_strategy": "DEFAULT"},
    )
    assert op._ray_remote_args == {"num_accs": 2, "scheduling_strategy": "DEFAULT"}
...
def test_task_pool_resource_reporting(ray_start_10_cpus_shared):
    ...
    assert op.current_resource_usage() == ExecutionResources(
        cpu=0, acc=0, object_store_memory=0
    )
...
def test_actor_pool_resource_reporting(ray_start_10_cpus_shared):
    ...
    op.start(ExecutionOptions())
    assert op.base_resource_usage() == ExecutionResources(cpu=2, acc=0)
    ...
    assert op.incremental_resource_usage() == ExecutionResources(cpu=0, acc=0)
    # Actors are pending creation, but they still count against CPU utilization.
    assert op.current_resource_usage() == ExecutionResources(
        cpu=2, acc=0, object_store_memory=0
    )
    ...
    assert inc_usage.acc == 0, inc_usage
