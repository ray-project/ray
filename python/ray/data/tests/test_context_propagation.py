...
def test_read(ray_start_regular_shared):
    ...
    class CustomDatasource(Datasource):
        def prepare_read(self, parallelism: int):
            value = DatasetContext.get_instance().foo
            meta = BlockMetadata(
                num_rows=1,
                size_bytes=8,
                schema=None,
...
def test_map(ray_start_regular_shared):
    context = DatasetContext.get_instance()
    context.foo = 70001
    ds = ray.data.range(1).map(lambda x: DatasetContext.get_instance().foo)
...
def test_flat_map(ray_start_regular_shared):
    context = DatasetContext.get_instance()
    context.foo = 70002
    ds = ray.data.range(1).flat_map(lambda x: [DatasetContext.get_instance().foo])
...
def test_map_batches(ray_start_regular_shared):
    context = DatasetContext.get_instance()
    context.foo = 70003
    ds = ray.data.range(1).map_batches(lambda x: [DatasetContext.get_instance().foo])
...
def test_filter(ray_start_regular_shared):
    context = DatasetContext.get_instance()
    context.foo = 70004
    ds = ray.data.from_items([70004]).filter(lambda x: x == DatasetContext.get_instance().foo)
