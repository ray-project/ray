...
def test_map_pipeline(ray_start_regular_shared):
    ...
    pipe = pipe.map(lambda x: DatasetContext.get_instance().foo)
    [a, b] = pipe.split(2)
...
def test_flat_map(ray_start_regular_shared):
    context = DatasetContext.get_instance()
    context.foo = 70002
    ds = ray.data.range(1).flat_map(
        lambda x: [DatasetContext.get_instance().foo])
...
def test_filter(ray_start_regular_shared):
    ...
    ds = ray.data.from_items(
        [70004]).filter(lambda x: x == DatasetContext.get_instance().foo)
