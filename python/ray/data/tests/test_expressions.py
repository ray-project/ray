...
def test_case_expression_error_handling(ray_start_regular_shared):
    ...
    with pytest.raises(Exception):  # UserCodeException or similar
        ds.with_column(
            "result",
            case([(col("nonexistent_column") > 30, lit("High"))], default=lit("Low")),
        ).materialize()
...
def test_case_expression_edge_cases_dataset(ray_start_regular_shared):
    ...
    result = empty_ds.with_column(
        "result", case([(col("age") > 30, lit("Adult"))], default=lit("Young"))
    )
...
def test_case_expression_api_consistency(ray_start_regular_shared):
    ...
    aggregated = result.aggregate(
        ray.data.aggregate.Count("id"), ray.data.aggregate.Sum("value")
    )
    ...
    grouped = result.groupby("category").aggregate(
        ray.data.aggregate.Count("id"), ray.data.aggregate.Mean("value")
    )
    ...
    ds2 = ray.data.from_items(
        [{"id": 1, "extra": "A"}, {"id": 2, "extra": "B"}, {"id": 3, "extra": "C"}]
    )
...
def test_case_expression_optimization_patterns():
    ...
    expr = case(
        [(lit(True), lit("always")), (lit(False), lit("never"))], default=lit("default")
    )
    ...
    expr = case(
        [(col("flag"), lit("flagged")), (~col("flag"), lit("not_flagged"))],
        default=lit("unknown"),
    )
    ...
    assert (
        expr.when_clauses[1][0].op == Operation.AND
    )  # NOT is implemented as AND with False
...
def test_case_expression_validation_optimization():
    ...
    with pytest.raises(
        ValueError, match="when_clauses\\[0\\] must be a tuple with exactly 2 elements"
    ):
        case([(col("age") > 30,)], default=lit("default"))  # Missing value

    # Test validation of invalid when_clauses structure
    with pytest.raises(
        ValueError, match="when_clauses\\[1\\] must be a tuple with exactly 2 elements"
    ):
        case(
            [(col("age") > 30, lit("adult")), "not a tuple"],  # Invalid structure
            default=lit("default"),
        )

    # Test that valid case statements are created successfully
    valid_expr = case(
        [(col("age") > 30, lit("adult")), (col("age") > 18, lit("young"))],
        default=lit("minor"),
    )
