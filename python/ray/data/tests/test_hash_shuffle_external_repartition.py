"""End-to-end test: ``ds.repartition()`` routed through the external
(file-transport) hash shuffle.

Unlike ``test_hash_shuffle_external_smoke.py`` which hand-drives the
operator pair, this test goes through the **real** Ray Data planner +
...
If the in-memory path regresses or the external wiring breaks, one of
these fails fast. Each test restores the flag in ``finally`` so
cross-test bleed doesn't poison later cases.
...
@contextmanager
def _external_flag(enabled: bool):
...
def test_external_repartition_row_count(ray_cluster, num_rows, num_partitions):
    """Row count must be preserved end-to-end."""
    with _external_flag(True):
...
def test_external_repartition_partition_count(ray_cluster):
...
def test_external_repartition_with_key(ray_cluster):
...
def test_external_same_key_lands_in_same_block(ray_cluster):
...
def test_external_multi_column_keys(ray_cluster):
...
def test_external_more_partitions_than_keys_emits_empty_blocks(ray_cluster):
...
def test_external_flag_off_keeps_v2_path(ray_cluster):
...
def test_external_repartition_sorted(ray_cluster):
...
def test_external_repartition_then_map(ray_cluster):
...
def test_external_two_repartitions_chained(ray_cluster):
