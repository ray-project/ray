...
def test_preserve_whether_base_datasets_can_be_cleared(ray_start_regular_shared):
    ...
    assert pipe._base_datasets_can_be_cleared == False
    # pipe has no transformation, so output blocks are blocks of dataset, so cannot
    # be cleared.
    assert pipe._can_clear_output_blocks_after_read() == False

    pipe = ds.repeat().map_batches(lambda x: x)
    assert pipe._base_datasets_can_be_cleared == False
    # pipe now has transformation, so output blocks are created on-the-fly and can
    # be cleared.
    assert pipe._can_clear_output_blocks_after_read() == True

    # rewindow()
    pipe = ds.repeat().map_batches(lambda x: x).rewindow(blocks_per_window=1)
    # The rewindow will collapase the stages we previously had.
    assert len(pipe._stages) == 0
    # The base datasets are generated by a pipeline prior to rewindow, so we
    # can clear them.
    assert pipe._base_datasets_can_be_cleared == True
    # When base datasets can be cleared, we can surely clear output blocks from this
    # pipeline.
    assert pipe._can_clear_output_blocks_after_read() == True

    # split(): Similar to rewindow, pipeline splitting will also collapase previous stages,
    # so we expect same.
    p1, p2 = ds.repeat().map_batches(lambda x: x).split(2)
    assert len(p1._stages) == 0
    assert len(p2._stages) == 0
    assert p1._base_datasets_can_be_cleared == True
    assert p2._base_datasets_can_be_cleared == True
    assert p1._can_clear_output_blocks_after_read() == True
    assert p2._can_clear_output_blocks_after_read() == True
    ...
    assert p2._base_datasets_can_be_cleared == False
