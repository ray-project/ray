...
import ray
from ray.data.experimental.sql_api import clear_tables, list_tables, register, sql
from ray.data.experimental.sql import SQLError, SQLParseError
...
def test_sql_aggregation(ray_start_regular_shared):
    ...
    numbers = ray.data.from_items([{"value": 10}, {"value": 20}, {"value": 30}])
...
def test_sql_config(ray_start_regular_shared):
    ...
    assert ctx.sql_enable_sqlglot_optimizer == False  # Default
    config.enable_sqlglot_optimizer = True
    assert ctx.sql_enable_sqlglot_optimizer == True
