...
def test_group_by_operations(ray_start_regular_shared, sql_test_data):
    ...
    result = sql("SELECT city, COUNT(*) as cnt, AVG(age) as avg_age FROM users GROUP BY city")
    rows = result.take_all()
    assert len(rows) == 2
...
def test_join_operations(ray_start_regular_shared, sql_test_data):
    ...
    # LEFT JOIN
    result = sql("SELECT u.name, o.amount FROM users u LEFT JOIN orders o ON u.id = o.user_id")
    rows = result.take_all()
    assert len(rows) == 4
    
    # Join with WHERE
    result = sql("SELECT u.name, o.amount FROM users u JOIN orders o ON u.id = o.user_id WHERE o.amount > 100")
    rows = result.take_all()
    assert len(rows) == 1
...
def test_null_handling(ray_start_regular_shared, sql_test_data):
    ...
    # COUNT with nulls
    result = sql("SELECT COUNT(*) as total, COUNT(value) as non_null_values FROM null_table")
    rows = result.take_all()
    assert len(rows) == 1
...
def test_complex_queries(ray_start_regular_shared, sql_test_data):
    ...
    assert all("city" in row and "user_count" in row and "total_sales" in row for row in rows)
...
def test_dataset_chaining(ray_start_regular_shared, sql_test_data):
    ...
    transformed = result.map(lambda row: {"name_upper": row["name"].upper(), "age_group": "adult"})
