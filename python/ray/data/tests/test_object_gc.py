...
def test_global_bytes_spilled(shutdown_only):
    ...
    ctx = ray.init(num_cpus=4, object_store_memory=90e6)
    # The size of dataset is 500*(80*80*4)*8B, about 100MB.
    ds = ray.data.range_tensor(500, shape=(80, 80, 4)).materialize()
    ...
    for row in ds.iter_rows():
        pass
    wait_for_condition(lambda: ds._get_stats_summary().global_bytes_restored > 0)
