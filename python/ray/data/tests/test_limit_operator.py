...
def test_limit_operator_preserves_producer_op_id(ray_start_regular_shared):
    """Test that LimitOperator propagates producer_op_id through output bundles."""
    refs = make_ref_bundles([[1, 2, 3], [4, 5, 6]])
    refs = [dataclasses.replace(r, producer_op_id="op_a") for r in refs]
    input_op = InputDataBuffer(DataContext.get_current(), refs)
    ...
    assert all(b.producer_op_id == "op_a" for b in outputs)
