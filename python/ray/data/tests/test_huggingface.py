...
def test_from_huggingface(hf_dataset, ray_start_regular_shared, num_par):
    ...
    ray_datasets = {
        "train": ray.data.from_huggingface(
            hf_dataset["train"], override_num_blocks=num_par, concurrency=1
        ),
    }
    ...
    ray_dataset_split_train = ray.data.from_huggingface(
        hf_dataset_split["train"], override_num_blocks=1, concurrency=1
    )
