...
def _normalize_df(df: pd.DataFrame) -> pd.DataFrame:
    """Convert all columns to Python-native types for dtype-agnostic comparison.

    Handles PyArrow-backed ArrowDtype columns produced by ray.data Iceberg reads
    vs standard numpy-backed pandas columns produced by _sorted_df.
    """
    result = {}
...
def test_iceberg_checkpoint_recovers_write_results_and_commits(
    ...
    assert _normalize_df(actual_df).equals(_normalize_df(expected_df))
