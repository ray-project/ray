...
def test_e2e_option_propagation(ray_start_10_cpus_shared, restore_data_context):
    ...
    DataContext.get_current().execution_options.resource_limits = DataContext.get_current().execution_options.resource_limits.copy(cpu=1)
    with pytest.raises(ValueError):
        run()
...
def test_backpressure_from_output(ray_start_10_cpus_shared, restore_data_context):
    ...
    ctx.execution_options.resource_limits = ctx.execution_options.resource_limits.copy(object_store_memory=block_size)
...
def test_e2e_autoscaling_down(ray_start_10_cpus_shared, restore_data_context):
    ...
    DataContext.get_current().execution_options.resource_limits = DataContext.get_current().execution_options.resource_limits.copy(cpu=2)
...
def test_e2e_liveness_with_output_backpressure_edge_case(
    ...
    ctx = DataContext.get_current()
    ctx.execution_options.preserve_order = True
    ctx.execution_options.resource_limits = ctx.execution_options.resource_limits.copy(object_store_memory=1)
