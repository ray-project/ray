...
def test_report_issues():
    ...
    assert (
        data[0]["event_data"]["event_type"]
        == ISSUE_TYPE_TO_EXPORT_OPERATOR_EVENT_TYPE[IssueType.HANGING]
    )
    ...
    assert (
        data[1]["event_data"]["event_type"]
        == ISSUE_TYPE_TO_EXPORT_OPERATOR_EVENT_TYPE[IssueType.HIGH_MEMORY]
    )
