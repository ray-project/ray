...
@pytest.mark.parametrize(
    ...
    [
        ...
        (col("id"), "int64", 0, int),  # int64 -> int64 (no change)
        (col("id"), "float64", 0.0, float),  # int64 -> float64
        (col("id"), "string", "0", str),  # int64 -> string
        (col("id") / 2, "int64", 0, int),  # float -> int64
        (col("id") / 2, "float64", 0.0, float),  # float -> float64 (no change)
        # Using PyArrow types
        (col("id"), pa.int64(), 0, int),
        (col("id"), pa.float64(), 0.0, float),
        (col("id"), pa.string(), "0", str),
        # Using DataType objects
        (col("id"), DataType.int64(), 0, int),
        (col("id"), DataType.float64(), 0.0, float),
        (col("id"), DataType.string(), "0", str),
    ],
)
...
def test_cast_expression_chained(ray_start_regular_shared, target_max_block_size_infinite_or_default):
    ...
    ds = ds.with_column("result", col("id").cast("float64") * 2.5)
    result = ds.take(1)[0]
    assert result["id"] == 0
    ...
    ds = ray.data.range(5)
    ds = ds.with_column("result", (col("id") + 1).cast("string"))
    result = ds.take(1)[0]
...
def test_cast_expression_safe_mode(ray_start_regular_shared, target_max_block_size_infinite_or_default):
    ...
    with pytest.raises((UserCodeException, ValueError, pa.ArrowInvalid)):
        ds.with_column("result", col("value").cast("int64")).materialize()
...
def test_cast_expression_multiple_types(ray_start_regular_shared, target_max_block_size_infinite_or_default):
    ...
    ds = ds.with_column("id_int", col("id").cast("int64"))
    ...
    assert isinstance(result["id_int"], int)
    assert result["id_float"] == 42.0
    assert isinstance(result["id_float"], float)
    assert result["id_str"] == "42"
    assert isinstance(result["id_str"], str)
    assert result["score_int"] == 3
