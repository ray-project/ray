...
class TestReservationOpResourceAllocator:
    ...
    def test_can_submit_new_task_fast_producer_backpressured(
        self, restore_data_context
    ):
        """Test that fast producer is backpressured when it exhausts its budget.

        In a fast producer → slow consumer scenario:
        - Fast producer uses up resources quickly
        - Budget becomes smaller than incremental_resource_usage
        - can_submit_new_task returns False (backpressured)
        """
        DataContext.get_current().op_resource_reservation_enabled = True
        DataContext.get_current().op_resource_reservation_ratio = 0.5

        o1 = InputDataBuffer(DataContext.get_current(), [])
        # Fast producer with large incremental usage
        o2 = mock_map_op(
            input_op=o1,
            incremental_resource_usage=ExecutionResources(
                cpu=1, gpu=0, object_store_memory=100
            ),
        )
        # Slow consumer with smaller incremental usage
        o3 = mock_map_op(
            input_op=o2,
            incremental_resource_usage=ExecutionResources(
                cpu=1, gpu=0, object_store_memory=50
            ),
        )
        ...
        # Global limits sized so fast producer can exhaust budget while slow consumer has enough
        global_limits = ExecutionResources(cpu=10, object_store_memory=1000)

        # Fast producer has used most of its budget
        # o2 usage is very high (800), exhausting reserved + shared budget
        # o3 usage is low (100), leaving plenty of budget
        op_usages = {
            o1: ExecutionResources.zero(),
            o2: ExecutionResources(cpu=8, object_store_memory=800),  # Very high usage
            o3: ExecutionResources(cpu=1, object_store_memory=100),  # Low usage
        }
        ...
        resource_manager.get_op_usage = MagicMock(side_effect=lambda op: op_usages[op])
        resource_manager._mem_op_internal = {o1: 0, o2: 700, o3: 50}
        resource_manager._mem_op_outputs = {o1: 0, o2: 100, o3: 50}
        resource_manager.get_global_limits = MagicMock(return_value=global_limits)
        ...
        #
        #   Budget = max(0, reserved - usage):
        #     cpu: max(0, 2.5 - 8) = 0
        #     mem: max(0, 125 - 700) = 0
        o2_budget = allocator.get_budget(o2)
        assert o2_budget.cpu == 0.0
        assert o2_budget.object_store_memory == 0.0
        # Budget (cpu=0, mem=0) < incremental (cpu=1, mem=100) → backpressured
        assert allocator.can_submit_new_task(o2) is False, (
            f"Fast producer should be backpressured when budget exhausted. "
            f"Budget: {o2_budget}, Incremental: {o2.incremental_resource_usage()}"
        )
        ...
        assert o3_budget.cpu == 1.5
        assert o3_budget.object_store_memory == 75.0
        # Budget (cpu=1.5, mem=75) >= incremental (cpu=1, mem=50) → can submit
        assert allocator.can_submit_new_task(o3) is True, (
            f"Slow consumer should be able to submit tasks. "
            f"Budget: {o3_budget}, Incremental: {o3.incremental_resource_usage()}"
        )
