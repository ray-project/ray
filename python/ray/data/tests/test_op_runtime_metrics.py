...
def test_average_max_rss_per_task():
    # No tasks submitted yet.
    metrics = OpRuntimeMetrics(MagicMock())
    assert metrics.average_max_rss_per_task is None

    def create_bundle(rss_bytes: int):
        block = ray.put(pa.Table.from_pydict({}))
        stats = BlockExecStats()
        stats.max_rss_bytes = rss_bytes
        stats.wall_time_s = 0
    ...
    assert metrics.average_max_rss_per_task == 1

    # Generate one output for the second task.
    bundle = create_bundle(rss_bytes=3)
    metrics.on_task_output_generated(0, bundle)
    assert metrics.average_max_rss_per_task == 2  # (1 + 3) / 2 = 2
