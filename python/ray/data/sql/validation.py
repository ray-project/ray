...
SUPPORTED_FUNCTIONS: Set[str] = {
    ...
    "concat", "replace", "split", "regexp_replace", "like",
...
SUPPORTED_OPERATORS: Set[Type[exp.Expression]] = {
    ...
    # Other operators
    exp.Is, exp.Like, exp.ILike,
    exp.Between, exp.Bracket,
    
    # Null checks
    exp.Null,
}

# Unsupported operators with reasons
...
SUPPORTED_CONSTRUCTS: Set[Type[exp.Expression]] = {
    ...
    # Expressions
    exp.Column, exp.Literal, exp.Star, exp.Alias,
    exp.Case, exp.When, exp.If,
    
    # Parentheses and grouping
    exp.Paren, exp.Tuple,
    
    # Basic table references
    exp.Table, exp.TableAlias,
}
...
UNSUPPORTED_DATA_TYPES: Dict[str, Dict[str, str]] = {
    ...
    "uuid": {
        "reason": "UUID type not supported",
...
def get_supported_features_summary() -> Dict[str, List[str]]:
    ...
    return {
        ...
        "data_types": sorted(list(SUPPORTED_DATA_TYPES))
    }
...
def get_feature_alternatives(feature_name: str) -> Optional[str]:
    ...
    for feature_dict in [UNSUPPORTED_AGGREGATES, UNSUPPORTED_FUNCTIONS, 
