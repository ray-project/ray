...
class SelectOperation:
    ...
    def __post_init__(self):
        ...
        if not self.expressions and not self.star_select:
            raise ValueError("Select operation must have expressions or be a star select")
...
class JoinOperation:
    ...
    def __post_init__(self):
        ...
        if self.join_type not in valid_types:
            raise ValueError(f"Invalid join type: {self.join_type}. Must be one of {valid_types}")
...
class AggregateOperation:
    ...
    def __post_init__(self):
        ...
        if not self.aggregates:
            raise ValueError("Aggregate operation must have at least one aggregate function")
...
class OrderOperation:
    ...
    def to_ray_sort_params(self) -> Union[str, List[str]]:
        ...
        if len(self.keys) == 1:
            key = self.keys[0]
            return key.column.output_name if key.ascending else f"-{key.column.output_name}"
...
class SQLOperationFactory:
    ...
    @staticmethod
    def create_join(join_type: str, left_table: str, right_table: str, 
                   left_on: List[str], right_on: List[str], **kwargs) -> JoinOperation:
        """Create a JOIN operation."""
        return JoinOperation(
            join_type=join_type, left_table=left_table, right_table=right_table,
            left_on=left_on, right_on=right_on, **kwargs
        )
    
    @staticmethod 
    def create_aggregate(group_by: List[ColumnReference], 
                        aggregates: List[SQLExpression], **kwargs) -> AggregateOperation:
...
class SQLOperationError(Exception):
    ...
    def __init__(self, message: str, operation: Optional[Any] = None, 
