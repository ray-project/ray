...
class TestRunner:
    ...
    def _define_test_cases(self) -> List[Tuple[str, str, List[Dict], bool]]:
        ...
        return [
            ...
            ("select_all", "SELECT * FROM ds1", [
                {"id": 1, "a": 10, "b": 100, "c": "foo", "flag": 1},
                {"id": 2, "a": 20, "b": 200, "c": "bar", "flag": 0},
                {"id": 3, "a": 30, "b": 300, "c": "bar", "flag": 1},
                {"id": 4, "a": 0, "b": 400, "c": "baz", "flag": 0}
            ], False),
            ("select_columns", "SELECT id, a FROM ds1", [
                {"id": 1, "a": 10},
                {"id": 2, "a": 20},
                {"id": 3, "a": 30},
                {"id": 4, "a": 0}
            ], False),
            ("select_with_alias", "SELECT a AS amount FROM ds1", [
                {"amount": 10},
                {"amount": 20},
                {"amount": 30},
                {"amount": 0}
            ...
            # WHERE clauses
            ("where_simple", "SELECT id FROM ds1 WHERE a > 15", [
                {"id": 2},
                {"id": 3}
            ], False),
            ("where_and", "SELECT id FROM ds1 WHERE a > 10 AND flag = 1", [
                {"id": 3}
            ], False),
            ("where_or", "SELECT id FROM ds1 WHERE a = 10 OR a = 30", [
                {"id": 1},
                {"id": 3}
            ], False),
            # ORDER BY
            ("order_asc", "SELECT id FROM ds1 ORDER BY a ASC", [
                {"id": 4},
                {"id": 1},
                {"id": 2},
                {"id": 3}
            ], False),
            ("order_desc", "SELECT id FROM ds1 ORDER BY a DESC", [
                {"id": 3},
                {"id": 2},
                {"id": 1},
                {"id": 4}
            ], False),
            ("order_multiple", "SELECT id FROM ds1 ORDER BY flag DESC, a ASC", [
                {"id": 1},
                {"id": 3},
                {"id": 4},
                {"id": 2}
            ...
            # LIMIT
            ("limit_simple", "SELECT id FROM ds1 LIMIT 2", [
                {"id": 1},
                {"id": 2}
            ], False),
            ("limit_with_order", "SELECT id FROM ds1 ORDER BY a DESC LIMIT 2", [
                {"id": 3},
                {"id": 2}
            ], False),
            # Aggregates without GROUP BY
            ("count_all", "SELECT COUNT(*) as total FROM ds1", [
                {"total": 4}
            ], False),
            ("sum_column", "SELECT SUM(a) as sum_a FROM ds1", [
                {"sum_a": 60}
            ], False),
            ("multiple_agg", "SELECT COUNT(*) as cnt, SUM(a) as sum_a, AVG(a) as avg_a FROM ds1", [
                {"cnt": 4, "sum_a": 60, "avg_a": 15.0}
            ], False),
            # GROUP BY
            ("group_by_count", "SELECT c, COUNT(*) as cnt FROM ds1 GROUP BY c", [
                {"c": "foo", "cnt": 1},
                {"c": "bar", "cnt": 2},
                {"c": "baz", "cnt": 1}
            ], False),
            ("group_by_sum", "SELECT c, SUM(a) as sum_a FROM ds1 GROUP BY c", [
                {"c": "foo", "sum_a": 10},
                {"c": "bar", "sum_a": 50},
                {"c": "baz", "sum_a": 0}
            ], False),
            ("group_by_multiple", "SELECT flag, COUNT(*) as cnt, SUM(a) as sum_a FROM ds1 GROUP BY flag", [
                {"flag": 0, "cnt": 2, "sum_a": 20},
                {"flag": 1, "cnt": 2, "sum_a": 40}
            ], False),
            # JOINs following Ray Data Join API
            ("inner_join", "SELECT ds1.id, ds1.a, ds2.d FROM ds1 JOIN ds2 ON ds1.id = ds2.id", [
                {"id": 1, "a": 10, "d": 5.5},
                {"id": 2, "a": 20, "d": 7.2},
                {"id": 4, "a": 0, "d": 42.0}
            ], False),
            ("left_join", "SELECT ds1.id, ds1.a, ds2.d FROM ds1 LEFT JOIN ds2 ON ds1.id = ds2.id", [
                {"id": 1, "a": 10, "d": 5.5},
                {"id": 2, "a": 20, "d": 7.2},
                {"id": 3, "a": 30, "d": None},
                {"id": 4, "a": 0, "d": 42.0}
            ], False),
            ("right_join", "SELECT ds2.id, ds1.a, ds2.d FROM ds1 RIGHT JOIN ds2 ON ds1.id = ds2.id", [
                {"id": 1, "a": 10, "d": 5.5},
                {"id": 2, "a": 20, "d": 7.2},
                {"id": 4, "a": 0, "d": 42.0}
            ], False),
            ("full_join", "SELECT ds1.id, ds1.a, ds2.d FROM ds1 FULL JOIN ds2 ON ds1.id = ds2.id", [
                {"id": 1, "a": 10, "d": 5.5},
                {"id": 2, "a": 20, "d": 7.2},
                {"id": 3, "a": 30, "d": None},
                {"id": 4, "a": 0, "d": 42.0}
            ...
            ("arithmetic", "SELECT id, a + b as sum_ab FROM ds1", [
                {"id": 1, "sum_ab": 110},
                {"id": 2, "sum_ab": 220},
                {"id": 3, "sum_ab": 330},
                {"id": 4, "sum_ab": 400}
            ], False),
            ("complex_expr", "SELECT id, (a + b) * 2 as complex FROM ds1", [
                {"id": 1, "complex": 220},
                {"id": 2, "complex": 440},
                {"id": 3, "complex": 660},
                {"id": 4, "complex": 800}
            ...
            # String literals
            ("string_literal", "SELECT 'Hello' as greeting FROM ds1 LIMIT 1", [
                {"greeting": "Hello"}
            ], False),
            ("mixed_literal", "SELECT id, 'constant' as const FROM ds1", [
                {"id": 1, "const": "constant"},
                {"id": 2, "const": "constant"},
                {"id": 3, "const": "constant"},
                {"id": 4, "const": "constant"}
            ...
            # NULL handling
            ("null_count", "SELECT COUNT(*) as total, COUNT(val) as non_null FROM dsnull", [
                {"total": 2, "non_null": 1}
            ], False),
            # Empty dataset handling
            ("empty_select", "SELECT * FROM dsempty", [], False),
            ("empty_count", "SELECT COUNT(*) as cnt FROM dsempty", [
                {"cnt": 0}
            ], False),
            # Error cases
    ...
    def _run_test_cases(self, test_cases: List[Tuple[str, str, List[Dict], bool]]) -> Dict[str, Any]:
        ...
        for test_name, sql_query, expected_data, should_fail in test_cases:
            try:
                if test_name in ["limit_simple", "limit_with_order", "left_join", "string_literal", "empty_count", "count_all", "null_count"]:
                ...
                if test_name in ["limit_simple", "limit_with_order", "left_join", "string_literal", "empty_count", "count_all", "null_count"]:
                    print(f"DEBUG: {test_name} returned {len(actual_rows)} rows: {actual_rows}")
                
                if should_fail:
                    results.append((test_name, "FAIL", f"Expected error but got {len(actual_rows)} rows", res_ds))
                    failed += 1
                else:
                    is_match, error_msg = self._compare_datasets(actual_rows, expected_data)
                    if is_match:
                        results.append((test_name, "PASS", f"{len(actual_rows)} rows match expected", res_ds))
                        passed += 1
                    else:
                ...
                if should_fail:
                    results.append((test_name, "PASS", f"Expected error: {str(e)[:50]}...", None))
                    passed += 1
                else:
                    results.append((test_name, "FAIL", f"Unexpected error: {str(e)[:50]}...", None))
                    failed += 1
    ...
    def _compare_datasets(self, actual_rows: List[Dict], expected_data: List[Dict]) -> Tuple[bool, str]:
        ...
        for i, (actual_row, expected_row) in enumerate(zip(actual_normalized, expected_normalized)):
            if actual_row != expected_row:
                return False, f"Row {i} mismatch: expected {expected_row}, got {actual_row}"
...
class ExampleRunner:
    ...
    def _run_sqlglot_features_example(self):
        ...
        employees = ray.data.from_items([
            ...
            {"id": 2, "name": "Bob", "dept": "Sales", "salary": 65000, "hire_date": "2019-03-20"},
            {"id": 3, "name": "Charlie", "dept": "Engineering", "salary": 90000, "hire_date": "2018-11-10"},
            {"id": 4, "name": "Diana", "dept": "Marketing", "salary": 70000, "hire_date": "2021-06-05"},
            {"id": 5, "name": "Eve", "dept": "Engineering", "salary": 85000, "hire_date": "2020-08-12"},
        ])
