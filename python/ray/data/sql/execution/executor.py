...
from ray.data.sql.utils import setup_logger, create_column_mapping, is_aggregate_function
from ray.data.sql.execution.analyzers import ProjectionAnalyzer, AggregateAnalyzer
from ray.data.sql.execution.handlers import JoinHandler, FilterHandler, OrderHandler, LimitHandler
...
class QueryExecutor:
    ...
    def _apply_ordered_operations(self, dataset: Dataset, ast: exp.Select, column_names: List[str], funcs: List[Callable]) -> Dataset:
    ...
    def _apply_projection(self, dataset: Dataset, column_names: List[str], exprs: List[Callable]) -> Dataset:
    ...
    def _create_literal_result(self, ast: exp.Select, select_exprs: List[exp.Expression]) -> Dataset:
    ...
    def _execute_group_by_query(self, ast: exp.Select, group_keys: List[str]) -> Dataset:
        ...
        aggregates, renames = self.aggregate_analyzer.build_aggregates(aggregates, dataset)
    ...
    def _resolve_group_keys(self, group_keys: List[str], dataset: Dataset) -> List[str]:
        ...
        for key in group_keys:
            ...
            if not actual_column:
                raise ValueError(f"GROUP BY column '{key}' not found. Available columns: {cols}")
            keys.append(actual_column)
        
        return keys

    def _execute_aggregate_query(self, dataset: Dataset, aggregates: List[Tuple[str, exp.Expression]]) -> Dataset:
    ...
    def _execute_count_star_query(self, dataset: Dataset, aggregates: List[Tuple[str, exp.Expression]]) -> Dataset:
        ...
        for output_name, agg_expr in aggregates:
            func_name = get_function_name_from_expression(agg_expr).lower()
            target_column = self.aggregate_analyzer._extract_target_column(agg_expr, column_mapping)
            ...
            if func_name == "count":
                if target_column is None:
                    ...
                    non_null_count = dataset.filter(lambda row: row.get(target_column) is not None).count()
