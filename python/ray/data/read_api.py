...
def read_tfrecords(
    ...
    *,
    ...
    Examples:
        ...
        >>> options = tf.io.TFRecordOptions(compression_type="ZLIB") # "GZIP" also supported by TensorFlow
        >>> with tf.io.TFRecordWriter(path=compressed_path, options=options) as writer:
        ...     writer.write(example.SerializeToString())

        >>> ds_decompressed = ray.data.read_tfrecords(
                [compressed_path],
                arrow_open_stream_args={"compression": "gzip"},
            )
        >>> ds_decompressed.to_pandas()  # doctest: +SKIP
           length  width    species
        0     5.1    3.5  b'setosa'
    ...
    Args:
        ...
        arrow_open_stream_args: Key-word arguments passed to
            ...
            pass the corresponding compression type into ``arrow_open_stream_args``
            (e.g. for ``GZIP`` or ``ZLIB``, use
