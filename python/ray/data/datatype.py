...
class DataType:
    ...
    _internal_type: Optional[Union[pa.DataType, np.dtype, type]]
    _logical_dtype: Optional[_LogicalDataType] = None  # Used for pattern matching

    # Sentinel value to represent "any type" for logical types in nested types
    ...
    def __eq__(self, other: "DataType") -> bool:
        ...
        self_is_pattern = self._internal_type is None
        other_is_pattern = other._internal_type is None
    ...
    @classmethod
    def list(
        cls, value_type: Union["DataType", _LogicalDataType, None] = None
    ) -> "DataType":
        ...
        Examples:
            ...
            >>> DataType.list(DataType.ANY)  # Pattern: matches any list
            DataType(logical_dtype:LIST)
            >>> DataType.list()  # Same as DataType.list(DataType.ANY)
            DataType(logical_dtype:LIST)
        """
        if value_type is None or isinstance(value_type, _LogicalDataType):
            # Create a special pattern-matching DataType for "any list"
            return cls(_internal_type=None, _logical_dtype=_LogicalDataType.LIST)
    ...
    @classmethod
    def struct(
        cls, fields: Union[List[Tuple[str, "DataType"]], _LogicalDataType, None] = None
    ) -> "DataType":
        ...
        Examples:
            ...
            DataType(logical_dtype:STRUCT)
        """
        if fields is None or isinstance(fields, _LogicalDataType):
            return cls(_internal_type=None, _logical_dtype=_LogicalDataType.STRUCT)
    ...
    def map(
        cls,
        key_type: Union["DataType", _LogicalDataType, None] = None,
        value_type: Union["DataType", _LogicalDataType, None] = None,
    ) -> "DataType":
        ...
        Examples:
            ...
            >>> DataType.map(DataType.ANY)  # Pattern: matches any map
            DataType(logical_dtype:MAP)
            >>> DataType.map()  # Same as DataType.map(DataType.ANY)
            DataType(logical_dtype:MAP)
            >>> DataType.map(DataType.string(), DataType.ANY)  # Pattern: matches any map
            DataType(logical_dtype:MAP)
        """
        if key_type is None or isinstance(key_type, _LogicalDataType):
            return cls(_internal_type=None, _logical_dtype=_LogicalDataType.MAP)
    ...
    def tensor(
        cls,
        shape: Union[Tuple[int, ...], _LogicalDataType, None] = None,
        dtype: Union["DataType", _LogicalDataType, None] = None,
    ) -> "DataType":
        ...
        Examples:
            ...
            DataType(logical_dtype:TENSOR)
            >>> DataType.tensor(shape=(3, 4), dtype=DataType.ANY)  # Pattern: matches any tensor
            DataType(logical_dtype:TENSOR)
        """
        if shape is None or isinstance(shape, _LogicalDataType):
            return cls(_internal_type=None, _logical_dtype=_LogicalDataType.TENSOR)

        if dtype is None or isinstance(dtype, _LogicalDataType):
            return cls(_internal_type=None, _logical_dtype=_LogicalDataType.TENSOR)
    ...
    def temporal(
        cls,
        temporal_type: Optional[Union[str, _LogicalDataType]] = None,
        ...
        Examples:
            ...
            >>> DataType.temporal(DataType.ANY)  # Pattern: matches any temporal
            DataType(logical_dtype:TEMPORAL)
            >>> DataType.temporal()  # Same as DataType.temporal(DataType.ANY)
            DataType(logical_dtype:TEMPORAL)
        """
        if temporal_type is None or isinstance(temporal_type, _LogicalDataType):
            return cls(_internal_type=None, _logical_dtype=_LogicalDataType.TEMPORAL)
    ...
    def is_numerical_type(self) -> bool:
        ...
        if self.is_arrow_type():
            ...
            return self._internal_type in (int, float, complex)
        return False
