...
class ReadTask(Callable[[], List[Block]]):
    """A function used to read block from the dataset.

    Read tasks are generated by ``datasource.prepare_read()``, and return
    a list of ``ray.data.Block`` when called. Metadata about the read operation
    can be retrieved via ``get_metadata()`` prior to executing the read.
    ...
    The metadata should reflect all the blocks returned by the read, e.g.,
    if the metadata says num_rows=1000, the read can return a single block of
    1000 rows, or multiple blocks with 1000 rows altogether.
    """
    ...
    def __call__(self) -> List[ObjectRef[Block]]:
        ...
        if not isinstance(result[0], ray.ObjectRef):
            raise ValueError(
