...
class Datasource:
    ...
    To write to a writable datasource, use ``Datastream.write_datasource()``.
...
class Reader:
    ...
    For example, it is useful for Datastreams to know the in-memory size of the read
...
class ReadTask(Callable[[], Iterable[Block]]):
    """A function used to read blocks from the datastream.
