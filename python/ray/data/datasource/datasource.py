...
class Datasource(_DatasourceProjectionPushdownMixin, _DatasourcePredicatePushdownMixin):
    ...
    Example:
        >>> class MyDatasource(Datasource): # doctest: +SKIP
        ...     def get_read_tasks(self, parallelism: int, **read_args) -> List["ReadTask"]: # doctest: +SKIP
        ...         return [ReadTask(lambda: [pa.Table.from_pydict({"data": range(10)})], BlockMetadata(10, 80, None, None))] # doctest: +SKIP
        ...     def estimate_inmemory_data_size(self) -> Optional[int]: # doctest: +SKIP
        ...         return 80
        >>> ds = MyDatasource() # doctest: +SKIP
        >>> tasks = ds.get_read_tasks(parallelism=1) # doctest: +SKIP
        >>> assert len(tasks) == 1 and tasks[0].metadata.num_rows == 10 # doctest: +SKIP
    """  # noqa: E501
