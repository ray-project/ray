...
def _snowflake_load_private_key(props: Dict) -> None:
    ...
    if private_key:
        if isinstance(private_key, str):
            ...
            raise ValueError(
                "The private_key property must be a string, bytes or an RSAPrivateKey."
            )
    ...
    if pk_bytes:
        pem = serialization.load_pem_private_key(
            pk_bytes, password=password, backend=default_backend()
        )
...
class SnowflakeConnector(DBAPI2Connector):
    def __init__(self, **connect_properties):
        ...
        super().__init__(
            connector.connect,
            to_block_fn=lambda d: d
            if isinstance(d, pandas.DataFrame)
            else d.fetch_pandas_all(),
    ...
    def write_pandas(self, table: str, block: Block, *args, **kwargs) -> None:
        ...
        write_pandas(
            self.connection, data, *args, table_name=table, **{"parallel": 1, **kwargs}
        )
    ...
    def get_read_tasks(self, parallelism: int) -> List[DatabaseReadTask]:
        ...
        for i, batch in enumerate(batches):
            ...
            queries = self.queries.templatize(
                metadata=metadata, partition=i, row_start=row_start, num_rows=num_rows
            )
...
class SnowflakeDatasource(DBAPI2Datasource):
    ...
    See [Snowflake connector API]
    (https://docs.snowflake.com/en/user-guide/python-connector-api.html#module-snowflake-connector).
    """
