...
#
# Each query declares only its event-specific stages. The preamble that finds
# Ray's annotation lines in the operator's log backend and scopes them to the
# cluster that emitted them is generated by `generate_annotation`, which pins the
# session name as a literal so no dashboard setting can widen it onto another
# cluster. The run filters below narrow further, to a single Train run, which is
# what separates concurrent runs sharing one cluster, as in a workspace.
#
# Must match `ray.train.v2._internal.constants.TRAIN_ANNOTATION_SOURCE`, which is
# not imported here to keep the dashboards independent of Ray Train internals.
