...
EXTERNAL_OUTQUEUE_BLOCKS_PANEL = Panel(
    ...
    description="Current number of blocks in an operator's external output queue. This queue typically stores references to results produced by the operator's tasks and awaiting collection by downstream operators.",
    unit="blocks",
    targets=[
        Target(
...
OBJECT_STORE_MEMORY_BUDGET_PANEL = Panel(
    ...
    description="Displays the allocated object store memory budget for an operator. This represents the portion of the total memory budget dedicated to storing data blocks in the Ray object store.",
    unit="bytes",
    targets=[
        Target(
