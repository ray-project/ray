...
class TaskMetadataBuffer:
    ...
    def __init__(self, max_buffer_size: int = 1000, max_dropped_attempts_per_metadata_entry: int = 100):
    ...
    def merge(self, new_metadata: Optional[events_event_aggregator_service_pb2.TaskEventsMetadata]) -> None:
        ...
        with self._lock:
            for new_attempt in new_metadata.dropped_task_attempts:
                if len(self._metadata.dropped_task_attempts) >= self._max_dropped_attempts:
                    # Flush current metadata to buffer
                    if self._buffer.full():
                        oldest_entry = self._buffer.get_nowait()
                        self._dropped_metadata_count += len(oldest_entry.dropped_task_attempts)
