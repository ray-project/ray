...
def test_ray_actor_events(ray_start_cluster, httpserver):
    ...
    def verify_alive_state():
        ...
        for req, _ in httpserver.log:
            ...
            for actorLifeCycleEvent in req_json:
                if "actorLifecycleEvent" in actorLifeCycleEvent:
                    assert (
                        base64.b64decode(actorLifeCycleEvent["nodeId"]).hex()
                        == head_node_id
                    )
                    assert (
                        base64.b64decode(
                    ...
                    for stateTransition in actorLifeCycleEvent["actorLifecycleEvent"][
                        ...
                        if stateTransition["state"] == "ALIVE":
                            ...
                            assert (
                                base64.b64decode(stateTransition["nodeId"]).hex()
                                in all_nodes_ids
                            )
                            assert (
                                base64.b64decode(stateTransition["workerId"]).hex()
                                != ""
                            )
        assert has_alive_state
        return True

    wait_for_condition(verify_alive_state, timeout=5)
    ...
    wait_for_condition(verify_dead_state, timeout=5)
