...
def test_ray_submission_job_events(ray_start_cluster, httpserver):
    ...
    cluster.add_node(
        env_vars={
            # Env var (instead of _system_config) so the dashboard agent
            # process also picks up the flag.
            "RAY_enable_ray_event": "1",
            "RAY_ENABLE_PYTHON_RAY_EVENT_TYPES": "SUBMISSION_JOB_DEFINITION_EVENT,SUBMISSION_JOB_LIFECYCLE_EVENT",
