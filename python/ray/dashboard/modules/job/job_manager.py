            ...
            try:
                ...
                if job_supervisor is None:
                    if job_status == JobStatus.PENDING:
                        ...
                        if not status_updated:
                            logger.info(
                                "Job %s changed while marking its missing "
                                "supervisor as failed; retrying with the latest state.",
                                job_id,
                            )
                            job_info = None
                            continue
                ...
                if ready:
                    try:
                        ...
                        # A completed error must not be reused after a status-write race.
                        ping_obj_ref = None
                else:
                    continue
