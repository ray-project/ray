...
def _extract_k8s_event_timestamp(event_obj: Any) -> Optional[datetime]:
    ...
    core/v1 EventSeries or events.k8s.io/v1). Otherwise falls back through
    `last_timestamp`, `event_time`, and `first_timestamp`.
    """
...
def _extract_k8s_event_component(event_obj: Any) -> str:
    ...
    and report the emitter in `reportingComponent` instead. Prefer
    `source.component` and fallback to `reportingComponent`, matching kubectl.
    """
    ...
    reporting_component = _get_event_attr(event_obj, "reporting_component")
    if isinstance(reporting_component, str) and reporting_component:
        return reporting_component
...
def _extract_k8s_event_count(event_obj: Any) -> Optional[int]:
    ...
    For repeated events represented as a series (core/v1 EventSeries
    or events.k8s.io/v1), prefers `series.count`. Otherwise falls back
    to the legacy top-level `count`.
    """
    series = _get_event_attr(event_obj, "series")
    if series is not None:
    ...
    val = _get_event_attr(event_obj, "count")
