        ...
        with zipfile.ZipFile(self.zip_file_path, mode='w') as zf:
            for folder_name, _, filenames in os.walk(self.prometheus_db_path):
                for filename in filenames:
                    ...
                    parent_path = os.path.relpath(file_path, self.prometheus_db_path)
...
if __name__ == "__main__":
    ...
    s3_writer = PrometheusS3Writer(prometheus_db_path, bucket, key, zip_temp_dir)
    zip_file_path = "{}/{}.zip".format(zip_temp_dir, key)

    while True:
        s3_writer.write()
        logger.info("Writing done. Waiting for {} seconds.".format(update_frequency))
