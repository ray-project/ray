...
def post_auth(auth_url, cluster_id):
    try:
        ...
        logger.error("Failed to connect to the server {}".format(auth_url))
        return None
...
def post_ingest(export_address: str,
                dashboard_id: str,
                access_token: str,
                ray_config: dict,
                node_info: dict,
    ...
    if response.status_code != 200:
        logger.error("Failed to export metrics\n"
                        "URL: {}.\n"
                        "Status code: {}"
                        .format(export_address, response.status_code))
