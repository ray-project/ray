...
const ActorClassGroups: React.FC<ActorClassGroupsProps> = ({ actorGroups }) => {
  const numInfeasible = (group: ActorGroup) => group.summary.stateToCount[ActorState.Infeasible] ?? 0;
  ...
  const children = stableSort(Object.entries(actorGroups), groupComparator)
  ...
  <React.Fragment>
    <Snackbar open={warningOpen}>
        ...
        There are one or more actors that cannot currently be
        created due to insufficient cluster resources. These
        have been sorted to the top of the list. If you
        are using autoscaling functionality, you may ignore
        this message.
            </Alert>
    </Snackbar>
    {children}
