...
export const useJobTable = (submitId?: string) => {
  const {
    msg,
    isLoading,
    isRefreshing,
      ...
      <Table>
        ...
        <TableBody>
          {list.map((job, index) => {
            ...
            return <JobRow key={job_id ?? submission_id ?? index} job={job} />;
          })}
        </TableBody>
      </Table>
    </TableContainer>
...
const JobList = () => {
  const { jobTable, msg, isLoading, isRefreshing, onSwitchChange } =
    useJobTable();
  return (
    <Box sx={{ padding: 2, width: "100%" }}>
      ...
      <TitleCard title="Job List">{jobTable}</TitleCard>
    </Box>
  );
};
