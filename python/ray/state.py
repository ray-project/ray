...
class GlobalState:
    ...
    def _check_connected(self):
        ...
        Raises:
            RuntimeError: An exception is raised if ray.init() has not been
                called yet.
        """
        if (self.gcs_client_options is not None
    ...
    def _initialize_global_state(self, gcs_client_options):
        """Set args for lazily initialization of the GlobalState object.

        It's possible that certain keys in gcs table may not have been fully
        populated yet. In this case, we will retry this method until they have
        been populated or we exceed a timeout.

        Args:
            gcs_client_options: The config to connect to gcs
        """
