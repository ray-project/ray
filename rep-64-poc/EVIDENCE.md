...
4. **macOS arm64 dev workflow is now green and APFS substrate evidence is captured.** `bazel build //src/ray/...`, `bazel build //:all //cpp:all`, and `pytest python/ray/tests/test_basic.py` all succeed on macOS arm64 (Apple Silicon, Bazel 7.5.0). The local changes that make this possible are documented in `MACOS_DEV_SETUP.md` and packaged as an applicable patch in `macos-dev.patch` — they are **not** committed to the tree, since Ray's supported build target is Linux. Four of the original six are general portability fixes (independent of REP-64) that would benefit any macOS contributor: `--dynamic_mode=off` to dodge upb dylib symbol resolution, two missing `target_compatible_with` Linux gates (`:jemalloc_files`, `:sysfs_cgroup_driver`), and routing the dashboard-agent port-file wait through the existing `agent_register_timeout_ms` config. **These four are upstream-worthy as a separate PR.** All four phases (1, 4, 7, 8) have now been run on APFS; results live in `reports/macos-apfs-evidence.md` and the `*_apfs*.json` files under each phase's `results/` dir. The Phase 4 writer was modified to issue `fcntl(F_FULLFSYNC, 0)` after each acked Put — Apple's documented media-flush barrier (plain `fsync()` is page-cache-bounded on macOS); this is the highest-evidence configuration and gives an apples-to-apples durability test vs. ext4's honest fsync. Surfaced one cross-platform compile bug (`recovery_bench.cc` had a stale `Postable<>` calling pattern that would fail on Linux too) — fixed in a separate commit on this branch.
...
| 5 | Ack ⇒ durable. `Put(WriteOptions{sync=true})` returns OK only after the write is recoverable from disk. | 4 | kill-9 harness: 1k and 5k writes, SIGKILL mid-run, 0 acked-but-missing keys on probe-verified honest ext4. **And on probe-verified honest macOS APFS** — the writer was modified to issue `fcntl(F_FULLFSYNC, 0)` after each Put (Apple's documented media-flush barrier; plain `fsync()` is page-cache-bounded on macOS). Negative control (50 ghost-writes) flagged exactly 50 acked-but-missing on **both** substrates. | **Verified for ext4 and APFS. Cloud PVs pending.** |
...
| 7 | RocksDB write 0.01–0.1 ms (REP perf table). | 7 | Measured **3.81 ms p50 sync-on-write** on probe-verified ext4 and **4.02 ms p50** on probe-verified macOS APFS. **REP claim contradicted on both substrates.** Holds only for memtable-only / sync=false writes. | **Contradicted on two substrates — REP needs revision** |
| 8 | RocksDB read 0.01–0.1 ms (REP perf table). | 7 | Measured 0.97 µs p50 (ext4), 3.25 µs p50 (APFS). Both inside the REP's range. | **Confirmed on two substrates** |
| 9 | RocksDB recovery time is equal to or better than Redis-based FT. | 8 | Storage-layer cold-open + scan: 81 ms at 10k entries on ext4, 61 ms at 10k entries on APFS — both under the sub-100 ms target. Strictly faster than "wait for Redis container + RPC fetch all state." End-to-end Redis comparison pending Docker Compose harness. | **Provisional yes on two substrates; head-to-head pending** |
...
| macOS APFS (user's MacBook) | **Honest** via `F_FULLFSYNC` (p50 = 4003 µs, same media-flush class as ext4's fsync); plain `fsync()` is page-cache-bounded as documented (p50 = 17 µs) | **PASS** (1k, 5k writes, sync=1 + per-Put `F_FULLFSYNC` barrier — see Phase 4 caveat below); ghost-write negative control catches exactly 50 | **Captured** (Put p50 4.02 ms, Get p50 3.25 µs) | **Captured** (61 ms cold open + scan at 10k entries) |
...
| R4 | fsync semantics on K8s PVs silently violate durability | **Closed on two honest substrates: ext4 (Phase 4) and APFS via F_FULLFSYNC (`reports/macos-apfs-evidence.md`).** Cloud PV substrates still pending. |
| R5 | Atomic GetNextJobID correctness under concurrency | **Closed for mutex-RMW on GCC 11** (Phase 5). TSAN/ASAN pending. |
| R6 | RocksDB sync-write latency materially worse than REP claims | **Open — REP claim is materially off**, correction documented in Phase 7 and confirmed on a second substrate (APFS, Put p50 4.02 ms; `reports/macos-apfs-evidence.md`). |
| R7 | Recovery time scales poorly | **Closed at the storage layer on two substrates: ext4 (Phase 8) and APFS (`reports/macos-apfs-evidence.md`).** Full GCS-process timing pending. |
...
6. ~~**macOS+F_FULLFSYNC** runs on the user's MacBook for substrate diversity at zero infrastructure cost.~~ **Done.** All four phases (1, 4, 7, 8) ran on APFS with `F_FULLFSYNC` as the honest barrier — see `reports/macos-apfs-evidence.md` and the `*_apfs*.json` files in the per-phase `results/` directories. The harness BUILD-gate relaxations and the writer.cc F_FULLFSYNC barrier are bundled into `macos-dev.patch`; the recovery_bench Postable-API fix surfaced during this run was committed separately as a cross-platform bug.
