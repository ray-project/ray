...
typedef struct {
  PyObject_HEAD
  local_scheduler_conn *local_scheduler_connection;
...
static int PyLocalSchedulerClient_init(PyLocalSchedulerClient *self,
  ...
  self->local_scheduler_connection =
      local_scheduler_connect(socket_name, actor_id);
  return 0;
}

static void PyLocalSchedulerClient_dealloc(PyLocalSchedulerClient *self) {
  local_scheduler_disconnect(((PyLocalSchedulerClient *) self)->local_scheduler_connection);
...
static PyObject *PyLocalSchedulerClient_log_event(PyObject *self, PyObject *args) {
  const char *key;
  int key_length;
  const char *value;
  int value_length;
  ...
  local_scheduler_log_event(((PyLocalSchedulerClient *) self)->local_scheduler_connection,
...
static PyObject *PyLocalSchedulerClient_notify_unblocked(PyObject *self) {
  local_scheduler_notify_unblocked(((PyLocalSchedulerClient *) self)->local_scheduler_connection);
  Py_RETURN_NONE;
}
...
static PyMethodDef PyLocalSchedulerClient_methods[] = {
    ...
    {"reconstruct_object", (PyCFunction) PyLocalSchedulerClient_reconstruct_object,
...
#else
  PyObject *m = Py_InitModule3("liblocal_scheduler", local_scheduler_methods,
