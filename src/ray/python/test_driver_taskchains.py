...
def submit_task_chains(num_chains, tasks_per_chain):
    ...
    for chain_num in range(num_chains):
        ...
        for i in range(tasks_per_chain):
            task_returns = submit_task_withdep(driver, task_object_dependencies=last_task_returns)
...
def TEST_run_task_chains(num_chains, tasks_per_chain):
    task_placement_map = submit_task_chains(num_chains=num_chains, tasks_per_chain=tasks_per_chain)
    ...
    for chain_num in range(len(task_placement_map)):
        task_placement_list = driver.get(task_placement_map[chain_num], timeout_ms=5000)
    ...
    task_placement_stats = [(v,task_placement_total.count(v)) for v in set(task_placement_total)]
...
def TEST_run_tasks_nodep(num_tasks):
    ...
    task_placement_map = submit_task_chains(num_chains=num_tasks, tasks_per_chain=1)
    logger.debug("[DRIVER]: task placement information, per chain:")
    task_placement_total = []
    for chain_num in range(len(task_placement_map)):
        task_placement_list = driver.get(task_placement_map[chain_num], timeout_ms=5000)
