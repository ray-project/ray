...
RayLogLevelNew RayLog::severity_threshold_ = RayLogLevelNew::INFO;
...
class DefaultStdErrLogger final {
 ...
 private:
  DefaultStdErrLogger() {
    default_stderr_logger_ = spdlog::stderr_color_mt("stderr_new");
...
static int GetMappedSeverity(RayLogLevelNew severity) {
  switch (severity) {
  case RayLogLevelNew::TRACE:
    return spdlog::level::trace;
  case RayLogLevelNew::DEBUG:
    return spdlog::level::debug;
  case RayLogLevelNew::INFO:
    return spdlog::level::info;
  case RayLogLevelNew::WARNING:
    return spdlog::level::warn;
  case RayLogLevelNew::ERROR:
    return spdlog::level::err;
  case RayLogLevelNew::FATAL:
    return spdlog::level::critical;
  default:
    RAY_LOG_NEW(FATAL) << "Unsupported logging level: " << static_cast<int>(severity);
...
void RayLog::StartRayLog(const std::string &app_name, RayLogLevelNew severity_threshold,
  ...
  if (var_value != nullptr) {
    ...
    if (data == "trace") {
      severity_threshold = RayLogLevelNew::TRACE;
    } else if (data == "debug") {
      severity_threshold = RayLogLevelNew::DEBUG;
    } else if (data == "info") {
      severity_threshold = RayLogLevelNew::INFO;
    } else if (data == "warning") {
      severity_threshold = RayLogLevelNew::WARNING;
    } else if (data == "error") {
      severity_threshold = RayLogLevelNew::ERROR;
    } else if (data == "fatal") {
      severity_threshold = RayLogLevelNew::FATAL;
    } else {
      RAY_LOG_NEW(WARNING) << "Unrecognized setting of RAY_BACKEND_LOG_LEVEL="
                           << var_value;
    }
    RAY_LOG_NEW(INFO)
...
void RayLog::UninstallSignalAction() {
  ...
  RAY_LOG_NEW(DEBUG) << "Uninstall signal handlers.";
  ...
  for (int signal_num : installed_signals) {
    RAY_CHECK_NEW(sigaction(signal_num, &sig_action, NULL) == 0);
  }
#endif
...
void WriteFailureMessage(const char *data) {
  ...
  // for pretty printing.
  if (nullptr != data) {
    RAY_LOG_NEW(ERROR) << std::string(data, strlen(data) - 1);
  }
...
bool RayLog::IsLevelEnabled(RayLogLevelNew log_level) {
...
RayLog::RayLog(const char *file_name, int line_number, RayLogLevelNew severity)
...
RayLog::RayLog(const char *prefix, RayLogLevelNew severity)
    : is_enabled_(severity >= severity_threshold_),
      severity_(severity),
      is_fatal_(severity == RayLogLevelNew::FATAL),
      str_(arena_),
...
RayLog::~RayLog() {
  ...
  if (loglevel_ == static_cast<int>(spdlog::level::critical)) {
    str_.append("\n*** StackTrace Information ***\n").append(ray_test::GetCallTrace());
  }
  // if (expose_osstream_) {
  //   *expose_osstream_ << "\n*** StackTrace Information ***\n"
  //                     << ray_test::GetCallTrace();
  // }
  ...
  //   }
  // }
  if (severity_ == RayLogLevelNew::FATAL) {
