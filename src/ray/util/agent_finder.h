...
static inline void FindAgentAddress(std::shared_ptr<gcs::RedisClient> redis_client, NodeID node_id,
                      GetAgentAddressCallback callback) {
...
static inline void FindAgentAddress(std::shared_ptr<gcs::GcsClient> gcs_client, NodeID node_id,
                      GetAgentAddressCallback callback) {
  auto key = std::string(kDashboardAgentAddressPrefix) + ":" + node_id.Hex();
...
static inline void UpdateAgentAddress(std::shared_ptr<gcs::GcsClient> gcs_client, NodeID node_id,
                        const std::string &value, UpdateAgentAddressCallback callback) {
  auto key = std::string(kDashboardAgentAddressPrefix) + ":" + node_id.Hex();
