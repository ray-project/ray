...
class RaySyncer {
 public:
  ...
  constexpr RayComponentId kComponentArraySize = ray::rpc::syncer::RayComponentId_ARRAYSIZE;
  ...
  bool Update(RaySyncMessage message) {
    ...
    iter->second[message.component_id()] = std::make_shared<RaySyncMessage>(std::move(message));
    return receivers_[message.component_id()]->Update(iter->second[message.component_id()]);
  }
  ...
  std::vector<std::shared_ptr<RaySyncMessages>> SyncMessages(const std::string& node_id) const {
 ...
 private:
  // Manage messages
  absl::flat_hash_map<std::string, std::array<std::shared_ptr<RaySyncMessage>, kComponentArraySize>> cluster_messages_;
...
class RaySyncerService : public ray::rpc::syncer::RaySyncer::CallbackService {
 public:
  RaySyncerService(instrumented_io_context& io_context)
