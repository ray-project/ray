...
#ifdef _WIN32
void SetHandleCloseOnExec(HANDLE handle) {
  if (handle != INVALID_HANDLE_VALUE && handle != nullptr) {
    SetHandleInformation(handle, HANDLE_FLAG_INHERIT, 0);
  }
}

int OpenHandleAsFd(intptr_t handle, int flags) {
  SetHandleCloseOnExec(reinterpret_cast<HANDLE>(handle));
  int fd = _open_osfhandle(handle, flags);
  if (fd < 0) {
    RAY_LOG(WARNING) << "_open_osfhandle failed for handle " << handle;
  }
  return fd;
}
#endif
...
Pipe::Pipe(Pipe &&other) noexcept { MoveFrom(std::move(other)); }
...
Pipe Pipe::FromWriterHandle(intptr_t writer_handle) {
  Pipe p;
  p.Close();
#ifdef _WIN32
...
Pipe Pipe::FromReaderHandle(intptr_t reader_handle) {
  Pipe p;
  p.Close();
#ifdef _WIN32
...
intptr_t Pipe::MakeReaderHandle() {
...
intptr_t Pipe::MakeWriterHandle() {
...
StatusOr<std::string> Pipe::Read(int timeout_s, size_t chunk_size) {
  if (read_fd_ < 0) {
...
Status Pipe::Write(const std::string &payload) {
  if (write_fd_ < 0) {
...
#ifdef _WIN32
    int n =
        _write(write_fd_, remaining.data(), static_cast<unsigned int>(remaining.size()));
#else
...
void Pipe::CloseReaderHandle() { ResetFd(read_fd_for_close_); }

void Pipe::CloseWriterHandle() { ResetFd(write_fd_for_close_); }
...
std::vector<intptr_t> Pipe::ParseHandlesAndSetCloexec(const std::string &handles_str) {
...
void Pipe::SetFdCloseOnExec(int fd) {
#ifdef _WIN32
...
void Pipe::ClearFdCloseOnExec(int fd) {
#ifdef _WIN32
  intptr_t handle = _get_osfhandle(fd);
  if (handle != -1) {
...
void Pipe::MoveFrom(Pipe &&other) noexcept {
