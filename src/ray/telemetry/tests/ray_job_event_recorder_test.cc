...
TEST_F(RayJobEventRecorderTest, TestRecordEvents) {
  ...
  ASSERT_EQ(recorded_events_[0].driver_job_event().driver_address().ip_address(),
            "127.0.0.1");
  ...
  ASSERT_EQ(recorded_events_[1].driver_job_event().driver_address().ip_address(),
            "192.168.1.100");
