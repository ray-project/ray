...
struct ReceiverInterface {
  /// Consume messages of one type generated by other nodes. The module should read the
  /// `sync_message` field of each and deserialize it to update its internal state. The
  /// messages were received together, so a module that reacts to the new state (e.g. by
  /// scheduling) can do it once per call, after all of them are applied.
  ///
  /// \param message_type The type every message in this call has.
  /// \param messages The messages received from remote nodes.
  virtual void ConsumeSyncMessages(
      MessageType message_type,
      std::vector<std::shared_ptr<const RaySyncMessage>> messages) = 0;
...
class RaySyncer {
 public:
  ...
  /// \param on_messages_received See MessagesReceivedCallback in common.h.
  ...
  /// Consume the messages locally and forward the ones this node did not already
  /// have to every connected node that doesn't have them.
  /// The messages can be generated by the local reporter or received from another node.
  ///
  /// \param messages The messages to be broadcasted.
  void BroadcastMessages(std::vector<std::shared_ptr<const RaySyncMessage>> messages);
 ...
 private:
  ...
  /// MessagesReceivedCallback (see common.h), handed to every reactor this syncer
  /// creates.
