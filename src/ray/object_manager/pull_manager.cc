  ...
  for (const auto &obj_id : bundle_it->second.objects_) {
    ...
    if (it != object_pull_requests_.end()) {
      ...
      if (it->second.bundle_request_ids.empty()) {
        pull_manager_object_request_time_ms_.Record(
...
void PullManager::RecordMetrics() const {
  absl::MutexLock lock(&active_objects_mu_);
  pull_manager_usage_bytes_.Record(num_bytes_available_, {{"Type", "Available"}});
  pull_manager_usage_bytes_.Record(num_bytes_being_pulled_, {{"Type", "BeingPulled"}});
  pull_manager_usage_bytes_.Record(pinned_objects_size_, {{"Type", "Pinned"}});
  pull_manager_requested_bundles_.Record(get_request_bundles_.requests.size(),
                                         {{"Type", "Get"}});
  pull_manager_requested_bundles_.Record(wait_request_bundles_.requests.size(),
                                         {{"Type", "Wait"}});
  pull_manager_requested_bundles_.Record(task_argument_bundles_.requests.size(),
                                         {{"Type", "TaskArgs"}});
  pull_manager_requested_bundles_.Record(next_req_id_, {{"Type", "CumulativeTotal"}});
  pull_manager_requests_.Record(object_pull_requests_.size(), {{"Type", "Queued"}});
  pull_manager_requests_.Record(active_object_pull_requests_.size(),
                                {{"Type", "Active"}});
  pull_manager_requests_.Record(pinned_objects_.size(), {{"Type", "Pinned"}});
  pull_manager_active_bundles_.Record(num_active_bundles_);
  pull_manager_retries_total_.Record(num_retries_total_);
  pull_manager_retries_total_.Record(num_tries_total_);

  pull_manager_num_object_pins_.Record(num_succeeded_pins_total_, {{"Type", "Success"}});
  pull_manager_num_object_pins_.Record(num_failed_pins_total_, {{"Type", "Failure"}});
}
