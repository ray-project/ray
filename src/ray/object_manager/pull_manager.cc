...
void PullManager::UpdatePullsBasedOnAvailableMemory(int64_t num_bytes_available) {
  ...
  while (ActivateNextPullBundleRequest(
      task_argument_bundles_,
      &highest_task_req_id_being_pulled_,
      /*respect_quota=*/true,
      &objects_to_pull)) {
...
void PullManager::UpdateRetryTimer(
...
std::string PullManager::BundleInfo(
    const Queue &bundles,
...
int64_t PullManager::NextRequestBundleSize(
    const Queue &bundles,
    uint64_t highest_id_being_pulled) const {
  // Get the next pull request in the queue.
...
void PullManager::RecordMetrics() const {
  ...
  ray::stats::STATS_pull_manager_usage_bytes.Record(
  ...
  ray::stats::STATS_pull_manager_requested_bundles.Record(
      get_request_bundles_.size(),
      "Get");
  ray::stats::STATS_pull_manager_requested_bundles.Record(
      wait_request_bundles_.size(),
      "Wait");
  ray::stats::STATS_pull_manager_requested_bundles.Record(
      task_argument_bundles_.size(),
      "TaskArgs");
  ray::stats::STATS_pull_manager_requests.Record(object_pull_requests_.size(), "Queued");
  ray::stats::STATS_pull_manager_requests.Record(
