...
class TestObjectManagerCommands : public TestObjectManager {
 public:
  ...
  void TestNotifications() {
    ...
    status =
        server1->object_manager_.SubscribeObjAdded([this](const ObjectInfoT &object_info) {
          object_added_handler_1(ObjectID::from_binary(object_info.object_id));
          if (v1.size() == num_expected_objects) {
            NotificationTestComplete(created_object_id, ObjectID::from_binary(object_info.object_id));
          }
        });
    RAY_CHECK_OK(status);
