...
void PushManager::RecordMetrics() const {
  push_manager_num_pushes_remaining_.Record(NumPushRequestsWithChunksToSend());
  push_manager_chunks_.Record(NumChunksInFlight(), {{"Type", "InFlight"}});
  push_manager_chunks_.Record(NumChunksRemaining(), {{"Type", "Remaining"}});
}
