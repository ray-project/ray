...
std::pair<const ObjectBufferPool::ChunkInfo &, ray::Status> ObjectBufferPool::GetChunk(
  ...
  if (get_buffer_state_.count(object_id) == 0) {
    ...
    if (object_buffer.data == nullptr) {
      RAY_LOG(INFO) << "Failed to get a chunk of the object: " << object_id <<". It is mostly because the object is already evicted or spilled when the pull request is received. The caller will retry the pull request again.";
