...
void PlasmaStoreRunner::Start(ray::SpillObjectsCallback spill_objects_callback,
  ...
  RAY_LOG(DEBUG) << "starting server listening on " << socket_name_;
  {
    ...
    allocator_ = std::make_unique<PlasmaAllocator>(
        ...
        ray::core::RayConfig::instance().plasma_unlimited());
    store_.reset(
        new PlasmaStore(main_service_, *allocator_, socket_name_,
                        ray::core::RayConfig::instance().object_store_full_delay_ms(),
                        ray::core::RayConfig::instance().object_spilling_threshold(),
