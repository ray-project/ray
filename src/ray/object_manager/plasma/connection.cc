...
static const std::vector<std::string> object_store_message_enum = GenerateEnumNames(
    flatbuf::EnumNamesMessageType(),
  ...
  if (used_fds_.find(fd) == used_fds_.end()) {
    ...
    if (!success) {
      ...
      return Status::IOError(
          "Fail to duplicate handle to PID = " + std::to_string(target_pid));
    }
