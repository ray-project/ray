...
void *fake_mmap(size_t size) {
  // In unlimited allocation mode, fail allocations done by
  // PlasmaAllocator::GetInstance().Memalign() after the initial allocation. Allow
  // allocations done by PlasmaAllocator::GetInstance().DiskMemalignUnlimited(), which
  // sets mmap_threshold to zero prior to calling dlmemalign().
