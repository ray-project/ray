...
class PlasmaBuffer : public SharedMemoryBuffer {
 public:
  ...
  PlasmaBuffer(std::shared_ptr<PlasmaClientInterface> client, const ObjectID &object_id,
 ...
 private:
  std::shared_ptr<PlasmaClientInterface> client_;
  ObjectID object_id_;
};
...
class RAY_NO_EXPORT PlasmaMutableBuffer : public SharedMemoryBuffer {
 public:
  PlasmaMutableBuffer(std::shared_ptr<PlasmaClientInterface> client,
...
class Impl : public std::enable_shared_from_this<Impl> {
 public:
  ...
  Status Get(const std::vector<ObjectID> &object_ids, int64_t timeout_ms,
             ...
             const std::function<Status(std::vector<MEMFD_TYPE> &store_fds,
 ...
 private:
  ...
  uint8_t *GetStoreFdAndMmap(MEMFD_TYPE store_fd, int64_t map_size, PlasmaObject &object);
...
uint8_t *Impl::GetStoreFdAndMmap(MEMFD_TYPE store_fd_val, int64_t map_size,
                                 PlasmaObject &object) {
  ...
  if (entry != mmap_table_.end()) {
    ...
    if (store_conn_) {
      ...
      RAY_CHECK(object.address);
      mmap_table_[store_fd_val] =
          std::make_unique<ClientMmapTableEntry>((uint8_t *)object.address, map_size);
    }
    RAY_CHECK(mmap_table_[store_fd_val]->pointer());
...
// process before.
uint8_t *Impl::LookupMmappedFile(MEMFD_TYPE store_fd_val) {
  auto entry = mmap_table_.find(store_fd_val);
  RAY_CHECK(entry != mmap_table_.end())
      << "store_fd " << store_fd_val.first << " " << store_fd_val.second;
  return entry->second->pointer();
}

bool Impl::IsInUse(const ObjectID &object_id) {
...
void Impl::IncrementObjectCount(const ObjectID &object_id, PlasmaObject *object,
                                bool is_sealed) {
  ...
  if (elem == objects_in_use_.end()) {
    RAY_LOG(INFO) << "IncrementObjectCount object_id " << object_id << " "
                  << object->store_fd.first << " " << object->store_fd.second;
...
Status Impl::HandleCreateReply(const ObjectID &object_id, const uint8_t *metadata,
  ...
  if (object.device_num == 0) {
    ...
    *data = std::make_shared<PlasmaMutableBuffer>(
        client, GetStoreFdAndMmap(fd, mmap_size, object) + object.data_offset,
        object.data_size);
...
Status Impl::GetBuffers(
  ...
  for (size_t i = 0; i < store_fds.size(); i++) {
    // RAY_CHECK(object_data[i].address);
    GetStoreFdAndMmap(store_fds[i], mmap_sizes[i], object_data[i]);
  }
...
Status Impl::Get(const std::vector<ObjectID> &object_ids, int64_t timeout_ms,
                 std::vector<ObjectBuffer> *out, bool is_from_worker,
                 const std::function<Status(std::vector<MEMFD_TYPE> &store_fds,
                                            std::vector<int64_t> &mmap_sizes,
                                            std::vector<ObjectID> &received_object_ids,
                                            std::vector<PlasmaObject> &object_data)>
  ...
  *out = std::vector<ObjectBuffer>(num_objects);
  return GetBuffers(&object_ids[0], num_objects, timeout_ms, wrap_buffer, &(*out)[0],
                    is_from_worker, get_from_store);
}

Status Impl::MarkObjectUnused(const ObjectID &object_id) {
...
Status Impl::Contains(const ObjectID &object_id, bool *has_object,
                      const std::function<Status()> contains_in_store) {
  std::lock_guard<std::recursive_mutex> guard(client_mutex_);
  ...
  if (objects_in_use_.count(object_id) > 0) {
    *has_object = 1;
    ...
    RAY_RETURN_NOT_OK(contains_in_store());
  }
  return Status::OK();
}
...
Status RemotePlasmaClient::Connect(const std::string &store_socket_name,
...
Status RemotePlasmaClient::CreateAndSpillIfNeeded(
...
Status RemotePlasmaClient::TryCreateImmediately(
    const ObjectID &object_id, const ray::rpc::Address &owner_address, int64_t data_size,
    const uint8_t *metadata, int64_t metadata_size, std::shared_ptr<Buffer> *data,
    fb::ObjectSource source, int device_num) {
  std::lock_guard<std::recursive_mutex> guard(client_mutex_);
...
Status RemotePlasmaClient::Get(const std::vector<ObjectID> &object_ids,
                               int64_t timeout_ms,
  ...
  return impl_->Get(
      ...
      [this, object_ids, timeout_ms, is_from_worker](
        ...
        const size_t num_objects = object_ids.size();
        RAY_RETURN_NOT_OK(SendGetRequest(store_conn_, &object_ids[0], num_objects,
                                         timeout_ms, is_from_worker));
        std::vector<uint8_t> buffer;
        RAY_RETURN_NOT_OK(
...
Status RemotePlasmaClient::HandleCreateReply(const ObjectID &object_id,
...
Status PlasmaClient::CreateAndSpillIfNeeded(
    const ObjectID &object_id, const ray::rpc::Address &owner_address, int64_t data_size,
    const uint8_t *metadata, int64_t metadata_size, std::shared_ptr<Buffer> *data,
...
Status PlasmaClient::TryCreateImmediately(
    const ObjectID &object_id, const ray::rpc::Address &owner_address, int64_t data_size,
    const uint8_t *metadata, int64_t metadata_size, std::shared_ptr<Buffer> *data,
    plasma::flatbuf::ObjectSource source, int device_num) {
  std::unique_lock<std::recursive_mutex> guard(client_mutex_);
  RAY_LOG(INFO) << "TryCreateImmediately " << object_id;
  uint64_t req_id;
  ray::ObjectInfo object_info;
  object_info.data_size = data_size;
  object_info.metadata_size = metadata_size;
  ...
  RAY_UNUSED(impl_->HandleCreateReply(
      object_id, metadata, data, object,
      std::dynamic_pointer_cast<PlasmaClient>((shared_from_this())), object.store_fd,
      object.mmap_size));
  return Status::OK();
}
          ...
          return plasma_store_.GetObjects(
              ...
              [&](const std::shared_ptr<GetRequest> &get_request,
                ...
                for (size_t i = 0; i < num_objects; ++i) {
                  received_object_ids[i] = object_ids[i];
                }
                for (size_t i = 0; i < num_objects; ++i) {
                  const ObjectID &object_id = object_ids[i];
                  object_data[i].store_fd.first =
                      get_request->objects[object_id].store_fd.first;
                  object_data[i].store_fd.second =
                      get_request->objects[object_id].store_fd.second;
                  object_data[i].data_offset =
                      get_request->objects[object_id].data_offset;
                  object_data[i].data_size = get_request->objects[object_id].data_size;
                  object_data[i].metadata_offset =
                      get_request->objects[object_id].metadata_offset;
                  object_data[i].metadata_size =
                      get_request->objects[object_id].metadata_size;
                  object_data[i].device_num = get_request->objects[object_id].device_num;
                  object_data[i].address = get_request->objects[object_id].address;
                }

                for (size_t i = 0; i < fds.size(); i++) {
                  store_fds.push_back({fds[i].first, fds[i].second});
                  mmap_sizes.push_back(sizes[i]);
                }

                get_return.set_value();
              });
        });
