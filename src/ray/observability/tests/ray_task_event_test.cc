...
TaskSpecification CreateNormalTaskSpec(const JobID &job_id) {
  ...
  auto function_descriptor =
      FunctionDescriptorBuilder::BuildPython("test_module", "TestClass", "test_func", "");
...
TaskSpecification CreateActorTaskSpec(const JobID &job_id, const ActorID &actor_id) {
  ...
  auto function_descriptor = FunctionDescriptorBuilder::BuildPython(
      "test_module", "TestActor", "test_method", "");
  rpc::Address caller_address;
  caller_address.set_node_id(NodeID::FromRandom().Binary());
  caller_address.set_ip_address("127.0.0.1");
  caller_address.set_port(1234);
...
TEST(RayTaskEventTest, TestTaskDefinitionEventCreation) {
  ...
  RayTaskDefinitionEvent event(task_spec, task_id, attempt_number, job_id, session_name);
  ...
  // Verify event type
  EXPECT_EQ(event.GetEventType(), rpc::events::RayEvent::TASK_DEFINITION_EVENT);
...
TEST(RayTaskEventTest, TestActorTaskDefinitionEventCreation) {
  JobID job_id = JobID::FromInt(1);
  ActorID actor_id = ActorID::Of(job_id, TaskID::Nil(), 0);
  TaskSpecification task_spec = CreateActorTaskSpec(job_id, actor_id);
  TaskID task_id = task_spec.TaskId();
  int32_t attempt_number = 0;
  std::string session_name = "test_session";

  RayActorTaskDefinitionEvent event(
  ...
  // Verify event type
  EXPECT_EQ(event.GetEventType(), rpc::events::RayEvent::ACTOR_TASK_DEFINITION_EVENT);
...
TEST(RayTaskEventTest, TestTaskProfileEventMerge) {
  TaskID task_id = TaskID::FromRandom(JobID::FromInt(1));
  int32_t attempt_number = 0;
  JobID job_id = JobID::FromInt(1);
  NodeID node_id = NodeID::FromRandom();
  ...
  RayTaskProfileEvent event1(task_id,
                             attempt_number,
                             job_id,
                             node_id,
                             "worker",
  ...
  RayTaskProfileEvent event2(task_id,
                             attempt_number,
                             job_id,
                             node_id,
                             "worker",
  ...
  // Serialize and verify both profile events are present
  auto serialized = std::move(event1).Serialize();
  EXPECT_TRUE(serialized.has_task_profile_events());
  const auto &profile_event = serialized.task_profile_events();
  EXPECT_EQ(profile_event.profile_events().events_size(), 2);
  EXPECT_EQ(profile_event.profile_events().events(0).event_name(), "event_1");
  EXPECT_EQ(profile_event.profile_events().events(1).event_name(), "event_2");
}
