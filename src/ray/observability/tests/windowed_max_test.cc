...
TEST(WindowedMaxTest, ReturnsWindowedMaxEveryCall) {
  ...
  // Lower value within the window: max stays 10 and is still returned.
  EXPECT_DOUBLE_EQ(window.Observe(kT0 + absl::Seconds(1), 4.0), 10.0);

  // Equal to the current max: still 10.
  EXPECT_DOUBLE_EQ(window.Observe(kT0 + absl::Seconds(2), 10.0), 10.0);

  // Higher value: max moves up.
...
TEST(WindowedMaxTest, EvictsSamplesOutsideWindow) {
  ...
  EXPECT_DOUBLE_EQ(window.Observe(kT0 + absl::Seconds(10), 20.0), 100.0);

  // 31s after the first sample, the high sample falls out of the 30s window. The
  // max now drops to the surviving samples.
