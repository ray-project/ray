...
class RayEventInterface {
 public:
  ...
  // Merge with another data point to form a time series.
  //
  // This function assumes that the two events have the same type and resource ID.
  // This function is used to merge the individual execution events into a single
  // execution event (single data point to a time series).
  virtual void Merge(RayEventInterface &other) = 0;

  // Serialize the event data to a RayEvent proto.
  virtual void Serialize(ray::rpc::events::RayEvent *event) const = 0;
