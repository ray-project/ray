...
class RayTaskEventRecorder : public RayEventRecorderBase {
 ...
 private:
  ...
  // (claude) Move up to one batch of buffered events, and the dropped task attempts to
  // report, out of the buffers. Events belonging to a dropped attempt are discarded so
  // that a task attempt is either sent in full or not at all.
