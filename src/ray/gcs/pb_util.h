...
inline std::shared_ptr<ray::rpc::ActorTableData> CreateActorTableData(
    ...
    ray::rpc::ActorStates::ActorState state, uint64_t num_restarts) {
  ...
  actor_info_ptr->mutable_states()->set_num_restarts(num_restarts);
  actor_info_ptr->mutable_states()->mutable_address()->CopyFrom(address);
  actor_info_ptr->mutable_owner_address()->CopyFrom(
      task_spec.GetMessage().caller_address());
  actor_info_ptr->mutable_states()->set_state(state);
