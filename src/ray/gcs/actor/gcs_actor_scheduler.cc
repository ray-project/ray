...
NodeID GcsActorScheduler::SelectForwardingNode(std::shared_ptr<GcsActor> actor) {
  ...
  if (!view_holder_nodes_.empty()) {
    ...
    // The node-removed listener re-designates before a dead node's actors are
    // rescheduled, so the shard target is normally alive. Should it be dead
    // anyway, probe the next view holders rather than fall through: a raylet
    // without a resource view could park the lease forever.
