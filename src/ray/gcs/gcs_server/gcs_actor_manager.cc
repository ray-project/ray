...
void GcsActorManager::HandleRegisterActor(rpc::RegisterActorRequest request,
  ...
  RAY_LOG(INFO).With(actor_id.JobId()).With(actor_id) << "Registering actor";
  Status status =
  ...
  if (!status.ok()) {
    RAY_LOG(WARNING).With(actor_id.JobId()).With(actor_id)
...
void GcsActorManager::HandleCreateActor(rpc::CreateActorRequest request,
  ...
  RAY_LOG(INFO).With(actor_id.JobId()).With(actor_id) << "Creating actor";
  Status status = CreateActor(
      ...
      [reply, send_reply_callback, actor_id](const std::shared_ptr<gcs::GcsActor> &actor,
        ...
        RAY_LOG(INFO).With(actor_id.JobId()).With(actor_id)
...
void GcsActorManager::HandleGetActorInfo(rpc::GetActorInfoRequest request,
  ...
  RAY_LOG(DEBUG).With(actor_id.JobId()).With(actor_id) << "Getting actor info";
  ...
  RAY_LOG(DEBUG).With(actor_id.JobId()).With(actor_id) << "Finished getting actor info";
  GCS_RPC_SEND_REPLY(send_reply_callback, reply, Status::OK());
...
void GcsActorManager::HandleKillActorViaGcs(rpc::KillActorViaGcsRequest request,
  ...
  GCS_RPC_SEND_REPLY(send_reply_callback, reply, Status::OK());
  RAY_LOG(DEBUG).With(actor_id.JobId()).With(actor_id)
...
void GcsActorManager::DestroyActor(const ActorID &actor_id,
                                   ...
                                   bool force_kill) {
  RAY_LOG(INFO).With(actor_id.JobId()).With(actor_id) << "Destroying actor";
  actor_to_register_callbacks_.erase(actor_id);
  auto it = registered_actors_.find(actor_id);
  if (it == registered_actors_.end()) {
    RAY_LOG(INFO).With(actor_id) << "Tried to destroy actor that does not exist";
    return;
  }
...
void GcsActorManager::OnNodeDead(std::shared_ptr<rpc::GcsNodeInfo> node,
  ...
  RAY_LOG(INFO).With(node_id) << "Node failed, reconstructing actors.";
...
void GcsActorManager::OnActorCreationSuccess(const std::shared_ptr<GcsActor> &actor,
  ...
  if (reply.is_application_error()) {
    ...
    RAY_LOG(INFO).With(actor_id.JobId()).With(actor_id) << "Actor created successfully";
  }
...
void GcsActorManager::Initialize(const GcsInitData &gcs_init_data) {
  ...
  for (auto &item : registered_actors_) {
    ...
    if (actor->GetState() == ray::rpc::ActorTableData::PENDING_CREATION ||
      ...
      RAY_LOG(INFO).With(actor->GetActorID().JobId()).With(actor->GetActorID())
