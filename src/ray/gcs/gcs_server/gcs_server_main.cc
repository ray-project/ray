...
int main(int argc, char *argv[]) {
  ...
  if (RayConfig::instance().event_log_reporter_enabled() && !log_dir.empty()) {
    // This GCS server process emits GCS standard events, and Node and Actor
    /// export events so the various source types are passed to RayEventInit.
    // The type of an event is determined by the schema of its event data.
