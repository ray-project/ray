...
grpc::ServerUnaryReactor* GcsDebugService::StartMemoryProfile(
  ...
  if(is_memory_profiling_.exchange(true)) {
    reactor->Finish(grpc::Status(grpc::StatusCode::RESOURCE_EXHAUSTED, "There is a request running profiling."));
    return reactor;
  }
  if(request->duration() <= 0 || request->dump_prefix().size() == 0) {
    reactor->Finish(grpc::Status(grpc::StatusCode::INVALID_ARGUMENT, "duration and dump_prefix must be set."));
    return reactor;
  }
...
//   if(prof_build == false) {
//     reactor->Finish(grpc::Status(grpc::StatusCode::FAILED_PRECONDITION, "Profiling is not enabled."));
//     return reactor;
//   }
...
//   timer->async_wait([this, timer, reactor, old_prefix](const boost::system::error_code&) {
