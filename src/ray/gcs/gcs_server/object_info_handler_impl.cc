...
void DefaultObjectInfoHandler::HandleGetObjectIdsOfNodeByJob(
  ...
  ClientID node_id = ClientID::FromBinary(request.node_id());
  RAY_LOG(DEBUG) << "Getting object ids of node, job id = " << job_id
                 << ", node id = " << node_id;

  auto on_done = [job_id, node_id, reply, send_reply_callback](
                     Status status, const std::vector<ObjectID> &result) {
    if (!status.ok()) {
      RAY_LOG(ERROR) << "Failed to getting object ids of node, job id = " << job_id
                     << ", node id = " << node_id;
    } else {
      for (const ObjectID &object_id : result) {
        reply->add_object_id_list(object_id.Binary());
      }
      RAY_LOG(DEBUG) << "Finished getting object ids of node, job id = " << job_id
                     << ", node id = " << node_id;
    }
  ...
  Status status =
      gcs_client_.Objects().AsyncGetObjectIdsOfNodeByJob(job_id, node_id, on_done);
  if (!status.ok()) {
