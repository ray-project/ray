 ...
 public:
  explicit GcsTable(
      ...
      std::function<void(const rpc::ChannelType channel_type,
                         rpc::PubMessage &pub_message, const std::string &key_id_binary)>
          publish_change)
...
class GcsActorTable : public GcsTableWithJobId<ActorID, ActorTableData> {
 public:
  ...
  Status Put(const ActorID &key, const ActorTableData &value,
             const StatusCallback &callback) override;
...
class GcsTaskTable : public GcsTableWithJobId<TaskID, TaskTableData> {
 public:
  explicit GcsTaskTable(std::shared_ptr<StoreClient> &store_client)
      : GcsTableWithJobId(store_client, nullptr) {
 ...
 public:
  explicit GcsTableStorage(
      ...
      : store_client_(store_client),
        ...
        heartbeat_table_(std::make_unique<GcsHeartbeatTable>(store_client_)),
