...
class GcsPlacementGroupManager : public rpc::PlacementGroupInfoHandler {
 public:
  ...
  /// `registered_placement_groups_` and its state is `ALIVE`. \return Status::Invalid if
  /// this is a named placement_group and an placement_group with the specified name
  /// already exists. The callback will not be called in this case.
  ...
  /// placement_group was not found. \param name The name of the detached placement_group
  /// to look up. \returns PlacementGroupID The ID of the placement_group. Nil if the
