...
void GcsPlacementGroupManager::RegisterPlacementGroup(
  ...
  registered_placement_groups_.emplace(
      placement_group->GetPlacementGroupID(),
      placement_group);
...
void GcsPlacementGroupManager::OnPlacementGroupCreationFailed(
  ...
  io_context_.post(
      [this] { SchedulePendingPlacementGroups(); },
...
void GcsPlacementGroupManager::OnPlacementGroupCreationSuccess(
  ...
  ray::stats::STATS_gcs_placement_group_creation_latency_ms.Record(
      creation_latency_us / 1e3);
...
void GcsPlacementGroupManager::AddToPendingQueue(
  ...
  pg->GetMutableStats()->set_highest_retry_delay_ms(
      absl::Nanoseconds(last_delay) / absl::Milliseconds(1));
  if (!exp_backer) {
...
void GcsPlacementGroupManager::RecordMetrics() const {
  ray::stats::STATS_gcs_placement_group_count.Record(
      pending_placement_groups_.size(),
      "Pending");
  ray::stats::STATS_gcs_placement_group_count.Record(
      registered_placement_groups_.size(),
      "Registered");
