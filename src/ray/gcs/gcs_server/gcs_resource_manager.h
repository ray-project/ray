...
class GcsResourceManager : public rpc::NodeResourceInfoHandler {
 public:
  ...
  explicit GcsResourceManager(std::shared_ptr<gcs::GcsTableStorage> gcs_table_storage,
                              ...
                              std::function<std::shared_ptr<rpc::ResourcesData>()>
                                  get_gcs_node_resources = nullptr);
  ...
  // Update node normal task and available resources.
  void UpdateNodeNormalTaskAndAvailableResources(const NodeID &node_id,
