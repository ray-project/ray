...
#include "ray/gcs/gcs_server/refactor/gcs_resource_scheduler.h"
...
double LeastResourceScorer::Score(const ResourceRequest &required_resources,
  ...
  for (size_t i = 0; i < node_resources.predefined_resources.size(); ++i) {
    ...
    if (i >= required_resources.predefined_resources.size()) {
...
bool GcsResourceScheduler::AllocateRemoteTaskResources(
  ...
  // TODO(Shanly): Implement it after gcs_resources_manager is refactored.
  return false;
}
...
const absl::flat_hash_map<NodeID, Node> &GcsResourceScheduler::GetResourceView() const {
  // TODO(Shanly): Implement it after gcs_resources_manager is refactored.
  static absl::flat_hash_map<NodeID, Node> resource_view;
