...
void GcsResourceManager::HandleGetAllAvailableResources(
  ...
  //       if (resource_iter !=
  //       node_resource_usages_[node_id].resources_available().end()) {
...
void GcsResourceManager::HandleGetAllTotalResources(
  ...
  GCS_RPC_SEND_REPLY(send_reply_callback, reply, Status::OK());
  //   auto local_scheduling_node_id = scheduling::NodeID(local_node_id_.Binary());
  //   for (const auto &node_resources_entry :
  ...
  //   // When gcs detects task pending, we may receive an local update. But it can be
  //   ignored
...
void GcsResourceManager::HandleGetAllResourceUsage(
  ...
  //     auto demand_proto =
  //     batch.mutable_resource_load_by_shape()->add_resource_demands();
