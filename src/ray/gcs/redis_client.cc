...
Status RedisClient::Connect(io_context_proxy &io_service) {
  std::vector<io_context_proxy *> io_services;
  io_services.emplace_back(&io_service);
  return Connect(io_services);
}

Status RedisClient::Connect(std::vector<io_context_proxy *> io_services) {
  ...
  if (!options_.is_test_client_) {
    ...
    for (size_t i = 0; i < addresses.size(); ++i) {
      ...
      io_context_proxy &io_service = *io_services[io_service_index];
...
void RedisClient::Attach() {
  ...
  for (std::shared_ptr<RedisContext> context : shard_contexts_) {
    io_context_proxy &io_service = context->io_service();
  ...
  io_context_proxy &io_service = primary_context_->io_service();
