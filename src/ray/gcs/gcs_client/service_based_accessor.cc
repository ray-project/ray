...
Status ServiceBasedPlacementGroupInfoAccessor::AsyncGetByName(
    const std::string &name, const OptionalItemCallback<rpc::PlacementGroupTableData> &callback) {
  ...
  client_impl_->GetGcsRpcClient().GetNamedPlacementGroup(
      request,
      [name, callback](const Status &status, const rpc::GetNamedPlacementGroupReply &reply) {
        if (reply.has_placement_group_table_data()) {
          callback(status, reply.placement_group_table_data());
        } else {
          callback(status, boost::none);
        }
        RAY_LOG(DEBUG) << "Finished getting named placement group info, status = " << status
                       << ", name = " << name;
      });
  return Status::OK();
}
