...
Status GcsClient::Connect(instrumented_io_context &io_service) {
  ...
  if (options_.server_ip_.empty() && options_.gcs_address_.empty()) {
  ...
  if (get_server_address_func_) {
  ...
  } else if (!options_.gcs_address_.empty()) {
    ...
    get_server_address_func_ = [this](std::pair<std::string, int> *addr) {
      *addr = std::make_pair(options_->gcs_address, options_->gcs_port_);
    };
  } else {
    // Connect to redis.
