...
void NodeInfoGrpcService::InitServerCallFactories(
  ...
  // GetClusterId replies inline on the gRPC polling thread: every client's
  // Connect() blocks on it, and dispatching it through the (single-threaded)
  // node_manager io_context lets mass RegisterNode/GetAllNodeInfo storms delay
  // a constant-value read past client deadlines.
  RPC_SERVICE_HANDLER_CUSTOM_AUTH_INLINE_DISPATCH(NodeInfoGcsService,
                                                  GetClusterId,
