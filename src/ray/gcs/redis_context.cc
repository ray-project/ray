...
void RedisCallbackManager::Add(const RedisCallback &function, bool is_subscription,
                               boost::asio::io_service &io_service,
...
int64_t RedisCallbackManager::IncreaseAndGetCallbackIndex() {
  std::lock_guard<std::mutex> lock(mutex_);
  return num_callbacks_++;
}
...
Status RedisContext::PSubscribeAsync(const std::string &pattern,
  ...
  RedisCallbackManager::instance().Add(redisCallback, true, io_service_, callback_index);
