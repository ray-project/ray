...
void GcsWorkerManager::HandleReportWorkerFailure(
  ...
  GetWorkerInfo(
      ...
      {[this, reply, send_reply_callback, worker_id, request = std::move(request)](
         ...
         sorted_dead_worker_deque_.emplace_back(worker_id,
                                                worker_failure_data->timestamp());
         // If limit enforced, replace one.
         if (max_num_dead_workers_ > 0 &&
             sorted_dead_worker_deque_.size() > max_num_dead_workers_) {
           RAY_LOG_EVERY_MS(WARNING, 10000)
               ...
               << ") allowed is reached. Old worker events will be overwritten. Set "
                  "`RAY_maximum_gcs_dead_worker_cached_count` to a higher value to store "
                  "more.";
           const auto &clean_worker_id = sorted_dead_worker_deque_.front().first;
           gcs_table_storage_.WorkerTable().Delete(
               clean_worker_id,
               Postable<void(Status)>([](const Status &status) { RAY_CHECK_OK(status); },
                                      io_context_));
...
void GcsWorkerManager::Initialize(const GcsInitData &gcs_init_data) {
  ...
  std::sort(
      sorted_dead_worker_deque_.begin(),
      sorted_dead_worker_deque_.end(),
      [](const auto &left, const auto &right) { return left.second < right.second; });

  RAY_LOG(INFO) << "Finished initialize gcs_worker_manager for dead workers size : "
                << sorted_dead_worker_deque_.size();
}
