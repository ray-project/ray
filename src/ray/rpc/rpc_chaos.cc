...
//     export RAY_testing_rpc_failure='{"method1":{"num_failures":3,"req_failure_prob":12,"resp_failure_prob":12,"in_flight_failure_prob":50}}'
//
// You can also use a wildcard to set probabilities for all rpc's and -1 as num_failures
// to have unlimited failures.
//     export RAY_testing_rpc_failure='{"*":{"num_failures":-1,"req_failure_prob":10,"resp_failure_prob":25,"in_flight_failure_prob":50}}'
...
// in-flight failure.
//     export RAY_testing_rpc_failure='{"*":{"num_failures":-1,"req_failure_prob":25,"resp_failure_prob":50,"in_flight_failure_prob":10,"num_lower_bound_req_failures":2,"num_lower_bound_resp_failures":3,"num_lower_bound_in_flight_failures":1}}'
 ...
 public:
  ...
  void Init() {
    ...
    if (!RayConfig::instance().testing_rpc_failure().empty()) {
      auto json =
          nlohmann::json::parse(RayConfig::instance().testing_rpc_failure(), nullptr, false);
