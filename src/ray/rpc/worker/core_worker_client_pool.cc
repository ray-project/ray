...
std::function<void()> CoreWorkerClientPool::GetDefaultUnavailableTimeoutCallback(
  ...
  return [addr,
    ...
    rpc::GcsNodeInfo keep_node_info_ptr_alive;  // To copy out to keep the pointer alive
                                                // in the not subscribed case.
    const rpc::GcsNodeInfo *node_info = nullptr;
    if (gcs_client->Nodes().IsSubscribedToNodeChange()) {
      node_info = gcs_client->Nodes().Get(node_id, /*filter_dead_nodes=*/true);
    } else {
      rpc::GetAllNodeInfoRequest_Filters filters;
      filters.set_node_id(node_id.Binary());
      filters.set_state(rpc::GcsNodeInfo::ALIVE);
      auto nodes = gcs_client->Nodes().GetAllNoCacheWithFilters(-1, std::move(filters));
      if (nodes.ok() && nodes->size() > 0) {
