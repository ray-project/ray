...
class CoreWorkerClientInterface : public pubsub::SubscriberClientInterface {
 public:
  ...
  /// Sets max_finished_seq_no to the given value. This is useful when an actor dies and restarts.
...
class CoreWorkerClient : public std::enable_shared_from_this<CoreWorkerClient>,
 ...
 public:
  ...
  /// Sets `max_finished_seq_no` to the given value. This is useful when an actor dies and restarts.
  /// A new core worker client will be created for the new core worker process. Because the sequence
  /// number is monotonically increasing for tasks from the same actor regardless of which core worker
  /// process they are submitted to, we need to set `max_finished_seq_no` to the correct value for the
  /// new core worker client. This allows the actor to determine whether it needs to wait for tasks
  /// with smaller sequence numbers that may have already been executed in previous core worker
  /// processes.
  void SetMaxFinishedSeqno(int64_t max_finished_seq_no);
