...
class EventAggregatorClient {
 public:
  ...
  virtual void AddEvents(const events::AddEventRequest &request,
                         const ClientCallback<events::AddEventReply> &callback) = 0;
};
...
class EventAggregatorClientImpl : public EventAggregatorClient {
 public:
  ...
  EventAggregatorClientImpl(const std::string &address,
    ...
    RAY_LOG(INFO) << "Initiating the event aggregator client with address: " << address
                  << " port: " << port;
    grpc_client_ = std::make_unique<GrpcClient<events::EventAggregatorService>>(
        address, port, client_call_manager);
  };

  void AddEvents(const events::AddEventRequest &request,
                 const ClientCallback<events::AddEventReply> &callback) override {
    grpc_client_->CallMethod<events::AddEventRequest, events::AddEventReply>(
        &events::EventAggregatorService::Stub::PrepareAsyncAddEvents,
        request,
        callback,
 ...
 private:
  // The RPC client.
  std::unique_ptr<GrpcClient<events::EventAggregatorService>> grpc_client_;
};
