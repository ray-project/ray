...
/// sent to the client.
using SendReplyCallback = std::function<
    void(Status status, std::function<void()> success, std::function<void()> failure)>;
...
using HandleRequestFunction =
    void (ServiceHandler::*)(const Request &, Reply *, SendReplyCallback);
 ...
 private:
  ...
  void LogProcessTime() {
    ...
    ray::stats::STATS_grpc_server_req_process_time_ms.Record(
        (end_time - start_time_) / 1000000.0,
        call_name_);
  }
