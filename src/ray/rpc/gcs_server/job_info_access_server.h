...
class JobInfoAccessHandler {
 public:
  ...
  virtual void HandleAddJob(const GcsJobInfo &request, AddJobReply *reply,
                            SendReplyCallback send_reply_callback) = 0;

  virtual void HandleMarkJobFinished(const FinishedJob &request,
 ...
 protected:
  ...
  void InitServerCallFactories(
    ...
    std::unique_ptr<ServerCallFactory> add_job_call_factory(
        new ServerCallFactoryImpl<JobInfoAccessService, JobInfoAccessHandler, GcsJobInfo,
                                  AddJobReply>(
    ...
    std::unique_ptr<ServerCallFactory> mark_job_finished_call_factory(
        new ServerCallFactoryImpl<JobInfoAccessService, JobInfoAccessHandler, FinishedJob,
                                  MarkJobFinishedReply>(
