...
bazel build --config=asan "//:object_manager_stress_test" "//:object_manager_test" "@plasma//:plasma_store_server"
...
bazel run --config=asan //:redis-cli -- -p 6379 shutdown || true
sleep 1s
bazel run --config=asan //:redis-server -- --loglevel warning ${LOAD_MODULE_ARGS} --port 6379 &
sleep 1s
# Run tests.
...
bazel run --config=asan //:redis-cli -- -p 6379 shutdown
sleep 1s
