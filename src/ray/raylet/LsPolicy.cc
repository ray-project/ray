...
std::unordered_map<TaskID, DBClientID, UniqueIDHasher> LsPolicy::Schedule(
  ...
  std::unordered_map<TaskID, DBClientID, UniqueIDHasher> decision;
  ...
  for (const auto &t : ready_tasks) {
    ...
    LsResources resource_supply = cluster_resources.at(DBClientID::nil());
    ...
    if (task_feasible) {
      ...
      decision[last_task_id] = DBClientID::nil();
    }
  }
  return decision;
}
