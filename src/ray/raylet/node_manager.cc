...
std::shared_ptr<AgentManager> NodeManager::CreateDashboardAgentManager(
  ...
  auto options = AgentManager::Options({self_node_id,
                                        ...
                                        agent_command_line,
                                        /*fate_shares=*/true});
  return std::make_shared<AgentManager>(
      std::move(options),
      /*delay_executor=*/
      [this](std::function<void()> task, uint32_t delay_ms) {
        return execute_after(io_service_, task, std::chrono::milliseconds(delay_ms));
...
std::shared_ptr<AgentManager> NodeManager::CreateRuntimeEnvAgentManager(
    const NodeID &self_node_id, const NodeManagerConfig &config) {
