...
// The budget comfortably exceeds the default actor_graceful_shutdown_timeout_ms
// (30s, after which the GCS force-kills the worker), so the worker is normally
// dead well within it. If the worker is still alive when the budget runs out we
// just give up: nothing is orphaned while the group leader is still alive.
...
// worker exits or `polls_remaining` reaches zero, so per-worker process-group
// cleanup does not interrupt the worker's own shutdown sequence (atexit /
// __ray_shutdown__ handlers).
void SchedulePostExitProcessGroupCleanup(
  ...
  // Current implementation: the worker is still running its own shutdown and
  // nothing is orphaned while the group leader is alive, so once the fixed
  // polling budget is exhausted we just give up and skip the sweep rather than
  // risk interrupting (or prematurely killing) a worker still within its
  // graceful-shutdown window.
  //
  // TODO(kevin85421): Derive `polls_remaining` from
  ...
  // the worker is already dead before we give up, letting the post-exit sweep
  // above reap any orphaned descendants instead of leaking them (except when the
  // timeout is -1, where the user has opted out of force-kill). We must not
  // SIGKILL the group here while the worker is still alive: that would prematurely
  // kill a worker still within its graceful-shutdown window.
  ...
  timer->async_wait([timer, worker_pid, pgid, wid, polls_remaining](
