...
inline ray::rpc::ObjectReference FlatbufferToSingleObjectReference(
    const flatbuffers::String &object_id,
    const ray::protocol::Address &address) {
...
NodeManager::NodeManager(
    ...
    : self_node_id_(self_node_id),
      ...
      object_manager_(
          ...
          [this](const ObjectID &object_id) {
            ...
            MarkObjectsAsFailed(
                rpc::ErrorType::OBJECT_FETCH_TIMED_OUT,
                {ref},
                JobID::Nil());
          }),
  ...
  RAY_CHECK(
      RayConfig::instance().max_task_args_memory_fraction() > 0 &&
  ...
  auto is_owner_alive =
      [this](const WorkerID &owner_worker_id, const NodeID &owner_node_id) {
        return !(
            failed_workers_cache_.count(owner_worker_id) > 0 ||
  ...
  placement_group_resource_manager_ = std::make_shared<NewPlacementGroupResourceManager>(
      ...
      [this](const std::vector<std::string> &resource_names) {
        RAY_CHECK_OK(gcs_client_->NodeResources()
                         .AsyncDeleteResources(self_node_id_, resource_names, nullptr));
      });
...
ray::Status NodeManager::RegisterGcs() {
  ...
  const auto job_subscribe_handler =
      [this](const JobID &job_id, const JobTableData &job_data) {
...
void NodeManager::FillNormalTaskResourceUsage(rpc::ResourcesData &resources_data) {
  ...
  if (!last_heartbeat_resources->GetNormalTaskResources().IsEqual(
    ...
    normal_task_map = {
        normal_task_resources.GetResourceMap().begin(),
...
void NodeManager::DoLocalGC() {
  ...
  for (const auto &worker : all_workers) {
    ...
    worker->rpc_client()->LocalGC(
        request,
        [](const ray::Status &status, const rpc::LocalGCReply &r) {
          if (!status.ok()) {
...
void NodeManager::HandleRequestObjectSpillage(
  ...
  local_object_manager_.SpillObjects(
      {object_id},
      [object_id, reply, send_reply_callback](const ray::Status &status) {
        if (status.ok()) {
...
void NodeManager::HandleReleaseUnusedBundles(
  ...
  for (int index = 0; index < request.bundles_in_use_size(); ++index) {
    ...
    in_use_bundles.emplace(std::make_pair(
        PlacementGroupID::FromBinary(bundle_id.placement_group_id()),
...
void NodeManager::ResourceDeleted(
    const NodeID &node_id,
...
void NodeManager::UpdateResourceUsage(
    const NodeID &node_id,
...
void NodeManager::ProcessRegisterClientRequestMessage(
  ...
  if (worker_type == rpc::WorkerType::WORKER ||
    ...
    auto cb = [this,
               ...
               job_id,
               job_config,
               send_reply_callback = std::move(
                   send_reply_callback)](const Status &status, int assigned_port) {
      if (status.ok()) {
...
void NodeManager::ProcessDisconnectClientMessage(
  ...
  if (exception_pb != nullptr) {
    ...
    creation_task_exception->ParseFromString(std::string(
        reinterpret_cast<const char *>(exception_pb->data()),
        exception_pb->size()));
  }
  ...
  wait_manager_.Wait(
      ...
      [this, resolve_objects, was_blocked, client, current_task_id](
        ...
        flatbuffers::Offset<protocol::WaitReply> wait_reply = protocol::CreateWaitReply(
            fbb,
            to_flatbuf(fbb, ready),
            to_flatbuf(fbb, remaining));
...
void NodeManager::HandleReturnWorker(
...
void NodeManager::HandleShutdownRaylet(
...
void NodeManager::HandleCancelWorkerLease(
...
void NodeManager::AsyncResolveObjects(
  ...
  if (ray_get) {
    ...
    dependency_manager_.StartOrUpdateWaitRequest(
        worker->WorkerId(),
...
void NodeManager::HandleObjectLocal(const ObjectInfo &object_info) {
  ...
  for (auto worker : waiting_workers) {
    worker->rpc_client()->PlasmaObjectReady(
        request,
        [](Status status, const rpc::PlasmaObjectReadyReply &reply) {
          if (!status.ok()) {
...
void NodeManager::DumpDebugState() const {
  std::fstream fs;
  fs.open(
      initial_config_.log_dir + "/debug_state.txt",
...
std::string compact_tag_string(
    const opencensus::stats::ViewDescriptor &view,
...
void NodeManager::HandlePinObjectIDs(
...
void NodeManager::HandleGetSystemConfig(
...
void NodeManager::HandleGetNodeStats(
    const rpc::GetNodeStatsRequest &node_stats_request,
...
rpc::ObjectStoreStats AccumulateStoreStats(
  ...
  for (const auto &reply : node_stats) {
    ...
    store_stats.set_spilled_bytes_total(
        store_stats.spilled_bytes_total() + cur_store.spilled_bytes_total());
    store_stats.set_spilled_objects_total(
        store_stats.spilled_objects_total() + cur_store.spilled_objects_total());
    store_stats.set_restored_bytes_total(
        store_stats.restored_bytes_total() + cur_store.restored_bytes_total());
    store_stats.set_restored_objects_total(
        store_stats.restored_objects_total() + cur_store.restored_objects_total());
    store_stats.set_object_store_bytes_used(
        store_stats.object_store_bytes_used() + cur_store.object_store_bytes_used());
    store_stats.set_object_store_bytes_avail(
        store_stats.object_store_bytes_avail() + cur_store.object_store_bytes_avail());
    ...
    store_stats.set_num_local_objects(
        store_stats.num_local_objects() + cur_store.num_local_objects());
    store_stats.set_consumed_bytes(
        store_stats.consumed_bytes() + cur_store.consumed_bytes());
...
void NodeManager::HandleFormatGlobalMemoryInfo(
  ...
  for (const auto &entry : remote_node_manager_addresses_) {
    ...
    client->GetNodeStats(
        stats_req,
        [replies,
         store_reply](const ray::Status &status, const rpc::GetNodeStatsReply &r) {
          if (!status.ok()) {
...
void NodeManager::HandleGlobalGC(
