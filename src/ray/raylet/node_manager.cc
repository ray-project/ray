    ...
    auto cb = [this, worker_ip_address, pid, job_id, job_config,
      ...
      if (status.ok()) {
        auto job_data_ptr = gcs::CreateJobTableData(job_id,
                                                    /*is_dead*/ false, worker_ip_address,
                                                    pid, job_config);
...
void NodeManager::ProcessFetchOrReconstructMessage(
  ...
  if (message->fetch_only()) {
    ...
    AsyncResolveObjects(client, refs, task_id,
                        /*ray_get=*/true,
...
void NodeManager::ProcessWaitRequestMessage(
  ...
  if (resolve_objects) {
    ...
    // value.
    AsyncResolveObjects(client, refs, current_task_id,
                        /*ray_get=*/false,
...
void NodeManager::ProcessWaitForDirectActorCallArgsRequestMessage(
  ...
  const auto refs =
      FlatbufferToObjectReference(*message->object_ids(), *message->owner_addresses());
  AsyncResolveObjects(client, refs, TaskID::Nil(),
                      /*ray_get=*/false,
...
void NodeManager::HandleReportWorkerBacklog(
  ...
  for (const auto &backlog_report : request.backlog_reports()) {
    ...
    local_task_manager_->SetWorkerBacklog(scheduling_class, worker_id,
