...
TEST(RuntimeEnvAgentClientTest, GetOrCreateRuntimeEnvWithPgFormattedResources) {
  RayConfig::instance().initialize(R"({"AUTH_MODE": "disabled"})");
  ray::UnsetEnv("RAY_AUTH_TOKEN");
  rpc::AuthenticationTokenLoader::instance().ResetCache();
  ...
  ResourceSet resource_requirements(
      absl::flat_hash_map<std::string, double>{{"CPU_group_0_xxx", 2.0}});

  auto pg_gpu_id = ResourceID("GPU_group_1_xxx");
  auto allocated_instances = std::make_shared<TaskResourceInstances>(
      absl::flat_hash_map<ResourceID, std::vector<FixedPoint>>{{pg_gpu_id, {0.0, 1.0}}});
