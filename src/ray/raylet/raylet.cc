...
GenerateEnumNames(const char *const *enum_names_ptr, int start_index, int end_index) {
...
static const std::vector<std::string> node_manager_message_enum = GenerateEnumNames(
...
Raylet::Raylet(
  ...
  self_node_info_.mutable_resources_total()->insert(
      resource_map.begin(),
      resource_map.end());
}
...
ray::Status Raylet::RegisterGcs() {
  auto register_callback = [this](const Status &status) {
    ...
    RAY_CHECK_OK(gcs_client_->NodeResources()
                     .AsyncUpdateResources(self_node_id_, resources, nullptr));
