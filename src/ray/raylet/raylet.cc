...
Raylet::Raylet(instrumented_io_context &main_service,
  ...
  // TODO(rickyx): We could also probably add this to a KV map when we support node
  auto instance_id = RayConfig::instance().cloud_instance_id();
  self_node_info_.set_instance_id(instance_id);
}
