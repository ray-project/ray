...
class MockGcsClient : public gcs::RedisGcsClient {
 public:
  MockGcsClient(const gcs::GcsClientOptions &options, const ClientID &node_id)
    ...
    task_accessor_.reset(new gcs::MockTaskInfoAccessor());
    node_accessor_.reset(new gcs::MockNodeInfoAccessor(node_id));
  }

  gcs::raylet::TaskTable &raylet_task_table() override { return *task_table_fake_; }

  gcs::MockTaskInfoAccessor &MockTasks() {
    return *dynamic_cast<gcs::MockTaskInfoAccessor *>(task_accessor_.get());
  }
