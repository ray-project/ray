...
void ClusterResourceManager::ReleaseResources(scheduling::NodeID node_id,
  ...
  for (const auto &[resource_id, value] : resources.ToResourceMap()) {
    ...
    auto new_value = value_fp + local_view->total.Get(ResourceID(resource_id));
    local_view->total.Set(ResourceID(resource_id), new_value);
    new_value = value_fp + local_view->available.Get(ResourceID(resource_id));
