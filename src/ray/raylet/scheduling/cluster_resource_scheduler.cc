...
void ClusterResourceScheduler::UpdateLocalResourceCapacity(
  ...
  if (idx != -1) {
    ...
    InitResourceInstances(resource_total_fp, is_unit_instance,
                          &local_resources_.predefined_resources[idx]);
  } else {
    string_to_int_map_.Insert(resource_name);
    int64_t resource_id = string_to_int_map_.Get(resource_name);
