...
std::promise<bool> global_promise;
...
class MockWorkerPool : public WorkerPoolInterface {
 public:
  ...
  void PopWorker(const TaskSpecification &task_spec, const PopWorkerCallback &callback,
    ...
    if (worker) {
      io_service.post([callback, worker]() {
        callback(worker, Status::OK());
        global_promise.set_value(true);
      });
    } else {
  ...
  void PushWorker(const std::shared_ptr<WorkerInterface> &worker) {
    ...
    auto it = callbacks.find(runtime_env_hash);
    if (it != callbacks.end()) {
      auto &list = it->second;
      RAY_CHECK(!list.empty());
      auto &callback = list.front();
      io_service.post([callback, worker]() {
        callback(worker, Status::OK());
        global_promise.set_value(true);
      });
...
TEST_F(ClusterTaskManagerTest, BasicTest) {
  ...
  io_service_.post([&task, &reply, &callback, this]() {
    task_manager_.QueueAndScheduleTask(task, &reply, callback);
  });
  global_promise.get_future().get();
  global_promise = std::promise<bool>();
...
TEST_F(ClusterTaskManagerTest, DispatchQueueNonBlockingTest) {
  ...
  io_service_.post([&task_B_1, &reply_B_1, &empty_callback, this]() {
    task_manager_.QueueAndScheduleTask(task_B_1, &reply_B_1, empty_callback);
  });
  global_promise.get_future().get();
  global_promise = std::promise<bool>();

  io_service_.post([&task_A, &reply_A, &callback, this]() {
    task_manager_.QueueAndScheduleTask(task_A, &reply_A, callback);
  });
  global_promise.get_future().get();
  global_promise = std::promise<bool>();

  io_service_.post([&task_B_2, &reply_B_2, &empty_callback, this]() {
    task_manager_.QueueAndScheduleTask(task_B_2, &reply_B_2, empty_callback);
  });
  // No enough resources, so can not call pop worker.
  // global_promise.get_future().get();
  // global_promise = std::promise<bool>();

  // Push a worker that can only run task A.
  io_service_.post([this, &serialized_runtime_env_A]() {
...
//   {
//     Task task = CreateTask({{ray::kCPU_ResourceLabel, 9}, {ray::kGPU_ResourceLabel,
//     5}}); rpc::RequestWorkerLeaseReply reply;
...
//   {
//     Task task = CreateTask({{ray::kCPU_ResourceLabel, 10}, {ray::kGPU_ResourceLabel,
//     1}}); rpc::RequestWorkerLeaseReply reply;
...
//   // Infeasible warning shouldn't be reprinted when the previous task is still
//   infeasible
...
//   ASSERT_EQ(node_resources.predefined_resources[PredefinedResources::CPU].available,
//   8);
//   ASSERT_EQ(node_resources.predefined_resources[PredefinedResources::GPU].available,
//   4);
...
//   ASSERT_EQ(replies[2]->retry_at_raylet_address().raylet_id(),
//   remote_node_id.Binary());
//   ASSERT_EQ(replies[3]->retry_at_raylet_address().raylet_id(),
//   remote_node_id.Binary());
