...
TEST_F(ClusterResourceSchedulerTest, SchedulingUpdateTotalResourcesTest) {
  absl::flat_hash_map<std::string, double> initial_resources = {{ray::kCPU_ResourceLabel, 1}, {"custom", 1}};
  ClusterResourceScheduler resource_scheduler(NodeID::FromRandom().Binary(), initial_resources, *gcs_client_, nullptr, nullptr);
  ...
  const auto &predefined_resources = resource_scheduler.GetLocalNodeResources().predefined_resources;
  ASSERT_EQ(predefined_resources[CPU].total.Double(), 3);

  const auto &custom_resources = resource_scheduler.GetLocalNodeResources().custom_resources;
