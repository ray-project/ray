...
void ClusterLeaseManager::QueueAndScheduleLease(
  ...
  if (internal::IsResourceViewFanoutRestricted() &&
  ...
  if (infeasible_leases_iter != infeasible_leases_.end()) {
    if (work->grant_or_reject_ && internal::IsResourceViewFanoutRestricted()) {
...
void ClusterLeaseManager::ScheduleAndGrantLeases() {
  ...
  ScheduleAndGrantPendingLeases();
}

void ClusterLeaseManager::ScheduleAndGrantPendingLeases() {
    ...
    for (auto work_it = work_queue.begin(); work_it != work_queue.end();) {
      ...
      if (scheduling_node_id.IsNil()) {
        ...
        if (!is_infeasible && work->grant_or_reject_ &&
            internal::IsResourceViewFanoutRestricted()) {
