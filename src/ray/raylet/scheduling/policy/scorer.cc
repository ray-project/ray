...
double LeastResourceScorer::Score(const ResourceRequest &required_resources,
  ...
  new_node_resources.total = node_resources.total.ToNodeResourceSet();
  new_node_resources.available = node_resources.available.ToNodeResourceSet();
  ...
  for (auto &resource_id : required_resources.ResourceIds()) {
    ...
    const auto &node_available_resource = new_node_resources.available.Get(resource_id);
