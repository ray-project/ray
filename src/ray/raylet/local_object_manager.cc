...
void LocalObjectManager::RecordMetrics() const {
  ...
  if (spilled_bytes_total_ != 0 && spill_time_total_s_ != 0) {
    spill_manager_throughput_mb_gauge_.Record(
  ...
  spill_manager_objects_gauge_.Record(pinned_objects_.size(), {{"State", "Pinned"}});
  spill_manager_objects_gauge_.Record(objects_pending_restore_.size(),
                                      {{"State", "PendingRestore"}});
  spill_manager_objects_gauge_.Record(objects_pending_spill_.size(),
                                      {{"State", "PendingSpill"}});

  spill_manager_objects_bytes_gauge_.Record(pinned_objects_size_, {{"State", "Pinned"}});
  spill_manager_objects_bytes_gauge_.Record(num_bytes_pending_spill_,
                                            {{"State", "PendingSpill"}});
  spill_manager_objects_bytes_gauge_.Record(num_bytes_pending_restore_,
                                            {{"State", "PendingRestore"}});
  spill_manager_objects_bytes_gauge_.Record(spilled_bytes_total_, {{"State", "Spilled"}});
  spill_manager_objects_bytes_gauge_.Record(restored_objects_total_,
                                            {{"State", "Restored"}});

  spill_manager_request_total_gauge_.Record(spilled_objects_total_,
                                            {{"Type", "Spilled"}});
  spill_manager_request_total_gauge_.Record(restored_objects_total_,
                                            {{"Type", "Restored"}});

  object_store_memory_gauge_.Record(spilled_bytes_current_,
                                    {{stats::LocationKey, "SPILLED"}});

  spill_manager_request_total_gauge_.Record(num_failed_deletion_requests_,
                                            {{"Type", "FailedDeletion"}});
}
