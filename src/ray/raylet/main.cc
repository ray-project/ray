...
DEFINE_string(rust_worker_command, "", "Rust worker command.");
...
int main(int argc, char *argv[]) {
  ...
  const std::string rust_worker_command = FLAGS_rust_worker_command;
  ...
  RAY_CHECK_OK(gcs_client->Nodes().AsyncGetInternalConfig(
      [&](::ray::Status status,
        ...
        if (!rust_worker_command.empty()) {
          node_manager_config.worker_commands.emplace(
              make_pair(ray::Language::RUST, ParseCommandLine(rust_worker_command)));
        }
