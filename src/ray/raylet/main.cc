...
DEFINE_string(runtime_env_agent_port_pipe_handles,
              "",
              "Comma-separated list of additional pipe handles to report runtime env "
...
int main(int argc, char *argv[]) {
  ...
  const std::string runtime_env_agent_port_pipe_handles_str =
      FLAGS_runtime_env_agent_port_pipe_handles;
  ...
  gcs_client->InternalKV().AsyncGetInternalConfig([&](::ray::Status status,
    ...
    // Parse additional pipe handles for reporting runtime env agent port.
    // These handles came from Python via pass_fds and are inheritable.
    // We immediately set close-on-exec to prevent dashboard_agent from inheriting them.
    // Later, when spawning runtime_env_agent, we'll clear close-on-exec for these fds.
    if (!runtime_env_agent_port_pipe_handles_str.empty()) {
      std::stringstream ss(runtime_env_agent_port_pipe_handles_str);
      std::string handle_str;
      while (std::getline(ss, handle_str, ',')) {
        if (!handle_str.empty()) {
          intptr_t handle = std::stoll(handle_str);
          // Set close-on-exec immediately to prevent other subprocesses from inheriting.
          ray::pipe_internal::SetFdCloseOnExec(static_cast<int>(handle));
          node_manager_config.runtime_env_agent_port_pipe_handles.push_back(handle);
        }
      }
    }
