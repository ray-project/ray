...
const std::list<Task> ReadyQueue::GetTasks(const ResourceIdSet* resources) const {
...
std::list<Task> SchedulingQueue::GetReadyTasks(const ResourceIdSet* resources) const {
  return ready_tasks_.GetTasks(resources);
}
