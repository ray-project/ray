...
WorkerUnfitForTaskReason WorkerPool::WorkerFitsForTask(
  ...
  if ((pop_worker_request.is_gpu.has_value()) && worker.GetIsGpu().has_value() &&
      (worker.GetIsGpu().value() != pop_worker_request.is_gpu.value())) {
    return WorkerUnfitForTaskReason::RUNTIME_ENV_MISMATCH;
  }
  if ((pop_worker_request.is_actor_worker.has_value()) &&
      worker.GetIsActorWorker().has_value() &&
      (worker.GetIsActorWorker().value() != pop_worker_request.is_actor_worker.value())) {
    return WorkerUnfitForTaskReason::RUNTIME_ENV_MISMATCH;
  }
...
void WorkerPool::PopWorkerInner(
  ...
  auto start_worker_process_fn = [this](
    ...
    if (status == PopWorkerStatus::OK) {
      ...
      WarnAboutSize();
      state.pending_registration_requests.emplace_back(std::move(pop_worker_request));
...
void WorkerPool::PopWorker(const TaskSpecification &task_spec,
  ...
  if (worker == nullptr) {
    ...
    PopWorkerCallbackAsync(task_callback, worker, PopWorkerStatus::OK);
  }
}
