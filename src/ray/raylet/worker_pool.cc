...
std::string WorkerPool::DebugString() const {
  ...
  for (const auto &entry : states_by_lang_) {
    ...
    result << "\n- num object spill callbacks queued: " << entry.second.spill_io_worker_state.pending_io_tasks.size();
    result << "\n- num object restore queued: " << entry.second.restore_io_worker_state.pending_io_tasks.size();
  }
