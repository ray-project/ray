...
void WorkerPool::SetAgentManager(
    std::shared_ptr<RuntimeEnvAgentManager> runtime_env_agent_manager) {
  runtime_env_agent_manager_ = runtime_env_agent_manager;
}
...
void WorkerPool::GetOrCreateRuntimeEnv(
  ...
  runtime_env_agent_manager_->GetOrCreateRuntimeEnv(
      job_id,
      serialized_runtime_env,
      runtime_env_config,
...
void WorkerPool::DeleteRuntimeEnvIfPossible(const std::string &serialized_runtime_env) {
  ...
  if (!IsRuntimeEnvEmpty(serialized_runtime_env)) {
    runtime_env_agent_manager_->DeleteRuntimeEnvIfPossible(
