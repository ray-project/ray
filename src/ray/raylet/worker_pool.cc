...
void WorkerPool::TryToKillWorker(std::shared_ptr<WorkerInterface> worker) {
  ...
  for (auto worker_it = workers_in_the_same_process.begin();
    ...
    RAY_LOG(INFO) << "The worker pool has " << registered_size
    ...
    RemoveWorker(state.idle, *worker_it);
    state.killed_workers.insert(*worker_it);
  }
...
bool WorkerPool::DisconnectWorker(const std::shared_ptr<WorkerInterface> &worker) {
  ...
  RemoveWorker(state.killed_workers, worker);
