...
class LeaseDependencyManager : public LeaseDependencyManagerInterface {
 ...
 private:
  ...
  /// A struct to represent the object dependencies of a lease.
  struct LeaseDependencies {
    LeaseDependencies(const absl::flat_hash_set<ObjectID> &deps,
                      ...
                      const TaskMetricsKey &lease_key)
        : dependencies(std::move(deps)),
          num_missing_dependencies(dependencies.size()),
          waiting_lease_counter_map(counter_map),
          lease_key(lease_key) {
      if (num_missing_dependencies > 0) {
        waiting_lease_counter_map.Increment(lease_key);
      }
    }
    ...
    /// must be zero before the lease is ready to execute.
    ...
    CounterMap<std::pair<std::string, bool>> &waiting_lease_counter_map;
    ...
    void DecrementMissingDependencies() {
      num_missing_dependencies--;
      if (num_missing_dependencies == 0) {
        waiting_lease_counter_map.Decrement(lease_key);
      }
    }
