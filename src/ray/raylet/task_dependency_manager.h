...
/// waiting or ready to execute locally. The manager determines which object
/// dependencies are both required by a local task and being created by a
/// remote node.
class TaskDependencyManager {
 public:
  ...
  /// are neither local nor going to be created by a queued task.
  ...
  /// remote object handler for any missing dependencies that need to be
  /// transferred from another node or reconstructed. The registered task ready
  /// handler will be called when the task's dependencies become locally
  /// available. The registered task waiting handler will be called when one or
  /// more of the task's dependencies is not locally available.
  ///
  /// \param task The locally queued task.
  void SubscribeTask(const Task &task);

  // TODO(swang): Call Unsubscribe from the node manager.
  ...
  void UnsubscribeExecutedTask(const TaskID &task_id);
 ...
 private:
  ...
  enum class ObjectAvailability : unsigned int {
    ...
    /// task.
    kRemote = 0,
    /// The task that creates this object is waiting to be executed.
    kWaiting,
    /// The task that creates this object finished execution and created its
    /// outputs. The object will be marked as local once the notification that
    /// it is local is received.
    kCreated,
    /// The object is local. The object will be marked as remote once the
  ...
  struct ObjectEntry {
    ...
    /// The object's availability on the local node, according to the queued
    /// tasks and the objects currently in the store.
