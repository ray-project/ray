...
void NormalTaskSubmitter::AddWorkerLeaseClient(
  ...
  LeaseEntry new_lease_entry{
      std::move(lease_client), expiration, assigned_resources, scheduling_key, lease_id};
        ...
        rpc::ErrorType error_type = rpc::ErrorType::WORKER_DIED;
        {
          ...
          if (status.ok()) {
            if (reply.canceled()) {
              ...
              RAY_LOG(DEBUG) << "Lease granted " << lease_id << " from raylet "
       ...
       assigned_resources](Status status, const rpc::PushTaskReply &reply) {
        bool resubmit_generator = false;
        {
          ...
          if (!status.ok()) {
            ...
            cur_lease_entry.lease_client->GetTaskFailureCause(cur_lease_entry.lease_id,
                                                              callback);
          }
          OnWorkerIdle(addr,
                       scheduling_key,
