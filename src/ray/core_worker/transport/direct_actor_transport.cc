...
void CoreWorkerDirectActorTaskSubmitter::ConnectActor(const ActorID &actor_id,
  ...
  if (num_restarts < queue->second.num_restarts) {
    ...
    RAY_LOG(INFO) << "Skip actor creation that has already been restarted, actor_id=" << actor_id;
    return;
  }
...
void CoreWorkerDirectActorTaskSubmitter::DisconnectActor(
  ...
  if (dead) {
    ...
    RAY_LOG(INFO) << "Failing pending tasks for actor " << actor_id << " because the actor is already dead.";
