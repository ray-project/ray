...
void CoreWorkerDirectTaskReceiver::HandleTask(
  ...
  auto accept_callback = [this, reply, task_spec,
    ...
    if (RayConfig::instance().ownership_transfer_enabled()) {
      ...
      transfer_handler_(
          caller_addr, return_contained_ids,
          [status, send_reply_callback, objects_valid, reply, num_returns](
              google::protobuf::RepeatedPtrField<rpc::SharedObjectInfo> shared_objs) {
            reply->mutable_shared_obj_info()->Swap(&shared_objs);
