...
void CoreWorkerDirectActorTaskSubmitter::TreatTaskAsFailed(
  ...
  for (int i = 0; i < num_returns; i++) {
    const auto object_id = ObjectID::ForTaskReturn(task_id, /*index=*/i + 1, /*transport_type=*/0);
...
void CoreWorkerDirectActorTaskReceiver::HandlePushTask(
  ...
  for (int i = 0; i < results.size(); i++) {
    ...
    ObjectID id = ObjectID::ForTaskReturn(task_spec.TaskId(), /*index=*/i + 1, /*transport_type=*/0);
