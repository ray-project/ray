      ...
      if (keep_executing) {
        ...
        if (!scheduling_key_entry.AllWorkersBusy()) {
          ...
          for (auto active_worker_addr : scheduling_key_entry.active_workers) {
            RAY_CHECK(
                worker_to_lease_entry_.find(active_worker_addr) !=
...
void CoreWorkerDirectTaskSubmitter::ReturnWorker(
    const rpc::WorkerAddress addr,
    bool was_error,
    bool worker_exiting,
    const SchedulingKey &scheduling_key) {
  ...
  auto status = lease_entry.lease_client
                    ->ReturnWorker(addr.port, addr.worker_id, was_error, worker_exiting);
  if (!status.ok()) {
...
void CoreWorkerDirectTaskSubmitter::CancelWorkerLeaseIfNeeded(
  ...
  for (auto &pending_lease_request : scheduling_key_entry.pending_lease_requests) {
    ...
    lease_client->CancelWorkerLease(
        task_id,
        [this,
         scheduling_key](const Status &status, const rpc::CancelWorkerLeaseReply &reply) {
          absl::MutexLock lock(&mu_);
  ...
  if (NodeID::FromBinary(raylet_address->raylet_id()) != local_raylet_id_) {
    ...
    if (it == remote_lease_clients_.end()) {
      ...
      it = remote_lease_clients_
               .emplace(
                   raylet_id,
                   lease_client_factory_(
                       raylet_address->ip_address(),
                       raylet_address->port()))
               .first;
    }
        ...
        if (status.ok()) {
          ...
          if (status.IsGrpcUnavailable()) {
            ...
            while (!task_queue.empty()) {
              ...
              RAY_UNUSED(task_finisher_->FailPendingTask(
                  task_spec.TaskId(),
                  rpc::ErrorType::LOCAL_RAYLET_DIED,
                  &status));
              task_queue.pop_front();
            }
            if (scheduling_key_entry.CanDelete()) {
...
Status CoreWorkerDirectTaskSubmitter::CancelTask(
        ...
        if (status.ok() && !reply.attempt_succeeded()) {
          if (cancel_retry_timer_.has_value()) {
            ...
            cancel_retry_timer_->async_wait(boost::bind(
                &CoreWorkerDirectTaskSubmitter::CancelTask,
                this,
                task_spec,
                force_kill,
                recursive));
...
Status CoreWorkerDirectTaskSubmitter::CancelRemoteTask(
