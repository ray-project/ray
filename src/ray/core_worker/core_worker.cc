...
void CoreWorker::WaitForShutdown() {
  ...
  if (options_.worker_type == WorkerType::WORKER) {
    ...
    // Asyncio coroutines could still run after CoreWorker is removed because it is running
    // in a different thread. This can cause segfault because coroutines try to access 
    // CoreWorker methods that are already garbage collected. We should complete all
    // coroutines before shutting down in order to prevent this.
