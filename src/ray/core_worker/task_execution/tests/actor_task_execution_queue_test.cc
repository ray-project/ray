...
TEST(OrderedActorTaskExecutionQueueTest, TestTaskEvents) {
  ...
  int n_executed = 0;
  int n_canceled = 0;
  auto execute_task = [&n_executed](TaskExecutionMetadata &task) { n_executed++; };
  auto cancel_task = [&n_canceled](const TaskExecutionMetadata &task,
                                   const Status &status) { n_canceled++; };
  ...
  queue.EnqueueTask(0, -1, MakeTaskExecutionMetadata(task_spec_without_dependency));
  ...
  queue.EnqueueTask(1, -1, MakeTaskExecutionMetadata(task_spec_with_dependency));
  waiter.Complete(0);
...
TEST(OrderedActorTaskExecutionQueueTest, TestInOrder) {
  ...
  queue.EnqueueTask(0, -1, MakeTaskExecutionMetadata(task_spec));
  queue.EnqueueTask(1, -1, MakeTaskExecutionMetadata(task_spec));
  queue.EnqueueTask(2, -1, MakeTaskExecutionMetadata(task_spec));
  queue.EnqueueTask(3, -1, MakeTaskExecutionMetadata(task_spec));
  io_service.run();
...
TEST(OrderedActorTaskExecutionQueueTest, ShutdownCancelsQueuedAndWaitsForRunning) {
  ...
  auto execute_task_blocking = [&running_started,
                                &allow_finish](TaskExecutionMetadata &task) {
    running_started.set_value();
    allow_finish.get_future().wait();
  };
  auto cancel_task_count = [&n_rejected](const TaskExecutionMetadata &task,
                                         const Status &status) {
  ...
  queue.EnqueueTask(0, -1, MakeTaskExecutionMetadata(ts));
  ...
  queue.EnqueueTask(1, -1, MakeTaskExecutionMetadata(ts_dep));
  io_service.poll();
...
TEST(OrderedActorTaskExecutionQueueTest, TestWaitForObjects) {
  ...
  queue.EnqueueTask(0, -1, MakeTaskExecutionMetadata(task_spec_without_dependency));
  queue.EnqueueTask(1, -1, MakeTaskExecutionMetadata(task_spec_with_dependency));
  queue.EnqueueTask(2, -1, MakeTaskExecutionMetadata(task_spec_with_dependency));
  queue.EnqueueTask(3, -1, MakeTaskExecutionMetadata(task_spec_with_dependency));
...
TEST(OrderedActorTaskExecutionQueueTest, TestSeqWaitTimeout) {
  ...
  queue.EnqueueTask(4, -1, MakeTaskExecutionMetadata(task_spec));
  queue.EnqueueTask(5, -1, MakeTaskExecutionMetadata(task_spec));
...
TEST(OrderedActorTaskExecutionQueueTest, TestSkipAlreadyProcessedByClient) {
  ...
  TaskSpecification task_spec;
  task_spec.GetMutableMessage().set_type(TaskType::ACTOR_TASK);
  queue.EnqueueTask(2, 2, MakeTaskExecutionMetadata(task_spec));
  queue.EnqueueTask(3, 2, MakeTaskExecutionMetadata(task_spec));
  queue.EnqueueTask(1, 2, MakeTaskExecutionMetadata(task_spec));
  io_service.run();
...
TEST(OrderedActorTaskExecutionQueueTest, TestRetryInOrderOrderedActorTaskExecutionQueue) {
  ...
  auto execute_task = [&accept_seq_nos, &n_accept](TaskExecutionMetadata &task) {
    accept_seq_nos.push_back(task.TaskSpec().ConcurrencyGroupSequenceNumber());
    n_accept++;
  };
  auto cancel_task = [&reject_seq_nos](const TaskExecutionMetadata &task,
                                       const Status &status) {
  ...
  queue.EnqueueTask(0, -1, MakeTaskExecutionMetadata(task_spec_0));
  auto task_spec_1 = CreateActorTaskSpec(1);
  queue.EnqueueTask(1, -1, MakeTaskExecutionMetadata(task_spec_1));
  auto task_spec_2_retry = CreateActorTaskSpec(3, /*is_retry=*/true);
  queue.EnqueueTask(3, -1, MakeTaskExecutionMetadata(task_spec_2_retry));
  auto task_spec_4 = CreateActorTaskSpec(4);
  queue.EnqueueTask(4, 2, MakeTaskExecutionMetadata(task_spec_4));
  auto task_spec_5_retry = CreateActorTaskSpec(6, /*is_retry=*/true, /*dependency=*/true);
  queue.EnqueueTask(6, -1, MakeTaskExecutionMetadata(task_spec_5_retry));
...
TEST(OrderedActorTaskExecutionQueueTest, TestPerConcurrencyGroupOrdering) {
  ...
  std::atomic<int> n_accept = 0;
  auto execute_task = [&accepted, &n_accept](TaskExecutionMetadata &task) {
    accepted.emplace_back(task.TaskSpec().ConcurrencyGroupName(),
                          task.TaskSpec().ConcurrencyGroupSequenceNumber());
    n_accept++;
  };
  auto cancel_task = [](const TaskExecutionMetadata &, const Status &) { FAIL(); };
  ...
  queue.EnqueueTask(1, -1, MakeTaskExecutionMetadata(task_a1));
  queue.EnqueueTask(0, -1, MakeTaskExecutionMetadata(task_b0));
  queue.EnqueueTask(1, -1, MakeTaskExecutionMetadata(task_b1));
  ...
  queue.EnqueueTask(0, -1, MakeTaskExecutionMetadata(task_a0));
  io_service.run_one();
  io_service.run_one();
...
TEST(UnorderedActorTaskExecutionQueueTest, TestSameTaskMultipleAttempts) {
  ...
  auto execute_task = [&attempt_1_start_promise,
                       ...
                       &attempt_2_start_promise](TaskExecutionMetadata &task) {
  ...
  queue.EnqueueTask(-1, -1, MakeTaskExecutionMetadata(task_spec_2));
  io_service.poll();
...
TEST(UnorderedActorTaskExecutionQueueTest, TestSameTaskMultipleAttemptsCancellation) {
  ...
  auto execute_task = [&attempt_1_start_promise,
                       &attempt_1_finish_promise](TaskExecutionMetadata &task) {
    if (task.TaskSpec().AttemptNumber() == 1) {
      attempt_1_start_promise.set_value();
      attempt_1_finish_promise.get_future().wait();
    } else {
  ...
  auto cancel_task = [&attempt_2_cancelled, &attempt_3_cancelled, &attempt_4_cancelled](
                         const TaskExecutionMetadata &task, const Status &status) {
  ...
  queue.EnqueueTask(-1, -1, MakeTaskExecutionMetadata(task_spec_4));
  ...
  queue.EnqueueTask(-1, -1, MakeTaskExecutionMetadata(task_spec_3));
