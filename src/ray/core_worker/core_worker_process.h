...
class CoreWorkerProcessImpl {
 ...
 private:
  ...
  // Comparison metrics - these wrappers record to all three implementations:
  ...
  std::unique_ptr<ray::stats::ComparisonMetric>
      task_total_submitter_preprocessing_time_ms_histogram_;
  std::unique_ptr<ray::stats::ComparisonMetric>
      task_dependency_resolution_time_ms_histogram_;
  std::unique_ptr<ray::stats::ComparisonMetric> task_push_time_ms_histogram_;
};
}  // namespace core
}  // namespace ray
