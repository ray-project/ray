...
struct WorkerThreadContext {
  ...
  const TaskID &GetCurrentTaskUniqueBytes() const { return current_task_unique_bytes_; }
  ...
  void SetCurrentTask(const TaskSpecification &task_spec) {
    ...
    current_task_unique_bytes_ = TaskID::FromRandom(task_spec.JobId());
  ...
  void ResetCurrentTask() {
    ...
    current_task_unique_bytes_ = TaskID::Nil();
    task_index_ = 0;
    put_counter_ = 0;
  }
 ...
 private:
  ...
  /// Unique bytes to use for any ObjectIDs generated by the
  /// current task (via ray.put or .remote calls).
  TaskID current_task_unique_bytes_;
...
const TaskID &WorkerContext::GetCurrentTaskUniqueBytes() const {
  return GetThreadContext().GetCurrentTaskUniqueBytes();
}
