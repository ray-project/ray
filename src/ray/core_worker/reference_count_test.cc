...
//         periodic_runner_(std::make_shared<PeriodicalRunner>(io_service_)),
//         publisher_(std::make_shared<pubsub::Publisher>(
...
//       ObjectID::FromBinary(request.reference().object_id()); ObjectID contained_in_id =
//       ObjectID::FromBinary(request.contained_in_id()); const auto owner_address =
//       request.reference().owner_address(); const auto subscriber_id =
...
//         rpc::ChannelType::WAIT_FOR_REF_REMOVED_CHANNEL, subscriber_id,
//         object_id.Binary());
...
//       {}, const rpc::Address &borrower_address = empty_borrower, const
//       ReferenceCounter::ReferenceTableProto &borrower_refs = empty_refs) {
...
//   // The owner receives the borrower's reply and merges the borrower's ref
//   // count into its own.
//   owner->HandleSubmittedTaskFinished(owner_id3, {}, borrower1->address_,
//   borrower_refs);
...
//   owner->HandleSubmittedTaskFinished(owner_id3, {}, borrower2->address_,
//   borrower_refs); ASSERT_TRUE(borrower2->FlushBorrowerCallbacks());
...
//   ASSERT_FALSE(borrower1->rc_.HasReference(owner_id3));
//   ASSERT_FALSE(borrower1->rc_.HasReference(borrower_id));
//   // Borrower 1 should not have merge any refs into the owner because borrower 2's ref
//   was
//   // already merged into the owner.
//   owner->HandleSubmittedTaskFinished(owner_id2, {}, borrower1->address_,
//   borrower_refs);
...
//   caller->HandleSubmittedTaskFinished(return_id, {}, borrower->address_,
//   borrower_refs);
...
//   auto refs =
//       worker->FinishExecutingTask(ObjectID::Nil(), return_id, &nested_return_id,
//       &addr);
...
//   auto nested_refs = nested_worker->FinishExecutingTask(ObjectID::Nil(),
//   nested_return_id,
...
//   root->HandleSubmittedTaskFinished(ObjectID::Nil(), {{return_id,
//   {nested_return_id}}}); worker->HandleSubmittedTaskFinished(ObjectID::Nil(),
//   {{nested_return_id, {inner_id}}}); root->FlushBorrowerCallbacks();
...
//   worker->HandleSubmittedTaskFinished(ObjectID::Nil(), {{nested_return_id,
//   {inner_id}}}); worker->FlushBorrowerCallbacks();
...
//   rpc::WorkerAddress addr(root->address_);
//   auto refs = worker->FinishExecutingTask(ObjectID::Nil(), return_id, &inner_id,
//   &addr);
