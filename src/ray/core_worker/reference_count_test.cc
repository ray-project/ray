...
TEST_F(ReferenceCountTest, TestGetLocalityData) {
  ...
  ASSERT_EQ(
      locality_data_obj1->nodes_containing_object,
...
TEST(DistributedReferenceCountTest, TestForeignOwner) {
  ...
  foreign_owner->rc_.AddOwnedObject(
      inner_id,
      {},
...
TEST_F(ReferenceCountLineageEnabledTest, TestPinLineageRecursive) {
  ...
  for (size_t i = 0; i < ids.size() - 1; i++) {
    ...
    ASSERT_FALSE(rc->SetDeleteCallback(id, [&](const ObjectID &object_id) {
      ASSERT_FALSE(true);
    }));
