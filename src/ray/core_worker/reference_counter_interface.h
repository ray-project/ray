 ...
 * reporting to users.
 *
 * @param eligibility The lineage eligibility to convert.
 * @return The corresponding ErrorType, or std::nullopt if the object is eligible for
 * reconstruction.
 */
...
class ReferenceCounterInterface {
 ...
 public:
  ...
  /**
   * @brief Wait for all object references to go out of scope, and then shutdown.
   *
  ...
  /**
   * @brief Return the size of the reference count table
   * (i.e. the number of objects that have references).
   */
  virtual size_t Size() const = 0;

  /**
   * @brief Return true if the object is owned by us.
   */
  virtual bool OwnedByUs(const ObjectID &object_id) const = 0;

  /**
   * @brief Increase the reference count for the ObjectID by one.
   *
   * If there is no entry for the ObjectID, one will be created. The object ID
   * will not have any owner information, since we don't know how it was
   * created.
   *
  ...
  /**
   * @brief Decrease the local reference count for the ObjectID by one.
   *
  ...
  /**
   * @brief Add references for the provided object IDs that correspond to them being
   * dependencies to a submitted task.
   *
   * If lineage pinning is enabled, then this will also pin the Reference entry
   * for each new argument until the argument's lineage ref is released.
   *
  ...
  /**
   * @brief Add references for the object dependencies of a resubmitted task.
   *
   * This does not increment the arguments' lineage ref counts because we should
  ...
  /**
   * @brief Update object references that were given to a submitted task.
   *
   * The task may still be borrowing any object IDs that were contained in its
  ...
  /**
   * @brief Add an object that we own.
   *
   * The object may depend on other objects. Dependencies for each ObjectID must
   * be set at most once. The local reference count for the ObjectID is set to
   * zero, which assumes that an ObjectID for it will be created in the language
   ...
   *
   * These are objects that were created by a task that we called, but that we
   * own.
   *
  ...
  /**
   * @brief Own an object that the current owner (current process) dynamically created.
   *
  ...
  /**
   * @brief Try to decrement the local ref count for the given objects, if they are
   * still in scope.
   *
  ...
  /**
   * @brief Check if a generator's lineage has gone out of scope.
   *
   * This checks if we still have entries for the generator ref and all refs
   * returned by the generator, including the sentinel EOF object. If true, then
   * the lineage (task and stream metadata) is safe to remove.
   *
  ...
  /**
   * @brief Update the size of the object.
   *
  ...
  /**
   * @brief Add an object that we are borrowing.
   *
  ...
  /**
   * @brief Get the owner address of the given object.
   *
  ...
  /**
   * @brief Check if the object has an owner.
   *
   * @param[in] object_id The ID of the object.
  ...
  /**
   * @brief Checks to see if objects have an owner.
   *
  ...
  /**
   * @brief Get the owner addresses of the given objects.
   *
   * The owner address must be registered for these objects.
   *
  ...
  /**
   * @brief Check whether an object value has been freed.
   *
  ...
  /**
   * @brief Mark an object that was freed as being in use again.
   *
   * If another copy of the object is subsequently pinned, we will not delete it
   * until free is called again, or the ObjectRef goes out of scope.
   *
  ...
  /**
   * @brief Release the underlying value from plasma (if any) for these objects.
   *
  ...
  /**
   * @brief Adds the callback that will be run when the object goes out of scope
   * (Reference.OutOfScope() returns true).
   *
   * @return true if the object was in scope and the callback was added, else false.
   */
  ...
  /**
   * @brief Stores the callback that will be run when the object reference is deleted
   ...
   * the later one.
   *
   * @return true if the object was in the reference table and the callback was added
   * else false.
   */
  ...
  /**
   * @brief So we call PublishRefRemovedInternal when we are no longer borrowing this
  ...
  /**
   * @brief Set a callback to call whenever a Reference that we own is deleted.
   *
   * A Reference can only be deleted if:
  ...
  /**
   * @brief Just calls PublishRefRemovedInternal with a lock.
   */
  virtual void PublishRefRemoved(const ObjectID &object_id) = 0;

  /**
   * @brief Returns the total number of ObjectIDs currently in scope.
   */
  virtual size_t NumObjectIDsInScope() const = 0;

  /**
   * @brief Returns the total number of objects owned by this worker.
   */
  virtual size_t NumObjectsOwnedByUs() const = 0;

  /**
   * @brief Returns the total number of actors owned by this worker.
   */
  virtual size_t NumActorsOwnedByUs() const = 0;

  /**
   * @brief Reports owner-side observability metrics to underlying monitoring system.
   */
  virtual void RecordOwnerMetrics() = 0;

  /**
   * @brief Returns a set of all ObjectIDs currently in scope (i.e., nonzero reference
   * count).
   */
  virtual std::unordered_set<ObjectID> GetAllInScopeObjectIDs() const = 0;

  /**
   * @brief Returns a map of all ObjectIDs currently in scope with a pair of their
  ...
  /**
   * @brief Populate a table with ObjectIDs that we were or are still borrowing.
   *
  ...
  /**
   * @brief Mark that this ObjectID contains another ObjectID(s).
   *
  ...
  /**
   * @brief Update the pinned location of an object stored in plasma.
   *
  ...
  /**
   * @brief Check whether the object is pinned at a remote plasma store node or
   * spilled to external storage.
   *
   * In either case, a copy of the object is available to fetch.
   *
   * @param[in] object_id The object to check.
  ...
  /**
   * @brief Get and reset the objects that were pinned or spilled on the given node.
   *
  ...
  /**
   * @brief Whether we have a reference to a particular ObjectID.
   *
  ...
  /**
   * @brief Write the current reference table to the given proto.
   *
  ...
  /**
   * @brief Add a new location for the given object.
   *
   * The owner must have the object ref in scope.
   *
   * @param[in] object_id The object to update.
  ...
  /**
   * @brief Remove a location for the given object.
   *
   * The owner must have the object ref in scope.
   *
   * @param[in] object_id The object to update.
  ...
  /**
   * @brief Get the locations of the given object.
   *
   * The owner must have the object ref in scope.
   *
  ...
  /**
   * @brief Publish the snapshot of the object location for the given object id.
   * and mark the object as subscribed, so its later location publishes are not
  ...
  /**
   * @brief Fill up the object information.
   *
   * @param[in] object_id The object id
   * @param[out] object_info The object information that will be filled by a given object
   * id.
   */
  ...
  /**
   * @brief Handle an object has been spilled to external storage.
   *
  ...
  /**
   * @brief Get locality data for object.
   *
   * This is used by the leasing policy to implement locality-aware leasing.
   *
  ...
  /**
   * @brief Report locality data for object.
   *
   * This is used by the FutureResolver to report locality data for borrowed refs.
   *
  ...
  /**
   * @brief Add borrower address in owner's worker.
   *
   * This function will add borrower address to the `object_id_refs_`, then call
   * WaitForRefRemoved() to monitor borrowed object in borrower's worker.
   *
  ...
  /**
   * @brief Check if an object is eligible for lineage-based reconstruction.
   *
  ...
  /**
   * @brief Evict lineage of objects that are still in scope.
   *
   * This evicts lineage in FIFO order, based on when the ObjectRef was created.
   *
  ...
  /**
   * @brief Update whether the object is pending creation.
   */
  virtual void UpdateObjectPendingCreation(const ObjectID &object_id,
                                           bool pending_creation) = 0;

  /**
   * @brief Whether the object is pending creation (the task that creates it is
   * scheduled/executing).
   */
  virtual bool IsObjectPendingCreation(const ObjectID &object_id) const = 0;

  /**
   * @brief Release all local references which registered on this local.
   */
  virtual void ReleaseAllLocalReferences() = 0;

  /**
   * @brief Get the tensor transport for the given object.
   */
   ...
   * workers only learn their job config with the first task they receive,
   * which is before they can own any objects.
   *
   * @param lineage_pinning_enabled whether to enable lineage pinning on this worker.
   */
