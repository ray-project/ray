...
Status CoreWorkerStoreProviderLayer::Put(StoreProviderType type, const RayObject &object,
                                         const ObjectID &object_id) {
  return store_providers_[type]->Put(object, object_id);
}

Status CoreWorkerStoreProviderLayer::Get(
    StoreProviderType type, const std::vector<ObjectID> &ids, int64_t timeout_ms,
...
CoreWorkerStoreProviderLayer::CreateStoreProvider(StoreProviderType type) const {
  switch (type) {
  ...
  case StoreProviderType::PLASMA:
    return std::unique_ptr<CoreWorkerStoreProvider>(new CoreWorkerPlasmaStoreProvider(
        worker_context_, store_socket_, raylet_client_));
    break;
