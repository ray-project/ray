...
class GetRequest {
 public:
  GetRequest(
      absl::flat_hash_set<ObjectID> object_ids,
      size_t num_objects,
...
CoreWorkerMemoryStore::CoreWorkerMemoryStore(
    ...
    std::function<std::shared_ptr<ray::RayObject>(
        ...
        const ObjectID &object_id)> object_allocator)
...
void CoreWorkerMemoryStore::Delete(
    const absl::flat_hash_set<ObjectID> &object_ids,
...
inline void CoreWorkerMemoryStore::EraseObjectAndUpdateStats(const ObjectID &object_id) {
  ...
  RAY_CHECK(
      num_in_plasma_ >= 0 && num_local_objects_ >= 0 && used_object_store_memory_ >= 0);
  objects_.erase(it);
}
