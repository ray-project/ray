 ...
 public:
  Status GetExperimentalMutableObject(
    ...
    if (!objects_.count(object_id)) {
      ...
      *mutable_object = MakeObject(/*min_size=*/1024);
...
TEST(MutableObjectProvider, HandleRetryOutOfOrderChunks) {
  ...
  constexpr size_t kTotalDataSize = kChunkSize * 3;  // 3 chunks
  ...
  for (int i = 0; i < 3; i++) {
    chunk_data[i].resize(kChunkSize, static_cast<uint8_t>(i));
  }
  ...
  // Chunk 1 arrives first (offset = kChunkSize)
  {
    ray::rpc::PushMutableObjectRequest request;
    ...
    request.set_data(chunk_data[1].data(), kChunkSize);
  ...
  // Chunk 0 arrives second (offset = 0) - simulates retry or out-of-order
  {
    ...
    request.set_offset(0);
    request.set_chunk_size(kChunkSize);
    request.set_data(chunk_data[0].data(), kChunkSize);
    request.set_metadata(metadata.data(), kMetadataSize);
  ...
  // Chunk 2 arrives last (offset = 2 * kChunkSize)
  {
    ray::rpc::PushMutableObjectRequest request;
    request.set_writer_object_id(writer_object_id.Binary());
    request.set_total_data_size(kTotalDataSize);
    request.set_total_metadata_size(kMetadataSize);
    request.set_offset(2 * kChunkSize);
    request.set_chunk_size(kChunkSize);
    request.set_data(chunk_data[2].data(), kChunkSize);
    request.set_metadata(metadata.data(), kMetadataSize);
  ...
  for (int chunk = 0; chunk < 3; chunk++) {
    for (size_t i = 0; i < kChunkSize; i++) {
      EXPECT_EQ(data_ptr[chunk * kChunkSize + i], static_cast<uint8_t>(chunk))
