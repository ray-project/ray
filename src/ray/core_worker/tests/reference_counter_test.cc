...
// Regression test for reference_counter.cc:770-771 RAY_CHECK failure.
//
// The RAY_CHECK at line 770-771 can fail if DeleteReferenceInternal is called
// twice on a borrowed outer object that contains nested references, where the
// first call successfully erases from contained_in_borrowed_ids but the outer's
// "contains" set is not cleared, causing the second call to fail the erase.
//
// This test sets up nested borrowed references and verifies that cleanup
// works correctly when the outer object goes out of scope. If the bug were
// present (double deletion), this test would crash with a RAY_CHECK failure.
TEST(DistributedReferenceCountTest, TestDeleteBorrowedObjectWithNestedRefs) {
  auto owner = std::make_shared<MockWorkerClient>("1");
  auto borrower = std::make_shared<MockWorkerClient>(
      "2", [&](const rpc::Address &addr) { return owner; });

  // Owner creates nested objects: outer contains inner
  auto inner_id = ObjectID::FromRandom();
  auto outer_id = ObjectID::FromRandom();
  owner->rc_.AddOwnedObject(
      inner_id, {}, owner->address_, "", 0, false, /*add_local_ref=*/true);
  owner->rc_.AddOwnedObject(
      outer_id, {inner_id}, owner->address_, "", 0, false, /*add_local_ref=*/true);

  // Borrower borrows the outer object and deserializes to get inner.
  // This sets up:
  // - outer's contains = {inner_id}
  // - inner's contained_in_borrowed_ids = {outer_id}
  borrower->rc_.AddLocalReference(outer_id, "");
  borrower->rc_.AddBorrowedObject(outer_id, ObjectID::Nil(), owner->address_);
  borrower->rc_.AddLocalReference(inner_id, "");
  borrower->rc_.AddBorrowedObject(inner_id, outer_id, owner->address_);
  ...
  // Remove inner's reference first
  borrower->rc_.RemoveLocalReference(inner_id, nullptr);
  // Inner should still exist because it's nested in outer (contained_in_borrowed_ids
  // is not empty)
  ASSERT_TRUE(borrower->rc_.HasReference(inner_id));

  // Remove outer's reference - this triggers cleanup of the nested relationship.
  // DeleteReferenceInternal(outer) will:
  // 1. Iterate over outer's "contains" set
  // 2. Erase outer_id from inner's contained_in_borrowed_ids
  // 3. Recursively call DeleteReferenceInternal(inner)
  //
  // The bug would occur if this cleanup ran twice on the same outer object
  // before it was erased from the table.
  borrower->rc_.RemoveLocalReference(outer_id, nullptr);
...
// Test that specifically exercises the scenario where AddBorrowedObject is called
// with RefCount()==0, which triggers an immediate DeleteReferenceInternal call.
// This verifies that the nested relationship is properly maintained even when
// the inner object has no local references.
TEST(DistributedReferenceCountTest, TestAddBorrowedObjectWithZeroRefCount) {
  auto owner = std::make_shared<MockWorkerClient>("1");
  auto borrower = std::make_shared<MockWorkerClient>(
      "2", [&](const rpc::Address &addr) { return owner; });

  // Owner creates nested objects
  auto inner_id = ObjectID::FromRandom();
  auto outer_id = ObjectID::FromRandom();
  owner->rc_.AddOwnedObject(
      inner_id, {}, owner->address_, "", 0, false, /*add_local_ref=*/true);
  ...
  // Borrower borrows outer
  borrower->rc_.AddLocalReference(outer_id, "");
  borrower->rc_.AddBorrowedObject(outer_id, ObjectID::Nil(), owner->address_);

  // Add borrowed object for inner WITHOUT a local reference.
  // This means RefCount()==0, triggering immediate DeleteReferenceInternal.
  // However, inner should survive because contained_in_borrowed_ids is not empty.
  borrower->rc_.AddBorrowedObject(inner_id, outer_id, owner->address_);

  // Inner should still be tracked because it's contained in outer
  ASSERT_TRUE(borrower->rc_.HasReference(inner_id));
  ASSERT_TRUE(borrower->rc_.HasReference(outer_id));

  // Now remove outer's reference
  borrower->rc_.RemoveLocalReference(outer_id, nullptr);

  // Both should be cleaned up - the RAY_CHECK at line 770-771 should not fail
  // because the cleanup only runs once
  ASSERT_FALSE(borrower->rc_.HasReference(outer_id));
  ASSERT_FALSE(borrower->rc_.HasReference(inner_id));
}

// Test that calling AddBorrowedObject multiple times for the same nested relationship
// is idempotent and doesn't cause issues during cleanup.
TEST(DistributedReferenceCountTest, TestAddBorrowedObjectIdempotent) {
  auto owner = std::make_shared<MockWorkerClient>("1");
  auto borrower = std::make_shared<MockWorkerClient>(
      "2", [&](const rpc::Address &addr) { return owner; });
  ...
  // Borrower borrows outer and inner
  borrower->rc_.AddLocalReference(outer_id, "");
  borrower->rc_.AddBorrowedObject(outer_id, ObjectID::Nil(), owner->address_);
  borrower->rc_.AddLocalReference(inner_id, "");

  // Call AddBorrowedObject multiple times for the same relationship.
  // This should be idempotent - the relationship should only be tracked once.
  borrower->rc_.AddBorrowedObject(inner_id, outer_id, owner->address_);
  borrower->rc_.AddBorrowedObject(inner_id, outer_id, owner->address_);
  borrower->rc_.AddBorrowedObject(inner_id, outer_id, owner->address_);
  ...
  // Cleanup should work correctly even after multiple AddBorrowedObject calls.
  // The RAY_CHECK at line 770-771 should not fail because:
  // 1. The insert() calls are idempotent (it's a set)
  // 2. The erase() is only called once during deletion
  borrower->rc_.RemoveLocalReference(inner_id, nullptr);
  borrower->rc_.RemoveLocalReference(outer_id, nullptr);
