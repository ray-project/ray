...
extern "C" CSerializedObjectArray *CNativeTaskExecutor_Execute(
  ...
  return ray::go::CgoErrorHandler::Execute(
      "CNativeTaskExecutor_Execute", [&]() -> CSerializedObjectArray * {
        ...
        // Caller is responsible for freeing the returned CSerializedObjectArray using
        // CNativeCommon_FreeCSerializedObjectArray
        using CSerializedObjectArrayPtr =
            ray::go::CgoUniquePtr<CSerializedObjectArray,
                                  &CNativeCommon_FreeCSerializedObjectArray>;
        auto result_ptr = CSerializedObjectArrayPtr(static_cast<CSerializedObjectArray *>(
            malloc(sizeof(CSerializedObjectArray))));
        if (!result_ptr) {
        ...
        for (int i = 0; i < result_ptr->count; ++i) {
          ...
          if (obj && obj->HasData()) {
            ...
            if (metadata && metadata->Size() > 0) {
              result_ptr->objects[i].metadata =
                  static_cast<char *>(malloc(metadata->Size()));
...
extern "C" CSerializedObjectArray *CNativeTaskExecutor_ExecuteActorTask(
    ...
    int num_returns) {
  return ray::go::CgoErrorHandler::Execute(
      "CNativeTaskExecutor_ExecuteActorTask", [&]() -> CSerializedObjectArray * {
