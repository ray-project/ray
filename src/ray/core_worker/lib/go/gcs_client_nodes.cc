...
int ray_gcs_client_nodes_check_alive(CGcsClient* client,
                                      ...
                                      int* alive_out,
                                      char** error_out) {
    if (!client || !client->global_state_accessor || !node_ids_hex || !alive_out || count <= 0) {
        set_error(error_out, "Invalid arguments");
        return 0;
    }

    try {
        std::vector<std::string> all_node_info = client->global_state_accessor->GetAllNodeInfo();
        ...
        for (const auto& serialized : all_node_info) {
            ...
            if (node_info.ParseFromString(serialized)) {
                std::string node_id_hex = ray::NodeID::FromBinary(
                    std::string(node_info.node_id())).Hex();
...
int ray_gcs_client_nodes_get_all(CGcsClient* client,
    ...
    try {
        ...
        if (!allocate_serialized_array(filtered_nodes, serialized_out, sizes_out, count_out)) {
...
int ray_gcs_client_nodes_drain(CGcsClient* client,
                               ...
                               char** error_out) {
    if (!client || !client->global_state_accessor || !drained_ids_hex_out || !drained_count_out) {
        set_error(error_out, "Invalid arguments");
        return 0;
    }
