...
CGcsClient* ray_gcs_client_create(const char* address,
    ...
    if (!cluster_id_hex) {
        set_error(error_out, "Invalid arguments: cluster_id_hex must not be null (use empty string for nil)");
        return nullptr;
    }
    ...
    try {
        ...
        if (cluster_id_hex[0] != '\0') {
            ...
            cluster_id = ray::ClusterID::FromBinary(
                std::string(reinterpret_cast<const char*>(cluster_id_bytes.data()), cluster_id_bytes.size())
            );
        } else {
        ...
        ray::Status status = client->gcs_client->Connect(*client->io_service, client->timeout_ms);
        if (!status.ok()) {
        ...
        client->global_state_accessor = std::make_unique<ray::gcs::GlobalStateAccessor>(options);
