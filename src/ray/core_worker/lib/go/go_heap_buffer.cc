...
size_t GoHeapBuffer::Size() const { return size_; }
...
std::shared_ptr<GoHeapBuffer> AllocateGoHeapBuffer(const char *object_id_data,
  ...
  void *go_handle = GoAllocateObject(const_cast<char *>(object_id_data),
                                     object_id_size,
                                     const_cast<char *>(data_ptr),
                                     data_size,
                                     const_cast<char *>(metadata_ptr),
                                     metadata_size);

  if (go_handle == nullptr) {
    RAY_LOG(ERROR) << "Failed to allocate Go heap buffer: "
                   << "object_id_size=" << object_id_size << ", data_size=" << data_size
                   << ", metadata_size=" << metadata_size;
    return nullptr;
  }

  // Wrap Go handle in GoHeapBuffer
  return std::make_shared<GoHeapBuffer>(static_cast<GoObjectRefHandle *>(go_handle));
}
