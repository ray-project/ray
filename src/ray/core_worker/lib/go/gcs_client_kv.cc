...
int ray_gcs_client_kv_get(CGcsClient *client,
  ...
  if (status.IsNotFound()) {
    // Keep the Go-side contract: the error string "Key not found" maps to
    // gcs.ErrKeyNotFound
...
int ray_gcs_client_kv_multi_get(CGcsClient *client,
                                ...
                                int *count_out,
                                char **error_out) {
  if (!client || !client->gcs_client || !keys || key_count <= 0 || !keys_out ||
      !values_out || !sizes_out || !count_out) {
    set_error(error_out, "Invalid arguments");
    return -1;
  }
  ...
  *count_out = count;
  ...
  for (const auto &kv : values) {
    ...
    // When size==0 val_arr[i] stays nullptr: Go only reads when size>0, free(nullptr) is
    // safe
    i++;
  }
...
int ray_gcs_client_kv_put(CGcsClient *client,
  ...
  // added=true means inserted; added=false when overwrite=false and the key exists (not
  // an error)
