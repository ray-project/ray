...
class MockActorCreator : public ActorCreatorInterface {
 public:
  ...
  void AsyncWaitForActorRegisterFinish(const ActorID &, gcs::StatusCallback callback)
      override {
    callbacks.push_back(callback);
  }
...
TEST(DirectTaskTransportTest, TestSpillbackRoundTrip) {
  ...
  ASSERT_TRUE(
      remote_lease_clients[7777]
          ->GrantWorkerLease("local", 1234, local_raylet_id, false, "", /*reject=*/true));
...
void TestSchedulingKey(
    const std::shared_ptr<CoreWorkerMemoryStore> store,
...
TEST(DirectTaskTransportTest, TestKillExecutingTask) {
  ...
  ASSERT_EQ(
      worker_client->kill_requests.front().intended_task_id(),
      task.TaskId().Binary());
