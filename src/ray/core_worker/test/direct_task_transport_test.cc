...
class MockRayletClient : public WorkerLeaseInterface {
 public:
  ...
  void GrantWorkerLease(const std::string &address, int port,
...
TEST(DirectTaskTransportTest, TestSubmitOneTask) {
  ...
  raylet_client->GrantWorkerLease("localhost", 1234, ClientID::Nil());
...
TEST(DirectTaskTransportTest, TestConcurrentWorkerLeases) {
  ...
  raylet_client->GrantWorkerLease("localhost", 1000, ClientID::Nil());
  ASSERT_EQ(worker_client->callbacks.size(), 1);
  ASSERT_EQ(raylet_client->num_workers_requested, 2);

  // Task 2 is pushed; worker 3 is requested.
  raylet_client->GrantWorkerLease("localhost", 1001, ClientID::Nil());
  ...
  raylet_client->GrantWorkerLease("localhost", 1002, ClientID::Nil());
