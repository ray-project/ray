...
TEST_P(DirectActorSubmitterTest, TestSubmitTask) {
  ...
  EXPECT_CALL(*task_finisher_, FailOrRetryPendingTask(_, _, _, _, _, _, _)).Times(0);
...
TEST_P(DirectActorSubmitterTest, TestActorDead) {
  ...
  EXPECT_CALL(*task_finisher_, FailOrRetryPendingTask(task1.TaskId(), _, _, _, _, _, _))
      .Times(1);
  ...
  EXPECT_CALL(*task_finisher_, FailOrRetryPendingTask(task2.TaskId(), _, _, _, _, _, _))
      .Times(1);
...
TEST_P(DirectActorSubmitterTest, TestActorRestartNoRetry) {
  ...
  EXPECT_CALL(*task_finisher_, FailOrRetryPendingTask(task2.TaskId(), _, _, _, _, _, _))
      .Times(1);
  EXPECT_CALL(*task_finisher_, FailOrRetryPendingTask(task3.TaskId(), _, _, _, _, _, _))
      .Times(1);
...
TEST_P(DirectActorSubmitterTest, TestActorRestartOutOfOrderGcs) {
  ...
  // the code.
  EXPECT_CALL(*task_finisher_, FailOrRetryPendingTask(task.TaskId(), _, _, _, _, _, _))
      .Times(1);
