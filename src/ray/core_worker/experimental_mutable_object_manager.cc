...
Status MutableObjectManager::ReadAcquire(const ObjectID &object_id,
  ...
  if (result) {
    ...
    std::shared_ptr<SharedMemoryBuffer> metadata_buf = SharedMemoryBuffer::Slice(
        channel->mutable_object->buffer,
        /*offset=*/channel->mutable_object->header->data_size,
        /*size=*/channel->mutable_object->header->metadata_size);

    result = std::make_shared<RayObject>(std::move(data_buf),
