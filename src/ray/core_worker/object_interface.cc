...
Status CoreWorkerObjectInterface::Get(const std::vector<ObjectID> &ids,
  ...
  for (const auto &entry : object_ids_per_transport) {
    ...
    RAY_RETURN_NOT_OK(Get(store_provider_type, entry.first, entry.second,
...
bool CoreWorkerObjectInterface::ShouldWaitObjects(
    TaskTransportType transport_type, const std::vector<ObjectID> &object_ids) {
  for (const auto &object_id : object_ids) {
    bool should_wait =
        task_submitter_layer_.ShouldWaitTask(transport_type, object_id.TaskId());
    if (should_wait) {
      return true;
    }
  }
...
Status CoreWorkerObjectInterface::Get(
  ...
  if (type == StoreProviderType::PLASMA) {
    ...
    RAY_RETURN_NOT_OK(
        store_provider_layer_.Get(type, unready_ids, timeout_ms, &result_objects));
  ...
  while (!unready.empty() && !should_break) {
    ...
    std::vector<std::shared_ptr<RayObject>> result_objects;
    RAY_RETURN_NOT_OK(
        store_provider_layer_.Get(type, unready_ids, get_timeout, &result_objects));
