...
std::string GetValidLocalIp(int port, int64_t timeout_ms) {
  ...
  if (async_client.Connect(kPublicDNSServerIp, kPublicDNSServerPort, timeout_ms,
    ...
    if (is_timeout || error_code == boost::system::errc::host_unreachable) {
      ...
      if (!error_code) {
        std::vector<boost::asio::ip::address> ip_candidates = NetIf::GetValidLocalIpCandidates();
  ...
  if (address == localhost_ip) {
    RAY_LOG(ERROR) << "Failed to find other valid local IP. Using "
      << localhost_ip << ", not possible to go distributed!";
  }
  ...
  std::vector<boost::asio::ip::address> GetValidLocalIpCandidates() {
    ...
    for (if_info = ifs_info; if_info != nullptr; if_info = if_info->ifa_next) {
      ...
      if (if_info->ifa_addr->sa_family == AF_INET) {
        ...
        ifnames_and_ips.push_back(std::make_pair(std::string(if_info->ifa_name), std::string(ip)));
      }
    }
    freeifaddrs(ifs_info);

    // Bigger prefixes must be tested first in CompNameAndIps
    std::sort(prefixes_and_priorities.begin(), prefixes_and_priorities.end(), CompPrefixLen);
    ...
    for(const auto &ifname_and_ip : ifnames_and_ips) {
      boost::asio::ip::address boost_addr = boost::asio::ip::make_address(ifname_and_ip.second);
