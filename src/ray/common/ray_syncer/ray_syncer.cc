  ...
  boost::asio::dispatch(
      io_context_.get_executor(), std::packaged_task<void()>([=]() {
        ...
        auto *reactor = new RayClientBidiReactor(
            ...
            /* cleanup_cb */
            [this, channel](RaySyncerBidiReactor *_reactor, bool restart) {
              const std::string &_node_id = _reactor->GetRemoteNodeID();
              auto iter = sync_reactors_.find(_node_id);
              if (iter != sync_reactors_.end()) {
                if (iter->second != _reactor) {
              ...
              if (restart) {
                execute_after(
                    io_context_,
                    [this, _node_id, channel]() {
                      RAY_LOG(INFO).WithField(NodeID::FromBinary(_node_id))
                          << "Connection is broken. Reconnect to node.";
                      Connect(_node_id, channel);
                    },
                    /* delay_microseconds = */ std::chrono::milliseconds(2000));
              } else {
                node_state_->RemoveNode(_node_id);
              }
            },
            /* stub */ std::move(stub));
        Connect(reactor);
...
ServerBidiReactor *RaySyncerService::StartSync(grpc::CallbackServerContext *context) {
  auto reactor = new RayServerBidiReactor(
      ...
      /*cleanup_cb=*/
      [this](RaySyncerBidiReactor *_reactor, bool reconnect) mutable {
        // No need to reconnect for server side.
        RAY_CHECK(!reconnect);
        const auto &node_id = _reactor->GetRemoteNodeID();
