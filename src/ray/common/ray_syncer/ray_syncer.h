...
class RaySyncer {
 public:
  ...
  void BroadcastMessage(std::shared_ptr<MergedRaySyncMessage> message);
 ...
 private:
  ...
  // Whether batching of sync messages is enabled.
  bool batching_enabled_;

  // Buffer for batching sync messages before sending.
  absl::flat_hash_map<std::string, std::shared_ptr<const InnerRaySyncMessage>>
      sync_message_batch_buffer_;

  // Timer for flushing batched sync messages.
  std::unique_ptr<boost::asio::steady_timer> sync_message_batch_timer_;

  // Whether the sync message batch timer is active.
  bool sync_message_batch_timer_active_ = false;

  // The maximum batch size for sync messages.
  size_t batch_size_ = static_cast<size_t>(RayConfig::instance().syncer_batch_size());

  // The maximum timeout for batching sync messages.
  std::chrono::milliseconds batch_timeout_{
      static_cast<int64_t>(RayConfig::instance().syncer_batch_timeout_ms())};

  // Sync message batching support
  void BufferOrFlushSyncMessage(std::shared_ptr<MergedRaySyncMessage> message);

  /// Merge and flush the buffered sync messages.
  void MergeAndFlushSyncMessage();

  /// Merge multiple sync messages (flat_hash_map) into one.
  std::shared_ptr<MergedRaySyncMessage> MergeInnerSyncMessages(
      absl::flat_hash_map<std::string, std::shared_ptr<const InnerRaySyncMessage>>
          &messages) const;
