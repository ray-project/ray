...
class LocalNode : public Reporter {
 public:
  ...
  ray::rpc::syncer::RaySyncMessage Snapshot() const override {
    ...
    msg.set_sync_message(std::string(reinterpret_cast<const char*>(&state_), sizeof(state_)));
    msg.set_node_id(node_id_);
    RAY_LOG(INFO) << node_id_ << " generate snapshot " << "(" << state_ << ", v:" << version_ << ")";
    return msg;
  }
...
class RemoteNodes : public Receiver {
 public:
  ...
  void Update(ray::rpc::syncer::RaySyncMessage& msg) override {
    ...
    if(infos_.end() == iter) {
      RAY_LOG(INFO) << "Got a new node " << msg.node_id() << " with (" <<  state << ", v:" << version << ")";
...
int main(int argc, char *argv[]) {
  ...
  syncer.Register(ray::rpc::syncer::RayComponentId::RESOURCE_MANAGER, local_node.get(), remote_node.get());
  if(server_port != ".") {
  ...
  if(leader_port != ".") {
    channel = grpc::CreateChannel("localhost:" + leader_port, grpc::InsecureChannelCredentials());
