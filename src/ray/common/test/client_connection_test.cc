...
TEST_F(ClientConnectionTest, NongracefulDisconnect) {
  const uint8_t arr[5] = {1, 2, 3, 4, 5};
  int server_num_messages = 0;
  ...
  MessageHandler server_message_handler = [&arr, &server_num_messages](
    ...
    server_num_messages += 1;
    if (server_num_messages == 1) {
      ASSERT_FALSE(std::memcmp(arr, message_data.data(), 5));
    } else {
      // Expect non-graceful disconnect after first successful message.
      auto fb_message_type = static_cast<protocol::MessageType>(message_type);
      ASSERT_EQ(fb_message_type, protocol::MessageType::DisconnectClientRequest);

      auto message =
          flatbuffers::GetRoot<protocol::DisconnectClientRequest>(message_data.data());
      ASSERT_FALSE(message->graceful());
      auto disconnect_type = static_cast<rpc::WorkerExitType>(message->disconnect_type());
      ASSERT_EQ(disconnect_type, ray::rpc::WorkerExitType::SYSTEM_ERROR);
      const auto &disconnect_detail = message->disconnect_detail()->str();
      ASSERT_TRUE(disconnect_detail.size() > 0);
    }
  };
  ...
  // Shut down client side of the connection. The server should get EOF and receive
  // a non-graceful disconnect message.
  shutdown(client->GetNativeHandle(), SHUT_RDWR);
  server->ProcessMessages();
  io_service_.run();
  ASSERT_EQ(server_num_messages, 1);
}
