...
/**
    ...
    It expects that base_cgroup has cpu, memory controllers enabled.
...
class SysFsCgroupDriverIntegrationTest : public ::testing::Test {
 protected:
  ...
  static void SetUpTestSuite() {
    ...
    // 4) Enable cpu and memory controllers for the base_cgroup to allow testing_cgroup
    // to use those controllers. This is a NOOP if the controllers are already enabled.
    ...
    for (const auto &op : subtree_control_ops) {
      ...
      ASSERT_NE(base_cgroup_subtree_control_fd, -1)
          << "Failed to open base cgroup's subtree_control file at path "
          << base_cgroup_subtree_control_path << " with error " << strerror(errno);
      ssize_t num_written = write(base_cgroup_subtree_control_fd, op.c_str(), op.size());
      ASSERT_EQ(num_written, op.size())
          << "Failed to write to base cgroup's subtree_control file at path "
          << base_cgroup_subtree_control_path << " with error " << strerror(errno);
      close(base_cgroup_subtree_control_fd);

      // 5) Enable cpu and memory controllers for the testing_cgroup to allow tests to
      ...
      for (const auto &op : subtree_control_ops) {
        ...
        ssize_t num_written =
            write(test_cgroup_subtree_control_fd, op.c_str(), op.size());
        ASSERT_EQ(num_written, op.size())
    ...
    /**
      ...
      If cleanup fails, cgroup directories may not be cleaned up properly. However,
      despite cleanup failure, tests can be rerun on the same machine/container without
      interference from the previous failed attempt.

      There are a few types of leaks possible
        1) In CI, the tests run inside the container and all processes inside the
      container will have been moved into the <root_cgroup>/leaf/cgroup.procs file. They
      may stay there. This has no impact on subsequent runs because this cgroup should
      have no constriants. 2) In all environments, cgroups may be leaked and will have to
      be manually cleaned up. Typically, a cgroup cannot be removed iff it has a running
      process in the cgroup.procs file or child cgroups.
    ...
    static void TearDownTestSuite() {
      ...
      //<processes_cgroup>/cgroup.procs file. This will allow the leaf_cgroup to be
      // deleted.
      std::string processes_cgroup_procs_file_path =
          processes_cgroup_path_ + "/cgroup.procs";
      std::string leaf_cgroup_procs_file_path = leaf_cgroup_->GetPath() + "/cgroup.procs";
  ...
  TEST_F(SysFsCgroupDriverIntegrationTest,
    ...
    auto parent_cgroup_dir_or_status =
        TempCgroupDirectory::Create(testing_cgroup_->GetPath(), S_IRWXU);
    ASSERT_TRUE(parent_cgroup_dir_or_status.ok())
        << parent_cgroup_dir_or_status.ToString();
  ...
  TEST_F(SysFsCgroupDriverIntegrationTest,
    ...
    ASSERT_TRUE(nested_cgroup_dir_or_status.ok())
        << nested_cgroup_dir_or_status.ToString();
  ...
  TEST_F(SysFsCgroupDriverIntegrationTest,
    ...
    Status invalid_argument_s =
        driver.EnableController(child_cgroup_dir->GetPath(), "cpu");
