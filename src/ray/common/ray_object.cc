...
bool RayObject::IsException(rpc::ErrorType *error_type) const {
  return false;
// TODO(ekl) this is super slow, we shouldn't iterate over the entire proto enum looking for a matching code
...
bool RayObject::IsInPlasmaError() const {
  ...
  const std::string metadata(reinterpret_cast<const char *>(metadata_->Data()),
                             metadata_->Size());
  return metadata == "4";  // XXX cache this std::to_string(ray::rpc::ErrorType::OBJECT_IN_PLASMA);
}
