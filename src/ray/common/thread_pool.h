...
class CPUThreadPool {
 public:
  ...
  auto post(F&& f) {
    ...
    boost::asio::post(pool_, [f = std::move(f), p = std::move(promise)] {
      p.set_value(f());
    });
    return future;
  }
...
class IOThreadPool {
 public:
  IOThreadPool()
      : io_service_(std::make_shared<instrumented_io_context>()){
  ...
  instrumented_io_context& GetIOService() {
    return *io_service_;
  }
 private:
