...
db_handle *db_connect(const char *address,
  ...
  while (1) {
    ...
    freeReplyObject(reply);
    reply = redisCommand(context, "HMSET %s:%b address %s:%d client_id %b",
                         client_type, (char *) client.id, sizeof(client_id),
                         client_addr, client_port, (char *) client.id,
                         sizeof(client_id));
    freeReplyObject(reply);
    reply = redisCommand(context, "PUBLISH %s %b", client_type,
                         (char *) client.id, sizeof(client_id));
    freeReplyObject(reply);
...
void db_disconnect(db_handle *db) {
  ...
  HASH_ITER(hh, db->service_cache, e, tmp) {
    free(e->addr);
    HASH_DELETE(hh, db->service_cache, e);
    free(e);
  }
  free(db->client_type);
  void **p = NULL;
 ...
 * @return Void.
 */
void redis_get_cached_service(db_handle *db,
                              client_id client,
                              const char **manager) {
  service_cache_entry *entry;
  HASH_FIND(hh, db->service_cache, &client, sizeof(client_id), entry);
  if (!entry) {
    /* This is a very rare case. It should happen at most once per service. */
    redisReply *reply =
        redisCommand(db->sync_context, "HGET %s:%b address", db->client_type,
                     (char *) client.id, sizeof(client_id));
    CHECK(reply->type == REDIS_REPLY_STRING);
    entry = malloc(sizeof(service_cache_entry));
    entry->service_id = client;
    entry->addr = strdup(reply->str);
    HASH_ADD(hh, db->service_cache, service_id, sizeof(client_id), entry);
    freeReplyObject(reply);
  }
  *manager = entry->addr;
}
...
void redis_object_table_get_entry(redisAsyncContext *c,
  ...
  client_id *managers = malloc(reply->elements * sizeof(client_id));
  ...
  if (reply->type == REDIS_REPLY_ARRAY) {
    ...
    for (int j = 0; j < reply->elements; ++j) {
      ...
      memcpy(managers[j].id, reply->element[j]->str, sizeof(client_id));
...
/*
 *  ==== local scheduler table callbacks ====
 */
...
void redis_local_scheduler_table_subscribe_callback(redisAsyncContext *c,
  ...
  client_id client;
  memcpy(client.id, payload->str, payload->len);
  if (data->subscribe_callback) {
    data->subscribe_callback(client, data->subscribe_context);
  }
}
