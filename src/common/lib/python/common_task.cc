  ...
  for (Py_ssize_t i = 0; i < num_args; ++i) {
    ...
    if (isPyObjectIDType(arg)) {
      TaskSpec_args_add_ref(
          g_task_builder, &(reinterpret_cast<PyObjectID *>(arg))->object_id, 1);
    } else {
      PyObject *data = callMethodObjArgs(arg);
      TaskSpec_args_add_val(g_task_builder,
...
PyObject *NonRayletTask::to_py_string() {
...
void NonRayletTask::to_serialized_flatbuf(
    flatbuffers::FlatBufferBuilder &fbb,
    std::vector<ray::ObjectID> execution_dependencies) {
  throw std::runtime_error("Method not implemented");
}

void NonRayletTask::to_submit_message(
    flatbuffers::FlatBufferBuilder &fbb,
    const std::vector<ObjectID> &dependencies) {
  TaskExecutionSpec execution_spec(dependencies, spec, size);
...
RayletTask::RayletTask(
  ...
  task_spec.reset(new ray::raylet::TaskSpecification(
      ...
      function_id, args, num_returns, required_resources));
}

PyObject *RayletTask::to_py_string() {
  flatbuffers::FlatBufferBuilder fbb;
...
std::unordered_map<std::string, double> RayletTask::get_required_resources() {
...
void RayletTask::to_serialized_flatbuf(
    flatbuffers::FlatBufferBuilder &fbb,
    std::vector<ray::ObjectID> execution_dependencies) {
  ray::raylet::TaskExecutionSpecification execution_spec(
...
void RayletTask::to_submit_message(
    flatbuffers::FlatBufferBuilder &fbb,
