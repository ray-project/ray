...
void photon_submit(photon_conn *conn, task_spec *task) {
  write_message(conn->conn, 0, SUBMIT_TASK, task_spec_size(task),
                (uint8_t *) task);
}

task_spec *photon_get_task(photon_conn *conn) {
  write_message(conn->conn, 0, GET_TASK, 0, NULL);
  int64_t type;
  int64_t length;
  uint8_t *message;
  /* Receive a task from the local scheduler. This will block until the local
   * scheduler gives this client a task. */
  read_message(conn->conn, 0, &type, &length, &message);
...
void photon_task_done(photon_conn *conn) {
  write_message(conn->conn, 0, TASK_DONE, 0, NULL);
}

void photon_disconnect(photon_conn *conn) {
  write_message(conn->conn, 0, DISCONNECT_CLIENT, 0, NULL);
}

void photon_log_message(photon_conn *conn) {
  write_message(conn->conn, 0, LOG_MESSAGE, 0, NULL);
}
