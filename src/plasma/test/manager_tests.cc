...
TEST request_transfer_test(void) {
  ...
  ARROW_CHECK_OK(PlasmaReceive(read_fd, MessageType_PlasmaDataRequest, request_data));
  ObjectID object_id2;
  char *address;
  int port;
  ARROW_CHECK_OK(ReadDataRequest(request_data.data(), &object_id2, &address, &port));
