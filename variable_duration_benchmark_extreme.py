...
LOG_FILE = "variable_duration_benchmark_extreme.log"
...
def main(is_flink: bool):
    ...
    def produce(batch):
        ...
        if int(batch["id"][0].item()) < NUM_ROWS_TOTAL / 2:
            time.sleep(TIME_UNIT * len(batch["id"]) * 8)
        else:
            time.sleep(TIME_UNIT * len(batch["id"]))
        for id in batch["id"]:
    ...
    timeline_utils.save_timeline(f"timeline_{'ray' if not is_flink else 'flink'}_variable_extreme.json")
    ray.shutdown()
