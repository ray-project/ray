...
std::shared_ptr<boost::dll::shared_library> FunctionHelper::LoadDll(
  ...
  if (!boost::filesystem::exists(absolute_path)) {
    CPP_LOG(WARNING) << lib_name << " dll not found, absolute_path: " << absolute_path
  ...
  try {
    ...
    CPP_LOG(WARNING) << "Load library failed, lib_name: " << lib_name
...
FunctionHelper::GetEntryFunction(const std::string &lib_name) {
  ...
  try {
    ...
    CPP_LOG(WARNING) << "Get execute function failed, lib_name: " << lib_name
                     << ", failed reason: " << e.what();
  } catch (...) {
    CPP_LOG(WARNING) << "Get execute function, lib_name: " << lib_name
                     << ", unknown failed reason.";
  }

  CPP_LOG(WARNING) << "Can't get execute function, lib_name: " << lib_name;
  return nullptr;
}
