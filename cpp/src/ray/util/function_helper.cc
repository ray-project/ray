...
static const char *first_external_symbol_for_image(const mach_header_t *header) {
  Dl_info info;
  if (dladdr(header, &info) == 0) return NULL;
  ...
  if ((seg_text == NULL) || (seg_linkedit == NULL) || (symtab == NULL)) return NULL;

  intptr_t file_slide = ((intptr_t)seg_linkedit->vmaddr - (intptr_t)seg_text->vmaddr) -
  ...
  for (uint32_t i = 0; i < symtab->nsyms; i++, sym++) {
    if ((sym->n_type & N_EXT) != N_EXT || !sym->n_value) continue;
...
static const uintptr_t BaseAddressForHandle(void *handle) {
  for (int32_t i = _dyld_image_count(); i >= 0; i--) {
    const char *first_symbol =
        first_external_symbol_for_image((const mach_header_t *)_dyld_get_image_header(i));
