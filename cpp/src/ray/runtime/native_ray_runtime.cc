...
NativeRayRuntime::NativeRayRuntime(std::shared_ptr<RayConfig> config) {
  config_ = config;
  worker_ =
      std::unique_ptr<WorkerContext>(new WorkerContext(WorkerType::DRIVER, WorkerID::Nil(), JobID::Nil()));
  ...
  //task_executor_ = std::unique_ptr<TaskSubmitter>(new NativeTaskExecutor(*this));
}
