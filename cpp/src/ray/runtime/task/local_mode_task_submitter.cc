...
std::list<std::unique_ptr<UniqueId> > LocalModeTaskSubmitter::buildReturnIds(const UniqueId &taskId,
                                                                int returnCount) {
...
std::unique_ptr<UniqueId> LocalModeTaskSubmitter::submit(const InvocationSpec &invocation, TaskType type) {
  ...
  boost::asio::post(*_pool.get(), std::bind([actor](std::unique_ptr<TaskSpec>& ts)
    {
...
std::unique_ptr<UniqueId> LocalModeTaskSubmitter::submitTask(const InvocationSpec &invocation) {
  return submit(invocation, TaskType::NORMAL_TASK);
}

std::unique_ptr<UniqueId> LocalModeTaskSubmitter::createActor(remote_function_ptr_holder &fptr, 
...
std::unique_ptr<UniqueId> LocalModeTaskSubmitter::submitActorTask(const InvocationSpec &invocation) {
