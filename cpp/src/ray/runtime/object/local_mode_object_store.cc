...
void LocalModeObjectStore::PutRaw(
...
std::shared_ptr<msgpack::sbuffer> LocalModeObjectStore::GetRaw(
    const ObjectID &object_id,
    int timeout_ms) {
...
std::vector<std::shared_ptr<msgpack::sbuffer>> LocalModeObjectStore::GetRaw(
  ...
  ::ray::Status status = memory_store_->Get(
      ids,
      (int)ids.size(),
      timeout_ms,
...
std::vector<bool> LocalModeObjectStore::Wait(
    const std::vector<ObjectID> &ids,
    int num_objects,
    int timeout_ms) {
  ...
  ::ray::Status status = memory_store_->Wait(
      memory_object_ids,
      num_objects,
      timeout_ms,
      local_mode_ray_tuntime_.GetWorkerContext(),
