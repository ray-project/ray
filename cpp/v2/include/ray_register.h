...
namespace ray {
template <typename OUT, typename IN> inline static OUT address_of(IN in) {
  union {
    IN in;
    OUT out;
  } u = {in};
...
template <typename F> static inline auto get_function(F f) {
...
template <typename F> static inline absl::string_view get_function_name(F f) {
...
template <typename S, typename T> inline static bool register_func(S s, T t) {
...
#define RAY_REGISTER(...) \
static auto ANONYMOUS_VARIABLE(var) = ray::register_functions(std::array<absl::string_view, GET_ARG_COUNT(__VA_ARGS__)>{MARCO_EXPAND(MACRO_CONCAT(CON_STR, GET_ARG_COUNT(__VA_ARGS__))(__VA_ARGS__))}, __VA_ARGS__);
