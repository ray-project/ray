...
When a code snippet fails in CI, two questions decide what to do about it: what kind of failure it is, and what the example was protecting.
...
- **A Sphinx warning failed the build.** Ray builds the docs with warnings treated as errors, so a malformed directive or a broken cross-reference fails the build even when your code is correct. This is a markup problem, not a code failure.
...
- **A breaking-change detector.** The snippet is user-facing code that broke because the library's behavior changed. Treat the failure as a real signal: either don't land the change, or land it with breaking-change communication. Don't silently rewrite the example to match the new behavior, because that hides the break from the users who copy it.
