...
When an API is declared, Ray promises that
users can use these APIs to develop their
...
Ray exposes APIs to users. If Ray provides incorrect
information, it can significantly impact the reliability
and maintainability of user applications. Based on
...
so caution is needed when moving APIs between different
...
.. list-table:: API Lifecycle Policy
    ...
    * - Can this API be demoted to a lower level? If so then how?
      ...
      - Yes
      - No annotations mean it's a developer API by default
    * - Can you remove or change this API's parameters?
      - Yes. The API should emit warning messages and you must set a deadline for the end-of-life of the original version that's **6 months or +25 Ray minor versions**. During the transition period, you must support both the new and old parameters.
