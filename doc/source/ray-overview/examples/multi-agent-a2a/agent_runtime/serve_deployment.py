...
# Convenience function for creating deployment classes (for multi-app configs)
def create_deployment_class(
    build_agent_fn: AgentBuilder,
    *,
    name: str = "LangGraphServeDeployment",
    title: str = "LangGraph Agent",
    description: str = "LangGraph agent with streaming SSE support",
    num_cpus: float = 1,
):
    """
    Create a Ray Serve deployment class (not bound) for use in multi-app configurations.

    This is useful when you need to create multiple deployment classes that will
    be bound separately (e.g., in a YAML config or multi-app deployment).
    ...
    Returns:
        Ray Serve deployment class (call .bind() to create the app)
    """
    fastapi_app = create_chat_app(
        build_agent_fn,
        title=title,
