...
class Dataset_ETT_hour(Dataset):
    def __init__(self, flag="train", size=None, features="S", target="OT", scale=True, train_only=False, smoke_test=False):
        ...
        assert flag in ["train", "test", "val"], "flag must be one of 'train', 'test', or 'val'"
    ...
    def __read_and_preprocess_data__(self):
        ...
        if self.is_smoke_test:
            ...
            split_start_indices = [0, max(0, num_train - self.encoder_seq_len), max(0, num_train + num_val - self.encoder_seq_len)]
            # Define end indices for each split
            split_end_indices = [num_train, num_train + num_val, num_train + num_val + num_test]
            ...
            split_start_indices = [0, 0, 0]  # Or consider num_train, num_train for val/test starts
            ...
            num_val = 4 * 30 * 24
            num_test = 4 * 30 * 24
            split_start_indices = [0, num_train - self.encoder_seq_len, num_train + num_val - self.encoder_seq_len]
            split_end_indices = [num_train, num_train + num_val, num_train + num_val + num_test]
        ...
        if self.enable_scaling:
            ...
            cols_for_scaler = raw_df.columns[1:] if self.features_type != "S" else [self.target_column]
            scaler_fitting_data = raw_df[cols_for_scaler][train_data_for_scaler_start:train_data_for_scaler_end]
        ...
        self.timeseries_data_for_inputs = processed_data[current_split_start_idx:current_split_end_idx]
        self.timeseries_data_for_targets = processed_data[current_split_start_idx:current_split_end_idx]
    ...
    def __getitem__(self, index):
        ...
        max_valid_start_index = len(self.timeseries_data_for_inputs) - self.encoder_seq_len - self.prediction_horizon
        ...
        encoder_input_sequence = self.timeseries_data_for_inputs[encoder_input_start_idx:encoder_input_end_idx]
        ...
        target_sequence_end_idx = decoder_context_start_idx + self.decoder_context_len + self.prediction_horizon

        target_sequence = self.timeseries_data_for_targets[decoder_context_start_idx:target_sequence_end_idx]
    ...
    def __len__(self):
        ...
        if len(self.timeseries_data_for_inputs) <= self.encoder_seq_len + self.prediction_horizon - 1:
            return 0  # Not enough data to form even one sample
        return len(self.timeseries_data_for_inputs) - self.encoder_seq_len - self.prediction_horizon + 1
