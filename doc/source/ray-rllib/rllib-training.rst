...
.. note::
    ...
    It's recommended that you run RLlib trainers with :doc:`Tune <../ray-tune/index>`, for easy experiment management and visualization of results. Just set ``"run": ALG_NAME, "env": ENV_NAME`` in the experiment config.

All RLlib trainers are compatible with the :ref:`Tune API <tune-60-seconds>`. This enables them to be easily used in experiments with :doc:`Tune <../ray-tune/index>`. For example, the following code performs a simple hyperparam sweep of PPO:
