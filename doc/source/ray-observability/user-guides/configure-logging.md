...
- ``dashboard.[log|err]``: The log file for old Ray Dashboard single-process modules. ``.log`` files contain logs generated from the dashboard's logger. ``.err`` files contain stdout and stderr printed from the dashboard. They are usually empty except when the dashboard crashes unexpectedly.
- ``dashboard_agent.log``: Every Ray node has one dashboard agent. This is a log file of the agent.
- ``dashboard-[module_name]-[incarnation].log``: The log files for the new Ray Dashboard subprocess modules. Incarnation is the number of times that the module has been restarted, starting from 0.
