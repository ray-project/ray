...
.. list-table:: Ray System Metrics
   ...
   * - `ray_actors`
     - `Name`, `State`
     - Current number of actors in one of IDLE, RUNNING states. The actor class name is available in the Name label.
   * - `ray_actor_tasks`
     - `Name`, `State`
     - Current number of running tasks per actor in one of EXECUTING, RUNNING_IN_GET, RUNNING_IN_WAIT states. The actor class name is available in the Name label.
   * - `ray_resources`
