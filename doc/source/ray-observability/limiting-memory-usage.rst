...
The application may consume a large amount of heap space and causes the node to run out of memory. When that happens, the operating system will start killing Ray worker or raylet processes, which will cause unexpected behaviors to occur. For example, it may stall the :ref:`dashboard <ray-dashboard>` or other control processes and causes Ray to fail.
...
If the combined usage exceeds a configurable threshold, the raylet will kill task or actor to free up memory and prevent Ray from failing.
...
The memory monitor can be configured by the following variables:
