...
The :func:`ray.get <ray.get>` function can also be used as usual to retrieve the result of an RDT object, via Ray's object store. The difference is that the tensor transport for :func:`ray.get <ray.get>` will be the one specified in the :func:`@ray.method <ray.method>` decorator by default.
Therefore, users need to specify the correct tensor transport explicitly set ``_tensor_transport`` in :func:`ray.get <ray.get>`.
