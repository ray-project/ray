...
Therefore, users need to specify the Ray object store as the tensor transport explicitly by using ``GetTensorOptions(use_object_store=True)`` in :func:`ray.get <ray.get>`.
