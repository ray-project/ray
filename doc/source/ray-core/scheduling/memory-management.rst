...
Application memory: this is memory used by your application
  ...
  - **Object store memory**: memory used when your application creates objects in the object store via ``ray.put`` and when returning values from remote functions. Objects are reference counted and evicted when they fall out of scope. There is an object store server running on each node. By default, 30% of available memory when starting a Ray instance is reserved to ``/dev/shm`` (shared memory) in Linux. In MacOS, it uses ``/tmp``, which can impact the performance compared to Linux (because ``/dev/shm`` uses the shared memory, but ``/tmp`` uses the disk). In Ray 1.3+, objects will be :ref:`spilled to disk <object-spilling>` if the object store fills up.
