...
When ``.bind()`` is called on a ``ray.remote`` decorated class or function, it
generates an intermediate representation (IR) node that act as backbone and
building blocks of the DAG that's statically holding the computation graph
...
executed as a Ray Task upon execution which is solved to the task output.
...
of the DAG, where all other non-reachable nodes from the root are ignored.
...
executed as a Ray Actor upon execution. The Actor is instantiated every
