...
**Summary:** Don't use global variables to share state with tasks and actors. Instead, encapsulate the global variables in an actor and pass the actor handle to other tasks and actors.
...
in one process, changes aren't reflected in other processes.
...
Note that using class variables to manage state between instances of the same class isn't supported.
Each actor instance is instantiated in its own process, so each actor has its own copy of the class variables.
