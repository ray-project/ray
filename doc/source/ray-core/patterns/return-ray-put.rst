...
**Summary:** Avoid calling :func:`ray.put() <ray.put>` on task return values and returning the resulting ObjectRefs.
...
- It disallows inlining small return values: Ray has a performance optimization to return small (<= 100KB) values inline directly to the caller, avoiding going through the distributed object store.
  On the other hand, ``ray.put()`` unconditionally stores the value to the object store which makes the optimization for small return values impossible.
