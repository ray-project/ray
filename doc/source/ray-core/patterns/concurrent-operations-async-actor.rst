...
This pattern uses ``await`` to yield control from the long running method call so other method calls can run concurrently.
...
With the default actor, the code looks like this:
...
You can solve this problem by using :ref:`async actors <async-actors>` and use ``await`` to yield control:
...
Here, instead of using the blocking :func:`ray.get() <ray.get>` to get the value of an ObjectRef, the code uses ``await`` so it can yield control while waiting for the object to be fetched.
