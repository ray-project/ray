...
This pattern simplifies the driver since it manages only a few supervisors and doesn't deal with failures from worker actors directly.
...
For each hyperparameter, you can launch a supervisor actor to do the orchestration and it creates worker actors to do the actual training per data shard.
