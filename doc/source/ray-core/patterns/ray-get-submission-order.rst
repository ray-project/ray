...
**TLDR:** Avoid processing independent results in submission order using :meth:`ray.get() <ray.get>` since results may be ready in a different order than the submission order.
...
Instead, we want to process the tasks in the order that they finish using :meth:`ray.wait() <ray.wait>` to speed up total time to completion.
