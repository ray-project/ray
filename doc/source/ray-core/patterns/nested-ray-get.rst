...
If all cores are already occupied, this can lead to a deadlock, as the task that produce the ``ObjectRef``'s value may need the caller task's resources in order to run.
