...
Instead you should get and process the results one batch at a time. Once a batch is processed, Ray evicts objects in that batch to make space for future batches.
...
Here besides getting one batch at a time to avoid failure, the code also uses ``ray.wait()`` to process results in the finish order instead of the submission order to reduce the runtime. See :doc:`ray-get-submission-order` for more details.
