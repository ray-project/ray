...
This pattern uses **generators** in Python to reduce the total heap memory usage during a task. The key idea is that for tasks that return multiple objects, you can return them one at a time instead of all at once. This allows a worker to free the heap memory used by a previous return value before returning the next one.
...
Using normal Python functions, you can write such a task like this. The following example returns numpy arrays of size 100 MB each:
...
However, this requires the task to hold all ``num_returns`` arrays in heap memory at the same time at the end of the task. If there are many return values, this can lead to high heap memory usage and potentially an out-of-memory error.

You can fix the preceding example by rewriting ``large_values`` as a **generator**. Instead of returning all values at once as a tuple or list, you can ``yield`` one value at a time.
