...
is pickled and shipped to the workers, and so if ``f`` hasn't been
defined yet, the definition is incomplete.

Yielding Resources While Blocked
--------------------------------

Ray releases CPU resources when being blocked. This prevents
...
When a ``g`` task is executing, it releases its CPU resources when it gets
blocked in the call to ``ray.get``. It reacquires the CPU resources when
``ray.get`` returns. It retains its GPU resources throughout the lifetime of
the task because the task most likely continues to use GPU memory.
