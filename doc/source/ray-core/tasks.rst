...
finished executing without blocking on all of them. This could be achieved by :meth:`ray.wait() <ray.wait>`. The function
works as follows.
...
Ray tasks can be canceled by calling :meth:`ray.cancel() <ray.cancel>` on the returned Object ref.
...
in :meth:`ray.remote() <ray.remote>` and :meth:`.options() <ray.remote_function.RemoteFunction.options>`.
