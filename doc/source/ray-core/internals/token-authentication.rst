...
For Ray clusters running on Kubernetes, Ray can leverage Kubernetes RBAC to handle authentication
and authorization. Kubernetes environments typically provide identity through Service Accounts or
integrations such as webhook token authentication or OpenID Connect tokens.

To enable Kubernetes-based authentication and authorization, set **RAY_ENABLE_K8S_TOKEN_AUTH**
in addition to ``RAY_AUTH_MODE=token``. When enabled, Ray will not validate tokens against a
single static token, but rather make two API calls to the Kubernetes API server:

1. **TokenReview**: Authenticates a token to a known user.
2. **SubjectAccessReview**: Checks whether a user or group can perform an action.

TokenReview
~~~~~~~~~~~

Upon receiving a request containing a token, Ray validates tokens by calling the ``TokenReview``
API. All ``TokenReview`` requests from Ray specify the ``ray.io`` audience to prevent token
misuse. Example request:
...
If the token is from an authenticated user, Ray receives a response with the following status:
...
After authentication, Ray uses the user/group information from the ``TokenReview`` status for
authorization via ``SubjectAccessReview``:
...
By default, the identity of the Raylet is bound to the Service Account token of the Pod. However,
the Raylet does not use the default token in ``/var/run/kubernetes.io/serviceaccount/token``.
Instead, a dedicated token at path ``/var/run/ray.io/serviceaccount/token`` is mounted using
`serviceAccountToken projected volumes <https://kubernetes.io/docs/concepts/storage/projected-volumes/#serviceaccounttoken>`_
with a token audience dedicated for Ray:
...
Most managed Kubernetes platforms provide integrations with external IAM systems. For example,
on GKE, Google Cloud IAM and Kubernetes RBAC are integrated to authorize users.

When Ray uses Kubernetes for authn/authz, users can connect to clusters using their external
identities. For example, on GKE you can set **RAY_AUTH_TOKEN** to a token from ``gcloud``:

.. code-block:: bash

   export RAY_AUTH_TOKEN=$(gcloud auth print-identity-token \
       --audiences=ray.io \
...
When enabled, KubeRay creates a ``RoleBinding`` that binds the Service Account used in the
``RayCluster`` to a ``ClusterRole`` granting the verb ``ray:write`` to all ``RayClusters``
in the namespace.

The KubeRay helm chart includes a default ``ClusterRole`` that grants access to the ``ray:write``
custom verb:
...
The helm chart also contains a ``ClusterRoleBinding`` for the KubeRay operator service account,
granting it access to every RayCluster. This is required for ``RayJob`` and ``RayService``
custom resources where KubeRay needs authenticated access to Ray clusters.
