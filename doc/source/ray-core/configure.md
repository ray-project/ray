...
Ray uses [Typesafe Config](https://lightbend.github.io/config/) to read options. There are several ways to set options:
...
- A [HOCON format](https://github.com/lightbend/config/blob/master/HOCON.md) configuration file. By default, Ray will try to read the file named `ray.conf` in the root of the classpath. You can customize the location of the file by setting system property `ray.config-file` to the path of the file.
