...
-  **MongoDB connector**: We use `PyMongo <https://pymongo.readthedocs.io/en/stable/>`__ to connect to MongDB.
...
-  **Parallel execution**: We ask the user to provide a list of MongoDB queries, with each corresponding to a shard (i.e. a :class:`~ray.data.ReadTask`) that can be executed in parallel.
...
list of MongDB queries. Each :class:`~ray.data.ReadTask` returns a list of blocks when called, and 
