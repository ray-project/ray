...
.. tabbed:: "Callable Class UDFs"
  ...
  *callable classes*, i.e., classes that implement the ``__call__`` magic method. You
  ...
  Callable classes are useful if you need to load an expensive state (such as a neural network) for the UDF. By using an actor class, the you only need to load the state once in the beginning, rather than for each batch.
