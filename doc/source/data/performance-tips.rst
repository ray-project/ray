...
You can view debug stats for your Dataset and DatasetPipeline executions via :meth:`ds.stats() <ray.data.Datastream.stats>`.
...
These operations include :meth:`Dataset.random_shuffle <ray.data.Datastream.random_shuffle>`,
:meth:`Dataset.sort <ray.data.Datastream.sort>` and :meth:`Dataset.groupby <ray.data.Datastream.groupby>`.
...
To try this out locally or on a cluster, you can start with the `nightly release test <https://github.com/ray-project/ray/blob/master/release/nightly_tests/dataset/sort.py>`_ that Ray runs for :meth:`Dataset.random_shuffle <ray.data.Datastream.random_shuffle>` and :meth:`Dataset.sort <ray.data.Datastream.sort>`.
To get an idea of the performance you can expect, here are some run time results for :meth:`Dataset.random_shuffle <ray.data.Datastream.random_shuffle>` on 1-10TB of data on 20 machines (m5.4xlarge instances on AWS EC2, each with 16 vCPUs, 64GB RAM).
