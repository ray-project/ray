...
For transformations that require setup, specify ``compute=ray.data.ActorPool(min, max)`` and Ray will use an autoscaling actor pool of ``min`` to ``max`` actors to execute your transforms.
For a fixed-size actor pool, specify ``ActorPool(n, n)``.
The following is an end-to-end example of reading, transforming, and saving batch inference results using Ray Data:

.. code-block:: python

    from ray.data import ActorPool
    ...
    ds = ds.map_batches(BatchInferModel, compute=ActorPool(10, 20), batch_size=256, num_gpus=1)
