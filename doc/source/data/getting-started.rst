...
For transformations that require setup, specify ``compute="actors[n]"`` and Ray will use an actor pool of size `n` to execute your transforms instead.
For an autoscaling actor pool, specify ``compute="actors"``.
...
.. code-block:: python
    ...
    ds = ds.map_batches(BatchInferModel, compute="actors[20]", batch_size=256, num_gpus=1)
