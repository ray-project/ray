...
Data loading is the process of importing data from various sources into Ray Data for processing. This guide covers loading data from files, databases, cloud storage, and in-memory sources.

What is data loading?
---------------------

Data loading is the foundation of any data processing workflow. Ray Data provides unified, high-performance loading capabilities that eliminate the complexity of working with multiple data source types.

**Core loading capabilities:**

* **File reading**: Import from 15+ file formats including Parquet, CSV, JSON, images, audio, and video
* **Database connectivity**: Native connectors for SQL databases, data warehouses, and NoSQL systems
* **Cloud storage access**: Optimized readers for S3, GCS, Azure Blob Storage with automatic credential handling
* **In-memory loading**: Efficient conversion from pandas DataFrames, NumPy arrays, and other Python objects
* **Enterprise integration**: Connect to legacy systems, APIs, and modern data platforms

**Business value:** Unified data loading reduces infrastructure complexity, eliminates custom connector development, and provides consistent performance across all data sources.

Why use Ray Data for loading?
-----------------------------

Based on codebase analysis, Ray Data provides verified technical advantages for data loading:

* **Comprehensive format support**: 26 verified native connectors including Parquet, CSV, JSON, Images, Audio, Video, SQL databases, Snowflake, BigQuery, MongoDB, Delta Lake, Iceberg, Hudi, Unity Catalog, and more
* **Intelligent distributed loading**: Automatic parallelization with configurable `override_num_blocks` and `concurrency` parameters for optimal resource utilization
* **Advanced cloud integration**: Native support for all major cloud storage platforms with automatic credential handling and optimized buffering (`streaming_read_buffer_size` = 32MB default)
* **Performance optimization**: Built-in predicate pushdown for Parquet/databases, column pruning, intelligent block sizing (128MB default), and metadata-based optimization
* **Robust schema handling**: Automatic schema inference with `enable_pandas_block` support and configurable type casting
* **Enterprise-grade error handling**: Configurable retry policies (`retried_io_errors`), partial failure tolerance (`max_errored_blocks`), and comprehensive error reporting

How to load data with Ray Data
------------------------------

Follow this approach for efficient data loading:

1. **Choose data source**: Select the appropriate read function for your data source
2. **Configure connection**: Set up authentication and connection parameters
3. **Optimize performance**: Use column pruning and filtering for better performance
4. **Handle errors**: Implement error handling for production reliability
5. **Validate schema**: Verify data schema and types after loading

This guide shows you how to:

* `Read files <#reading-files>`_ like images, documents, and tabular data
* `Load in-memory data <#loading-data-from-other-libraries>`_ like pandas DataFrames
* `Read databases <#reading-databases>`_ like MySQL, PostgreSQL, and data warehouses
...
.. tab-set::
    ...
    .. tab-item:: Spark
        ...
        ..
            Note: This code snippet demonstrates the concept but should be tested in your specific environment.
...
Always validate your data loading results to ensure data integrity:

.. code-block:: python

    # Load data with validation
    dataset = ray.data.read_parquet("s3://data-source/")
    
    # Validate loading completed successfully
    record_count = dataset.count()
    print(f"Loaded {record_count} records successfully")
...
**Loading Best Practices Checklist:**
- [ ] **Validate record counts**: Ensure expected number of records loaded
- [ ] **Check schema**: Verify column names and data types match expectations
- [ ] **Handle missing data**: Plan for missing files or connection failures
- [ ] **Monitor performance**: Track loading times and resource usage
- [ ] **Test with sample data**: Validate loading logic with small datasets first

Next Steps: Transform and Process Your Data
-------------------------------------------

Now that you understand how to load data into Ray Data, continue your journey based on your goals:

**Transform Your Data**
Learn how to process and modify your loaded data:

* :ref:`Transforming Data <transforming_data>` - Apply business logic and data cleaning
* :ref:`Joining Data <joining-data>` - Combine datasets for comprehensive analysis
* :ref:`Aggregations <aggregations>` - Calculate metrics and summary statistics

**Use Case-Specific Workflows**
Apply data loading to specific business scenarios:

* **ETL Workflows**: Build data pipelines → :ref:`ETL Pipeline Guide <etl-pipelines>`
* **Business Intelligence**: Create analytics → :ref:`Business Intelligence Guide <business-intelligence>`
* **AI/ML Workflows**: Prepare training data → :ref:`Working with AI <working-with-ai>`

**Advanced Data Loading**
Explore specialized data loading patterns:

* **Enterprise Integration**: Connect legacy systems → :ref:`Enterprise Integration <enterprise-integration>`
* **Performance Optimization**: Optimize loading performance → :ref:`Performance Optimization <performance-optimization>`
* **Custom Data Sources**: Build custom connectors → :ref:`custom-datasource-example`

**Production Deployment**
Prepare for production data loading:

* **Best Practices**: Production-ready patterns → :ref:`Best Practices <best_practices>`
* **Monitoring**: Track loading performance → :ref:`Monitoring Your Workload <monitoring-your-workload>`
* **Error Handling**: Handle loading failures → :ref:`Fault Tolerance <fault-tolerance>`
