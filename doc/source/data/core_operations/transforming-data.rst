...
Use transformations to process and modify your dataset for ETL, data cleaning, enrichment, and business logic. You can compose multiple transformations to create complex data processing workflows.
...
Transform individual rows with map
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Use :meth:`~ray.data.Dataset.map` when you need to transform each row individually with a one-to-one mapping. This is ideal for simple transformations that don't require batch-level computations.

**When to use `map`:**
- Simple field additions or modifications
- Row-level business logic that varies by record
- Transformations that don't benefit from vectorization
...
.. testcode::
    ...
    def parse_filename(row: Dict[str, Any]) -> Dict[str, Any]:
        """Extract filename from full path for easier reference."""
        row["filename"] = os.path.basename(row["path"])
        return row

    # Load images with file paths included
    ds = ray.data.read_images(
        "s3://anonymous@ray-example-data/image-datasets/simple", 
        include_paths=True
    )
    
    # Apply filename extraction to each row
    ds_with_filenames = ds.map(parse_filename)

**Why this approach works:** Filename extraction is a simple string operation that doesn't require batch processing or GPU resources. Using `map` provides optimal performance for this row-level transformation.
...
Use :meth:`~ray.data.Dataset.map_batches` for vectorized operations that can process multiple rows simultaneously. This approach provides significant performance benefits for mathematical operations, statistical computations, and operations that benefit from batch processing.

**When to use `map_batches`:**
- Vectorized NumPy or pandas operations
- Statistical computations across multiple rows
- Operations that benefit from GPU acceleration
- Transformations that can process data in parallel

**Performance benefit:** `map_batches` can be 5-10x faster than `map` for vectorized operations (like NumPy array processing) because it processes multiple rows simultaneously and reduces function call overhead. Performance improvement depends on operation type and data characteristics.
...
.. testcode::
    ...
    def increase_brightness(batch: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        """Increase image brightness using vectorized operations."""
        # Apply brightness adjustment to entire batch simultaneously
        batch["image"] = np.clip(batch["image"] + 4, 0, 255)
        return batch

    # Load images and apply batch transformation
    ds = ray.data.read_images("s3://anonymous@ray-example-data/image-datasets/simple")
    brightened_images = ds.map_batches(increase_brightness)

**Why this works better:** Image brightness adjustment is a vectorized operation that benefits from processing multiple images simultaneously. The `np.clip` operation can process entire batches efficiently, reducing computation time significantly compared to row-by-row processing.
...
.. code-block:: python
    ...
    brightened_images = ds.map_batches(increase_brightness)
    result_count = brightened_images.count()  # Trigger execution
    end_time = time.time()
    
    print(f"Processed {result_count} images in {end_time - start_time:.2f} seconds")
    
    # View execution statistics
    print(brightened_images.stats())

**Performance optimization tips:**
- **Batch size**: Use larger batches (64-128 images) for better GPU utilization
- **Actor pool**: Increase pool size for more parallelism with sufficient GPU memory
- **Memory monitoring**: Track GPU memory usage to avoid out-of-memory errors
