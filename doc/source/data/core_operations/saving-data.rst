...
Save your processed data to any destination with Ray Data's flexible output capabilities.

This guide shows you how to:

* `Write data to files <#writing-data-to-files>`_ - Save processed results for business use
* `Convert Datasets to other Python libraries <#converting-datasets-to-other-python-libraries>`_ - Integrate with existing tools
...
To save your :class:`~ray.data.dataset.Dataset` to cloud storage, first authenticate all nodes with your cloud service provider. Then call a method like :meth:`Dataset.write_parquet <ray.data.Dataset.write_parquet>` and specify a URI with the appropriate scheme. The URI can point to buckets or folders.
