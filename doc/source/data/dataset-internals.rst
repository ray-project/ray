...
Certain types of intermediate data must fit in memory. When a task has multiple input blocks, only one input block needs to fit in memory at any given time. When a task has multiple output blocks, before Ray 2.2 all output blocks need to fit in memory. After Ray 2.2 with feature of "dynamic blocks splitting", only one output block needs to fit in memory at any given time. Under the hood, Ray Datasets uses :ref:`remote generators <generators>` to put output blocks in the object store one by one, without holding all blocks in memory. Dynamic blocks splitting is enabled by default after Ray 2.2. It supports Ray task, and is not yet supported for Ray actor and Ray client. It support one-to-one :ref:`transformations <transform_datasets_transformations>` (such as ``ray.data.read_*()`` and :meth:`ds.map_batches() <ray.data.Dataset.map_batches>`), and is not yet supported for all-to-all transformations (such as :meth:`ds.random_shuffle() <ray.data.Dataset.random_shuffle>`).
