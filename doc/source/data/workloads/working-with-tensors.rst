...
**Navigation:** :ref:`Ray Data <data>` → :ref:`User Guides <data_user_guide>` → Working with Tensors

**Learning Path:** This guide covers tensor and NumPy array processing. After completing this guide, explore :ref:`Working with Images <working-with-images>` for computer vision or :ref:`Working with AI <working-with-ai>` for ML workflows.

N-dimensional arrays (tensors) are fundamental to ML workloads and scientific computing. This guide describes Ray Data's tensor support, limitations, and best practices for working with NumPy arrays and tensor data at scale.
...
.. testcode::
    ...
    # Load image dataset with variable-shaped tensors
    ds = ray.data.read_images("s3://anonymous@air-example-data/AnimalDetection")

    def increase_brightness(row: Dict[str, Any]) -> Dict[str, Any]:
        """Increase image brightness using row-level processing."""
        # Add brightness offset and clip to valid pixel range (0-255)
        row["image"] = np.clip(row["image"] + 4, 0, 255)
        return row

    # Apply transformation row by row (slower but simple)
    brightened_rows = ds.map(increase_brightness)

    def batch_increase_brightness(batch: Dict[str, np.ndarray]) -> Dict:
        """Increase image brightness using vectorized batch processing."""
        # Vectorized operation for better performance
        batch["image"] = np.clip(batch["image"] + 4, 0, 255)
        return batch

    # Apply transformation in batches (faster, recommended approach)
    brightened_batches = ds.map_batches(batch_increase_brightness)
