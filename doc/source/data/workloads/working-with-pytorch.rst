...
Ray Data enhances PyTorch workflows by providing scalable data processing capabilities that complement PyTorch's machine learning framework. While Ray Data works with any ML framework, this guide focuses on PyTorch-specific patterns.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
Ray Data integrates seamlessly with :ref:`Ray Train <train-docs>` for distributed model training workflows. This integration provides easy data ingest for data parallel training, with support for PyTorch, PyTorch Lightning, and Hugging Face training.

**Key integration benefits:**
- **Unified workflow**: Data preprocessing with Ray Data flows directly into Ray Train
- **Resource optimization**: Shared cluster resources between data processing and training
- **Fault tolerance**: Distributed training with automatic failure recovery
- **Scaling efficiency**: Linear scaling from single-node to multi-node training

For complete Ray Train documentation, see :ref:`Ray Train User Guide <train-user-guide>`.
...
Ray Data integrates seamlessly with PyTorch's built-in transforms from ``torchvision``, ``torchtext``, and ``torchaudio``. This integration allows you to use familiar PyTorch preprocessing within Ray Data's distributed processing framework.

**Integration benefits:**
- **Familiar API**: Use the same transforms you know from PyTorch
- **Distributed processing**: Apply transforms across multiple nodes automatically
- **GPU acceleration**: Leverage GPU resources for transform operations
- **Consistent results**: Maintain identical preprocessing between development and production
