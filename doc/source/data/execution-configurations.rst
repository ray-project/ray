...
* `log_internal_stack_trace`: Whether to include Ray Data/Ray Core internal code stack frames when logging a user-code error. By default these frames are omitted from both ``stdout`` and the Ray Data log file. Off by default.
