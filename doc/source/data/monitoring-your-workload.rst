...
.. list-table::
   ...
   * - Metric name
     - Description
   * - `data_spilled_bytes`
     - Bytes spilled by dataset operators. You must set `DataContext.enable_get_object_locations_for_metrics` to `True` to report this metric.
   * - `data_freed_bytes`
...
.. list-table::
   ...
   * - `average_total_task_completion_time_s`
     - Average task completion time in seconds including throttling
   ...
   * - `average_max_uss_per_task`
     - Average USS (Unique Set Size) memory usage of tasks
