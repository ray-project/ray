...
This guide uses a text tokenization collate function as an example to demonstrate the optimization. You'll learn how to address the technical challenges of moving collate functions to Ray Data and see the complete implementation.
...
**Challenge 1: Batch boundaries**
  The collate function needs to process complete batches, but :meth:`ray.data.Dataset.map_batches` doesn't guarantee the batch size would be ensured for each function call.
