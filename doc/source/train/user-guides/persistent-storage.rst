...
cloud storage (e.g., S3, GCS, Azure Blob Storage) or a shared filesystem
(e.g., AWS EFS, Google Filestore, Azure Files, HDFS) for multi-node training.
...
Shared filesystem (NFS, HDFS, Azure Files)
------------------------------------------
...
.. testcode::
    ...
    trainer = TorchTrainer(
        ...
        run_config=train.RunConfig(
            ...
            # Azure Files example (mount the file share on every node first):
