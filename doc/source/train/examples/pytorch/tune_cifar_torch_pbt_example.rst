:orphan:

.. _tune_train_torch_example:

Tuning Hyperparameters of a Distributed PyTorch Model with PBT using Ray Train & Tune
=====================================================================================

.. raw:: html

    <a id="try-anyscale-quickstart-tune_cifar_torch_pbt_example" target="_blank" href="https://console.anyscale.com/register/ha?render_flow=ray&utm_source=ray_docs&utm_medium=docs&utm_campaign=tune_cifar_torch_pbt_example">
      <img src="../../../_static/img/run-on-anyscale.svg" alt="Run on Anyscale" />
      <br/><br/>
    </a>

.. literalinclude:: /../../python/ray/train/examples/pytorch/tune_cifar_torch_pbt_example.py
