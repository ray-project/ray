...
You can find an example of this in the :doc:`Keras MNIST example </ray-tune/examples/tune_mnist_keras>`.
...
1. **Model with an existing Ray integration**
  ...
  * Keras (:doc:`example </ray-tune/examples/tune_mnist_keras>`)
  * Tensorflow (:ref:`example <ray-train-tftrainer-example>`)
  * LightGBM (:ref:`example <lightgbm-ray-tuning>`)

2. **Custom training function**

  * All this means is that your function has to expose a ``checkpoint_dir`` argument in the function signature, and call ``tune.checkpoint_dir``. See :doc:`this example </ray-tune/examples/custom_func_checkpointing>`, it's quite simple to do.
...
Note that you must use the ``tune.checkpoint_dir`` API to trigger syncing (or use a model type with a built-in Ray Tune integration as described here). See :doc:`/ray-tune/examples/custom_func_checkpointing` for an example.
