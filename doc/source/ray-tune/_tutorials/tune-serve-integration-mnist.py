...
class MNISTDataInterface(object):
    ...
    def get_incremental_data(self, day=0):
        ...
        train_n = int(0.8 *
                      (end - start))  # 80% train data, 20% validation data
...
# our :doc:`PyTorch MNIST example </ray-tune/examples/mnist_pytorch>`.
...
def tune_from_scratch(num_samples=10, num_epochs=10, gpus_per_trial=0., day=0):
    ...
    analysis = tune.run(partial(train_mnist,
                                start_model=None,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--from_existing",
                        action="store_true",
    ...
    parser.add_argument("--query",
                        help="Query endpoint with example",
                        type=int,
                        default=-1)
    ...
    if args.query >= 0:
        ...
        # Query our model
        response = requests.post("http://localhost:8000/mnist",
