...
.. list-table:: TrialScheduler Feature Compatibility Matrix
   ...
   * - :ref:`ASHA <tune-scheduler-hyperband>`
     - No
     - Yes
     - :doc:`Link </ray-tune/examples/async_hyperband_example>`
   ...
   * - :ref:`HyperBand <tune-original-hyperband>`
     - Yes
     - Yes
     - :doc:`Link </ray-tune/examples/hyperband_example>`
   * - :ref:`BOHB <tune-scheduler-bohb>`
     - Yes
     - Only TuneBOHB
     - :doc:`Link </ray-tune/examples/bohb_example>`
   * - :ref:`Population Based Training <tune-scheduler-pbt>`
     - Yes
     - Not Compatible
     - :doc:`Link </ray-tune/examples/pbt_function>`
   * - :ref:`Population Based Bandits <tune-scheduler-pb2>`
     - Yes
     - Not Compatible
     - :doc:`Basic Example </ray-tune/examples/pb2_example>`, :doc:`PPO example </ray-tune/examples/pb2_ppo_example>`
...
Compared to the original version of HyperBand, this implementation provides better parallelism and avoids straggler issues during eliminations. **We recommend using this over the standard HyperBand scheduler.** An example of this can be found here: :doc:`/ray-tune/examples/async_hyperband_example`.
...
The primary motivation for PB2 is the ability to find promising hyperparamters with only a small population size. With that in mind, you can run this :doc:`PB2 PPO example </ray-tune/examples/pb2_ppo_example>` to compare PB2 vs. PBT, with a population size of ``4`` (as in the paper). The example uses the ``BipedalWalker`` environment so does not require any additional licenses.
...
An example of this in use can be found here: :doc:`/ray-tune/examples/bohb_example`.
...
An example of this in use can be found here: :doc:`/ray-tune/examples/xgboost_dynamic_resources_example`.
