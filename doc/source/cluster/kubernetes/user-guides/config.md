...
Memory is not a compressible resource when using Kubernetes with Linux cgroupv1 but is a
compressible resource with cgroupv2. If you're using cgroupv1, set both memory requests and limits.
If you're using cgroupv2, you can omit memory limits with the same rationale for CPU.
