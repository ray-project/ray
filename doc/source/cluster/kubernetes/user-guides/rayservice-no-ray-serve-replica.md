...
ProxyActor is responsible for forwarding incoming requests to the corresponding Ray Serve replicas. Hence, if a Pod without a running ProxyActor receives requests, those requests will fail. KubeRay's readiness probe fails, rendering the Pods unready and preventing ProxyActor from sending requests to them.
...
# Step 4.2: List all Pods in the `default` namespace.
...
# Step 7.3: List all Pods in the `default` namespace.
kubectl get pods -l=ray.io/is-ray-node=yes
