...
When a `RayCluster` enables in-tree autoscaling (`spec.enableInTreeAutoscaling: true`),
KubeRay creates and reconciles a `ServiceAccount`, `Role`, and `RoleBinding` for the
autoscaler in the RayCluster namespace. The KubeRay operator's `ServiceAccount` must be
permitted to manage these resources.

If you use custom RBAC or restrict the rules generated by the KubeRay installation, retain
the following namespaced permissions for the operator in every namespace it watches:
...
these reads; if an informer cannot list one of these resources, autoscaling reconciliation
...
```bash
OPERATOR_NAMESPACE=ray-system
OPERATOR_SERVICE_ACCOUNT=kuberay-operator
RAYCLUSTER_NAMESPACE=default

kubectl auth can-i --as=system:serviceaccount:${OPERATOR_NAMESPACE}:${OPERATOR_SERVICE_ACCOUNT} \
  --list --namespace=${RAYCLUSTER_NAMESPACE}
```
