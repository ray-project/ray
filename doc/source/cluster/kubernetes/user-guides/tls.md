...
* A Kubernetes ConfigMap containing the scripts `gencert_head.sh` and `gencert_worker.sh`, which allow the head and worker Pods to generate private keys (`tls.key`) and self-signed certificates (`tls.crt`) (**Step 2**)
