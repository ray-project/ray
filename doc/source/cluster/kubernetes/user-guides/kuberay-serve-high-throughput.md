...
- `RAY_SERVE_ENABLE_HA_PROXY=1`: Enables HAProxy ingress for each Pod's proxy ingress (serving on port 8000 by default), which is a highly optimized, battle-tested open-source load balancer written in C.
