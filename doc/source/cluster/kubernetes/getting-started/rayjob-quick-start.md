...
* Submission configuration   
  * `submissionMode` (Optional): Specifies how RayJob submits the Ray job to the RayCluster. There are three possible values, with the default being `K8sJobMode`.
    ...
    * `SidecarMode`: The KubeRay operator injects a container into the Ray head Pod to submit the Ray job. This mode doesn't support `clusterSelector`, `submitterPodTemplate`, or `submitterConfig`. The head Pod's restart policy must be `Never`.
