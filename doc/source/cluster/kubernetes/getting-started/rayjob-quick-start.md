...
* Submission configuration   
  ...
  * `SidecarSubmitterRestart` (alpha in v1.7): This feature gate (disabled by default), when set, allows the submitter container to restart on transient submitter failures.
    * The submitter container's `restartPolicy` is set to `OnFailure` at the container level. It is independent of the head Pod's pod-level `restartPolicy: Never`, so a non-zero exit code restarts only the submitter container in place without restarting the `ray-head` container. Note that a failure of the Ray job's own code does not make the submitter exit non-zero, so it won't trigger a restart by itself.
    * On restart, the submitter checks the Ray job status first. If the Ray job is still running, the submitter reattaches to the log stream instead of resubmitting, preventing KubeRay from force-killing a running job if the log-follow connection drops.
    ...
    * Exceeding `backoffLimit` still marks the RayJob as failed even if the Ray job itself is still running.
    * See {ref}`kuberay-rayjob-sidecar-submitter-restart` for a full walkthrough including how to enable the feature gate.
