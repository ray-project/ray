...
If you don't find an answer to your question here, don't hesitate to connect through the [community channels](https://github.com/ray-project/kuberay#getting-involved).
...
The init container uses `ray health-check` to check the GCS server status continuously.
...
* The GCS server process has failed in the head Pod. Inspect the log directory `/tmp/ray/session_latest/logs/` in the head Pod for errors related to the GCS server.
* The `ray` executable isn't included in the `$PATH` for the image, so the init container fails to run `ray health-check`.
* The `CLUSTER_DOMAIN` environment variable isn't set correctly. See the section [cluster domain](#cluster-domain) for more details.
...
Refer to [#1069](https://github.com/ray-project/kuberay/pull/1069) and the [KubeRay Helm chart](https://github.com/ray-project/kuberay/blob/ddb5e528c29c2e1fb80994f05b1bd162ecbaf9f2/helm-chart/kuberay-operator/values.yaml#L83-L87) for instructions on how to set the environment variable.
Once disabled, you can add your custom init container to the worker Pod template.

(cluster-domain)=
## Cluster domain

In KubeRay, Fully Qualified Domain Names (FQDN) establish connections between workers and the head.
...
For more information, refer to [#951](https://github.com/ray-project/kuberay/pull/951) and [#938](https://github.com/ray-project/kuberay/pull/938) for more details.
