...
Note that the autoscaler only considers logical resource requests for scaling (specifically, those specified in ``@ray.remote`` and displayed in `ray status`), not physical machine utilization. If a user tries to launch an actor, task, or placement group but there are insufficient resources, the request is queued. The autoscaler adds nodes to satisfy resource demands in this queue.
...
`max_workers[default_value=2, min_value=0]`: The max number of cluster worker nodes to launch. Note that this doesn't include the head node.

`min_workers[default_value=0, min_value=0]`: The min number of cluster worker nodes to launch, regardless of utilization. Note that this doesn't include the head node. This number must be less than the ``max_workers``.

.. note::

  If `max_workers` is modified at runtime, the autoscaler immediately removes nodes until this constraint
...
`upscaling_speed[default_value=1.0, min_value=1.0]`: The number of nodes allowed to be pending as a multiple of the current number of nodes. The higher the value, the more aggressive upscaling is. For example, if this is set to 1.0, the cluster can grow in size by at most 100% at any time, so if the cluster currently has 20 nodes, at most 20 pending
...
autoscaler. The smaller the value, the more aggressive downscaling is. Worker nodes are considered idle when they hold no active tasks, actors, or referenced objects (either in-memory or spilled to disk). This parameter doesn't affect the head node.
