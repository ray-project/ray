...
.. code-block:: python
    ...
    ParIterator[...].for_each()

    # Batch together items into a lists of 32 elements.
    >>> it = it.batch(32)
    ParIterator[...].for_each().batch(32)
...
.. code-block:: python
    ...
    >>> it = it.gather_sync()
    LocalIterator[ParIterator[from_range[1000000, shards=1]].gather_sync()]
...
only one process should read from a shard of the iterator:

.. code-block:: python

    # Get local iterators representing the shards of this ParIterator:
