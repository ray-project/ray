...
For image-based environments like [Atari](https://ale.farama.org/environments/), use the
...
dimensions. For example, for an [Atari](https://ale.farama.org/environments/) environment, you can
...
[example that uses a default RLModule with an LSTM layer](https://github.com/ray-project/ray/blob/master/rllib/examples/algorithms/ppo/stateless_cartpole_ppo.py).
...
[tiny_atari_cnn_rlm example](https://github.com/ray-project/ray/blob/master/rllib/examples/rl_modules/classes/tiny_atari_cnn_rlm.py)
and in the [lstm_containing_rlm example](https://github.com/ray-project/ray/blob/master/rllib/examples/rl_modules/classes/lstm_containing_rlm.py).
:::
...
See here for an [example script utilizing a self-supervised loss RLModule](https://github.com/ray-project/ray/blob/master/rllib/examples/curiosity/intrinsic_curiosity_model_based_curiosity.py).
Losses can be defined over either policy evaluation inputs, or data read from {doc}`offline storage </rllib/offline>`.
...
See an [example for a "correlated actions" environment](https://github.com/ray-project/ray/blob/master/rllib/examples/envs/classes/correlated_actions_env.py) here.
