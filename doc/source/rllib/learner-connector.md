...
Each {py:class}`~ray.rllib.core.learner.learner.Learner` actor holds a single Learner connector pipeline that compiles the train batch for the {py:class}`~ray.rllib.core.rl_module.rl_module.RLModule` from a list of episodes. See the following figure.
...
The Learner connector pipeline transforms a list of {ref}`Episode objects <single-agent-episode-docs>` into an `RLModule`-readable tensor batch, the train batch. The {py:class}`~ray.rllib.core.learner.learner.Learner` actor sends the pipeline output directly into the {py:meth}`~ray.rllib.core.rl_module.rl_module.RLModule.forward_train` method of the {py:class}`~ray.rllib.core.rl_module.rl_module.RLModule`.
...
* {py:class}`~ray.rllib.connectors.learner.add_columns_from_episodes_to_batch.AddColumnsFromEpisodesToBatch`: Places all other columns, such as rewards, actions, and termination flags, from the incoming episodes into the batch.
* *Relevant for stateful models only:* {py:class}`~ray.rllib.connectors.common.add_time_dim_to_batch_and_zero_pad.AddTimeDimToBatchAndZeroPad`: If the {py:class}`~ray.rllib.core.rl_module.rl_module.RLModule` is stateful, adds a time-dimension of size `max_seq_len` at axis=1 to all data in the batch and right-zero-pads where episodes end at timesteps not divisible by `max_seq_len`. To change `max_seq_len`, call `config.rl_module(model_config_dict={'max_seq_len': ...})` on your {py:class}`~ray.rllib.algorithms.algorithm_config.AlgorithmConfig` object.
...
* {py:class}`~ray.rllib.connectors.common.batch_individual_items.BatchIndividualItems`: Converts all data in the batch, which so far are lists of individual items, into batched structures, meaning NumPy arrays whose batch axis is axis 0.
...
The order of these transforms matters for how the pipeline works.


(writing_custom_learner_connectors)=

## Writing custom Learner connectors

To customize the Learner connector pipeline, specify a function in your {py:class}`~ray.rllib.algorithms.algorithm_config.AlgorithmConfig`. The function takes the observation and action spaces as input arguments and returns a single {py:class}`~ray.rllib.connectors.connector_v2.ConnectorV2` piece or a list of pieces.

RLlib prepends these {py:class}`~ray.rllib.connectors.connector_v2.ConnectorV2` instances to the {ref}`default Learner pipeline <default-learner-pipeline>` in the order returned. If you set `add_default_connectors_to_learner_pipeline=False` in your config, RLlib uses only the provided {py:class}`~ray.rllib.connectors.connector_v2.ConnectorV2` pieces and adds no default behavior.

For example, to prepend a custom {py:class}`~ray.rllib.connectors.connector_v2.ConnectorV2` piece to the {py:class}`~ray.rllib.core.learner.learner.Learner` connector pipeline, do this in your config:
...
RLlib adds the connector pieces your function returns to the beginning of the Learner pipeline, before the default connector pieces it provides automatically:
...
Plug this custom {py:class}`~ray.rllib.connectors.connector_v2.ConnectorV2` piece into the pipeline through the algorithm config with `config.learners(learner_connector=lambda env: CountBasedIntrinsicRewards())`. Your loss function then receives the altered reward signals in the `rewards` column of the incoming batch.

:::{note}
Your custom logic writes the new rewards back into the given episodes instead of into the train batch. Writing the data you pulled from episodes back into the same episodes ensures that only the changed data is visible to the subsequent connector pieces. The batch remains unchanged at first. One of the subsequent {ref}`default Learner connector pieces <default-learner-pipeline>`, {py:class}`~ray.rllib.connectors.learner.add_columns_from_episodes_to_batch.AddColumnsFromEpisodesToBatch`, fills the batch with rewards data from the episodes. RLlib therefore adds to the train batch any changes you make to the episode objects.
:::


### Example: Stacking the N most recent observations

Another application of the Learner connector API, combined with a {ref}`custom env-to-module connector piece <writing_custom_env_to_module_connectors>`, is efficient observation frame stacking. This approach avoids deduplicating the stacked, overlapping observation data, and it avoids storing these additional observations in your episodes or sending them through the network for inter-actor communication.
...
in the batch. Use dummy, zero-filled observations, shown in red in the batch, where the stacking happens close to the beginning of
the episode.
```

Because you aren't overriding the original, non-stacked observations in the collected episodes, you have to apply the same batch construction logic for the observation stacking twice, once for the action computation on the {py:class}`~ray.rllib.env.env_runner.EnvRunner` actors and once for the loss computation on the {py:class}`~ray.rllib.core.learner.learner.Learner` actors.

Batches that a connector pipeline produces are ephemeral, and RLlib discards them right after the {py:class}`~ray.rllib.core.rl_module.rl_module.RLModule` forward pass. Frame stacking happens directly on the batch under construction because you don't want to overload the episodes with deduplicated, stacked observations. As a result, you have to apply the stacking logic twice, in the {ref}`env-to-module pipeline <env-to-module-pipeline-docs>` and the Learner connector pipeline.
