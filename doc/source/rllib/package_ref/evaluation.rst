...
.. figure:: ../../images/rllib/rollout_worker_class_overview.svg
    ...
    **A typical RLlib WorkerSet setup inside an RLlib Trainer:** Each ``WorkerSet`` contains
    ...
    a ``Sampler`` (either synchronous or asynchronous) which controls the environment data
    collection loop.
    In the online (environment is available) as well as the offline case (no environment),
    ``Trainers`` use the ``RolloutWorkers`` `sample()` method to get ``SampleBatch`` objects for training.
