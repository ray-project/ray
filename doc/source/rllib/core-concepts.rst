...
1) Vectorization of sub-environments (i.e. individual `gym.Env <https://github.com/Farama-Foundation/Gymnasium>`_ instances, stacked to form a vector of envs) in order to batch the action computing model forward passes.
...
For example, if you provide a custom `gym.Env <https://github.com/Farama-Foundation/Gymnasium>`_ class to RLlib, auto-conversion to :py:class:`~ray.rllib.env.base_env.BaseEnv` goes as follows:

- User provides a `gym.Env <https://github.com/Farama-Foundation/Gymnasium>`_ -> :py:class:`~ray.rllib.env.vector_env._VectorizedGymEnv` (is-a :py:class:`~ray.rllib.env.vector_env.VectorEnv`) -> :py:class:`~ray.rllib.env.base_env.BaseEnv`
