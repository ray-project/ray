...
Replay buffers in reinforcement learning generally mean a buffer that stores and replays experiences collected from interactions of agents with the environment.
...
When sampling from a replay buffer, you choose which experiences to train the agent with. A straightforward strategy that has proven effective for many algorithms is to pick these
samples uniformly at random.
A more advanced strategy (proven better in many cases) is Prioritized Experiences Replay (PER) (`see paper <https://arxiv.org/abs/1511.05952>`__).
In PER, single items in the buffer are assigned a (scalar) priority value, which denotes their significance, or in simpler terms, how much the system expects
to learn from these items. Experiences with a higher priority are more likely to be sampled.

Eviction Strategy
-----------------

A buffer is naturally limited in its capacity to hold experiences. In the course of running an algorithm, a buffer eventually reaches
its capacity and to make room for new experiences, older ones must be deleted (evicted). This is generally done on a first-in-first-out basis.
...
RLlib provides a base :py:class:`~ray.rllib.utils.replay_buffers.replay_buffer.ReplayBuffer` class from which you can build your own buffer.
In most algorithms, RLlib requires :py:class:`~ray.rllib.utils.replay_buffers.multi_agent_replay_buffer.MultiAgentReplayBuffer`\s.
This is because they need to generalize to the multi-agent case. Therefore, these buffer's ``add()`` and ``sample()`` methods require a ``policy_id`` to handle experiences per policy.
Have a look at the :py:class:`~ray.rllib.utils.replay_buffers.multi_agent_replay_buffer.MultiAgentReplayBuffer` to get a sense of how it extends the base class.
...
You rarely have to define your own replay buffer sub-class when running an experiment, but rather configure existing buffers.
...
.. tip:: Because of its prevalence, most Q-learning algorithms support PER. The priority update step that's needed is embedded into their training iteration functions.
.. warning:: If your custom buffer requires extra interaction, you have to change the training iteration function, too.
...
#. Parameters that define how algorithms interact with replay buffers.
    For example, ``worker_side_prioritization`` to decide where to compute priorities

#. Constructor arguments to instantiate the replay buffer.
    For example, ``capacity`` to limit the buffer's size

#. Call arguments for underlying replay buffer methods.
    For example, ``prioritized_replay_beta`` is used by the :py:class:`~ray.rllib.utils.replay_buffers.multi_agent_prioritized_replay_buffer.MultiAgentPrioritizedReplayBuffer` to call the ``sample()`` method of every underlying :py:class:`~ray.rllib.utils.replay_buffers.prioritized_replay_buffer.PrioritizedReplayBuffer`
...
Advanced buffer types add capabilities while trying to retain compatibility through inheritance.
...
The ``storage_unit`` constructor argument of a replay buffer defines how experiences are stored, and therefore the unit in which they're sampled from.
When later calling the ``sample()`` method, num_items relates to said ``storage_unit``.
...
As noted previously, RLlib's :py:class:`~ray.rllib.utils.replay_buffers.multi_agent_replay_buffer.MultiAgentReplayBuffer`\s
...
The :py:class:`~ray.rllib.utils.replay_buffers.multi_agent_replay_buffer.MultiAgentReplayBuffer` can stay the same. You only need to specify your own buffer along with a default call argument:
