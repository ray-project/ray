...
processed by their custom pipelines and models. For example, Compare this to having to handle

Another advantage the the more efficient memory footprint of episodes. For example, using tensor batch
format for an algorithm like DQN, we would have to store both observations and
next observations (to compute the TD error-based loss) in your batch, thereby duplicating an
already large observation tensor. On the other hand, in episode objects we only have to
store a single observation-track containing all observations from reset to terminal.

We'll explain in more detail on this page what working with RLlib's Episode APIs looks like.
