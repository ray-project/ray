...
`NovelD`_                        tf + torch **Yes** `+parametric`_  Yes                **Yes**     `+RNN`_     
`RE3`_                           tf         **Yes** `+parametric`_  Yes                **Yes**     `+RNN`_
================================ ========== ======================= ================== =========== =====================
...
**Activating NovelD** 
The NovelD plugin can be easily activated by specifying it as the Exploration class to-be-used in the main Trainer configuration. Most of its parameters do not have to be specified as the module uses the values from the paper and the model from the Policy as defaults. For example:
...
.. code-block:: python
    ...
    .exploration(
        exploration_config= {
            # Use the NovelD module for exploring.
            "type": "NovelD",
            # Dimensionality of the distillation network's output layer,
            # the state embeddings.
            "embed_dim": 128,
            # Configuration of the distillation networks. Used to compute
            # the state novelties.
            "distill_net_config": {
                "fcnet_hiddens": [],
                "fcnet_activation": "relu",
            },
            # The distillation network's optimizer's learning rate.
            "lr": 0.0001,
            # The scaling factor for the novelty difference.
            "alpha": 0.5,
            # The clipping factor of the novelty difference.
            "beta": 0.0,
            # The coefficient of the intrinsic reward. Scales the ratio 
            # between intrinsic and extrinsic reward.
            "intrinsic_reward_coeff": 0.005,
            # If intrinsic rewards should be normalized. This shows 
            # more stable training and helps to set comparable 
            # hyperparameters in experiments.
            "normalize": True,
            # Configuration of the sub-exploration that is used during 
            # rollouts.
            "sub_exploration": {
                "type": "StochasticSampling",
            },
...
NovelD when configured as the Exploration class in the Trainer config (see for more details the code above) adds two networks to the agent. These networks are (1) a trainable prediction network and (2) a random target network, both with the same architecture (as a default the architecture of the Policy model is chosen). These networks 
...
actions in the `Montezuma Revenge`, `PrivateEye <https://www.gymlibrary.ml/environments/atari/private_eye/?highlight=privateeye>` or `Pitfall <https://www.gymlibrary.ml/environments/atari/pitfall/?highlight=pitfall>`__ in the ATARI environments or `Obstructed Maze` in the `MiniGrid` environment. For this reason NovelD might be a good choice for 
stochastic environments and/or environments with local sources of entropy. For example, due to sticky actions in `Montezuma Revenge` it is hard for curiosity exploration to predict the next state due to the stochasticity in actions leading to the agent oscillating between two rooms. Because of the distillation of the trainable network in NovelD 
the agent cannot exploit such non-determinism in the environment to collect intrinsic reward. Furthermore, it should be mentioned that the NovelD approach uses only a single trainable network in contrast to :ref:`Curiosity` that maintains three neural nets. If computing resources are an issue single network exploration algorithms like `NovelD` 
or :ref:`RE3` should be considered. 
