...
def get_train_val_datasets():
    ...
    def label_function(examples):
        return {'labels': [1 if label == 'positive' else 0 for label in examples['label']]}
    ...
    train_dataset = train_dataset.shuffle(
        10000).batch(32).prefetch(tf.data.AUTOTUNE)
...
def train_loop_per_worker(config):
    ...
    with strategy.scope(): 
        ...
        optimizer = tf.keras.optimizers.Adam(
            learning_rate=config["lr"], epsilon=config["epsilon"], clipnorm=config["clipnorm"])
    ...
    for epoch in range(config["epochs"]):
        for batch in train_dataset:
            inputs = {key: batch[key]
                      for key in batch.keys() if key != 'labels'}
            labels = batch['labels']
        ...
        for batch in val_dataset:
            ...
            val_acc += (tf.argmax(outputs.logits, axis=-1).numpy()
                        == labels.numpy()).mean()
