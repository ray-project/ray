...
.. note:: Note that the HyperBand scheduler requires your trainable to support checkpointing, which is described in `Tune User Guide <tune-checkpointing.html>`__. Checkpointing enables the scheduler to multiplex many concurrent trials onto a limited size cluster.
