...
Once you find the component to override LD_PRELOAD, specify the env var, `f"RAY_{process_type.upper()}_JEMALLOC_LD_PRELOAD")`.
For example, if you'd like to profile gcs_server, specify `RAY_GCS_SERVER_JEMALLOC_LD_PRELOAD`.
...
.. code-block:: bash
  ...
  export MALLOC_CONF=prof:true,lg_prof_interval:33,lg_prof_sample:17,prof_final:true,prof_leak:true

  # Specify jemalloc shared library path.
  RAY_GCS_SERVER_JEMALLOC_LD_PRELOAD=~/jemalloc-5.2.1/lib/libjemalloc.so ray start --head

  # You will find the profiling data generated by Jemalloc for the GCS server.
