...
The example tries to set ``MKL_NUM_THREADS=1`` and ``OPENBLAS_NUM_THREADS=1`` in the actor,
which should cover most systems. In that case, numpy may be using some other multiprocessing
library that you need to config to use less cores.
