...
number of requests across replicas: for example, balancing ongoing input tokens, balancing
...
API to achieve custom load balancing across replicas of a given deployment. It
...
request is attempted to be routed to the replica with the shortest request queue in
...
is called again with a backoff delay until a replica is able to process the
request.
...
imported class or as the string of import path to the class. Deploy a simple app
...
"`on_request_routed` callback is called!!" messages are printed to the console. The
response is also randomly routed to one of the replicas. You can test this by
...
(utility-mixin)=
## Utility mixins for request routing
Ray Serve provides utility mixins that can be used to extend the capability of the
...
- [`FIFOMixin`](../api/doc/ray.serve.request_router.FIFOMixin.rst): This mixin implements first in first out (FIFO)
  request routing. The default behavior for the request router is out-of-order (OOO)
  ...
  matching algorithm is updated to route requests FIFO. There are no additional
  flags that need to be configured and no additional helper methods provided by this mixin.
