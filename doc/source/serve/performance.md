...
You can set an end-to-end timeout for HTTP requests by setting the `request_timeout_s` in Serve config's http_options` field. HTTP Proxies will wait for that many seconds before terminating an HTTP request and retrying it at another replica. This config is global to your Ray cluster, and it cannot be updated during runtime.
