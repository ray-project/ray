...
These guidelines are a great starting point. If you decide to further tune your autoscaling config for your application, you can read on to the following sections.

## Autoscaling concepts overview

Before diving into an example, we go over some foundational Serve autoscaling concepts.
...
Similarly, when Ray Serve scales down and terminates replica Actors, it attempts to make as many nodes idle as possible so the Ray Autoscaler can remove them. To learn more about the architecture underlying Ray Serve Autoscaling, see {ref}`serve-autoscaling-architecture`.
:::
...
This is an example of a synchronous workload that runs ResNet50. The application code is as follows:
...
This YAML file is the autoscaling configuration. `target_num_ongoing_requests_per_replica = 1` because the ResNet model runs synchronously, and the goal is to keep the latencies low. Note that the deployment starts with 0 replicas.
```yaml
applications:
  - name: default
