...
The import path (for example, `text_ml:app`) must be importable by Serve at runtime.
...
As with all other Ray actor options, pass the runtime environment in through `ray_actor_options` in
...
pre-compiling such packages in a private repository or Docker image.
:::
