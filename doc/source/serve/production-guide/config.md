...
The `logging_config` is global config, you can configure controller & proxy & replica logs. Note that you can also set application and deployment level logging config, which takes precedence over the global config. See logging config API [here](../../serve/api/doc/ray.serve.schema.LoggingConfig.rst) for more details.
...
- **`route_prefix`**: An application can be called through HTTP at the specified route prefix. It defaults to `/`. The route prefix for each application must be unique.
...
Below is a config for the [`Text ML Model` example](serve-in-production-example) that follows the format explained preceding:
...
Note that the `runtime_env` field is always empty when using `serve build` and must be set manually. In this case, if `torch` and `transformers` aren't installed globally, you should include these two pip packages in the `runtime_env`.
...
To enable the `user_config` feature, implement a `reconfigure` method that takes a JSON-serializable object (for example, a Dictionary, List, or String) as its only argument:
