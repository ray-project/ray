...
* `deployments`: A list of entries representing each deployment's status. Each entry maps a deployment's name to three fields:
    * `status`: A Serve deployment has six possible statuses:
        ...
        * `"UNHEALTHY"`: The deployment has updated and has become unhealthy afterwards. This condition may be due replicas failing to upscaling, replicas failing health checks, or a general system or machine error.
