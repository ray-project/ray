...
In this section, explore Serve's key architectural concepts and components. It offers insight and overview into:
...
- **HTTP Proxy**: By default there is one HTTP proxy actor on the head node. This actor runs a [Uvicorn](https://www.uvicorn.org/) HTTP
  ...
  responds once they're completed. For scalability and high availability,
  you can also run a proxy on each node in the cluster through the `proxy_location` field inside [`serve.start()`](core-apis) or [the config file](serve-in-production-config-file).
- **gRPC Proxy**: If Serve is started with valid `port` and `grpc_servicer_functions`,
  ...
  incoming requests, forwards them to replicas, and responds once they're completed.
...
3. For each request in a deployment's queue, an available replica is looked up
  and the request is sent to it. If no replicas are available (more
...
using `asyncio` to process them concurrently. If the handler (the deployment function or the `__call__` method of the deployment class) is declared with `async def`, the replica doesn't wait for the
handler to run. Otherwise, the replica blocks until the handler returns.

When making a request through a [DeploymentHandle](serve-key-concepts-deployment-handle) instead of HTTP or gRPC for [model composition](serve-model-composition), the request is placed on a queue in the `DeploymentHandle`, and skip to step 3 preceding.
...
wrapped. A 500 status code is returned with the traceback information. The
replica is able to continue to handle requests.
...
- If you aren't using KubeRay, when the Ray cluster fails, Ray Serve can't recover.
...
router and the replica (like network connections and internal request queues) is lost for this kind of failure.
...
:::{note}
When the controller dies, requests can still be sent through HTTP, gRPC and `DeploymentHandle`, but autoscaling is paused. When the controller recovers, the autoscaling resumes, but all previous metrics collected are lost.
:::
...
of replicas through the `num_replicas` option of your deployment.
...
request is sent from one replica to another through the handle, the
...
store. Small request objects are directly sent between actors through network
call. Larger request objects (100KiB+) are written to the object store and the replica can read them through zero-copy read.
