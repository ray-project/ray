...
@serve.deployment(
    ...
    autoscaling_config={
        "min_replicas": 2,
        "max_replicas": 16,
        # Must be <= max_ongoing_requests (2) or the autoscaler never upscales.
...
class VideoEncoder:
    ...
    def __init__(self):
        ...
        # Load SigLIP model and image processor. Use AutoImageProcessor (not
        # AutoProcessor): we only compute image features, so we skip the text
        # tokenizer, which avoids a sentencepiece/protobuf load incompatibility.
    ...
    def encode_frames(self, frames: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            with torch.amp.autocast(device_type=self.device, enabled=self.device == "cuda"):
                # In transformers 4.x, get_image_features returns the pooled image
                # embeddings as a Tensor directly (shape (T, D)); normalize it.
