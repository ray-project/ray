...
class IndexingIngress:
    ...
    async def index(self, req: IndexRequest, debug: bool = False):
        ...
        def _enqueue():
            ...
            return result, t_start, time.perf_counter()

        result, t_start, t_end = await loop.run_in_executor(self.enqueue_pool, _enqueue)
        resp = {"task_id": result.id, "status": result.status}
        if debug:
            resp["t_ms"] = {
                "executor_wait": round((t_start - t0) * 1e3, 2),
                "celery_publish": round((t_end - t_start) * 1e3, 2),
                "handler_total": round((time.perf_counter() - t0) * 1e3, 2),
            }
        return resp

    @fastapi_app.get("/debug/redis")
    def debug_redis(self, n: int = 20):
        """p50s for raw Redis PING and LPUSH+LPOP from this replica (broker RTT floor)."""
        ...
        def p50(fn):
            ...
            return round(sorted(ts)[n // 2], 3)
        out = {"ping_p50_ms": p50(r.ping)}
        out["lpush_lpop_p50_ms"] = p50(lambda: (r.lpush("_rtt_probe", "x"), r.lpop("_rtt_probe")))
        return out
    ...
    async def ping(self, req: IndexRequest):
        """Ping-pong probe: same request shape as /index, no enqueue (HTTP-path floor)."""
        return {"pong": True, "video_id": req.video_id}
