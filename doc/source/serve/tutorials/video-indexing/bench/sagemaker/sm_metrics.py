...
def count_outputs(s3, bucket, prefix):
    # Count result objects under the async output prefix, excluding the nested
    # failures/ path (deploy_endpoint writes failures to <output>/failures/) so
    # failed invocations don't inflate the completed count.
    n = 0
    failures_prefix = prefix.rstrip("/") + "/failures/"
    for page in s3.get_paginator("list_objects_v2").paginate(Bucket=bucket, Prefix=prefix):
        for obj in page.get("Contents", []):
            if not obj["Key"].startswith(failures_prefix):
                n += 1
    return n
