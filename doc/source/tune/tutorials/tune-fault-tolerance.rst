...
In this guide, learn how to enable different types of fault tolerance offered by Ray Tune.
...
1. The driver script that calls :meth:`Tuner.fit() <ray.tune.Tuner.fit>` errors out (for example, due to the head node running out of memory or out of disk).
...
Say your initial Tune experiment is configured as follows.
...
If the experiment has been interrupted due to one of the reasons listed preceding, use this path to resume:
...
- Unfinished trials left in the ``RUNNING`` state are resumed by default.
- Trials that have ``ERRORED`` can be resumed or retried from scratch.
- ``TERMINATED`` trials *can't* be resumed.
...
Running this script the first time launches the initial training run.
Running this script the second time attempts to resume from the outputs of the first run.
...
If object references are saved along with experiment state (for example, within each trial's config),
then attempting to retrieve these objects won't work properly after restoration:
...
Say there's some large pre-trained model to use in some way in the training loop.
For example, this could be an image classification model used to calculate an Inception Score
to evaluate the quality of a generative model.
There may be multiple models to tune over, where each trial samples one of the models to use.
...
To restore, just re-specify the ``param_space`` through :meth:`Tuner.restore <ray.tune.Tuner.restore>`:
...
Assuming the ``trainable`` from the previous example that implements
trial checkpoint saving and loading is used, here is how to configure :class:`~ray.tune.FailureConfig`:
...
When a trial encounters a runtime error, the preceding configuration re-schedules that trial
up to ``max_failures=3`` times.

Similarly, if a node failure occurs for node ``X`` (for example, pre-empted or lost connection),
this configuration reschedules all trials that lived on node ``X`` up to ``3`` times.


Summary
-------

In this user guide, learn how to enable experiment-level and trial-level fault tolerance in Ray Tune.
