...
It describes in detail what happens when you call ``Tuner.fit()``, what the lifecycle of a Tune trial looks like,
...
Every Tune run consists of a "driver process" and many "worker processes."
...
The definition of the user-provided Trainable is
:ref:`serialized via cloudpickle <serialization-guide>`) and sent to each actor process.
Each Ray actor starts an instance of the Trainable to be executed.

If the Trainable is a class, it's executed iteratively by calling ``train/step``.
After each invocation, the driver is notified that a "result dict" is ready.
The driver then pulls the result with ``ray.get``.

If the trainable is a callable or a function, it's executed on the Ray actor process on a separate execution thread.
...
After pulling, the actor's execution thread automatically resumes.


Resource Management in Tune
~~~~~~~~~~~~~~~~~~~~~~~~~~~

Before running a trial, the Ray Tune driver checks whether there are available
resources on the cluster (see :ref:`resource-requirements`).
It compares the available resources with the resources required by the trial.

If there is space on the cluster, then the Tune Driver starts a Ray actor (worker).
This actor is scheduled and executed on some node where the resources are available.
...
* **PENDING**: A pending trial is a trial to be executed on the machine.
  ...
  Tune runs the trial (by starting a ray actor holding the config and the training function.
...
* **ERRORED**: If a running trial throws an exception, Tune catches that exception and sets the trial status to ERRORED.
  Note that exceptions can be propagated from an actor to the main Tune driver process.
  If `max_retries` is set, Tune sets the trial back into "PENDING" and later starts it from the last checkpoint.

* **TERMINATED**: A trial is terminated if it's stopped by a Stopper/Scheduler.
  If using the Function API, the trial is also terminated when the function stops.

* **PAUSED**: A trial can be paused by a Trial scheduler. This means that the trial's actor is stopped.
...
is enabled, the trial is then reverted to a ``"PENDING"`` state and resumed
from the last available checkpoint when it's run.
...
used for querying new hyperparameter configurations to evaluate.

SearchAlgorithms are notified every time a trial finishes
...
the training process. If a class is provided, it's expected to conform to the
Trainable interface. If a function is provided, it's wrapped into a
Trainable class, and the function itself is executed on a separate thread.

Trainables execute one step of ``train()`` before notifying the TrialRunner.
