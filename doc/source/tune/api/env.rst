...
* **TUNE_ONLY_STORE_CHECKPOINT_SCORE_ATTRIBUTE**:  If set to `1`, only the metric defined by `checkpoint_score_attribute` will be stored with each `Checkpoint`. 
As a result, `Result.best_checkpoints` will contain only this metric, omitting others that would normally be included.  
This can significantly reduce memory usage, especially when many checkpoints are stored or when metrics are large.  
Defaults to `0` (i.e., all metrics are stored).
