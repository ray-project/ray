...
XGBoost uses `decision trees <https://en.wikipedia.org/wiki/Decision_tree>`_. Just like
in `bagging <https://en.wikipedia.org/wiki/Bootstrap_aggregating>`_ algorithms,
many different small
decision trees are built, and their individual outputs are combined
to make the final prediction. This is called an ensemble learner.

The intuition of ensemble learners is that even though single decision trees
can be inaccurate and suffer from high variance,
...
In bagging algorithms, different decision trees are generated by training on a random
subset of the training data. Since the input data is different for every decision tree,
different trees emerge.
Boosting algorithms like XGBoost are very similar. The main
difference is that they don't sample uniformly from the input data, but sample those
examples more often that have been misclassified in the previous step. This is useful
because it avoids overfitting to samples that can be easily classified and instead
tries to come up with models that are able to classify hard examples, too.

Additionally, bagging algorithms retain all decision trees and output a simple average
over their predictions. Boosting algorithms on the other hand assign weights to the
decision trees according to their classification error, and trees with an error rate
higher than a specific threshold (e.g. 50 %) are usually discarded.
...
`Asynchronous hyperband scheduling <https://openreview.net/forum?id=S1Y7OOlRZ>`_ is
a parallel implementation of the `Hyperband scheduler <https://arxiv.org/abs/1603.06560>`_.
...
are selected according to some loss metric, usually an evaluation loss. The remaining
candidates are then trained for some more iterations, after which again only a subset of
configurations is retained. Thus, at each step only the most promising combinations
are promoted to the next round, until we end up with the best configurations.

We need to tell the ``AsyncHyperBandScheduler`` three things. First, it needs to now
which metric to evaluate to select those trials that should continue to train. Second,
we need to tell it if this metric should be maximized or minimized. Third, we should
tell it the maximum number of training iterations per trial - this way the scheduler
can estimate how long to train the trials before it stops the bad performing
configurations.

There are other parameters, like the ``reduction_factor`` which tells the scheduler
how aggressively to prune the search space. All parameters are explained in the
...
Each task thus works with 10% of the available GPU memory.
