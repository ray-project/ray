...
By default, syncing occurs every 300 seconds. To change the frequency of syncing, set the ``TUNE_CLOUD_SYNC_PERIOD_SEC`` environment variable in the driver to the desired syncing period. Note that the ``global_checkpoint_period`` argument also affects the frequency of uploading.
