...
:::{admonition} Ray allows any clients to run arbitrary code. Be extremely careful about what's allowed to access your Ray Cluster
:class: caution
...
* Indirectly: By calling the Dashboard HTTP APIs of these services
...
* gRPC communication can be encrypted with TLS, but that's not a replacement for network isolation.
...
* Ray faithfully executes code that's passed to it – Ray doesn't differentiate between a tuning experiment, a rootkit install, or an S3 bucket inspection.
...
* Starting in Ray 2.52.0, Ray supports built-in token authentication that provides an additional measure to prevent unauthorized access to the cluster (including code execution from unknown sources). See {ref}`Ray token authentication <token-auth>` for details.
* Token authentication isn't an alternative to deploying Ray clusters in a controlled network environment. Rather, it's a defense-in-depth measure that adds to network-level security.
