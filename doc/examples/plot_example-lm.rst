...
To use Ray cluster launcher on AWS, install boto (``pip install boto3``) and configure your AWS credentials in ``~/.aws/credentials`` as described on  :ref:`Automatic Cluster Setup page <ref-automatic-cluster>`.
