...
// PublishErrors publishes the error stream.
func (c *cgoClient) PublishErrors(ctx context.Context) (<-chan gcs.ErrorData, error) {
	var cErr *C.char

	ok := C.ray_gcs_client_publisher_publish_errors(c.getPtr(), &cErr)
	if cErr != nil {
		defer C.free(unsafe.Pointer(cErr))
		return nil, fmt.Errorf("publish errors failed: %s", C.GoString(cErr))
	}
	if ok == 0 {
		return nil, fmt.Errorf("publish errors failed")
	}

	// Create the channel and start the polling goroutine.
	ch := make(chan gcs.ErrorData, getSubscriberChannelSize())

	// Create an error subscriber to poll with.
	sub, err := CreateErrorSubscriber(c.Address(), c.ClusterID().Hex())
	if err != nil {
		close(ch)
		return nil, fmt.Errorf("create error subscriber failed: %w", err)
	}

	if err := sub.Subscribe(); err != nil {
		close(ch)
		sub.Close() // Release the subscriber resources.
		...
		for {
			...
			case <-ticker.C:
				...
				if data != nil {
					...
					case ch <- *data:
					default:
						// Drop the data when the channel is full.
					}
				}
			}
		}
...
// PublishLogs publishes the log stream.
func (c *cgoClient) PublishLogs(ctx context.Context) (<-chan gcs.LogData, error) {
	var cErr *C.char

	ok := C.ray_gcs_client_publisher_publish_logs(c.getPtr(), &cErr)
	if cErr != nil {
		defer C.free(unsafe.Pointer(cErr))
		return nil, fmt.Errorf("publish logs failed: %s", C.GoString(cErr))
	}
	if ok == 0 {
		return nil, fmt.Errorf("publish logs failed")
	}

	// Create the channel and start the polling goroutine.
	ch := make(chan gcs.LogData, getSubscriberChannelSize())

	// Create a log subscriber to poll with.
	sub, err := CreateLogSubscriber(c.Address(), c.ClusterID().Hex())
	if err != nil {
		close(ch)
		return nil, fmt.Errorf("create log subscriber failed: %w", err)
	}
