...
UNIT_SECONDS = 0.35
W, K = 3, 3
OUTAGE_S = 42          # < C29's 60s cluster-death ceiling
DIE_INTO_OUTAGE = 30   # enough for Q=16 (48 units) to saturate
Q_FIXED = 8
DEATH_PHASES = [25.0, 26.3, 27.6, 28.9, 30.2, 31.5]
...
def run_q(q: int, idx: int, phase: float, log) -> dict:
    ...
    log(f"  [r{idx} phase={phase}] executed={m['executed_at_death']} "
        ...
        f"p={m['p_inferred']} interval=[{res['predicted_lo']},{res['predicted_hi']}] "
        f"in={m['in_interval']} saturated={m['saturated']}")
...
def main() -> int:
    ...
    res = {"arms": {}, "config": {"N": N_UNITS, "W": W, "K": K,
                                  ...
                                  "outage_s": OUTAGE_S, "Q": Q_FIXED,
                                  "death_phases": DEATH_PHASES}}
    try:
        for idx, phase in enumerate(DEATH_PHASES):
            log(""); log("=" * 66)
            log(f"repeat {idx}  Q={Q_FIXED}  death at +{phase}s into outage")
            log("=" * 66)
            res["arms"][str(idx)] = run_q(Q_FIXED, idx, phase, log)
    finally:
        sh([RAY, "stop", "--force"])

    arms = res["arms"]
    got = [(k, a["measured"]) for k, a in arms.items() if a.get("measured")]
    redos = [m["redo"] for _, m in got]
    ps = [m["p_inferred"] for _, m in got]
    sat = [m["saturated"] for _, m in got]
    lo, hi = Q_FIXED * W + W + 1, W * (Q_FIXED + 2)
    res["verdict"] = {
        "repeats_measured": len(got),
        "NC_saturated_count": sum(1 for s in sat if s),
        "NC_enough_saturated": sum(1 for s in sat if s) >= 4,
        "redo_values": redos,
        "p_values": ps,
        "distinct_redo_values": sorted(set(redos)),
        "n_distinct": len(set(redos)),
        "predicted_interval": [lo, hi],
        "all_in_interval": bool(redos) and all(lo <= r <= hi for r in redos),
        "modal_is_p_equals_W": (max(set(ps), key=ps.count) == W) if ps else None,
        "contiguous_everywhere": all(
            a.get("after", {}).get("contiguous") is True for a in arms.values()),
        "C30_proven": (bool(redos)
                       and sum(1 for s in sat if s) >= 4
                       and all(lo <= r <= hi for r in redos)
                       and len(set(redos)) >= 2),
        "C30_refuted_constant": bool(redos) and len(set(redos)) == 1,
    }
