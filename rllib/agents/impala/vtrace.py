...
to support MultiDiscrete action spaces. behavior_policy_logits,
...
def from_logits(behavior_policy_logits,
    ...
    res = multi_from_logits(
        [behavior_policy_logits], [target_policy_logits], [actions],
        discounts,
        rewards,
        values,
        bootstrap_value,
    ...
    return VTraceFromLogitsReturns(
        ...
        behavior_action_log_probs=tf.squeeze(
            res.behavior_action_log_probs, axis=0),
        target_action_log_probs=tf.squeeze(
            res.target_action_log_probs, axis=0),
    )


def multi_from_logits(behavior_policy_logits,
                      target_policy_logits,
                      actions,
                      discounts,
                      rewards,
  ...
  behavior policy refers to the policy that generated the given
  rewards and actions.
  ...
  Args:
    behavior_policy_logits: A list with length of ACTION_SPACE of float32
      tensors of shapes
      [T, B, ACTION_SPACE[0]],
      ...,
      [T, B, ACTION_SPACE[-1]]
      with un-normalized log-probabilities parameterizing the softmax behavior
      policy.
    ...
    behavior_action_log_probs: precalculated values of the behavior actions
  ...
  Returns:
    A `VTraceFromLogitsReturns` namedtuple with the following fields:
      ...
      behavior_action_log_probs: A float32 tensor of shape [T, B] containing
        behavior policy action log probabilities (log \mu(a_t)).
      target_action_log_probs: A float32 tensor of shape [T, B] containing
        target policy action probabilities (log \pi(a_t)).
  """

    for i in range(len(behavior_policy_logits)):
        behavior_policy_logits[i] = tf.convert_to_tensor(
            behavior_policy_logits[i], dtype=tf.float32)
        target_policy_logits[i] = tf.convert_to_tensor(
            target_policy_logits[i], dtype=tf.float32)

        # Make sure tensor ranks are as expected.
        # The rest will be checked by from_action_log_probs.
        behavior_policy_logits[i].shape.assert_has_rank(3)
    ...
    with tf.name_scope(
        ...
        if (len(behavior_policy_logits) > 1
                or behavior_action_log_probs is None):
            ...
            behavior_action_log_probs = (
                multi_log_probs_from_logits_and_actions(
                    behavior_policy_logits, actions, dist_class, model))
...
def from_importance_weights(log_rhos,
  ...
  Args:
    log_rhos: A float32 tensor of shape [T, B] representing the
      ...
      log(target_policy(a) / behavior_policy(a)). V-trace performs operations
...
def get_log_rhos(target_action_log_probs, behavior_action_log_probs):
    ...
    b = tf.stack(behavior_action_log_probs)
