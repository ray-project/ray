...
def multi_log_probs_from_logits_and_actions(policy_logits, actions, dist_class,
    ...
    for i in range(len(policy_logits)):
        ...
        policy_logits_flat = torch.reshape(
            policy_logits[i], (-1, ) + tuple(p_shape[2:]))
        actions_flat = torch.reshape(
            actions[i], (-1, ) + tuple(a_shape[2:]))
        log_probs.append(torch.reshape(
...
def from_logits(behaviour_policy_logits,
    ...
    return VTraceFromLogitsReturns(
        ...
        behaviour_action_log_probs=torch.squeeze(
            res.behaviour_action_log_probs, dim=0),
        target_action_log_probs=torch.squeeze(
            res.target_action_log_probs, dim=0),
    )
...
def multi_from_logits(behaviour_policy_logits,
    ...
    target_policy_logits: A list with length of ACTION_SPACE of float32 tensors
        of shapes [T, B, ACTION_SPACE[0]], ..., [T, B, ACTION_SPACE[-1]]
    ...
    discounts: A float32 tensor of shape [T, B] with the discount encountered
        when following the behavior policy.
    ...
    behaviour_action_log_probs: precalculated values of the behavior actions
...
def from_importance_weights(log_rhos,
    ...
    if clip_rho_threshold is not None:
        clip_rho_threshold = torch.from_numpy(clip_rho_threshold).float()
    if clip_pg_rho_threshold is not None:
        clip_pg_rho_threshold = torch.from_numpy(clip_pg_rho_threshold).float()
    ...
    if clip_rho_threshold is not None:
        assert len(clip_rho_threshold.size()) == 0
    if clip_pg_rho_threshold is not None:
        assert len(clip_pg_rho_threshold.size()) == 0
    ...
    for i in reversed(range(len(discounts))):
        ...
        vs_minus_v_xs = delta_t + discount_t * c_t * vs_minus_v_xs
