...
from ray.rllib.agents.sac.sac_torch_policy import build_sac_model_and_action_dist, \
    _get_dist_class, stats, optimizer_fn, ComputeTDErrorMixin, TargetNetworkMixin, \
...
def cql_loss(
    ...
    if policy.cur_iter >= bc_iters:
        ...
        if twin_q:
            twin_q = model.get_twin_q_values(
    ...
    critic_loss = [
        nn.MSELoss()(q_t, q_t_target)
    ]
    if twin_q:
        critic_loss.append(
            nn.MSELoss()(twin_q_t, q_t_target))

    # CQL Loss (We are using Entropy version of CQL (the best version))
    rand_actions = convert_to_torch_tensor(torch.FloatTensor(actions.shape[0] * num_actions, actions.shape[-1]).uniform_(action_low, action_high))
    ...
    cat_q1 = torch.cat(
        [q1_rand - random_density, q1_next_actions - next_logp.detach(), q1_curr_actions - curr_logp.detach()], 1
    )
    if twin_q:
        cat_q2 = torch.cat(
            [q2_rand - random_density, q2_next_actions - next_logp.detach(), q2_curr_actions - curr_logp.detach()], 1
        )

    min_qf1_loss = torch.logsumexp(cat_q1 / cql_temp, dim=1).mean() * min_q_weight * cql_temp
    min_qf1_loss = min_qf1_loss - q_t.mean() * min_q_weight
    if twin_q:
        min_qf2_loss = torch.logsumexp(cat_q2 / cql_temp, dim=1).mean() * min_q_weight * cql_temp
