...
from ray.rllib.agents.cql import CQLSACTrainer, CQLSAC_DEFAULT_CONFIG
...
class TestCQLSAC(unittest.TestCase):
    @classmethod
    ...
    def test_cqlsac_compilation(self):
        """Tests whether an SACTrainer can be built with all frameworks."""
        config = CQLSAC_DEFAULT_CONFIG.copy()
        ...
        config["train_batch_size"] = 256 #10
        config["input"] = "rllib/tests/data/moab/*.json"
        config["input_evaluation"] = []
        config["bc_iters"] = 5
        config["temperature"] = 1.0
        config["num_actions"] = 10
        config["lagrangian"] = True # False
        # Lagrangian Threshold
        config["lagrangian_thresh"] = 5.0
        config["min_q_weight"] = 5.0
        # Initial value to use for the Alpha Prime (in CQL Loss).
        config["initial_alpha_prime"] = 1.0
        ...
        for fw in framework_iterator(config=config, frameworks=("tf2", "tf", "tfe")):
            # Test for different env types (discrete w/ and w/o image, + cont).
            for env in [
                    # RandomEnv,
                    # "MsPacmanNoFrameskip-v4",
                    # "CartPole-v0",
                    MOAB_MOVE_TO_CENTER_ENV_NAME,
            ]:
                ...
                trainer = CQLSACTrainer(config=config, env=env)
    ...
    @unittest.skip("TODO(Edi): Adapt...")
    def test_cqlsac_loss_function(self):
        self.skipTest("TODO(Edi): Adapt...")
