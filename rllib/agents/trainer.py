...
class Trainer(Trainable):
    ...
    def setup(self, config: PartialTrainerConfigDict):
        ...
        # Update the user provided callbacks with RE3 specific changes
        if self.config["exploration_config"].get("type", None) == "RE3":
            ...
            embeds_dim = self.config["exploration_config"].get(
                "embeds_dim", 128)
            k_nn = self.config["exploration_config"].get("k_nn", 50)
            beta = self.config["exploration_config"].get("beta", 0.1)
            rho = self.config["exploration_config"].get("rho", 0.0001)
            beta_schedule = self.config["exploration_config"].get(
                "beta_schedule", "constant")

            class UpdateCallbacks(self.config["callbacks"]):
                """Update input callbacks method to mutate batch with states entropy rewards."""
                ...
                def on_learn_on_batch(
                    ...
                    states_entropy = update_beta(
                        beta_schedule, beta, rho,
                        UpdateCallbacks._step) * np.reshape(
            ...
            self.config["callbacks"] = UpdateCallbacks
