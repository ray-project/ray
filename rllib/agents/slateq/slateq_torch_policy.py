...
def action_distribution_fn(
    ...
    #if user.shape[0] == 1:
    #    dots = torch.stack([torch.dot(val.squeeze(), user.squeeze()) for val in obs["doc"].values()])
    #    best_dot = torch.argmax(dots)
    #    best_per_slate_q_values = torch.argmax(per_slate_q_values)

        #return dots.unsqueeze(0), TorchCategorical, []

        #print(f"best_dot: {best_dot} best_per_slate_q={best_per_slate_q_values}")
