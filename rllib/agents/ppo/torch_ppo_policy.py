...
from ray.rllib.agents.ppo.tf_ppo_policy import postprocess_ppo_gae, setup_config
...
def kl_and_loss_stats(policy, train_batch):
    return {
        ...
        "vf_explained_var": explained_variance(
            train_batch[Postprocessing.VALUE_TARGETS],
            policy.model.value_function().detach(), backend="torch").detach().numpy(),
...
def vf_preds_and_logits_fetches(policy, input_dict, state_batches, model, action_dist):
