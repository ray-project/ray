...
class ConvEncoder(nn.Module):
  ...
  """
  def __init__(self, depth: int = 32, act: ActFunc = None, shape: List = [3, 64, 64]):
...
class ConvDecoder(nn.Module):
  ...
  """
  def __init__(self, input_size: int, depth: int=32, act: ActFunc=None, shape: List=[3, 64, 64]):
...
class DenseDecoder(nn.Module):
  """Fully Connected network that outputs a distribution for calculating log_prob later in DreamerLoss
  """
  def __init__(self, input_size: int, output_size: int, layers: int, units: int, dist: str ='normal', act: ActFunc=None):
    ...
    Arguments:
      ...
      dist (str): Output distribution, parameterized by logits of network output
...
class ActionDecoder(nn.Module):
  """ActionDecoder is the policy module in Dreamer. It outputs a distribution parameterized by 
  mean and std, later to be transformed by a custom TanhBijector in utils.py for Dreamer.
  """
  def __init__(self, 
    input_size: int, 
    action_size: int, 
...
class RSSM(nn.Module):
  """RSSM is the core recurrent part of the PlaNET module. It consists of two networks,
  one (obs) to calculate posterior beliefs and states and the second (img) to calculate
  prior beliefs and states. The prior network takes in the previous state and action,
  while the posterior network takes in the previous state, action, and a latent embedding
  of the most recent observation.
  """
  def __init__(self, action_size: int, embed_size: int, stoch: int=30, deter: int=200, hidden: int=200, act: ActFunc=None):
    """Initializes RSSM
  ...
  def get_initial_state(self, batch_size: int) -> List[TensorType]:
    ...
    for the stochastic state, the sampled stochastic hidden state (from mean, std),
    and the deterministic hidden state, which is pushed through the GRUCell.
  ...
  def observe(self, embed: TensorType, action: TensorType, state: List[TensorType]=None) -> Tuple[List[TensorType], List[TensorType]]:
  ...
  def imagine(self, action: TensorType, state: List[TensorType]=None)->List[TensorType]:
  ...
  def obs_step(self, prev_state: TensorType, prev_action: TensorType, embed: TensorType) -> Tuple[List[TensorType], List[TensorType]]:
  ...
  def img_step(self, prev_state: TensorType, prev_action: TensorType) -> List[TensorType]:
  ...
  def get_feature(self, state: List[TensorType]) -> TensorType:
    # Constructs feature for input to reward, decoder, actor, and critic network
