...
from dreamerv3.utils import TwoHotEncoder
...
class ConvEncoder(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            act: Activation for Encoder, default SiLU
...
class DenseDecoder(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(self.num_layers):
            self.layers.extend([Linear(cur_size, self.units), nn.LayerNorm(self.units), self.act()])
            cur_size = units
    ...
    def forward(self, x):
        ...
        elif self.dist == 'softmax':
            return F.softmax(x, dim=-1)
        elif self.dist == 'sigmoid':
...
class ActionDecoder(nn.Module):
    ...
    def __init__(
        ...
        cur_size = input_size
        for _ in range(self.num_layers):
            self.layers.extend([Linear(cur_size, self.units), nn.LayerNorm(self.units), self.act()]) # TODO layernorm?
            cur_size = self.units
    ...
    def forward(self, x):
        ...
        if self.dist == "tanh_normal":
            ...
            mean, std = torch.chunk(x, 2, dim=-1)
            dist = td.Normal(mean, F.softplus(std))
            dist = td.Independent(dist, 1)

        self.action_dist = dist
...
class DreamerV3Model(TorchModelV2, nn.Module):
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        self.reward = DenseDecoder(
            self.stoch_size + self.deter_size, self.model_config['bucket_size'],
            2, self.hidden_size, 'softmax'
        )
        self.continue_ = DenseDecoder(
            self.stoch_size + self.deter_size, 1, 2, self.hidden_size, 'sigmoid'
        )  # TODO: bias?
        ...
        self.twohot = TwoHotEncoder(device=self.device)
        # self.cuda_bins = torch.FloatTensor(self.twohot.bins).to(self.device)
        # self.bucket_boundary = torch.arange(-129., 128.).to(self.device)

        # _table, _rev = combins_table(model_config['bucket_size'], 2)
        # self.twohot_table = torch.tensor(_table, dtype=torch.float32, device=self.device)
        # self.rev_twohot_table = _rev
        pprint(self)
