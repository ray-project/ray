...
from ray.rllib.contrib.bandits.agents.linear import LinTSTrainer, TS_CONFIG
...
if __name__ == "__main__":
    ...
    # Actual training_iterations will be 20 * timesteps_per_iteration (100 by default) = 5,000
    training_iterations = 20

    print("Running training for %s time steps" % training_iterations)

    start_time = time.time()
    analysis = tune.run(LinTSTrainer, config=TS_CONFIG,
    ...
    x = frame.groupby("num_steps_trained")["learner/cumulative_regret"].aggregate(["mean", "max", "min", "std"])

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(8,4))


    ax1.plot(x["mean"])
    # ax1.fill_between(x.index, x["mean"] - x["std"], x["mean"] + x["std"], color='b', alpha=0.2)
