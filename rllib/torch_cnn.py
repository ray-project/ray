...
class TF2V2NatureCNN(TFModelV2):
    def __init__(
        ...
        self.cnn.add(keras.layers.Conv2d(32, 8, 4, activation="relu", input_shape=(feature_size[-1], 3)))
...
torchv2_model = TorchV2NatureCNN(env.observation_space, env.action_space, 6, ModelConfigDict(), "bork").to(device)
obs_space = gym.spaces.Box(dtype=np.uint8, low=0, high=255, shape=(84,84,1))
# 16
# 32
# 256
torch_visionnet = VisionNetwork(obs_space, env.action_space, 6, ModelConfigDict(vf_share_layers=True), "bork").to(device)
# 21, 21, 16
# 11, 11, 32
# 1, 1, 256,
# 1, 1, 6
tf_visionnet = KerasVisionNetwork(obs_space, env.action_space, 6, ModelConfigDict(vf_share_layers=True), "bork")
...
for model in torch_models:
    ...
    for i in range(num_epochs):
        start = time.time()
        torch_opt.zero_grad()
        loss = torch_policy.loss(model=model, dist_class=TorchCategorical, train_batch=datas[i])
        loss.backward()
        torch_opt.step()
        if device != 'cpu':
            torch.cuda.synchronize()
