...
def ask_user_for_action(observations):
...
if __name__ == "__main__":
    ...
    def convert_box(convert_obs_fn, old_box):
        ...
        return gym.spaces.Box(low=new_low, high=new_high, dtype=new_low.dtype)
    ...
    if args.num_episodes_human_play > 0:
        ...
        env = flattened_pettingzoo_env()

        while num_episodes < args.num_episodes_human_play:
            print("\nYou play as {}".format("black" if human_player else "red"))
            env.reset()
            ...
            for agent in env.agent_iter():
                ...
                if termination or truncation:
                    ...
                    print(f"\nAgent: {agent_name}, Reward: {reward}")
                    if agent_name != "human":
                        if reward > 0:
                            print("You win")
                        elif reward < 0:
                    ...
                    if agent == env.possible_agents[human_player]:
                        action = ask_user_for_action(observation)
                    else:
                    ...
                    env.step(action)  # TODO: should this be [action]
                env.render(mode="human")
            env.render(mode="human")
