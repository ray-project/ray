...
class FrameStackingCartPoleModel(TFModelV2):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config,
        ...
        input_ = tf.keras.layers.Input(shape=(self.num_frames, obs_space.shape[0]))
        ...
        self.view_requirements["prev_n_obs"] = ViewRequirement(
            data_col="obs",
            shift="-{}:0".format(num_frames - 1), space=obs_space)
...
class TorchFrameStackingCartPoleModel(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config,
        ...
        assert len(obs_space.shape) == 1
        self.layer1 = SlimFC(
            in_size=obs_space.shape[0] * self.num_frames, out_size=64,
            activation_fn="relu")
        self.out = SlimFC(
            in_size=64, out_size=self.num_outputs,
            activation_fn="linear")
        self.values = SlimFC(
            in_size=64, out_size=1, activation_fn="linear")
