...
class TestViewRequirementConnector(unittest.TestCase):
    ...
    def test_vr_connector_respects_training_or_inference_vr_flags(self):
        ...
        For training,
            the returned data should be the data itself. The higher level policy collector in env_runner will construct the proper data structure.
        """
        view_rq_dict = {
            "both": ViewRequirement("obs",
                used_for_training=True, used_for_compute_actions=True
            ),
            "only_inference": ViewRequirement("obs",
                used_for_training=False, used_for_compute_actions=True
            ),
            "none": ViewRequirement("obs",
                used_for_training=False, used_for_compute_actions=False
            ),
            "only_training": ViewRequirement("obs",
    ...
    def test_vr_connector_shift_by_one(self): 
        """Test that the ViewRequirementConnector can handle shift by one correctly and can ignore future refrencing view_requirements to respect causality"""
        view_rq_dict = {
        ...
        for is_training in [True, False]:
            ...
            for i, obs_arr in enumerate(obs_arrs):
                ...
                if i == 0:
                    ...
                    check(for_action['prev_strided_states_even'], obs_arrs[i-4:i:2])
                    check(for_action['prev_strided_states_even'], obs_arrs[i-3:i:2])



    def test_vr_connector_with_multiple_buffers(self): 
        """Test that the ViewRequirementConnector can handle slice shifts correctly when it has multiple buffers to shift.
        """
        context_len = 5
        # This view requirement simulates the use-case of a decision transformer without reward-to-go.
        view_rq_dict = {
        ...
        for is_training in [True, False]:
            ...
            for i in range(n_steps):
                data = AgentConnectorDataType(
                    0, 1, 
                    dict(obs=obs_arrs[i], act=act_arrs[i])
                )
                processed = c([data])
                for_action = processed[0].data.for_action
