...
from ray.rllib.models.torch.torch_distributions import (
    TorchDeterministic, TorchCategorical)
...
def do_rollouts(env: gym.Env, module_for_inference: RLModule, num_rollouts: int) -> SampleBatch:
    _returns = []
    _rollouts = []
...
class BCTorchOptimizer(RLOptimizer):
    ...
    def compute_loss(self, batch: NestedDict, fwd_out: Mapping[str, Any]) -> Mapping[str, Any]:
        """Compute a loss"""
    ...
    def get_state(self):
        return {"module" : optim.state_dict() for optim in self._optimizers}

    def set_state(self, state: Mapping[Any, Any]) -> None:
        assert set(state.keys()) == set(self.get_optimizers().keys())
...
class BCTorchTrainer:
    ...
    @staticmethod
    def on_after_compute_gradients(gradients_dict: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    def compile_results(
        self, batch: NestedDict, fwd_out: Mapping[str, Any], postprocessed_loss: Mapping[str, Any], post_processed_gradients: Mapping[str, Any]
    )-> Mapping[str, Any]:
    ...
    def update(self, batch: NestedDict) -> Mapping[str, Any]:
        ...
        results = self.compile_results(
            batch, fwd_out, loss, post_processed_gradients
        )
        return results

    def compute_gradients(self, optimization_vars: Mapping[str, Any]) -> Mapping[str, Any]:
...
def test_rl_optimizer_torch():
    ray.init()
    torch.manual_seed(1)
    env = gym.make("CartPole-v1")
    module_config = FCConfig(input_dim=sum(env.observation_space.shape),
        output_dim=sum(env.action_space.shape or [env.action_space.n]), hidden_layers=[32])
    ...
    for _ in range(num_epochs):
        ...
        if avg_return > 50:
            break
    assert avg_return > 50, f"Return for training behavior cloning is too low: avg_return={avg_return}!"
