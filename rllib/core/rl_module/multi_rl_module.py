...
class MultiRLModuleSpec:
    ...
    #: Optional global observation space for the MultiRLModule. Useful for shared
    #: network components that live only inside the MultiRLModule and don't have their
    #: own ModuleID and own RLModule within `self._rl_modules`.
    observation_space: Optional[gym.Space] = None

    #: Optional global action space for the MultiRLModule. Useful for shared network
    #: components that live only inside the MultiRLModule and don't have their own
    #: ModuleID and own RLModule within `self._rl_modules`.
    action_space: Optional[gym.Space] = None

    #: An optional global inference_only flag. If not set (None by default), considers
    #: the MultiRLModule to be inference_only=True, only if all submodules also have
    #: their own inference_only flags set to True.
    inference_only: Optional[bool] = None
    ...
    #: An optional global model_config dict. Useful to configure shared network
    #: components that only live inside the MultiRLModule and don't have their own
    #: ModuleID and own RLModule within `self._rl_modules`.
    model_config: Optional[dict] = None
    #: The module specs for each individual module. It can be either a RLModuleSpec used
    #: for all module_ids or a dictionary mapping from module IDs to RLModuleSpecs for
