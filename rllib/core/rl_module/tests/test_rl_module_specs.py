...
class TestRLModuleSpecs(unittest.TestCase):
    ...
    def test_single_agent_module_spec_load_from_checkpoint(self):
        """Test if building a SingleAgentRLModuleSpec from a checkpoint works."""
        env = gym.make("CartPole-v1")
        for module_class in MODULES:
            spec = SingleAgentRLModuleSpec(
                module_class=module_class,
            ...
            with tempfile.TemporaryDirectory() as checkpoint_path:
                ...
                check(module.get_state(), module_from_checkpoint.get_state())

    def test_multi_agent_module_spec_load_from_checkpoint(self):
        """Test if building a MultiAgentRLModuleSpec from a checkpoint works."""
        env = gym.make("CartPole-v1")
        num_agents = 2
        for module_class in MODULES:
            module_specs = {}
    ...
    def test_multi_agent_module_spec_load_some_modules_from_other_checkpoints(self):
