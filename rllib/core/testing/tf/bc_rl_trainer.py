...
class BCTfRLTrainer(TfRLTrainer):
    ...
    def compute_loss(self, fwd_out: MultiAgentBatch, batch: MultiAgentBatch) -> Mapping[str, Any]:

        loss_dict = {}
        loss_total = None
        for module_id in fwd_out:
            action_dist = fwd_out[module_id]["action_dist"]
            loss = -tf.math.reduce_mean(action_dist.log_prob(batch[module_id][SampleBatch.ACTIONS]))
