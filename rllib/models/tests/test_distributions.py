...
class TestDistributions(unittest.TestCase):
    ...
    def test_diag_gaussian(self):
        ...
        for fw, sess in framework_iterator(
            ...
            # sampling and logp'ing result in NaN or +/-inf values.
#            self._stability_test(
#                cls, input_space.shape, fw=fw, sess=sess)

            #TODO currently failing the stability test by returning +-inf when std is 10e34
            ...
            # NN output.
            means = np.array([[0.1, 0.2, 0.3, 0.4, 50.0],
                              [-0.1, -0.2, -0.3, -0.4, -1.0]], dtype=np.float32)
            log_stds = np.array([[0.8, -0.2, 0.3, -1.0, 2.0],
                                 [0.7, -0.3, 0.4, -0.9, 2.0]],dtype=np.float32)

            print(means.dtype)  #TODO should be float32 instead of float64
            ...
            print(values.dtype, log_prob.dtype)
            #print(diag_distribution.logp(values))
