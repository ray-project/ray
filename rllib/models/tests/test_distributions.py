...
class TestDistributions(unittest.TestCase):
    ...
    def test_multi_action_distribution(self):
        ...
        for fw, sess in framework_iterator(session=True):
            ...
            inputs[2] = np.clip(inputs[2], np.log(SMALL_NUMBER),
                                -np.log(SMALL_NUMBER))
            ...
            expected_det = [
                ...
                (1.0 / (1.0 + inputs[2][:, 2:] / inputs[2][:, 0:2])) *
                (high - low) + low,
            ]
            ...
            check(
                np.mean(out.batches[1], 0),
                np.mean(expected_mean[1], 0),
                decimals=1)
            check(
                np.mean(out.batches[2], 0),
            ...
            log_prob_beta = np.log(
                beta.pdf(values[2], inputs[2][:, :2], inputs[2][:, 2:]))
