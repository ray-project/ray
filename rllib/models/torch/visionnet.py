...
class Torch_VisionNetwork(nn.Module):
    """Generic vision network as native torch.nn.Module."""
    ...
    def __init__(
        ...
        # Post FC net config.
        post_fcnet_activation = get_activation_fn(
            post_fcnet_activation, framework="torch")
        ...
        layers = []
        # Perform Atari framestacking via traj. view API.
        if num_framestacks != "auto" and num_framestacks > 1:
            (w, h) = input_space.shape
            in_channels = num_framestacks
            self.traj_view_framestacking = True
        else:
            (w, h, in_channels) = input_space.shape
    ...
    def forward(self, input_dict: SampleBatch) -> \
        ...
        # Calculate the value outputs:
        # Separate value branch.
        if self._value_branch_separate:
            value = self._value_branch_separate(self._features)
            value = value.squeeze(3)
            value = value.squeeze(2)
            value_outs = value.squeeze(1)
        # Shared value network, just the output layer is separate.
        else:
            if not self.last_layer_is_flattened:
                features = self._features.squeeze(3)
                features = features.squeeze(2)
                ...
                features = self._features
            value_outs = self._value_branch(features).squeeze(1)

        return model_out, [], {SampleBatch.VF_PREDS: value_outs}
