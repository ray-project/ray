...
class SquashedGaussian(TFActionDistribution):
    ...
    @override(TFActionDistribution)
    def sampled_action_logp(self):
        unsquashed_action = self.distr.sample()
        action = tf.math.tanh(unsquashed_action)
        log_prob = self.distr.log_prob(unsquashed_action)
        log_prob -= tf.math.reduce_sum(
            tf.math.log(1 - action**2 + SMALL_NUMBER), axis=-1, keepdims=True)

        squashed_action = (action + 1) / 2 * (self.high - self.low) + self.low
