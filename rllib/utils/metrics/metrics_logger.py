...
class MetricsLogger:
    ...
    def merge_and_log_n_dicts(
        ...
        *,
        ...
        for key in all_keys:
            ...
            all_stats = incoming_stats.copy() # copy to avoid mutating the original list
            ...
            if base_stats is None:
                assert self._key_in_stats(extended_key), f"Trying to merge metrics for a key {extended_key} where the merging MetricsLogger {self} has not Stats object for the key and no Stats object was found in the incoming values {incoming_stats}."
                base_stats = self._get_key(extended_key)
            ...
            if base_stats._reduce_method == "mean":
                ...
                if own_stats is not None:
                    all_stats += [own_stats]
                # First reduce any Stats objects that have new values
                for stat in all_stats:
                    if stat._has_new_values:
                        stat.reduce()
                
                # Collect all values from all available Stats objects
                all_values = []
                for stat in all_stats:
                    all_values.extend(stat.values)
    ...
    def reduce(self) -> Dict:
        ...
        Returns:
            ...
            logged keys to this MetricsLogger) with the leafs being the reduced values
