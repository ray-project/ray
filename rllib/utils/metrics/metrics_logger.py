...
class MetricsLogger:
    ...
    def __init__(self):
        ...
        # Track whether reduce() has been called yet
    ...
    def merge_and_log_n_dicts(
        ...
        for key in all_keys:
            incoming_stats = [
                self._get_key(key, stats=s if isinstance(s, dict) else s.stats)
                for s in stats_dicts_or_loggers
                if self._key_in_stats(key, stats=s if isinstance(s, dict) else s.stats)
            ]
    ...
    def reduce(self) -> Dict:
        ...
        On first call to this method, it returns a dictionary of Stats objects instead
        of reduced values. This allows us to properly merge Stats objects downstrea.
        On subsequent calls, it returns the reduced values as usual.
        ...
        Returns:
            ...
            logged keys to this MetricsLogger). On first call, the leafs are the Stats
            objects themselves. On subsequent calls, the leafs are the reduced values
            from the underlying Stats objects.
        """
        ...
        def _reduce(path, stats: Stats):
            ...
            if not self._reduce_called:
                # If this is the first call to reduce(), return a Stats object such that downstream MetricsLoggers can merge on it
        ...
        # At the end, check if this is the first call to reduce()
        if not self._reduce_called:
