...
class TestSchedules(unittest.TestCase):
    ...
    def test_linear_schedule(self):
        ...
        for fw in ["tf", "torch", None]:
            linear = from_config(LinearSchedule, {
    ...
    def test_polynomial_schedule(self):
        ...
        for fw in ["tf", "torch", None]:
            polynomial = from_config(dict(
                type="ray.rllib.utils.schedules.polynomial_schedule."
                "PolynomialSchedule",
                schedule_timesteps=100,
                initial_p=2.0, final_p=0.5, power=2.0, framework=fw))
            for t in ts:
                out = polynomial(t)
    ...
    def test_exponential_schedule(self):
        ts = [0, 5, 10, 100, 90, 2, 1, 99, 23]
        for fw in ["tf", "torch", None]:
            exponential = from_config(ExponentialSchedule, dict(
                initial_p=2.0, decay_rate=0.99,
                schedule_timesteps=100, framework=fw))
            for t in ts:
                out = exponential(t)
                check(out, 2.0 * 0.99 ** (t/100))

    def test_piecewise_schedule(self):
        piecewise = from_config(PiecewiseSchedule, dict(
