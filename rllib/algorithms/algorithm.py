...
class Algorithm(Trainable, AlgorithmBase):
    ...
    def _evaluate_async_with_env_runner(
        ...
        # units.
            ...
            def remote_fn(worker):
                # Pass in seq-no so that eval workers may ignore this call if no
                # update has happened since the last call to `remote_fn` (sample).
                worker.set_weights(
                    weights=ray.get(weights_ref), weights_seq_no=weights_seq_no
                )
                # By episode: Run always only one episode per remote call.
                # By timesteps: By default EnvRunner runs for the configured number
                # of timesteps (based on `rollout_fragment_length` and
                # `num_envs_per_worker`).
