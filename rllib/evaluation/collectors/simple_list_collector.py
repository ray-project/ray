...
class _AgentCollector:
    ...
    def _build_buffers(self, single_row: Dict[str, TensorType]) -> None:
        ...
        for col, data in single_row.items():
            ...
            if col in [SampleBatch.INFOS, SampleBatch.ACTIONS] or col.startswith(
                "state_out_"
            ):
...
class SimpleListCollector(SampleCollector):
    ...
    def add_init_obs(
        ...
        view_reqs = (
            policy.model.view_requirements
            if getattr(policy, "model", None)
            else policy.view_requirements
        )
