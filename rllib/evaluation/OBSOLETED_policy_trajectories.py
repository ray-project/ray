...
class PolicyTrajectories:
    ...
    def __init__(self, B: Optional[int] = None,
                 T: Optional[int] = None):
        ...
        self.B = B or 1000
        self.buffers = {}
        # The offset on the B axis to start the next SampleBatch build from.
        self.sample_batch_offset = 0
        self.T = T
        assert T, "Only supports RNN style PolicyTrajectories for now!"
        self.seq_lens = []
    ...
    def add_sample_batch(self, sample_batch):
        ...
        #import math
        fill_up = ts % self.T
        chunks = (ts // self.T) + (1 if fill_up else 0)

        # Extend (re-alloc) buffers if full.
        if self.B_cursor >= self.B:
            self._extend_buffers(sample_batch)

        for k, column in sample_batch.items():
            for c in range(chunks):
                if c == chunks - 1 and fill_up:
                    self.buffers[k][:fill_up, self.B_cursor + c] = column[
                                                            self.T * c:self.T * (
                                                                        c + 1)]
                else:
                    self.buffers[k][:,self.B_cursor + c] = column[self.T * c:self.T * (c+1)]
        ...
        self.seq_lens.extend(([self.T] * (chunks - 1)) + [fill_up])
        #else:
        #    self.cursor += ts

        self.B_cursor += chunks
    ...
    def get_sample_batch_and_reset(self) -> SampleBatch:
        ...
        for k, v in self.buffers.items():
            data[k] = to_float_array(
                v[:,self.sample_batch_offset:self.B_cursor])  #, reduce_floats=True)
        batch = SampleBatch(
            data,
        ...
        self.sample_batch_offset = self.B_cursor
        self.seq_lens = []
        return batch
    ...
    def _build_buffers(self, sample_batch: SampleBatch) -> None:
        ...
        for col, data in sample_batch.items():
            ...
            # np.ndarray, torch.Tensor, or tf.Tensor.
                ...
                shape = (self.T, self.B, ) + data.shape[1:]
                dtype = data.dtype
...
class _OBSOLETEPolicyTrajectories:
    """Aggregator for many single-agent SampleBatches (all using same Policy).
