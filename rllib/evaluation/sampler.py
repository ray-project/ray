        ...
        for agent_id, raw_obs in all_agents_obs.items():
            ...
            if last_observation is None:
                sample_collector.add_init_obs(
                    episode, agent_id, env_id, policy_id,
                    episode.length - 1, filtered_obs)
            elif agent_infos is None or agent_infos.get("training_enabled",
                                                        True):
                # Add actions, rewards, next-obs to collectors.
                values_dict = {
                    SampleBatch.T: episode.length - 1,
                    SampleBatch.ENV_ID: env_id,
                    SampleBatch.AGENT_INDEX: episode._agent_index(
                        agent_id),
                    ...
                    SampleBatch.REWARDS: rewards[env_id].get(
                        agent_id, 0.0),
                    # After taking action=a, did we reach terminal?
                    SampleBatch.DONES: (False if
                                        (no_done_at_end
