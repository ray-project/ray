...
class _EnvRunner(Iterator[SampleBatch]):
    def __init__(self,
        ...
        self.simple_image_viewer: Optional["SimpleImageViewer"] = None
    ...
    def __next__(self) -> SampleBatch:
        while not self.outputs:
            ...
            if self.render:
                ...
                rendered = self.base_env.try_render()
                ...
                if isinstance(rendered, np.ndarray) and len(rendered.shape) == 3:
                    ...
                    if self.simple_image_viewer is None:
                        try:
                            ...
                            self.render = False  # disable rendering
                            logger.warning(
                                "Could not import gym.envs.classic_control."
                                "rendering! Try `pip install gym[all]`.")
                    if self.simple_image_viewer:
                        self.simple_image_viewer.imshow(rendered)
                self.perf_stats.env_render_time += time.time() - t5
    ...
    def _process_observations(self,
        ...
        for env_id, all_agents_obs in unfiltered_obs.items():
            ...
            if not is_new_episode:
                self.sample_collector.episode_step(episode)
