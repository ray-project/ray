...
class _MoabBaseWrapper(gym.Env):
    ...
    def reset(self):
        ...
        initial_x = self.np_random.uniform(low=-_MoabBaseWrapper.RadiusOfPlate * 0.5,
                                           high=_MoabBaseWrapper.RadiusOfPlate * 0.5)
        initial_y = self.np_random.uniform(low=-_MoabBaseWrapper.RadiusOfPlate * 0.5,
                                           high=_MoabBaseWrapper.RadiusOfPlate * 0.5)
        # initial ball state after updating plate
        self.model.set_initial_ball(initial_x, initial_y, self.model.ball.z)

        initial_vel_x = self.np_random.uniform(low=-_MoabBaseWrapper.MaxVelocity * 0.02,
        ...
        self.model.ball_vel.x = initial_vel_x
        self.model.ball_vel.y = initial_vel_y

        initial_pitch = self.np_random.uniform(low=-0.2, high=0.2)
        initial_roll = self.np_random.uniform(low=-0.2, high=0.2)
        # initial control state. these are all [-1..1] unitless
        self.model.roll = initial_roll
        self.model.pitch = initial_pitch
