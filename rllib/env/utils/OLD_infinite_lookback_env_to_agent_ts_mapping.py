...
class InfiniteLookbackEnvToAgentTsMapping:
    """Stores and manages mappings from env- to agent timesteps.

    This class stores a list of "global" env timesteps, i.e. environment steps in a
    multi-agent setup. Each agent in such a multi-agent setup is provided with
    an instance of this class. The list indices hereby are the agent timesteps from 0
    (first time the agent stepped/acted), and from then on increasing by one. Note
    that also the observations at agent ts=0 is the reset observation, the action at
    ts=0 is the first action taken (after receiving the reset observation), the reward
    at ts=0 is the first reward received after taking action 0 after having received
    the reset observation. 
    Each time an agent steps in the multi-agent environment, another (global) env
    timestep is added to this class.

    The infinite lookback functionality allows views into timesteps of a previous
    chunk of the multi-agent episode. This is useful for cases in which episode data
    had been collected and returned before the episode was actually terminated or done
    as of the decision of the Env and thus is continued in a second rollout. The
    lookback enables a view into past env timesteps at which agents had stepped.

    See also the `MultiAgentEpisode` class.
    """

    def __init__(self, timesteps: list = None, lookback: int = 0, t_started: int = 0):
        """Initializes a InfiniteLookbackEnvToAgentTsMapping instance.
        ...
        Args:
            ...
            t_started: The env timestep at which the lookback should start
    ...
    def __add__(self, other):
        if self.timesteps[-1] > other[other.lookback]:
            ...
            return InfiniteLookbackEnvToAgentTsMapping(
    ...
    def cut(self, lookback, t_started):
        ...
        indices = self.get_agent_timesteps(slice(t_started - lookback, t_started + 1))
    ...
    def get_agent_timesteps(
        self,
        env_timesteps: Optional[Union[int, List[int], slice]] = None,
        *,
        ...
        """Maps env timestep(s) to corresponding agent timestep(s).
        ...
        if not self.timesteps:
            return None if isinstance(env_timesteps, int) else []

        # If no env timesteps are provided return all of them.
        if env_timesteps is None:
            return self._get_all_local_timesteps()
        # If a `slice` is requested, return a `slice` as long as `return_none=False`.
        elif isinstance(env_timesteps, slice):
            return self._get_slice_agent_timesteps(
                env_timesteps,
        ...
        elif isinstance(env_timesteps, list):
            return self._get_list_of_int_agent_timesteps(
                env_timesteps,
                neg_timesteps_left_of_zero=neg_timesteps_left_of_zero,
                t=t,
                #shift=shift,
        ...
        # Must be single time step.
            ...
            assert isinstance(env_timesteps, int)
            return self._get_int_agent_timestep(
                agent_timestep=env_timesteps,
                neg_timesteps_left_of_zero=neg_timesteps_left_of_zero,
                #return_none=return_none,
                t=t,
                #shift=shift,
    ...
    def _get_all_local_timesteps(self):
        ...
        # env lookback: t_started=100, lookback=10, self.lookback=0,
        # self.timesteps[self.lookback=0]=88).
        return self.timesteps

    def _get_int_agent_timestep(
        self, agent_timestep, neg_timesteps_left_of_zero, t, #return_none, t #, shift=0
    ):
        # User wants negative timesteps.
        if agent_timestep < 0:
            # User wants to index into the lookback before `t_started`.
            if neg_timesteps_left_of_zero:
                # Translate negative timestep to env timestep.
                agent_timestep = agent_timestep + self.t_started
            # User wants to look back from the end.
            else:
                # Translate negative timestep to env timestep
                agent_timestep = len(self) + self.lookback + agent_timestep
        ...
        if agent_timestep in self.timesteps:
            return self.timesteps.index(agent_timestep) + self.t_started - 1
    ...
    def _get_slice_agent_timesteps(
        self,
        agent_timesteps: slice,
        ...
        start = agent_timesteps.start
        stop = agent_timesteps.stop
        step = agent_timesteps.step
        ...
        if stop is None:
            ...
            if step is None or step > 0:
                # Stop after the last recorded env timestep.
        ...
        # performance.
        local_timesteps = self._get_list_of_int_agent_timesteps(
            env_timesteps,
            neg_timesteps_left_of_zero=neg_timesteps_left_of_zero,
            #return_none=return_none,
            #t=t,
    ...
    def _get_list_of_int_agent_timesteps(
        self, agent_timesteps, neg_timesteps_left_of_zero #, return_none, t, shift=0
    ):
        # Get all local timesteps.
        local_timesteps = [
            self._get_int_agent_timestep(
                ts, neg_timesteps_left_of_zero=neg_timesteps_left_of_zero, #return_none, #t, #shift=shift
            )
            for ts in agent_timesteps
        ]

        # Non-existent timesteps should be returned as `None`.
        #if return_none:
        #    return [ts[0] if ts else ts for ts in local_timesteps]
        # Return only agent timesteps for env timesteps that exist in `self.timesteps`.
        #else:
