...
class TestSingelAgentEpisode(unittest.TestCase):
    ...
    def test_get_and_from_state(self):
        ...
        self.assertEqual(episode_2.multi_agent_episode_id, episode.multi_agent_episode_id)
        self.assertEqual(episode_2.t, episode.t)
        self.assertEqual(episode_2.t_started, episode.t_started)
        self.assertIsNone(np.testing.assert_array_almost_equal(episode_2.observations[5], episode.observations[5]))
        self.assertEqual(episode_2.actions[5], episode.actions[5])
        self.assertEqual(episode_2.rewards[5], episode.rewards[5])
        self.assertDictEqual(episode_2.infos[5], episode.infos[5])
        self.assertEqual(episode_2.is_terminated, episode.is_terminated)
        self.assertEqual(episode_2.is_truncated, episode.is_truncated)
        self.assertEqual(type(episode_2._observation_space), type(episode._observation_space))
        self.assertEqual(type(episode_2._action_space), type(episode._action_space))
        self.assertEqual(episode_2._start_time, episode._start_time)
        self.assertEqual(episode_2._last_step_time, episode._last_step_time)
        self.assertDictEqual(episode_2._temporary_timestep_data, episode._temporary_timestep_data)
