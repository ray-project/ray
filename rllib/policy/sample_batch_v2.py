...
def attempt_count_timesteps(tensor_dict: dict):
    ...
    on INFOS or any state_in* and state_out* keys. The number of timesteps we count
    ...
    for k, v in copy_.items():
        ...
        if (
            k == SampleBatch.INFOS
            or k.startswith("state_in")
            or k.startswith("state_out")
        ):
...
@PublicAPI
class SampleBatchV2(SampleBatch):
        ...
        if (
            ...
            if start < 0:
                data = {
                    ...
                    if k != SampleBatch.SEQ_LENS and not k.startswith("state_in")
                }
            else:
                data = {
            ...
            if state_start is not None:
                ...
                data["state_in"] = self["state_in"][state_start:state_end]
    ...
    def right_zero_pad(self, max_seq_len: int, exclude_states: bool = True):
        ...
        def _zero_pad_in_place(path, value):
            ...
            if (exclude_states is True and path[0].startswith("state_in")) or path[
                0
