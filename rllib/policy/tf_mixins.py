...
class ComputeAndClipActorCriticGrads:
    """Compute and maybe clip gradients for actor/critic models.
    """
    def __init__(self):
        pass
    ...
    def compute_gradients_fn(
        ...
        Note: For actor/critic, optimizer and loss are ignored b/c we have 3
        ...
        if self.config["framework"] in ["tf2", "tfe"]:
            ...
            if self.config["twin_q"]:
                ...
                grads_1 = tape.gradient(self.critic_loss[0], q_weights[:half_cutoff])
                grads_2 = tape.gradient(self.critic_loss[1], q_weights[half_cutoff:])
                ...
                critic_grads_and_vars = list(
                    zip(tape.gradient(self.critic_loss[0], q_weights), q_weights)
                )

            alpha_vars = [self.model.log_alpha]
            alpha_grads_and_vars = list(
                zip(tape.gradient(self.alpha_loss, alpha_vars), alpha_vars)
            )
        # Tf1.x: Use optimizer.compute_gradients()
            ...
            if self.config["twin_q"]:
                half_cutoff = len(q_weights) // 2
                base_q_optimizer, twin_q_optimizer = self._critic_optimizer
                critic_grads_and_vars = base_q_optimizer.compute_gradients(
                    self.critic_loss[0], var_list=q_weights[:half_cutoff]
                ) + twin_q_optimizer.compute_gradients(
                    self.critic_loss[1], var_list=q_weights[half_cutoff:]
                )
            else:
                critic_grads_and_vars = self._critic_optimizer[0].compute_gradients(
                    self.critic_loss[0], var_list=q_weights
                )
            alpha_grads_and_vars = self._alpha_optimizer.compute_gradients(
                self.alpha_loss, var_list=[self.model.log_alpha]
            )
