...
class JsonReader(InputReader):
    ...
    def _from_json(self, data: str) -> SampleBatchType:
        ...
        if data_type == "SampleBatch":
            if self.ioctx.worker is not None and len(self.ioctx.worker.policy_map) != 1:
                raise ValueError(
            ...
            if self.ioctx.worker is not None:
                ...
                json_data = self._adjust_obs_actions_for_policy(json_data, policy)
    ...
    def _adjust_obs_actions_for_policy(self, json_data: dict, policy: Policy) -> dict:
