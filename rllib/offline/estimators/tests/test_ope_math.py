...
class FakePolicy(TorchPolicyV2):
    ...
    In case of an improved policy, we assign higher probs to those actions that attained a higher reward and lower probs to those actions that attained a lower reward. We do the reverse in case of a worse policy.
    """
    ...
    def action_distribution_fn(
        self, model, obs_batch=None, **kwargs
    ):  
        ...
        dist_inputs = torch.ones(
            (len(inds), self.action_space.n), dtype=torch.float32
        )
...
class TestOPEMath(unittest.TestCase):
    ...
    def test_dm_dr_math(self):
        """Tests that the Direct Method and Doubly Robust methods are correct in terms of RL vs. bandits."""
