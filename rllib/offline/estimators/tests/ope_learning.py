...
def test_fqe(checkpoint: str, batch: SampleBatch, mean_ret: float):
    config = (
        DQNConfig()
...
def collect_episodes(algo, num_episodes):
    batches = []
    mean_ret = []
    gamma = algo.config["gamma"]
...
class TestOPELearning(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.random_batch, cls.random_reward = collect_episodes(algo, cls.num_episodes)

        results = algo.train()
        while results["episode_reward_mean"] < 120:
            results = algo.train()
        cls.mixed_path = algo.save_checkpoint(cls.checkpoint_dir)
        cls.mixed_batch, cls.mixed_reward = collect_episodes(algo, cls.num_episodes)

        while results["episode_reward_mean"] < 200:
            results = algo.train()
        cls.optimal_path = algo.save_checkpoint(cls.checkpoint_dir)
        cls.optimal_batch, cls.optimal_reward = collect_episodes(algo, cls.num_episodes)
        algo.stop()
    ...
    def test_random(self):
        test_fqe(self.random_path, self.random_batch, self.random_reward)

    def test_mixed(self):
        test_fqe(self.mixed_path, self.mixed_batch, self.mixed_reward)

    def test_optimal(self):
        test_fqe(self.optimal_path, self.optimal_batch, self.optimal_reward)
