...
from ray.rllib.offline.estimators import (
    ImportanceSampling, WeightedImportanceSampling, DirectMethod, DoublyRobust
)
...
class TestOPE(unittest.TestCase):
    ...
    def test_build_ope_methods(self):
        ...
        estimators = [ImportanceSampling, WeightedImportanceSampling, DirectMethod, DoublyRobust]
        for estimator_cls in estimators:
            estimator = estimator_cls(trainer.get_policy(), gamma=0.99, config={
                "k": 5, "n_iters": 600, "lr": 1e-3, "delta": 1e-5}
            )
