...
from ray.rllib.offline.dataset_reader import (
    DatasetReader, get_dataset_and_shards, _unzip_if_needed
)
...
class TestDatasetReader(unittest.TestCase):
    ...
    def test_dataset_shard_with_only_local(self):
        """Tests whether the dataset_shard function works correctly for a single shard for the local worker.""" 
        config = {
            "input": "dataset", 
            "input_config": {"format": "json", "paths": self.dset_path}
        }
        ...
        _, shards = get_dataset_and_shards(
            config, num_workers=0, local_worker=False
        )
        ...
        _, shards = get_dataset_and_shards(
            config, num_workers=0, local_worker=True
        )
    ...
    def test_dataset_shard_only_remote_workers(self):
        """Tests whether the dataset_shard function works correctly for the remote workers with just the dataset shards for the remote workers."""
        ...
        assert all(isinstance(remote_shard, ray.data.Dataset) for remote_shard in shards)
...
class TestUnzipIfNeeded(unittest.TestCase):
    ...
    #     """Tests whether the unzip_if_needed function works correctly on s3 zip files"""
    ...
    def test_relative_zip(self):
        """Tests whether the unzip_if_needed function works correctly on relative zip files"""
    ...
    def test_absolute_zip(self):
        """Tests whether the unzip_if_needed function works correctly on absolute zip files"""
    ...
    def  test_s3_json(self):
        """Tests whether the unzip_if_needed function works correctly on s3 json files"""
    ...
    def test_relative_json(self):
        """Tests whether the unzip_if_needed function works correctly on relative json files"""
    ...
    def test_absolute_json(self):
        """Tests whether the unzip_if_needed function works correctly on absolute json files"""
