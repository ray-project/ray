...
# Override the Go toolchain registered by grpc_extra_deps() (Go 1.18) with the
# version required by the Go runtime. go_download_sdk re-registers the
# toolchains, and within a WORKSPACE evaluation block the last declaration of
# a repository wins, so this additive declaration swaps the SDK without
# patching the grpc dependency. The load above must stay in the same block as
# (i.e. before) grpc_extra_deps: a load() between the two calls starts a new
# evaluation block and the override would be silently ignored.
# go_register_toolchains cannot be used for this: it fails when an SDK rule
# already exists.
