...
- A new or changed Ray Data datasource that yields a `pa.Table` deserialized from external input (parquet, Arrow IPC, a service or library returning Arrow) without calling `raise_on_pickle_object_columns(table)` before yielding or materializing it is a remote code execution bug (GHSA-2ch8-9c5v-84jf). Flag it in review; see `python/ray/data/.claude/rules/datasource-pickle-gate.md`.
