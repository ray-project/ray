...
class TestValidation(unittest.TestCase):
    """Test _validate() checks build-supported combinations."""

    def test_valid_ray_cpu(self):
        ImageBuildConfig._validate("ray", "3.10", "cpu")

    def test_valid_ray_llm(self):
        ImageBuildConfig._validate("ray-llm", "3.11", "cu12.8.1-cudnn")

    def test_unknown_build_image_type(self):
        with self.assertRaises(BuildError) as ctx:
            ImageBuildConfig._validate("ray-ml", "3.10", "cpu")
        self.assertIn("Unknown image type", str(ctx.exception))

    def test_invalid_python_raises_build_error(self):
        with self.assertRaises(BuildError):
            ImageBuildConfig._validate("ray-llm", "3.10", "cu12.8.1-cudnn")
    ...
    def test_from_args_creates_config(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch(
                ...
                ImageBuildConfig, "_find_ray_root", return_value=ray_root
            ), mock.patch.object(
                ImageBuildConfig, "_get_git_commit", return_value="abc1234"
            ):
