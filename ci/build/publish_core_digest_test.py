...
from ci.build.publish_core_digest import (
    image_tag,
    publish,
    s3_key,
)
...
class TestPublish(unittest.TestCase):
    ...
    @patch("ci.build.publish_core_digest.compute_digest")
    ...
    @patch("ci.build.publish_core_digest.upload_to_s3")
    @patch("ci.build.publish_core_digest.export_image")
    @patch("ci.build.publish_core_digest.compute_digest")
    @patch.dict(os.environ, _ENV, clear=False)
