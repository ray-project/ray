...
if [[ "$PYTHON" == "2.7" ]] && [[ "$platform" == "linux" ]]; then
  ...
  pip install -q scipy tensorflow cython==0.29.0 cmake gym==0.10.11 opencv-python pyyaml pandas==0.23.4 requests \
  ...
  bash miniconda.sh -b -p $HOME/miniconda
  export PATH="$HOME/miniconda/bin:$PATH"
  pip install -q scipy tensorflow cython==0.29.0 cmake gym opencv-python pyyaml pandas==0.23.4 requests \
