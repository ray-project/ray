...
if [[ "$PYTHON" == "2.7" ]] && [[ "$platform" == "linux" ]]; then
  ...
  pip install -q scipy torch tensorflow cython==0.29.0 gym opencv-python-headless pyyaml pandas==0.24.2 requests \
  ...
  bash miniconda.sh -b -p $HOME/miniconda
  export PATH="$HOME/miniconda/bin:$PATH"
  pip install -q cython==0.29.0 torch tensorflow gym opencv-python-headless pyyaml pandas==0.24.2 requests \
    feather-format lxml openpyxl xlrd py-spy setproctitle faulthandler pytest-timeout mock flaky networkx tabulate psutil
