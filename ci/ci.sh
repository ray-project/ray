...
test_python() {
  ...
  if [ "${OSTYPE}" = msys ]; then
    ...
    args+=(
      ...
      -python/ray/tests:batching_node_provider/test_batch_node_provider_unit.py # irrelevant on windows
      -python/ray/tests:batching_node_provider/test_batch_node_provider_integration.py # irrelevant on windows
    )
  fi
