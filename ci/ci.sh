...
compile_pip_dependencies() {
  ...
  # Add python_version < 3.11 to scikit-image and scipy as they need more recent
  # versions in python 3.11. These will be automatically resolved. Remove as
  # soon as we resolve to versions of scikit-image that are built for py311.
  sed -iE 's/((scikit-image|scipy)==[\.0-9]+\b)/\1 ; python_version < "3.11"/g' "${WORKSPACE_DIR}/python/$TARGET"
