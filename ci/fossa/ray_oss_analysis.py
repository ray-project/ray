...
def generate_fossa_deps_file(askalono_results: List[Dict]) -> Dict:
    # Group licenses by dependency
    dependency_licenses = {}
    for result in askalono_results:
        logger.info("result: %s", result)
        dep = result["dependency"]
        license_name = result["license"]
        if dep not in dependency_licenses:
            dependency_licenses[dep] = set()
        dependency_licenses[dep].add(license_name)
    
    # Create custom dependencies with aggregated licenses
    custom_dependencies = []
    for dependency, licenses in sorted(dependency_licenses.items()):
        logger.info(f"Dependency: {dependency}, Licenses: {licenses}")
