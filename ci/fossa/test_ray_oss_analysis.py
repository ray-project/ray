...
class TestRayOssAnalysis(unittest.TestCase):
    ...
    def test_isBuildTool(self):
        ...
        self.assertTrue(ray_oss_analysis._isBuildTool("@local_config_cc//:builtin_include_directory_paths"))
        self.assertFalse(ray_oss_analysis._isBuildTool("//src/ray/util:subreaper.h"))
        self.assertFalse(ray_oss_analysis._isBuildTool("@upb//upbc:stage0/google/protobuf/compiler/plugin.upb.c"))
    ...
    def test_get_dependency_info(self):
        ...
        info = ray_oss_analysis._get_dependency_info({
            "type": "SOURCE_FILE",
            "sourceFile": {"name": "name", "location": "loc"}
        })
    ...
    def test_get_bazel_dependencies(self, mock_check_output):
        ...
        with patch("ci.fossa.ray_oss_analysis._isOwnCode") as mock_is_own:
            ...
            package_names, file_paths = ray_oss_analysis._get_bazel_dependencies("//:target", "bazel")
    ...
    def test_askalono_crawl(self, mock_run):
        mock_run.return_value.stdout = '{"path": "p", "result": {"score": 1.0, "license": {"name": "MIT"}}}\n'
