...
class TestRayDockerContainer(RayCITestBase):
    ...
    def test_run(self) -> None:
        ...
        with mock.patch(
            ...
            assert cmd == (
                ...
                f"rayproject/ray:nightly-{pv}-cu118 "
                f"ray:nightly-{pv}-cu118_pip-freeze.txt"
            )
            ...
            assert cmd == (
                ...
                "requirements_compiled.txt "
                f"rayproject/ray-ml:nightly-{pv}-cpu "
                f"ray-ml:nightly-{pv}-cpu_pip-freeze.txt"
            )
    ...
    def test_canonical_tag(self) -> None:
        ...
        assert container._get_canonical_tag() == f"nightly-{pv}-cpu"

        container = RayDockerContainer(v, "cpu", "ray", "aarch64")
        assert container._get_canonical_tag() == f"nightly-{pv}-cpu-aarch64"

        container = RayDockerContainer(v, "cu11.8.0", "ray-ml")
        assert container._get_canonical_tag() == f"nightly-{pv}-cu118"
