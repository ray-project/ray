...
from ci.ray_ci.doc.build_cache import BuildCache, ENVIRONMENT_PICKLE

class FakeBuildEnv:
    def __init__(self, srcdir: str, doctreedir: str, project: Project, all_docs: dict):
        self.srcdir = "srcdir"
        self.doctreedir = "doctreedir"
        self.project = Project("srcdir", {".rst": "restructuredtext", ".md": "myst-nb", ".ipynb": "myst-nb"})
...
def _generate_test_env():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        env = FakeBuildEnv(
            ...
            Project(os.path.join(temp_dir, "source"), {".rst": "restructuredtext", ".md": "myst-nb", ".ipynb": "myst-nb"}),
            {},
        )
        p = Project(
            env.srcdir,
...
def test_update_environment_pickle():
    with tempfile.TemporaryDirectory() as temp_dir, tempfile.TemporaryDirectory() as temp_dir2:
        ...
        with open(
            os.path.join(temp_dir, "doc", ENVIRONMENT_PICKLE), "wb+"
        ) as f:
        ...
        with open(
            os.path.join(temp_dir, "doc", ENVIRONMENT_PICKLE), "rb+"
        ) as f:
            env = pickle.load(f)
