...
class TestStatistics:
    ...
    async def gen_analyze(
        cls, test: Test, test_history_length: int, use_premerge_data: bool
    ) -> "TestStatistics":
        """
        ...
        stat._result_histories = test.get_test_results(
            ...
            use_async=True,
            aws_bucket="ray-ci-pr-results" if use_premerge_data else None,
            newer_than_days=30,
        )
    ...
    def _compute_p75_duration(self) -> float:
        durations = [
            result.duration_ms
            for result in self._result_histories
...
@click.option(
    ...
    default=False,
    help=(
        "Use the pre-merge data instead of the post-merge data. This is useful for "
        "analyzing tests that are not yet merged to the main branch."
    ),
)
...
def main(
    ...
    test_stats = asyncio.run(
        gen_test_stats(tests, test_history_length, use_premerge_data)
    )
    test_stats.sort(
...
async def gen_test_stats(
    ...
    async def gen_analyze(test, use_premerge_data):
        stats = await TestStatistics.gen_analyze(
            test, test_history_length, use_premerge_data
        )
        logger.info(f"Got stats {stats}")
        return stats

    return await asyncio.gather(
        *[gen_analyze(test, use_premerge_data) for test in tests]
    )
