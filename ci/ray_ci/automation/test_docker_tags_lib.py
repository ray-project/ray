...
def test_get_docker_registry_auth_token(mock_requests):
    ...
    mock_requests.return_value = mock.Mock(
        status_code=200,
        json=mock.MagicMock(return_value={"token": "test_token"})
    )

    token = get_docker_registry_auth_token(namespace, repository)

    assert mock_requests.call_count == 1
    assert mock_requests.call_args[0][0] == f"https://auth.docker.io/token?service=registry.docker.io&scope=repository:{namespace}/{repository}:pull"
...
def test_get_docker_registry_auth_token_failure(mock_requests):
    ...
    mock_requests.return_value = mock.Mock(
        status_code=400,
        json=mock.MagicMock(return_value={"error": "test_error"})
    )
...
def test_get_docker_hub_auth_token(mock_requests):
    ...
    assert mock_requests.call_args[1]["json"] == {"username": "test_username", "password": "test_password"}
    assert token == "test_token"
...
def test_get_image_creation_time_failure_unknown(mock_crane_config):
    mock_crane_config.return_value = "Error: Failed to get tag. MANIFEST_UNKNOWN: Tag manifest unknown"
...
def test_delete_tag(mock_get_token, mock_requests):
    ...
    assert mock_requests.call_args[0][0] == f"https://hub.docker.com/v2/repositories/{namespace}/{repository}/tags/{tag}"
    assert mock_requests.call_args[1]["headers"] == {"Authorization": "Bearer test_token"}
    assert deleted == True
...
def test_safe_to_delete(mock_get_image_creation_time):
    # Image creation time is more than 30 days ago
    mock_get_image_creation_time.return_value = datetime.now(timezone.utc) - timedelta(days=60)
...
def test_safe_to_delete(mock_get_image_creation_time):
    ...
    mock_get_image_creation_time.return_value = datetime.now(timezone.utc) - timedelta(days=15)
    registry_tags = ["test_tag1", "test_tag2"]
    is_safe = _safe_to_delete("test_tag1", 30, registry_tags)
...
def test_copy_tag_to_aws_ecr(mock_call_crane_cp):
    ...
    mock_call_crane_cp.return_value = "aws-ecr/name/repo:test_tag: digest: sha256:sample-sha256 size: 1788"

    is_copied = copy_tag_to_aws_ecr(tag, "aws-ecr/name/repo")
    mock_call_crane_cp.assert_called_once_with(tag="test_tag", source=tag, aws_ecr_repo="aws-ecr/name/repo")
...
def test_copy_tag_to_aws_ecr_failure(mock_call_crane_cp):
    ...
    is_copied = copy_tag_to_aws_ecr(tag, "aws-ecr/name/repo")
    mock_call_crane_cp.assert_called_once_with(tag="test_tag", source=tag, aws_ecr_repo="aws-ecr/name/repo")
    assert is_copied == False

def _make_docker_hub_response(tag: str, page_count: int, namespace: str, repository: str, page_limit: int):
    next_url = f"https://hub.docker.com/v2/namespaces/{namespace}/repositories/{repository}/tags?page={page_count+1}&page_size=100"
    return mock.Mock(
        status_code=200,
        json=mock.MagicMock(return_value={
            "count": 100,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            ["2.7.0", "2.7.0.a1s2d3-py38", "2.7.0-py38", "2.7.0rc0-py38-cu123", "2.7.0.post1"], 
            ["2.7.0", "2.7.0-py38"]
        ),
    ]
)
@mock.patch("requests.get")
@mock.patch("ci.ray_ci.automation.docker_tags_lib.get_docker_hub_auth_token")
def test_query_tags_from_docker_hub(mock_get_token, mock_requests, filter_func, tags, expected_tags):
    mock_get_token.return_value = "test_token"
    mock_requests.side_effect = [
        _make_docker_hub_response(tags[i], i+1, "test_namespace", "test_repo", len(tags)) for i in range(len(tags))
    ]
    queried_tags = query_tags_from_docker_hub(filter_func=filter_func, namespace="test_namespace", repository="test_repo", num_tags=100)
...
@mock.patch("requests.get")
@mock.patch("ci.ray_ci.automation.docker_tags_lib.get_docker_hub_auth_token")
def test_query_tags_from_docker_hub_failure(mock_get_token, mock_requests, filter_func, tags, expected_tags):
    mock_get_token.return_value = "test_token"
    mock_requests.side_effect = [
...
def test_query_tags_from_docker_registry(mock_call_crane_ls, mock_get_token):
    ...
    mock_call_crane_ls.assert_called_once_with(namespace="test_namespace", repository="test_repo")
    assert tags == ["test_namespace/test_repo:test_tag1", "test_namespace/test_repo:test_tag2", "test_namespace/test_repo:test_tag3"]
