...
class TestGetImageTags:
    ...
    def test_master_branch_no_build_id(self):
        tags = _get_image_tags(python_version="3.11", platform="cpu")

        assert tags == ["abc123-py311-cpu"]
    ...
    def test_gpu_platform_includes_alias(self):
        ...
        gpu_tags = _get_image_tags(python_version="3.10", platform=GPU_PLATFORM)
        other_cuda_tags = _get_image_tags(
            python_version="3.10", platform="cu12.3.2-cudnn9"
        )
        cpu_tags = _get_image_tags(python_version="3.10", platform="cpu")
