...
from ci.ray_ci.automation.ray_wheels_lib import _get_wheel_names, download_wheel_from_s3, download_ray_wheels_from_s3, _check_downloaded_wheels, PYTHON_VERSIONS, PLATFORMS, RAY_TYPES
...
def test_get_wheel_names():
    ...
    for wheel_name in wheel_names:
        ...
        ray_type, ray_version, python_version, python_version2, platform = wheel_name.split("-")
...
def test_download_wheel_from_s3(mock_boto3_client):
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        for key in keys:
            ...
            mock_boto3_client.return_value.download_file.assert_called_with("ray-wheels", key, f"{tmp_dir}/{key.split('/')[-1]}")
...
def test_download_ray_wheels_from_s3(mock_get_wheel_names, mock_check_wheels, mock_download_wheel):
    commit_hash = "1234567"
    ray_version = "1.0.0"

    mock_get_wheel_names.return_value = SAMPLE_WHEELS
    with tempfile.TemporaryDirectory() as tmp_dir:
        download_ray_wheels_from_s3(commit_hash=commit_hash, ray_version=ray_version, directory_path=tmp_dir)
        ...
        for i, call_args in enumerate(mock_download_wheel.call_args_list):
            assert call_args[0][0] == f"releases/{ray_version}/{commit_hash}/{SAMPLE_WHEELS[i]}.whl"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (Exception(f"Expected N *.whl files in tmp_dir, but found 0 instead")),
    ]
)
@mock.patch("ci.ray_ci.automation.ray_wheels_lib.download_wheel_from_s3")
@mock.patch("ci.ray_ci.automation.ray_wheels_lib._check_downloaded_wheels")
@mock.patch("ci.ray_ci.automation.ray_wheels_lib._get_wheel_names")
def test_download_ray_wheels_from_s3_fail_check_wheels(mock_get_wheel_names, mock_check_wheels, mock_download_wheel, check_error):
    commit_hash = "1234567"
    ray_version = "1.0.0"
...
@mock.patch("ci.ray_ci.automation.ray_wheels_lib._check_downloaded_wheels")
@mock.patch("ci.ray_ci.automation.ray_wheels_lib._get_wheel_names")
def test_download_ray_wheels_from_s3_fail_download(mock_get_wheel_names, mock_check_wheels, mock_download_wheel):
    commit_hash = "1234567"
    ray_version = "1.0.0"
