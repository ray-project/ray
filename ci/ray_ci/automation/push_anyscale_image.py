...
def _format_platform_tag(platform: str) -> str:
    ...
    if len(parts) < 2:
        raise PushAnyscaleImageError(f"Unrecognized GPU platform format: {platform}")
...
def _get_image_tags(python_version: str, platform: str) -> List[str]:
    ...
    commit_full = os.environ.get("BUILDKITE_COMMIT", "")
    if not commit_full:
        raise PushAnyscaleImageError("BUILDKITE_COMMIT environment variable not set")
    commit = commit_full[:6]
    ...
    if branch == "master":
        ...
        prefix = f"pr-{pr}.{commit}" if pr != "false" else commit

    tags = [f"{prefix}{py_tag}{ptag}" for ptag in platform_tags]
    if rayci_build_id:
...
def _copy_image(source: str, destination: str, dry_run: bool = False) -> None:
    ...
    try:
        ...
        raise PushAnyscaleImageError(f"Crane copy failed: {e}")
