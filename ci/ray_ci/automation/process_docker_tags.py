...
from ci.ray_ci.automation.docker_tags_lib import backup_release_tags, delete_old_commit_tags

@click.command()
@click.option("--task", help="Task to perform", required=True, type=click.Choice(["backup_release_tags", "delete_commit_tags"]))
...
@click.option("--release_versions", help="Release versions to backup", required=False, type=list)
...
def main(task, namespace, repository, release_versions, aws_ecr_repo, n_days, num_tags):
    if task == "backup_release_tags":
        ...
        raise ValueError("Invalid task choice. Choose from 'backup_release_tags' or 'delete_commit_tags'")
