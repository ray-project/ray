...
public final class Ray extends RayCall {
  ...
  public static <T> ObjectRef<T> put(T obj, BaseActorHandle owner) {
    return internal().put(obj, owner, false);
  }

  /**
   * Store Arrow-serialized data in the object store.
   *
   * @param obj The Java object to be stored.
   * @return A ObjectRef instance that represents the in-store object.
   */
  public static <T> ObjectRef<T> putArrow(T data) {
    return internal().put(data, true);
  }
   ...
   * @return A ObjectRef instance that represents the in-store object.
   */
  public static <T> ObjectRef<T> putArrow(T data, BaseActorHandle owner) {
    return internal().put(data, owner, true);
  }
