...
public class ObjectStoreProxy {
  ...
  public ObjectStoreProxy(WorkerContext workerContext, ObjectStore objectStore) {
  ...
  public <T> List<T> get(List<ObjectId> ids) {
    ...
    List<NativeRayObject> dataAndMetaList = objectStore.get(ids, -1);
  ...
  public ObjectId put(Object object) {
    return objectStore.put(serialize(object));
  }
  ...
  public List<Boolean> wait(List<ObjectId> objectIds, int numObjects, long timeoutMs) {
    return objectStore.wait(objectIds, numObjects, timeoutMs);
  }
