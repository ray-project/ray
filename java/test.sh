...
echo "Running tests under cluster mode with 5 workers per Java worker process."
# bazel test //java:all_tests --action_env=ENABLE_MULTI_LANGUAGE_TESTS=1 --jvmopt="-Dray.raylet.config.num_workers_per_process_java=5" --test_output="errors" || cluster_exit_code=$?
ENABLE_MULTI_LANGUAGE_TESTS=1 run_testng java -Dray.raylet.config.num_workers_per_process_java=5 -cp $ROOT_DIR/../bazel-bin/java/all_tests_deploy.jar org.testng.TestNG -d /tmp/ray_java_test_output $ROOT_DIR/testng.xml
