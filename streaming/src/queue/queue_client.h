...
/// A ReaderClient should be created before DataReader created in Cython/Jni, and hold by Jobworker.
/// When DataReader receive a buffer from upstream DataWriter (DataReader's raycall function is called),
/// it calls `OnReaderMessage` to pass the buffer to its own downstream queue, or `OnReaderMessageSync`
/// to wait handle result.
class ReaderClient {
public:
  /// Construct a ReaderClient object.
  /// \param[in] core_worker CoreWorker C++ pointer of current actor
  /// \param[in] async_func DataReader's raycall function descriptor to be called by DataWriter, asynchronous semantics
  /// \param[in] sync_func DataReader's raycall function descriptor to be called by DataWriter, synchronous semantics
