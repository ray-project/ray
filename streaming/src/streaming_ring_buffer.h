...
template <class T>
class BlockingQueue : public AbstractRingBufferImpl<T> {
 private:
 ...
 public:
  BlockingQueue(size_t size) : capacity_(size), is_started_(true) {}
...
class EventQueue : public AbstractRingBufferImpl<T> {
 ...
 public:
  ...
  void Pop() {
    ...
    if (Size() + 1 >= capacity_) {
      full_cv_.notify_one();
      STREAMING_LOG(WARNING) << "Pop notify success"
                             << " all_Size:" << Size();
      ;
    }
  }
  ...
  bool Empty() { return buffer_.empty() && urgent_buffer_.empty(); }

  bool Full() { return buffer_.size() + urgent_buffer_.size() == capacity_; }

  size_t Size() { return buffer_.size() + urgent_buffer_.size(); }

  size_t UrgentBufferSize() { return urgent_buffer_.size(); }
