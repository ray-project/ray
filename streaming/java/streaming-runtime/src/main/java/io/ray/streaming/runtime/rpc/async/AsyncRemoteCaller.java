...
public class AsyncRemoteCaller {
   ...
   */
  public void checkIfNeedRollbackAsync(BaseActorHandle actor, Callback<Boolean> callback, ExceptionHandler<Throwable> onException) {
   ...
   * @param onException callback function on exception
   */
  public void rollback(BaseActorHandle actor,
                       final Long checkpointId,
                       Callback<CallResult<QueueRecoverInfo>> callback,
    ...
    if (actor instanceof PyActorHandle) {
      ...
      remoteCallPool.bindCallback(call, obj -> {
        CallResult<QueueRecoverInfo> res = (CallResult<QueueRecoverInfo>) obj;
        callback.handle(res);
      }, onException);
    }
  }
   ...
   * @param onException callback function on exception
   */
  public void batchRollback(List<BaseActorHandle> actors, final Long checkpointId, Collection<String> abnormalQueues,
                            Callback<List<CallResult<QueueRecoverInfo>>> callback,
                            ExceptionHandler<Throwable> onException) {
    ...
    remoteCallPool.bindCallback(rayCallList, objList -> {
      List<CallResult<QueueRecoverInfo>> results = new ArrayList<>();
      for (int i = 0; i < objList.size(); ++i) {
        Object obj = objList.get(i);
        if (isPyActor.getOrDefault(i, false)) {
          results.add(PbResultParser.parseRollbackResult((byte[]) obj));
        } else {
          results.add((CallResult<QueueRecoverInfo>) obj);
        }
      }
