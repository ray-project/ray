...
class StreamTask(ABC):
    ...
    def __init__(self, task_id: int, processor: 'Processor', worker: 'JobWorker', last_checkpoint_id: int):
    ...
    def do_checkpoint(self,
        ...
        op_checkpoint_info = OpCheckpointInfo(operator_checkpoint, input_points, output_points, checkpoint_id)
    ...
    def __save_cp_state_and_report(self, op_checkpoint_info, checkpoint_id):
        logger.info("Start to save cp state and report, checkpoint id is {}.".format(checkpoint_id))
    ...
    def __save_cp(self, op_checkpoint_info, checkpoint_id):
        logger.info("save operator cp, op_checkpoint_info={}".format(op_checkpoint_info))
        cp_bytes = pickle.dumps(op_checkpoint_info)
        self.worker.state_backend.put(self.__gen_op_checkpoint_key(checkpoint_id), cp_bytes)

    def __report_commit(self, checkpoint_id: int):
        logger.info("Report commit, checkpoint id {}.".format(checkpoint_id))
        report = WorkerCommitReport(self.vertex_context.actor_id.binary(), checkpoint_id)
    ...
    def prepare_task(self, is_recreate: bool):
        ...
        if is_recreate:
            ...
            logger.info("Getting task checkpoints from state, cpKey={}, checkpointId={}."
                        .format(cp_key, self.last_checkpoint_id))
            cp_bytes = self.worker.state_backend.get(cp_key)
            if cp_bytes is None:
                msg = "Task recover failed, checkpoint is null! cpKey={}".format(cp_key)
        ...
        if len(output_actors_map) > 0:
            ...
            logger.info(
                "Create DataWriter channel_ids {}, target_actors {}, output_points={}."
                    .format(channel_str_ids, target_actors, op_checkpoint_info.output_points))
            ...
            for edge in execution_vertex_context.output_execution_edges:
                collectors.append(OutputCollector(
                    self.writer, channel_str_ids, target_actors,
                    edge.partition))
        ...
        if len(input_actor_map) > 0:
            ...
            logger.info("Create DataReader, channels {}, input_actors {}, input_points={}."
                        .format(channel_str_ids, from_actors, op_checkpoint_info.input_points))
        ...
        runtime_context = RuntimeContextImpl(
            ...
            config=channel_conf, job_config=channel_conf)
...
class InputStreamTask(StreamTask):
    ...
    def run(self):
        ...
        try:
            ...
            logger.exception("Last success checkpointId={}, now occur error.".format(self.last_checkpoint_id))
...
class SourceStreamTask(StreamTask):
    ...
    def __init__(self, task_id: int, processor_instance: 'SourceProcessor', worker: 'JobWorker', last_checkpoint_id):
        super().__init__(task_id, processor_instance, worker, last_checkpoint_id)
        self.running = True
        self.stopped = False
    ...
    def commit_trigger(self, barrier):
        if self.__pending_barrier is not None:
            logger.warning("Last barrier is not broadcast now, skip this barrier trigger.")
            return False
