...
class JobWorker(object):
    ...
    def __init__(self, execution_vertex_pb_bytes):
        ...
        try:
            ...
            if was_reconstructed:
                ...
                logger.info(
                    "Worker get checkpoint state by key: {}.".format(
                ...
                if context_bytes is not None and context_bytes.__len__() > 0:
                    ...
                    logger.error(
                        ...
                        ", only reliable state backend supports fail-over.".format(
                            job_worker_context_key
                        )
                    )
        except Exception:
        ...
        logger.info(
            "Creating job worker succeeded. worker config {}".format(self.config)
        )
    ...
    def init(self, worker_context_bytes):
        ...
        try:
            ...
            # save context
            job_worker_context_key = self.__get_job_worker_context_key()
            self.context_backend.put(job_worker_context_key, worker_context_bytes)
            ...
            logger.info(
                "Initializing job worker, exe_vertex_name={},"
    ...
    def create_stream_task(self, checkpoint_id):
        if isinstance(self.stream_processor, processor.SourceProcessor):
            ...
            raise Exception(
                "Unsupported processor type: " + str(type(self.stream_processor))
            )
    ...
    def rollback(self, checkpoint_id_bytes):
        ...
        try:
            ...
            logger.info(
                "Rollback success, checkpoint is {}, qRecoverInfo is {}.".format(
                    checkpoint_id, q_recover_info
                )
            )
    ...
    def clear_expired_cp(self, state_checkpoint_id_bytes, queue_checkpoint_id_bytes):
        ...
        logger.info(
            "Start to clear expired checkpoint, checkpoint_id={},"
        ...
        ret = remote_call_pb2.BoolResult()
        ret.boolRes = (
            self.__clear_expired_cp_state(state_checkpoint_id)
            if state_checkpoint_id > 0
            else True
        )
    ...
    def __clear_expired_cp_state(self, checkpoint_id):
        if self.__need_rollback:
            logger.warning(
                "Need rollback, skip clear_expired_cp_state"
                ", checkpoint id: {}".format(checkpoint_id)
            )
            return False

        logger.info(
            "Clear expired checkpoint state, cp id is {}.".format(checkpoint_id)
        )
    ...
    def __clear_expired_queue_msg(self, checkpoint_id):
        ...
        logger.info(
            "Clear expired queue msg, checkpoint_id is {}.".format(checkpoint_id)
        )
    ...
    def request_rollback(self, exception_msg="Python exception."):
        ...
        if not request_ret:
            logger.warning(
                "Request failed after retry {} times,"
                "now worker shutdown without reconstruction.".format(
                    Config.REQUEST_ROLLBACK_RETRY_TIMES
                )
            )
    ...
    def _gen_unique_key(self, key_prefix):
        return (
            ...
            + "_"
            + str(self.execution_vertex.execution_vertex_id)
        )
